...
$result = db_query('SELECT acctid1, acctid2, turn FROM pvp WHERE acctid1='.$session['user']['acctid'].' OR acctid2='.$session['user']['acctid']);
...
    addnav('Spieler herausfordern','pvp.php');
...
    addnav('WanderhÃ¤ndler','vendor.php');
...
if (@file_exists('pavilion.php'))

    addnav('P?AuffÃ¤lliger Pavilion','pavilion.php');

addnav('Tavernenstrasse');

addnav('E?Schenke zum Eberkopf','inn.php');
...
output('`@`c`bDorfplatz`b`cDie Einwohner rennen geschÃ¤ftig umher. Keiner bemerkt wirklich, dass Du dort stehst. Du siehst verschiedene GeschÃ¤fte und LÃ¤den entlang der Strasse. Es gibt einen merkwÃ¼rdig aussehenden Felsen auf einer Seite. ');

$result = db_query('SELECT * FROM news WHERE 1 ORDER BY newsid DESC LIMIT 1');
...
output('Die Uhr an der Kneipe zeigt `^'.getgametime().'`@.`n');
...
viewcommentary('Dorfplatz','HinzufÃ¼gen',25);
