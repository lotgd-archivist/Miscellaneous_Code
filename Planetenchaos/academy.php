...
        output(" `iDunklen KÃ¼nste`i");

        else if ($session['user']['specialty'] == 2)

        output(" `iMystischen KrÃ¤fte`i");
...
        output("Ã¶ffnet es fÃ¼r Dich. Es enthÃ¤lt eine Preisliste:`n`n<ul>",true);
...
                output("Fasziniert lauschst Du den geheimen Worten und lernst wirklich etwas Ã¼ber");

                if ($session['user']['specialty'] == 1)

                output(" `iDunkle KÃ¼nste`i");

                else if ($session['user']['specialty'] == 2)

                output(" `iMystische KrÃ¤fte`i");

                else

                output(" `iDiebeskunst`i");

                output(". Breit grinsend und stolz auf Dein neues Wissen verlÃ¤sst Du die Akademie.`n`n");
...
            $result=db_query("SELECT * FROM items WHERE id=$_GET['id']");

              $row = db_fetch_assoc($result);

            output("`vDer alter Zauberer begutachtet $row['name']`v. Dann Ã¼berreicht er dir sorgfÃ¤ltig abgezÃ¤hlt ".($row['gold']?"`^$row['gold'] `vGold":"")." ".($row['gems']?"`#$row['gems']`v Edelsteine":"")." und lÃ¤sst den Zauber verschwinden. WÃ¶rtlich. ");
...
            output("`^Warchild steht in der NÃ¤he des Eingangs zur Akademie und wartet, bis Du den Hof Ã¼berquert hast, um Dich anzureden.`n");
