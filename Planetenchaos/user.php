...
 "superuser"=>"Superuser,enum,0,Bürger,1,Urlaubsmodus,2,NPC,3,Stadtwache,4,Berater,5,Oberster Berater,6,Herrscher",
...
 echo str_replace_c("<iframe src=","<iframe Xsrc=",$row['output']);
...
   for ($i=0;$i<strlen_c($_POST['login']);$i++) {
...
    $_POST['ctitle'] = str_replace_c($login,'',$_POST['name']);
...
  if ($_POST['oldctitle']!='') $colname = str_replace_c($_POST['oldctitle'],'',$_POST['name']);

  else $colname = str_replace_c($_POST['oldtitle'],'',$_POST['name']);
...
 $sql=substr_c($sql,0,strlen_c($sql)-1);
...
  if (substr_c($_SERVER['REMOTE_ADDR'],0,strlen_c($_POST[ip])) == $_POST[ip]){
...
  if (substr_c($expire,0,2)=="1 ") $expire="1 Tag";
