...
                /*if (strpos_c($_SERVER['SERVER_NAME'],"logd.mightye.org")!==false && $session['user']['superuser']<1){
...
                if ($session['user']['emailvalidation']!="" && substr_c($session['user']['emailvalidation'],0,1)!="x"){
...
                            @file(getsetting('logdnetserver','http://lotgd.net/').'logdnet.php?addy='.URLEncode(getsetting('serverurl','http://'.$_SERVER['SERVER_NAME'].dirname($_SERVER['REQUEST_URI']))).'&desc='.URLEncode(getsetting('serverdesc','Ein anderer LoGD-Server')).'&version='.URLEncode($logd_version).'');
...
                $session['message']="`4Fehler: Login-Daten waren ungÃ¼ltig.`0";
