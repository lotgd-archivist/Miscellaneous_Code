...
    $sql = "SELECT petitionid FROM petitions WHERE status=2 AND date<'".date("Y-m-d H:i:s",strtotime(date('c')."-7 days"))."'";
...
            output(preg_replace("'[^a-zA-Z0-91234567890\\[\\]= @.!,?-]'","",substr_c($row['body'],0,strpos_c($row['body'],"[email"))));
...
    $body = preg_replace("'([[:alnum:]_.-]+[@][[:alnum:]_.-]{2,}([.][[:alnum:]_.-]{2,})+)'i","<a href='mailto:\\1?subject=RE: Hilfeanfrage&body=".str_replace_c("+"," ",URLEncode("\n\n----- Deine Anfrage -----\n".$row['body']))."'>\\1</a>",$body);
