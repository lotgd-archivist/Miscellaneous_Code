...
            output("`\$Ramius`) verflucht dich fÃ¼r deine Feigheit.`n`n");
...
            output("`)Als du zu fliehen versuchst, wirst du zum Kampf zurÃ¼ckberufen!`n`n");
...
            $taunt = str_replace_c("%s",($session['user']['sex']?"sie":"ihn"),$taunt['taunt']);

            $taunt = str_replace_c("%o",($session['user']['sex']?"sie":"er"),$taunt);

            $taunt = str_replace_c("%p",($session['user']['sex']?"ihre(m/r)":"seine(r/m)"),$taunt);
...
            output("Du kannst heute keine weiteren Seelen mehr quÃ¤len.");

            $session['user']['donation']+=1;

            $session['user']['gravefights']=0;

            addnav("F?ZurÃ¼ck zum Friedhof","graveyard.php");

        }else{

            addnav("Q?QuÃ¤len","graveyard.php?op=fight");
...
                addnav("5 Runden quÃ¤len","graveyard.php?op=fight&auto=five");
...
    output("Dein Geist wandert auf einen einsamen mit Unkraut Ã¼berwucherten Friedhof. Die Pflanzen scheinen nach deinem Geist im Vorbeischweben zu greifen.

    Du bist umgeben von den Ãœberresten alter Grabsteine. Einige liegen auf dem Gesicht, andere sind in StÃ¼cke zerbrochen. Fast kannst du das Wehklagen 

    der hier gefangenen Seelen hÃ¶ren.

    `n`nMitten im Friedhof steht ein altertÃ¼mliches Mausoleum, dem die Spuren ungezÃ¤hlter Jahre deutlich anzusehen sind. 

    Ein bÃ¶se schauender Steingargoyle ziert die Dachspitze; seine Augen scheinen dir zu folgen und sein aufklaffender Mund ist gespickt mit scharfen SteinzÃ¤hnen.

    Auf der Gedenktafel Ã¼ber der TÃ¼r ist zu lesen: `\$Ramius, Herr Ã¼ber den Tod`).");



    addnav("Etwas zum QuÃ¤len suchen","graveyard.php?op=search");
...
    output("Du betrittst das Mausoleum und siehst dich in einer kalten, kahlen Kammer aus Marmor. Die Luft um dich herum trÃ¤gt die KÃ¤lte des Todes selbst.

    Aus der Dunkelheit starren zwei schwarze Augen direkt in deine Seele. Ein feuchtkalter Griff scheint deine Seele zu umklammern und sie mit den Worten des Todesgottes `\$Ramius`) hÃ¶chstpersÃ¶nlich zu erfÃ¼llen.`n`n

    \"`0Dein sterblicher KÃ¶rper hat dich im Stich gelassen. Und jetzt wendest du dich an mich. Es gibt in diesem Land diejenigen, die sich meinem Griff entziehen konnten und ein Leben Ã¼ber das Leben hinaus besitzen. Um mir deinen Wert fÃ¼r mich zu beweisen 

    und dir Gefallen zu verdienen, gehe raus und quÃ¤le deren Seelen. Solltest du mir genug Gefallen getan haben, werde ich dich belohnen.`)\"");
...
            output("`\$Ramius`) nennt dich einen SchwÃ¤chling, weil du nach Wiederherstellung deiner Seele fragst. Aber da du genug Gefallen bei ihm gut hast, gibt er deiner Bitte zum Preis von `4$favortoheal`) Gefallen nach.");
...
            output("`\$Ramius`) verflucht dich und wirft dich aus dem Mausoleum. Du muÃŸt ihm erst genug Gefallen getan haben, bevor er dir die Wiederherstellung deiner Seele gewÃ¤hrt.");

        }

    }else{

        output("`\$Ramius`) seufzt und murmelt etwas von \"`7Nur weil sie tot sind, heiÃŸt das doch nicht, dass sie nicht zu denken brauchen, oder?`)\"`n`n");

        output("Vielleicht solltest du erstmal eine Wiederherstellung `inÃ¶tig`i haben, bevor du danach fragst.");
...
        output("`\$Ramius`) spricht: \"`7Du hast mich tatsÃ¤chlich beeindruckt. Ich sollte dir die MÃ¶glichkeit gewÃ¤hren, deine Feinde in der Welt der Sterblichen zu besuchen.`)\"");

        addnav("Ramius' Gefallen");

        addnav("h?Feind heimsuchen (25 Gefallen)","graveyard.php?op=haunt");

        if ($session['user']['reputation']<=-10) output(" Er weist dich noch darauf hin, dass er keinen Einfluss auf das GedÃ¤chtnis der Lebenden - und besonders der HÃ¤ndler -  hat.");

        if ($session['user']['reputation']<=-40) output("`n`n\"`7Wegen der Unehrenhaftigkeit deines Lebens kann ich dir nicht erlauben, vorzeitig zu den Lebenden zurÃ¼ckzukehren, obwohl du mir gute Dienste geleistet hast.`)\"");
...
        output("`\$Ramius`) spricht: \"`7Ich bin nicht wirklich beeindruckt von deinen BemÃ¼hungen, aber einen kleinen Gefallen werde ich dir gewÃ¤hren. FÃ¼hre meine Arbeit fort und ich kann dir vielleicht mehr meiner Kraft anbieten.`)\""); 
...
        output("`\$Ramius`) spricht: \"`7Ich bin von deinen BemÃ¼hungen noch nicht beeindruckt. FÃ¼hre meine Arbeit fort und wir kÃ¶nnen weiter reden.`)\"");
...
    output("`\$Ramius`)' GelÃ¤chter lÃ¤sst den Boden erbeben. \"`7Du verzichtest fÃ¼r ein paar Punkte auf das Leben? Bitte, soll mir nur Recht sein.`)\" Mit diesen Worten gibt er deiner Bitte nach.`nDu bekommst 5 Donationpoints.");

    $session['user']['deathpower']-=100;

    $session['user']['donation']+=5; 

    addnav("ZurÃ¼ck zum Mausoleum","graveyard.php?op=enter");
...
    output("`\$Ramius`) raunzt dir genervt entgegen, dass du noch `^".date("G\\h, i\\m, s\\s",strtotime("1980-01-01 00:00:00 + $realsecstotomorrow seconds"))."`) hier bleiben musst, wenn du nicht weiter fÃ¼r ihn arbeiten willst.");
...
    output("`\$Ramius`)  ist von deinen Aktionen beeindruckt und gewÃ¤hrt dir die Macht, einen Feind heimzusuchen.`n`n");
...
        output("`\$Ramius`) denkt, du solltest die Zahl derer, die du heimsuchen willst, etwas einschrÃ¤nken.");
...
    addnav("M?ZurÃ¼ck zum Mausoleum","graveyard.php?op=enter");
...
            output("Diese Person wurde bereits heimgesucht. WÃ¤hle eine andere");
...
                    output("Die Heimsuchung von `7{$row['name']}`) lÃ¤uft richtig gut. Leider schlÃ¤ft dein Opfer tief und fest und bekommt von deiner Anwesenheit absolut nichts mit.");

                    break;

                case 2:

                    output("Du machst dich zur Heimsuchung von `7{$row['name']}`) bereit, stolperst aber Ã¼ber deinen Geisterschwanz und landest flach auf deinem .... Ã¤hm ... Gesicht.");

                    break;

                case 3:

                    output("Du willst `7{$row['name']}`) im Schlaf heimsuchen, doch dein Opfer dreht sich nur im Bett um und murmelt etwas von 'nie wieder WÃ¼rstchen so kurz vor dem Schlafengehen'.");
...
                    output("Du versuchst `7{$row['name']}`) zu erschrecken, siehst dich dabei im Augenwinkel selbst im Spiegel und gerÃ¤tst in Panik, weil du einen Geist gesehen hast!");
