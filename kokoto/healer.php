...
if ($config['healer'] || $session['user']['acctid']==getsetting("hasegg",0)) $yuvi = 1;

if ($yuvi) {
	page_header("Yuvileés Hütte");
	output("`#`b`cYuvileés Hütte`c`b`n");
} else {
...
if ($session['user']['guildID']!=0) {
    ...
    if (isset($MyGuild)) {
        $guilddiscount = $MyGuild['HealDiscount'];
        if ( $guilddiscount > 0 ) $guilddisc = ( 1  ($guilddiscount100) );
        $cost=$cost$guilddisc;
        $cost = round($cost,0);
    } else {
        // Error
...
if ($session['user']['jailtime'] > 0) {redirect("jail.php");}
if ($_GET['op']==''){
	...
	if ($session['user']['hitpoints'] < $session['user']['maxhitpoints']){
		if ($yuvi) {
			...
			output('"`6Sehen kann ich dich. Bevor du sehen konntest mich, hmm?`3" bemerkt das alte Wesen. "`6Ich kenne dich, ja; Heilung du suchst. Bereit zu heilen dich ich bin, wenn bereit zu bezahlen du bist.`3"`n`n"`5Oh-oh. Wieviel?`3" fragst du, bereit dich von diesem stinkenden alten Dings ausnehmen zu lassen.`n`nDas alte Wesen pocht dir mit einem knorrigen Stab auf die Rippen: "`6Für dich... `$`b'.$cost.'`b`6 Goldstücke für eine komplette Heilung!!`3". Dabei krümmt es sich und zieht ein Tonfläschchen hinter einem Haufen Schädel hervor. Der Anblick dieses Dings, das sich über den Schädelhaufen krümmt, um das Fläschchen zu holen, verursacht wohl genug geistigen Schaden, um eine größere Flasche zu verlangen.  "`6Ich auch habe einige - ähm... "günstigere" Tränke im Angebot.`3" sagt das Wesen, während es auf  einen verstaubten Haufen zerbrochener Tonkrüge deutet. "`6Sie werden heilen einen bestimmten Prozentsatz deiner `iBeschädigung`i.`3"');
                ...
                output('`n`n`PFalls du in einer Gilde bist die Vergünstigungen auf die Heilung bietet, werden diese Vergünstigungen auch hier angewendet!');
		addnav("Heiltränke");
		addnav("`^Komplette Heilung`0","healer.php?op=buy&pct=100");
		for ($i=90;$i>0;$i-=10){
			addnav("$i% - ".round($cost$i100,0)." Gold","healer.php?op=buy&pct=$i");
		}
		addnav("`bZurück`b");
		...
		if ($yuvi) {
			output("`3Yuvileé untersucht dich sehr sorgfältig. \"`6Nun, du hast diesen leicht eingewachsenen Zehennagel hier, aber ansonsten bist du vollkommen gesund. `^Ich`6 glaube, du bist nur hier her gekommen, weil du einsam warst.`3\", kichert sie.`n`nDu erkennst, dass sie Recht hat und dass du sie von ihren anderen Patienten abhältst. Deswegen gehst du zurück in den Wald.");
		} else {
	...
	$newcost=round((int)$_GET['pct']$cost100,0);
	if ($session['user']['gold']>=$newcost){
		$session['user']['gold']-=$newcost;
		//debuglog("spent $newcost gold on healing");
		$diff = round(($session['user']['maxhitpoints']$session['user']['hitpoints'])(int)$_GET['pct']100,0);
		$session['user']['hitpoints'] += $diff;
		if ($yuvi) {
			output("`3Du erwartest ein fauliges Gesöff und kippst den Trank herunter, aber als die Flüssigkeit dir den Rachen hinunter läuft, schmeckst du Zimt, Honig und irgendetwas fruchtiges. Du fühlst Wärme durch deinen Körper strömen und deine Muskeln fangen an, sich von selbst zusammenzufügen. Mit klarem Kopf und wieder bei bester Gesundheit gibst du Yuvileé ihr Gold und verlässt die Hütte in Richtung Wald.");
		} else {
		...
		if ($yuvi) {
			output("`3\"`6Tss, tss!`3\", murmelt Yuvileé. \"`6Vielleicht solltest du erstmal zur Bank gehen und wiederkommen, sobald du `b`\$$newcost`6`b Gold hast?`3\"`n`nDu fühlst dich ziemlich blöde, weil du ihre kostbare Zeit vergeudet hast.`n`n\"Oder vielleicht wäre ein billigerer Trank besser für dich?`3\", schlägt sie freundlich vor.");
		} else {
