...
if ($_GET['op'] == "wkkauf"){
	if ($session['user']['gold']<$wkcost){
		...
		output(' Mit einem hohen Kichern flöst sie dir drei Portionen dieser braunen Brühe ein. Obwohl du gerade noch unfähig warst, dich aus dem Griff der Hexe zu befreien, fühlst du dich plötzlich wieder stark und bereit, einen weiteren Gegner im Wald zu bekämpfen.`nDu blickst dich noch einmal zu der immer noch kichernden Hexe um, aber die beugt sich schon wieder über ihren Punsch, ohne dir weitere Beachtung zu schenken. So gehst du zurück in den Wald.`n`n');
	}
	forest(true);
	...
	if ($session['user']['gold']<$spcost){
		...
		if ($handle = opendir("special")){
			  ...
			  while (false !== ($file = readdir($handle))){
				  if (strpos_c($file,".php")>0){
			  	  	// Skip the darkhorse if the horse knows the way
				  	if ($session['user']['hashorse'] > 0 && $playermount['tavern'] > 0 && strpos_c($file, "darkhorse") !== false) continue;
...
$HTTP_GET_VARS['op']=$y;
	...
	output('`!Die Hexe nimmt deinen Edelstein und holt eine Puppe aus einer Truhe in der Ecke, die genauso aussieht wie dein Meister. Sie sticht der Puppe eine krumme, rostige Nadel in den Kopf und sagt: "`%Gehe ruhig zu deinem Meister. Du hast heute eine zweite Chance, ihn zu schlagen. Es muß aber bald geschehen und bereite dich gut vor!`!"`nDu weißt nicht, ob jetzt tatsächlich der Meister oder du selbst verwirrt sein soll. Auf jeden Fall aber hast du wieder den Mut, deinen Meister heute doch noch einmal herauszufordern.`n`n');
	$session['user']['gems']--;
	$session['user']['seenmaster']=0;
	forest(true);
} else if ($_GET['op'] == 'drachen'){
	output('`!Du nimmst 3 deiner schwer verdienten Edelsteine und streckst sie der Hexe auf der flachen Hand entgegen. Die Hexe nimmt deine Hand und drückt so fest zu, daß dir schwindelig wird."`%Hiermit erhältst du die Möglichkeit, des Drachen erneut aufzuspüren. Doch diesmal mache es richtig!`!" Sie läßt deine Hand los und die Edelsteine sind verschwunden.`nDu kannst deinen letzten Waldkampf jetzt dem Kampf gegen den Drachen widmen...`n`n');
	$session['user']['gems']-=3;
	...
	output('`!"`%Soso, der Barde will nicht mehr für dich singen. Hättest du ihm diesen Edelstein gegeben statt mir, hätte er sicher gesungen. Weißt du was? Ich werde ihm diesen Edelstein vor die Füße zaubern und ihn wissen lassen, daß er von dir ist. So wie ich ihn kenne, steckt er ihn sich in die löchrige Hosentasche und verliert ihn in der Kneipe wieder ... aber was solls.`!" Damit legt die Hexe den Edelstein auf den Tisch und schüttet etwas von ihrem Punsch darüber. "`%Schon gut, du kannst gehen.`!" sagt sie noch zu dir und während du dich Richtung Wald umdrehst, siehst du den Edelstein verschwinden... `n`n');
	$session['user']['gems']--;
	$session['user']['seenbard']=0;
	forest(true);
} else if ($_GET['op'] == 'lotto'){
	output('`!"`%Nach schnellem Reichtum steht dir der Sinn? Weshalb verpulverst du dann deine Edelsteine auf diese Weise? Nunja, dein alter Lottoschein ist ungültig, du kannst dein Glück nochmal versuchen. Aber jammer mir nicht die Ohren voll, wenn es nicht klappt. Den Edelstein geb ich nicht wieder her!`!" Die Hexe wendet sich von dir ab, ohne ein weiteres Wort zu sagen. `nRichtung Wald umdrehst, siehst du den Edelstein verschwinden... `n`n');
	$session['user']['gems']--;
	$session['user']['lottery']=0;
	forest(true);
} else if ($_GET['op'] == 'freeale'){
	output('`!Du erzählst der Hexe davon, dass Thalion dir bei deiner Freiale-Politik einen Strich durch die Rechnung macht. Sie nimmt dir deine 350 Gold ab und sagt: "`%Jaja, der olle Thalion.  Ich glaube, er hat beim Zwergenweitwurf gerade einen Zwerg an den Schädel bekommen und kann sich nicht mehr an dich erinnern.`!" Dabei schnippt sie  einen Kieselstein vom Tisch in Richtung einer Puppe, die dir merkwürdig vertraut vorkommt, und trifft sie am Kopf. "`%So, und jetzt verschwinde, bevor ich mit dir auch sowas mach.`!" `n`n');
	$session['user']['gold']-=350;
	$session['user']['gotfreeale']=0;
	forest(true);
} else if ($_GET['op'] == 'cursep'){
	if ((int)$_GET['id']>'' && (int)$_GET['pid']>''){
		$sql="SELECT * FROM items WHERE id=".(int)$_GET['id'];
		$row = db_fetch_assoc(db_query($sql));
		$goldcost=$row['gold']$session['user']['level'];
		$klappt=e_rand(1,10);
		$sql="SELECT * FROM items WHERE owner=".(int)$_GET['pid']." AND name='".$row['name']."' AND class='Fluch'";
		$result2=db_query($sql);
		if (db_num_rows($result2)>0){
			output('`!Die Hexe sucht aus einem Regal voller Puppen eine heraus, die wie dein Opfer aussieht. Sie stutzt kurz, dann dreht sie sich zu dir um: "`%Jaja, es ist alles in Ordnung. Dein Opfer leidet bereits unter '.$row['name'].'`%. Behalte dein Geld. Einen schönen Tag noch. Und jetzt ... lass mich alleine.`!"');
			...
			systemmail((int)$_GET['pid'],"`\$Verflucht!`0","`9{$session['user']['name']}`9 hat dir den Fluch `T'".$row['name']."'`9 angehext!`n".$row['description']);
		}
		db_query($sql);
		$session['user']['witch']++;
		forest(true);
		...
		if (!$_GET['limit']){
			...
			$page=(int)$_GET['limit'];
			addnav('Vorherige Seite',"hexe.php?op=cursep&id=$id&limit=".($page1)."&search=".$_POST['search']);
		}
		$limit="".($page$ppp).",".($ppp1);
		...
		$result = db_query($sql);
		if (db_num_rows($result)>$ppp) addnav("Nächste Seite","hexe.php?op=cursep&id=$id&limit=".($page1)."&search=".$_POST['search']);
		output("`%Und wer darf das Opfer sein?`n`!<form action='hexe.php?op=cursep&id=$id' method='POST'>Nach Name suchen: <input name='search' value='".$_POST['search']."'><input type='submit' class='button' value='Suchen'></form>",true);
		allownav("hexe.php?op=cursep&id=$id");
		...
		for ($i=0;$i<db_num_rows($result);$i++){
			...
			output("<tr class='".($i2?"trlight":"trdark")."'><td><a href='hexe.php?op=cursep&id=$id&pid=".$row['acctid']."'>",true);
			output($row['name']);
			rawoutput('</a></td><td>');
			output($row['level']);
			output("</td><td align='center'><img src='images/".($row['sex']?"female":"male").".gif'></td></tr>",true);
			allownav("hexe.php?op=cursep&id=$id&pid=".$row['acctid']);
		}
		rawoutput('</table>');
		...
		$result = db_query($sql);
		output("`!Die alte Hexe reibt sich die knochigen Finger. \"`%Sehr schön, sehr schön! Womit kann ich deinen größten Feind quälen?`!\" Sie erzählt dir, welche Flüche sie gerne mal an jemandem ausprobieren würde. `nWähle ein Schicksal für dein Opfer:`4");
		rawoutput('<ul>');
		for ($i=0;$i<db_num_rows($result);$i++){
			$row = db_fetch_assoc($result);
			$goldcost=$row['gold']$session['user']['level'];
			output("<li><a href='hexe.php?op=cursep&id=".$row['id']."'>".$row['name']."</a>`4: ".htmlspecialchars($row['description'])."`4`nDauer: ".($row['hvalue']>0?"".$row['hvalue']." Tage":"unbegrenzt")."`nPreis: `^$goldcost`4 Gold, `#".$row['gems']."`4 Edelsteine.`n`n",true);
			allownav("hexe.php?op=cursep&id=".$row['id']);
		}
		rawoutput('</ul>');
		$session['user']['witch']--;
		addnav('Lieber nicht','hexe.php');
	...
	$result = db_query($sql);
	output('`!Die alte Hexe murmelt ein paar unverständliche Worte, bevor sie dir mit leicht arroganter Miene wie ein Arzt nach erfolgreicher Diagnose erzählt, was du wieder alles falsch gemacht hast."`%Also, dich von deinen Übeln zu befreien wird dich eine Kleinigkeit kosten. ');
	for ($i=0;$i<db_num_rows($result);$i++){
		$row = db_fetch_assoc($result);
	...
	if ($session['user']['gold']<$row['gold'] || $session['user']['gems']<$row['gems']){
		output('"`%Aha! Dachte ichs mir doch. Ich soll dich von einem Fluch befreien und du willst nicht einmal dafür bezahlen? Scher dich hier raus, bevor ich dir noch einen schlimmeren Fluch dazu hexe!`!"`nOhne dich wehren zu können, schwebst du nach draußen und die Tür der Hexenhütte knallt hinter dir ins Schloss. Tja, du hättest vielleicht genug Kleingold mitnehmen sollen.`n`n');
		forest();
	}else{
		output(' Schließlich scheint die Hexe gefunden zu haben, was sie offenbar gesucht hat, lässt '.$row['gold'].' Gold und '.$row['gems'].' Edelsteine von dir in ihrer Schatzkiste verschwinden und schenkt dir keine weitere Beachtung mehr. Gerade, als du den Mund zum Protestieren öffnen willst, fühlst du die Veränderung: `bDer Fluch wurde aufgehoben!`b. Glücklich verlässt du die Hütte der Hexe.');
	...
	if ($session['user']['witch']<getsetting("witchvisits",3)){
		output("`n\"`%Na, mein".($session['user']['sex']?"e Kleine":" Kleiner")."? Hast du dich verlaufen? Oder kann ich sonst etwas für dich tun? Du siehst erschöpft aus! Wenn du mir`^  $wkcost `%von deinem Gold gibst, lasse ich dich von meinem Aufputschpunsch kosten und du könntest noch ein paar Monster mehr erschlagen. ");
		...
		if ($session['user']['gotfreeale'] && $session['user']['gold']>350) addnav('Thalion verwirren (350 Gold)','hexe.php?op=freeale');
		output("`!\"");
