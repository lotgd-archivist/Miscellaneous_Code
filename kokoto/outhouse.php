...
if ($_GET['op'] == "pay"){
	...
	output("`7Du bezahlst die $cost Gold an den Klo-Gnom für die Erlaubnis, das private Klo zu benutzen.`nDies ist das sauberste Plumpsklo im ganzen Land!`nDer Klopapier-Gnom sagt dir noch, dass du einfach fragen sollst, wenn du noch etwas brauchst.`n");
	...
	$session['user']['usedouthouse'] = 1;
	output("`2Der furchtbare Gestank treibt dir Tränen in die Augen und deine Nasenhaare kräuseln sich!`nNachdem er sich die Nase damit geputzt hat, überreicht dir der Klopapier-Gnom ein Blatt einlagiges Klopapier.`nDu entschliesst dich, dieses Teil lieber nicht zu benutzen, nachdem du seine Hände gesehen hast.`n`nBeinahe rutschst du in das große Loch in der Mitte des Raumes, während du " . ($session['user']['sex']?"darüber in die Hocke gehst":"dich darüber stellst") . ". Der Klopapier-Gnom beobachtet dich bei deinem Geschäft sehr genau.`nDu machst so schnell du kannst, denn so arg lange kannst du die Luft nicht anhalten.`n");
	if ($session['user']['prefs']['nosounds']==0) output("<embed src=\"media/ogpissd.wav\" width=10 height=10 autostart=true loop=false hidden=true volume=100>",true);
	...
	output("`2Deine Hände sind schmutzig und stinken!`nHat dir deine Mutter denn gar nichts beigebracht?`n");
	...
	if ($session['user']['usedouthouse'] == 0){
		output('`2Das Dorf verfügt über 2 Klohäuschen, die wegen der monsterabwehrenden Wirkung des Gestanks etwas ausserhalb im Wald stehen.`n`nIn typischer Klassenmanier gibt es ein bevorzugtes und ein heruntergekommenes Häuschen. Du hast die Wahl!`0`n`n');
		addnav("Toiletten");	
		if ($canpay){
		...
		if (getsetting("dailyspecial","Keines")=="Orkburg"){
			...
			output("`2Neben den Klohäuschen findest du, halb von Gras bewachsen, einen uralten, abgebrochenen Wegweiser am Boden. Er scheint aber noch immer in die Richtung zu zeigen, für die er einst gedacht war. Die Aufschrift lautet \"`tOrkburg`2\". Du könntest diese Chance nutzen...");
		}
	}else{
