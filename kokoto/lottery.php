...
switch($_GET['op']){
	...
	if($session['user']['lottery']<1){
		...
		output('`^Du kannst jeden Tag ein Los kaufen und dein Glück damit versuchen, den Jackpot zu knacken. Um zu gewinnen, muss die Zahl auf deinem Los mit der Gewinn-Nummer übereinstimmen. Ein Los kostet dich '.$cost.' Gold. Je mehr Leute Lose kaufen, umso höher steigt der Jackpot. Sobald der Jackpot geknackt ist, wird eine neue Gewinn-Nummer festgelegt. `n`n `7     `iJackpot: `^'.$jackpot.'`7 Gold!`i`n `7     `iGewinn-Nummer: `@'.$winnumber.'`i`n`n');
	}else{
	...
	if($session['user']['gold']<$cost){
		...
		addnav("Zurück zur Kneipe","inn.php");
		$session['user']['lottery'] = mt_rand(100,500);
