...
// The vendor Erich sells furniture for houses and buys items found at beaten monsters in the forest.
...
if ($_GET['op']=="buy"){ // Wig-Wam Bam
	if (!$_GET['id']){
		...
		output("`qStolz präsentiert dir der Händler `tErich`q seinen Wagen. Zu jedem der seltsamen Gegenstände, Artefakte und Zauber scheint er eine kleine Geschichte zu kennen. Dabei scheint er auffällig oft darauf hinzuweisen, dass viele Leute, von denen er etwas gekauft hat, den wahren Wert dieser Dinge nicht zu kennen scheinen.`n ");
		$ppp=25; // Player Per Page to display
		if (!$_GET['limit']){
			$page=0;
		}else{
			$page=(int)$_GET['limit'];
			addnav("Vorherige Waren","vendor.php?op=buy&sorti=$sorti&limit=".($page1));
		}
		$limit="".($page$ppp).",".($ppp1);
		$sql="SELECT * FROM items WHERE owner=0 AND (class='Möbel.Prot') ORDER BY $sorti ASC LIMIT $limit";
		$result=db_query($sql);
		if (db_num_rows($result)>$ppp) addnav("Mehr Waren","vendor.php?op=buy&sorti=$sorti&limit=".($page1));
		if (db_num_rows($result)){
rawoutput("<table border='0' cellpadding='2' cellspacing='2'> <tr class='trhead'><td><b><a href='vendor.php?op=buy&sorti=name&limit=".$_GET['limit']."'>Name</a></b></td><td><b><a href='vendor.php?op=buy&sorti=".urlencode("gems ASC,gold")."&limit=".$_GET['limit']."'>Preis</a></b></td><td><b><a href='vendor.php?op=buy&sorti=".urlencode("class DESC,name")."&limit=".$_GET['limit']."'>Klasse</a></b></td></tr>");
			allownav("vendor.php?op=buy&sorti=name&limit=".$_GET['limit']);
			allownav("vendor.php?op=buy&sorti=".urlencode("gems ASC,gold")."&limit=".$_GET['limit']);
			allownav("vendor.php?op=buy&sorti=".urlencode("class DESC,name")."&limit=".$_GET['limit']);
			for ($i=0;$i<db_num_rows($result);$i++){
				...
				allownav("vendor.php?op=buy&id=".$row['id']);
			}
			rawoutput('</table>');
		
		} else {
			output('`qDa `tErich `qheute schon ein gutes Geschäft gemacht hat, will er sich leider nicht von seinen verbliebenen Sachen trennen. Enttäuscht schlenderst du zurück zum Dorfplatz.');
		}
		...
		if ($session['user']['gems']<$row['gems'] || $session['user']['gold']<$row['gold']){
			...
			$sql="UPDATE items SET owner=".$session['user']['acctid']." WHERE id=".(int)$_GET['id'];
	...
	if (!$_GET['id']){
		...
		$result=db_query($sql);
		if (db_num_rows($result)){
			output("<table border='0' cellpadding='0'><tr class='trhead'><td>`bName`b</td><td>`bPreis`b</td></tr>",true);
			for ($i=0;$i<db_num_rows($result);$i++){
			  	$row = db_fetch_assoc($result);
				$bgcolor=($i2==1?"trlight":"trdark");
				...
				allownav("vendor.php?op=sell&id=".$row['id']);
			}
			rawoutput('</table>');
		
		} else {
			output("Du hast aber nichts, was `tErich`q interessieren würde. Enttäuscht schlenderst du zurück zum Dorfplatz.");
		}
	}else{ // Hell Raiser
		...
		if ($row['class']=="Möbel" || $row['class']=="Beute" || $row['class']=="Waffe" || $row['class']=="Schmuck" AND $row['name']=="Elfenkunst") $sql="DELETE FROM items WHERE id=".(int)$_GET['id'];
