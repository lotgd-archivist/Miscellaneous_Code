...
if ($_GET['op']=="" && $session['user']['turns']>0){
	output("`rDu betrittst den Geschenkeladen und siehst eine Menge einzigartiger Gegenstände.`nEin".($session['user']['sex']?" junger Mann":"e junge Frau")." steht hinter der Ladentheke und lächelt dich sanft an.`nEin Schild an der Wand verspricht \"`iGeschenkverpackung und Lieferung frei.`i\"`n");
	...
	rawoutput("<br /><ul><a href=\"newgiftshop.php?op=send&op2=gift1\">Tulpe</a><br />");
	allownav("newgiftshop.php?op=send&op2=gift1");
	if ($session['user']['gold'] > 10){
		rawoutput("<a href=\"newgiftshop.php?op=send&op2=gift2\">Rose - 10 Gold</a><br />");
		allownav("newgiftshop.php?op=send&op2=gift2");
	}
	if ($session['user']['gold'] > 20){
		rawoutput("<a href=\"newgiftshop.php?op=send&op2=gift3\">Strauß Rosen - 20 Gold</a><br />");
		allownav("newgiftshop.php?op=send&op2=gift3");
	}
	if ($session['user']['gold'] > 40){
		rawoutput("<a href=\"newgiftshop.php?op=send&op2=gift4\">Pralinen - 40 Gold</a><br />");
		allownav("newgiftshop.php?op=send&op2=gift4");
	}
	if ($session['user']['gold'] > 60){
		rawoutput("<a href=\"newgiftshop.php?op=send&op2=gift5\">Freundschaftsbändchen - 60 Gold</a><br />");
		allownav("newgiftshop.php?op=send&op2=gift5");
	}
	if ($session['user']['gold'] > 100){
		rawoutput("<a href=\"newgiftshop.php?op=send&op2=gift6\">Freundschaftsanhänger - 100 Gold</a><br />");
		allownav("newgiftshop.php?op=send&op2=gift6");
	}
	if ($session['user']['gold'] > 200){
		rawoutput("<a href=\"newgiftshop.php?op=send&op2=gift7\">Halskette - 200 Gold</a><br />");
		allownav("newgiftshop.php?op=send&op2=gift7");
	}
	if ($session['user']['gold'] > 500){
		rawoutput("<a href=\"newgiftshop.php?op=send&op2=gift8\">Plüschdrachen - 500 Gold</a><br />");
		allownav("newgiftshop.php?op=send&op2=gift8");
	}
        	
	if ($session['user']['gold'] > 1000){
		rawoutput("<a href=\"newgiftshop.php?op=send&op2=gift9\">Beutel Heilkräuter - 1000 Gold</a><br />");
		allownav("newgiftshop.php?op=send&op2=gift9");
	}
	if ($session['user']['gold'] > 1500){
	rawoutput("<a href=\"newgiftshop.php?op=send&op2=gift10\">Drachenei - 1500 Gold</a><br />");
		allownav("newgiftshop.php?op=send&op2=gift10");
	}
	if ($session['user']['gold'] > 2000){
		rawoutput("<a href=\"newgiftshop.php?op=send&op2=gift11\">Goldenes Amulett - 2000 Gold</a><br />");
		allownav("newgiftshop.php?op=send&op2=gift11");
	}
	if ($session['user']['gold'] > 3000){
		rawoutput("<a href=\"newgiftshop.php?op=send&op2=gift12\">Seltsamer Schädel - 3000 Gold</a><br />");
		allownav("newgiftshop.php?op=send&op2=gift12");
	}
	rawoutput('</ul>');
	addnav("Sonstiges");
...
if ($_GET['op']=="send"){
	...
	if (!$_GET['limit']){
		...
		$page=(int)$_GET['limit'];
		addnav("Vorherige Seite","newgiftshop.php?op=send&op2=$gift&limit=".($page1)."&search=".$_POST['search']);
	}
	$limit="".($page$ppp).",".($ppp1);
	$sql = "SELECT login,name,level,sex,acctid FROM accounts WHERE ".$search." locked=0 AND acctid<>".$session['user']['acctid']." ORDER BY login,level LIMIT $limit";
	$result = db_query($sql);
	if (db_num_rows($result)>$ppp) addnav("Nächste Seite","newgiftshop.php?op=send&op2=$gift&limit=".($page1)."&search=".$_POST['search']);
	output("`rWem willst du das Geschenk schicken?`n`n<form action='newgiftshop.php?op=send&op2=$gift' method='POST'>Nach Name suchen: <input name='search' value='".$_POST['search']."'><input type='submit' class='button' value='Suchen'></form>",true);
	allownav("newgiftshop.php?op=send&op2=$gift");
	...
	for ($i=0;$i<db_num_rows($result);$i++){
		...
		output("<tr class='".($i2?"trlight":"trdark")."'><td><a href='newgiftshop.php?op=send2&op2=$gift&name=".$row['acctid']."'>",true);
		output($row['name']);
		rawoutput('</a></td><td>');
		output($row['level']);
		output("</td><td align='center'><img src='images/".($row['sex']?"female":"male").".gif'></td></tr>",true);
		allownav("newgiftshop.php?op=send2&op2=$gift&name=".$row['acctid']);
	}
	rawoutput('</table>');
...
if ($_GET['op']=="send2"){
	...
	if ($_GET['op2']=="gift5"){
		...
		db_query("INSERT INTO items (name,owner,class,gold,description) VALUES ('Freundschaftsbändchen',$name,'Geschenk',11,'Ein Freundschaftsbändchen von ".$session['user']['name']."')");
		$session['user']['gold']-=60;
	}
	if ($_GET['op2']=="gift6"){
		$gift="Freundschaftsanhänger";
		db_query("INSERT INTO items (name,owner,class,gold,description) VALUES ('Freundschaftsanhänger',$name,'Geschenk',15,'Ein Freundschaftsänhänger von ".$session['user']['name']."')");
		$session['user']['gold']-=100;
	}
	if ($_GET['op2']=="gift7"){
		$gift="Halskette";
		db_query("INSERT INTO items (name,owner,class,gold,description) VALUES ('Halskette',$name,'Geschenk',30,'Diese Halskette hat dir ".$session['user']['name']." geschenkt.')");
		$session['user']['gold']-=200;
	}
	if ($_GET['op2']=="gift8"){
		$gift="Plüschdrachen";
		$effekt="`RDer ist ja soooooo süüüüüüüüüüüssss!!!`0";
		db_query("INSERT INTO items (name,owner,class,gold,description) VALUES ('Plüschdrachen',$name,'Geschenk',60,'`REin `@Grüner Drachen`R aus Plüsch zum Kuscheln. Der ist von ".$session['user']['name'].".')");
	...
	if ($_GET['op2']=="gift9"){
		...
		db_query("INSERT INTO items (name,owner,class,gold,description,hvalue,buff) VALUES ('Beutel Heilkräuter',$name,'Geschenk',100,'$effekt',1,'$buff')");
	...
	if ($_GET['op2']=="gift11"){
		...
		db_query("INSERT INTO items (name,owner,class,gold,description,hvalue,buff) VALUES ('Goldenes Amulett',$name,'Geschenk',200,'$effekt',3,'$buff')");
		$session['user']['gold']-=2000;
	}
	if ($_GET['op2']=="gift12"){
		$gift="Seltsamer Schädel";
		$gefallen=mt_rand(2,10);
		$effekt="Du untersuchst dieses merkwürdige Geschenk genauer. Dabei rutscht es dir aus der Hand und zerplatzt am Boden in 1000 Stücke. Doch eine seltsame Kraft wird frei, die dir $gefallen Gefallen bei Luzifer bringt!";
...
if ($_GET['op']=="cake"){ // this part was done for claymore's birthday :)
	if (!isset($_POST['throw'])){
		...
		$geb = str_replace_c($find,$replace,getsetting('gamedateformat','%Y-%m-%d'));
		...
		if ($wer=="" && db_num_rows($result)<=0){
			...
			for ($i=0;$i<db_num_rows($result);$i++){
				$row = db_fetch_assoc($result);
				output("<option value='".rawurlencode($row['login'])."'>".preg_replace_c("'[`].'","",$row['name'])."</option>",true);
			}
			output("</select>`n`n<input type='submit' class='button' value='Torte kaufen'></form>",true);
			allownav("newgiftshop.php?op=cake");
		}
	}else{
		...
		if (db_num_rows($result)<=0){
			...
			$buff=serialize($buff);
			output("`rDu suchst dir eine $wie Torte für `&".$row['name']."`r aus. Damit bewaffnet machst du dich auf die Suche nach `&".$row['name']."`r und als du ".($row['sex']?"sie":"ihn")." gefunden hast...`n`n `&`b`c<font size='5'>*PLATSCH*</font>`b`c`r`n",true);
			output("`r...wirfst du sie ".($row['sex']?"ihr":"ihm")." mitten ins Gesicht und brüllst ".($row['sex']?"ihr":"ihm")." ein fröhliches `b`RHAPPY BIRTHDAY`b`r entgegen.`n`n `&".$row['name']."`r wird an den Tortenresten sicher noch lange ".($row['sex']?"ihre":"seine")." Freude haben. ".($row['sex']?"Sie":"Er")." trägt damit die klebrigen Reste von ".$item['hvalue']." Torten an sich herum." );
