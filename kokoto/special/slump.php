...
output('`6Plötzlich stürzt du - waren es deine etwas ungeschickten Füße, oder doch ein Ast?`n Das spielt jetzt auch keine Rolle mehr, denn nun liegst du am Boden und bemerkst, dass dein Goldbeutel zerrissen und das ganze Gold auf dem Boden verteilt ist.`n`n`0');
...
// oder kein juengster spieler und drachenkaempfer in den settings
}else if (($rownew == "" && $rowdk == "") ||($session['user']['name'] == $rownew['name'])||($session['user']['name'] == $rowdk['name'])){
	output("`^Schnell sammelst du dein Gold wieder ein und gehst deinen Weg weiter.`n`n`0");

}else if($rownew == "" && $rowdk != ""){ //kein juengster spieler - nur juengster drachenkaempfer

	output('`6Schnell willst du das Gold wieder einsammeln, doch bei ein paar Goldstücken ist jemand schneller.`n`n `3Du denkst: "`#Der hat doch gerade den `@grünen Drachen `#erfolgreich bekämpft  - wie war der Name noch gleich?`n Ah,`0 '.$rowdk['name'].' `#muss es gewesen sein.`3"`n`n`0 `6Jetzt ist es zu spät, so flink er beim Aufheben war, so flink ist er auch verschwunden.`0`n`n');
	//goldverteilung
...
}else if($rowdk == "" && $rownew != ""){ //kein juengster drachenkaempfer nur juengster spieler
	output('`6Schnell willst du das Gold wieder einsammeln, doch bei ein paar Goldstücken ist jemand schneller.`n`n `3Du denkst: "`#der ist doch gerade neu in dieser Welt  - wie war der Name noch gleich?`n Ah,`0 '.$rownew['name'].' `#muss es gewesen sein.`3"`n`n`0');
	//goldverteilung
	$save = round($session['user']['gold']0.8, 0);
	$lost = round($session['user']['gold']0.2, 0);
	$mailmessage = "`^".$session['user']['name']."`2 stürzte im Wald über einen Ast und verlor dabei ".($session['user']['sex']?"ihr":"sein")." Gold.`n`n"
	...
	output('`6Schnell willst du das Gold wieder einsammeln, doch bei ein paar Goldstücken ist jemand schneller.`n`n Den einen Dieb hast du doch gerade erst hier neu in der Welt gesehen, und der andere hatte eben den `@grünen Drachen `6erfolgreich bekämpft - wie waren ihre Namen noch gleich?`n`n`0 '.$rownew['name'].'`# und `0'.$rowdk['name'].'`# müssen es gewesen sein.`3"`6, denkst du. `n`n Jetzt ist es zu spät, so flink sie beim Aufheben waren, so flink sind sie auch verschwunden.`n`n`0');
	// goldverteilung
