...
if ($_GET['op']=='' || $_GET['op']=='search'){
  output('`#Du wanderst auf der Suche nach etwas zum Bekämpfen ziellos durch den Wald. Plötzlich stehst du mitten auf einem Feld. In der Mitte kannst du einen Steinkreis sehen. Du hast das legendäre Stonehenge gefunden! Du hast die Leute im Dorf über diesen mystischen Ort reden hören, aber du hast eigentlich nie geglaubt, dass es wirklich existiert. Sie sagen, der Kreis hat große magische Kräfte und dass diese Kräfte unberechenbar sind. Was wirst du tun?');
	...
	allownav('forest.php?op=stonehenge');
	allownav('forest.php?op=leavestonehenge');
	$session['user']['specialinc']='stonehenge.php';
}else{
  $session['user']['specialinc']='';
	if ($_GET['op']=='stonehenge'){
	  $rand = mt_rand(1,22);
		output('`#Obwohl du weißt, daß die Kräfte der Steine unvorhersagbar wirken, nimmst du diese Chance wahr. Du läufst in die Mitte der unzerstörbaren Steine und bist bereit, die fantastischen Kräfte von Stonehenge zu erfahren. Als du die Mitte erreichst, wird der Himmel zu einer schwarzen, sternenklaren Nacht. Du bemerkst, dass der Boden unter deinen Füssen in einem schwachen Licht lila zu glühen scheint, fast so, als ob sich der Boden selbst in Nebel verwandeln will. Du fühlst ein Kitzeln, das sich durch deinen gesamten Körper ausbreitet. Plötzlich umgibt ein helles, intensives Licht den Kreis und dich. Als das Licht verschwindet, ');
		switch ($rand){
		  case '1':
		  case '2':
                output('bist du nicht mehr länger in Stonehenge.`n`nÜberall um dich herum sind die Seelen derer, die in alten Schlachten und bei bedauerlichen Unfällen umgekommen sind. Jede trägt Anzeichen der Niedertracht, durch welche sie ihr Ende gefunden haben. Du bemerkst mit steigender Verzweiflung, daß der Steinkreis dich direkt ins Land der Toten transportiert hat! `n`n`^Du wurdest aufgrund deiner dümmlichen Entscheidung in die Unterwelt geschickt.`n Da du physisch dorthin transportiert worden bist, hast du noch dein ganzes Gold.`n Du verlierst aber 5% deiner Erfahrung.`n Du kannst morgen wieder spielen.');
				...
				break;
			case '3':
     				output(' liegt dort nur noch der Körper eines Kriegers, der die Kräfte von Stonehenge herausgefordert hat. `n`n`^Dein Geist wurde aus deinem Körper gerissen!`n Da dein Körper in Stonehenge liegt, verlierst du all dein Gold.`n Du verlierst 10% deiner Erfahrung.`n Du kannst morgen wieder spielen.');
				$session['user']['alive']=false;
				$session['user']['hitpoints']=0;
			...
			case '9':
			case '10':
			    $reward = mt_rand(1, 4); 		// original value: 1,4
