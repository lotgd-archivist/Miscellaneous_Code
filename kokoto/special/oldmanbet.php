...
if($_GET['op']=='yes'){
  if ($session['user']['gold']>0){
        ...
        if ($bet<=0){
            ...
            output("`3\"`!Wie hoch ist ein Einsatz, ".($session['user']['sex']?"junge Dame":"junger Mann")."?`3\"<form action='forest.php?op=yes' method='POST'><input name='bet' id='bet'><input type='submit' class='button' value='Setze'></form><script language='JavaScript'>document.getElementById('bet').focus();</script>",true); // Bravebrain
            ...
            if ($_POST['guess']!==NULL){
              ...
              if ($_POST['guess']==$session['user']['specialmisc']){
                  if ($try == 1) {
                        ...
                        output("`3\"`!AAAH!!!!`3\", schreit der alte Mann, \"`!Du hast die Zahl mit nur $try Versuchen erraten!  Es war `^".$session['user']['specialmisc']."`!!!  Nun, ich gratuliere dir , und denke ich werde jetzt besser gehen...`3\" Er will ins Unterholz verschwinden, doch mit einem flinken Schlag mit ".$session['user']['weapon']." schlägst du ihn KO. Du hilfst ihm dabei, dir die `^$bet`3 Goldmünzen zu geben, die er dir schuldet.");
                    }
	...
	$session['user']['specialmisc']=''; 
                  ...
                  if ($_GET['try']>=6&&((int)$_POST['guess']>=0&&(int)$_POST['guess']<=100)){
                      output("`3Der Mann gluckst vor Freude: \"`!Die Zahl war `^".$session['user']['specialmisc']."`!.`3\" Als der ehrenwerte Bürger, der du bist gibst du dem Mann die `^$bet`3 Goldmünzen, die du ihm schuldest, bereit, von hier zu verschwinden.");
                        $session['user']['specialinc']='';
	$session['user']['specialmisc']=''; 
                        $session['user']['gold']-=$bet;
                        //debuglog("lost $bet gold to the old man in the forest");
                          ...
                          $try--;
                          output("`3Der Alte lacht: \"`!Das ist wie einem Baby ein Schwert abzunehmen, wenn du wirklich glaubst, $_POST[guess] ist zwischen 1 und 100!`3\"`n\"`!Du hast noch `^".(6$try)."`! Versuche übrig.`3\"`n");
                        ...
                        allownav("forest.php?op=yes&bet=$bet&try=$try");
                    }
                }
            }else{
                output("`3Du hast `^$bet`3 Gold gesetzt.  Was schätzt du?");
                ...
                allownav("forest.php?op=yes&bet=$bet&try=1");
            }
        }
    }else{
      output('`3Der alte Mann streckt seinen Stock aus und klopft deinen Goldbeutel ab. "`!Leer?!?! Wie kannst du etwas setzen ohne Gold??`3", brüllt er. Damit dreht er sich mit einem HARUMF um und verschwindet im Unterholz.');
        //addnav("Zurück in den Wald","forest.php");
        $session['user']['specialinc']='';
    }
}else if($_GET['op']=='no'){
