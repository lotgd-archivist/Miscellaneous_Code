...
if ($_GET['op']==''){
	...
	{
		output('Deine Echsensinne sträuben sich vor dem Geruch menschlichen Back-Wahns doch noch kämpfst Du mit Dir. `n`7Wirst Du den Keks nehmen und trotz des Ekels hinunterschlingen?`n`7Oder lässt Du lieber Deine schuppigen Finger davon?');
		addnav('Keks nehmen','forest.php?op=cookie');
		addnav('Den Ort verlassen','forest.php?op=nocookie');
	}
	else
	{
		output('`bNun liegt es an Dir:`b`n `n`7Nimmst Du den Keks, da Du dem Duft einfach nicht wiederstehen kannst?!`n`7Oder lässt Du den Keks liegen wo er ist und läufst zurück in den Wald, da Dir sofort klar ist: `n`^Kekse im Wald? Das ist nicht normal!');
		addnav('Keks nehmen','forest.php?op=cookie');
		addnav('Den Ort verlassen','forest.php?op=nocookie');
	}
...
else
{
  $session['user']['specialinc']='';
	if ($_GET['op']=='cookie')
	{
		if ($session['user']['race'] == 5) $rand = mt_rand(1,6); // Echsen kriegen eher schlechte Kekse
		else $rand = mt_rand(1,7);
		...
		switch ($rand)
			...
			case '1':
				...
				$lifelost = mt_rand(0,$session['user']['hitpoints']5);
			...
			case '3':
				...
				break;
			case '4':
				output('`n`^`beinen Keks mit Orangenfüllung`b!`n`6Der ekelige Geschmack ist durchdringend und Du spuckst sofort alles aus. Die Füllung muss wohl schon schlecht gewesen sein. `^Du fühlst Dich ziemlich schlecht `6und musst Dich erst einmal ein wenig ausruhen, bevor Du weiterziehen kannst.`n`n Du verlierst einen Waldkampf!');
