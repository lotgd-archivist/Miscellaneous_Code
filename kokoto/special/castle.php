...
function castlenav($what, $runden){
	...
	switch ($what){
		...
		addnav('Ork Bar','forest.php?op=bar');
	...
	if ($where == false){
		switch (mt_rand(1,5)){
			...
			switch(mt_rand(1,5)){
				...
				$gems = mt_rand(1,3);
				$gold = mt_rand($session['user']['level']11,$session['user']['level']100);
				...
				break;
				case '3':
				output('`^ Vorwärts!`n`n`%Du erreichst eine verschlossene Tür und drückst sie auf. Dahinter findest du ... `bnichts`b! Ein anderer war wohl schneller als du.`n `nSchwer enttäuscht suchst du einen Ausgang aus den Katakomben.');
				break;
				case '4':
				output('`^ Vorwärts!`n`n`%Du erreichst eine verschlossene Tür und drückst sie auf. Dahinter findest du ... `bnichts`b! Ein anderer war wohl schneller als du.`n Schwer enttäuscht suchst du einen Ausgang aus den Katakomben.`n`nWenigstens hast du durch das Herumirren etwas an `^Erfahrung`% gewonnen.');
				$session['user']['experience']+=$session['user']['experience']0.02;
				break;
				case '5':
				output('`^ Vorwärts!`n`n`%Du erreichst eine verschlossene Tür und drückst sie auf. Dahinter findest du einen besonders schönen Edelstein und steckst ihn ein!`n `n`^Du hast 1 Edelstein gefunden!');
				$session['user']['gems']+=1;
				break;
			}
			output("`n`n`n<a href='forest.php?op=exitlab'>Katakomben verlassen</a>",true);
			addnav('Katakomben verlassen','forest.php?op=exitlab');
			allownav('forest.php?op=exitlab');
			break;
		}
	}
}
...
if ($_GET['op']=='enter'){
	...
	viewcommentary("Courtyard","Frage nach Tipps",10,"fragt");
...
/*********Bar*******/
	...
	$session['user']['specialinc']='castle.php';
	output('Du Betrittst die Ork Bar du siehst was eigentlich nicht verwunderlich sein sollte lauter Orks die dich grimmig anstarren du siehst ein paar Freunde an einem Tisch und gehst zu ihnen: `n`n');
	viewcommentary("orkbar","Reden",10,"Sagt");
	addnav('zurück','forest.php?op=enter');
/********Armourer********/
}elseif ($_GET['op']=='armourer'){
	$session['user']['specialinc']='castle.php';
	if (strstr_c($session['user']['armor'],"High-Grade")){
		output('`#Du betrittst `@Thorics`# Rüstungsladen. Du siehst Thoric in ein Buch vertieft in einer Ecke sitzen. Er schaut auf und wirft sofort einen Blick auf dein(e/n) `^'.$session['user']['armor'].'`#. `%"Schön zu sehen, daß du meine Handwerkskunst trägst."`# murmelt er, bevor er sich wieder seinem Buch zuwendet.');
	}else{
		$newdefence = $session['user']['armordef']  2;
		$cost = $session['user']['armordef']  200;
		output('`#Du betrittst `@Thorics`# Rüstungsladen. Du siehst Thoric in ein Buch vertieft in einer Ecke sitzen. Er schaut auf und wirft sofort einen Blick auf dein(e/n) `^'.$session['user']['armor'].'`#. ');
		if ($cost == 0){
			...
			allownav('forest.php?op=upgradearmour');
...
/********Blacksmith********/
	...
	$session['user']['specialinc']='castle.php';
	if (strstr_c($session['user']['weapon'],"High-Grade")){
		...
		if ($cost == 0){
			...
			allownav('forest.php?op=upgradeweapon');
...
/********Training Room********/
	...
	if ($session['user']['turns']<=0){
		...
		$exp = $session['user']['level']mt_rand(5,12)mt_rand(0,9);
		$totalexp = $exp$trai;
...
/********Well********/
	...
	$castleoptions = unserialize($session['user']['specialmisc']);
	if ($session['user']['gems'] < 1 && (mt_rand(1,10) != 7 || $castleoptions['well_uses']>=1)){
		...
		switch ($rand2){
			...
			switch ($rand1){
				case '1':
				$reward = mt_rand($session['user']['experience']  0.05,$session['user']['experience']  0.1);
				$session['user']['experience'] += $reward;
				output("`6$reward`^ Erfahrungspunkte...");
				break;
				case '2':
				$gold = mt_rand($session['user']['level']10,$session['user']['level']100);
				$session['user']['gold'] += $gold;
...
/********Healer********/
	...
	$session['user']['specialinc']='castle.php';
	output('`#`b`cKalas Beautyshop`c`b`n');
	$loglev = log($session['user']['level']);
	$cost = ($loglev  ($session['user']['maxhitpoints']$session['user']['hitpoints']))  ($loglev10);
	$cost=$cost0.9;
	$cost = round($cost,0);
	...
	output('Du fragst sie, was genau sie damit meint. "`6Also: Heilung dürfte dir klar sein. Mit einer Gesichtsmaske - vorzugsweise aus Gurken von Ivalaine - kann ich dich attraktiver machen. Und wenn du willst, kannst du mir etwas von deiner Schönheit ... überlassen und dafür etwas Erfahrung gewinnen.`3"');
...
/********Guard Fight********/
	...
	if (mt_rand(1,100) <95){
		...
		if ($session['user']['dragonkills']>0){
			...
			allownav('forest.php?op=guardfight');
		}
		allownav('forest.php?op=leave');
