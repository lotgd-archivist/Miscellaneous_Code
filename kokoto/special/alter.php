...
if ($_GET['op']=='dagger'){
  ...
  $session['user']['turns']--;
	if (mt_rand(0,1)==0){
	  $gold = mt_rand($session['user']['level']30,$session['user']['level']90);
	  $gems = mt_rand(1,4);
	  output('`#Du nimmst das Rechenbrett von seinem Platz.  Das Rechenbrett verwandelt sich in einen Beutel voller Gold und Edelsteine!`n`n ');
          output("Du bekommst $gold Goldstücke und $gems Edelsteine!");
		$session['user']['gold']+=$gold;
		$session['user']['gems']+=$gems;
	}else{
		$gold = $session['user']['gold']($session['user']['level']20);
    output('`@`#Du nimmst das Rechenbrett von seinem Platz.  Das Rechenbrett verwandelt sich in einen Beutel voller Gold!`n`n Du bekommst');
  ...
  $session['user']['turns']--;
	if (mt_rand(0,1)==0){
	  ...
	  output('`#Du nimmst das Buch und beginnst darin zu lesen. Das Wissen in diesem Buch hilft dir viel weiter und du legst es an seinen Platz zurück, damit ein anderer auch noch davon profitieren kann.`n`nDu bekommst'); output("$exp Erfahrungspunkte!");
		$session['user']['experience']+=$exp;
	}else{
		$ffights = mt_rand(1,5);
    output('`@`#Du nimmst das Buch und beginnst darin zu lesen.  Das Buch enthält ein Geheimnis, wie du deine heutigen Streifzüge durch den Wald profitabler gestalten kannst.  Du legst das Buch an seinen Platz zurück, damit ein anderer auch noch davon profitieren kann.`n`n'); output("Du bekommst $ffights zusätzliche Waldkämpfe!");
