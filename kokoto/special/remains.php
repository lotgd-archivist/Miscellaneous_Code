...
1.2 bugfix and added some addnews //Tidus (www.kokoto.de)
...
// überarbeitet von Tidus www.kokoto.de (angepasst, verschönert)
*/

if (!isset($session)) exit();
$result = db_query("SELECT name,sex FROM accounts WHERE alive=0 ORDER BY rand(".mt_rand().") LIMIT 1");
...
if ($_GET['op']=='bury'){
	...
	//check_align();
	output('`GDu verbringst einen großen Teil des Tages damit, diesem gefallenen Abenteuerer ein Grab zu Schaufeln und ihm ein ordentliches Begräbnis zukommen zu lassen. Du verzichtest auf Reichtum, wenn du ihn stehlen müsstest - erst Recht von Toten.`n `n`nAußerdem willst du nicht die Rache der Toten hinter dir wissen, zusätzlich zu den Kreaturen des Waldes.`n`n Als du endlich mit dem Grab fertig bist, erscheint eine kleine Fee vor dir. "`#Das war sehr edel, was du getan hast. Ich werde dich dafür belohnen.`G"`n`n');
        if ($tname>''){
        addnews("`P{$session[user][name]}`P hat $tname`P's überresten ein würdiges Begräbnis beschert!");
        }else{
        addnews("`P{$session[user][name]}`P hat edel wie ".($session['user']['sex']?"sie":"er").' ist, die Menschlichen überreste eines Kriegers begraben.');
        }
	$reward = rand(1,12);
	switch ($reward) {
		case '1':
		...
		case '9':
			...
			break;
		default:
			output('Du fühlst dich gut bei dem, was du getan hast.`n Du bekommst mehr Charme!');
			// debuglog("gained two charm for burying a corpse");
			$session['user']['charm']+=mt_rand(1,2);
			break;
	}
	...
	switch (rand(1,8)) {
		...
		case '2':
			...
			$session['user']['specialinc']='';
                       if ($tname>''){ 
                       addnews("`P{$session[user][name]}`P hat $tname`P's Leiche geplündert!!");
                       }else{
                       addnews("`P{$session[user][name]}`P hat eine Leiche geplündert!!");
                       }
		...
		case '4':
			...
			$session['user']['specialinc']='';
                        if ($tname>''){
                        addnews("`P{$session[user][name]}`P hat versucht $tname`P's Leiche zu plündern!!");
                        }else{
                        addnews("`P{$session[user][name]}`P hat versucht eine Leiche zu plündern!!");
                        }
			//debuglog("looted a corpse and found nothing");
			break;
		default:
			output('Als du die Leiche nach Reichtümern durchwühlst, lenkt eine plötzliche Bewegung deine Aufmerksamkeit auf sich. Du machst einen Satz zurück und  siehst, wie sich dieses tote Skelett aufrappelt. Seine leeren Augenhöhlen starren dich mit einem roten Glühen an. Wenn es reden könnte, würde es dich sicher verfluchen, aber schon das kranke Schleifgeräusch von Knochen auf Knochen lässt es dir eiskalt den Rücken runterlaufen. ');
			output('`n`n');
			$badguy = array(
...
if ($tname==''){
output('`GDu stoplerst über etwas. Es sind die skelettierten Überreste dessen, was einst ein Abenteuer wie du gewesen sein könnte. Tatsächlich deutes vieles darauf hin, dass dies die Überreste von jemand sind den du kennst aber du kannst dich gerade nicht an seinen/ihren namen erinnern. Die sterblichen Überreste scheinen unberührt und was immer '.($row2['sex']?"sie":"er").' für Schätze dabei gehabt haben mag, könnte immer noch dort sein. ');
	...
	output("`GDu stoplerst über etwas. Es sind die skelettierten Überreste dessen, was einst ein Abenteuer wie du gewesen sein könnte. Tatsächlich deutes vieles darauf hin, dass dies die Überreste von `%$tname`G sind. Die sterblichen Überreste scheinen unberührt und was immer ".($row2['sex']?"sie":"er")." für Schätze dabei gehabt haben mag, könnte immer noch dort sein. ");
	output('`n`nWas wirst du tun?`n`n');
	addnav('Nach Reichtümern suchen','forest.php?op=desecrate');
	addnav('Die Leiche in Ruhe lassen','forest.php?op=leave');
	if ($session['user']['turns'] > 2) {
...
if ($battle) {
	include("battle.php");
    	...
    	if ($victory){
        		$badguy=array();
            		$session['user']['badguy']='';
            		output('`n`GNach einem heftigen Kampf hast du den Skelettkrieger besiegt. Du hoffst, dass er vielleicht jetzt endlich seine ewige Ruhe finden wird.`n`n Oder wenigstens, bis der nächste Abenteurer auf der Suche nach dem schnellen Geld über seine Leiche stolpert.`n`n');
                        if ($tname>''){
                        addnews("`P{$session['user']['name']}`P hat $tname`P's Leiche geschändet!!");
                        }else{
                        addnews("`P{$session['user']['name']}`P hat eine Leiche geschändet!!");
                        }
			...
			$session['user']['specialinc']='';
               ...
               if ($tname>''){
               addnews("`P{$session['user']['name']}`P hat $tname`P's Leiche geschändet, und wurde mit einem Fluch belegt!!");
               }else{
               addnews("`P{$session['user']['name']}`P hat eine Leiche geschändet, und wurde mit einem Fluch belegt!!");
               }
            		$badguy=array();
            		$session['user']['badguy']='';
		...
		$session['user']['specialinc']='';
		$result=db_query("SELECT * FROM items WHERE class='Fluch.Prot' ORDER BY rand(".mt_rand().") LIMIT 1");
		$row = db_fetch_assoc($result);
