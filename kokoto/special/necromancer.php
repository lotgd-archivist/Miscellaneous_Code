...
$maxhp=6$session['user']['dragonkills']10$session['user']['level']5$dkhp;
...
if ($_GET['op']=='necromancer'){
	...
	switch(mt_rand(1,15)){
		case '1':
      		output('"`#Hat dir deine Mama nicht beigebracht, niemals mit Fremden zu sprechen?`@" Der alte Totenbeschwörer verfällt in ein gemeines Lachen, zieht einen schwarzen  Stab hervor und schwenkt ihn über deinem Kopf. Du spürst einen brennenden Schmerz während deine Seele aus deinem Körper gesogen wird. `n`n`^Dein Geist wurde vom Körper getrennt!`n Dieser heimtückische alte Mann durchsucht deine Leiche und nimmt all dein Gold!`n Du verlierst 10% deiner Erfahrung.`nDu kannst morgen wieder spielen.');
		...
		output('Der Totenbeschwörer schreit dieses letzte Wort, zieht einen schwarzen Zauberstab und schockt deinen Körper mit Schmerz.  Es ist, als ob geschmolzenes Feuer durch deine Knochen, Venen und deine Haut fließt. Gerade als der Tod nach dir greifen will,  hört der Schmerz auf. Du stehst noch vom Schmerz zitternd auf. Der alte Totenbeschwörer ist nirgends in Sicht. Du fühlst, dass du nie wieder so sein wirst, wie du einmal warst...');
		...
		case '10':
		case '11':
	  	output(' Du kommst näher und er murmelt: "`#Einen Edelstein für mich, ja?`@"`n`n Gibst du ihm einen Edelstein?');
		...
		case '14':
		case '15':
		output('Du stehst schon fast vor ihm, als du ein knirschendes Geräusch von rechts hörst. Du drehst dich hin und siehst eine der Dorfwachen Richtung Dorf zurück durch den Wald laufen. Du wendest dich wieder dem alten Mann zu, aber das Geräusch hat ihn wohl verscheucht. Nun wirst du wohl nie erfahren, was er von dir wollte. ');
		break;
	}
	...
	$session['user']['specialinc']='';
  	...
  	if ($session['user']['gems']>0){
		...
		switch (mt_rand(1,8)){
			...
			output("`\$Luzifer`# ein.`@\"`n Der Totenbeschwörer zieht einen schwarzen Zauberstab hervor und tippt dir damit auf den Kopf. ");
			output(' Dann verschwindet er im Wald.`n`n');
			$favor = mt_rand(5, 35);
			output("`^Du erhältst `&$favor`^ Gefallen bei `\$Luzifer`^, dem Gott der Toten.");
			...
			viewcommentary("shade","Sprich",10,"spricht");
			$session['user']['specialinc'] = 'necromancer.php';
			addnav('Zurück in den Wald','forest.php?op=return');
			break;
	  	}
	}else{
		output('`@Du greifst in deinen Beutel und stellst fest, dass du keine Edelsteine hast. Der alte Mann schaut dich erwartungsvoll an. Als er bemerkt, dass du keine Edelsteine hast, runzelt er die Stirn. Du spürst Ärger in der Luft und flüchtest so schnell du kannst.');
	}
