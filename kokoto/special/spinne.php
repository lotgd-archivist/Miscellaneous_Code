...
if ($_GET['op']=='betreten'){
	if ($session['user']['sex']==0) $rand = mt_rand(1,2); else $rand = mt_rand(3,4);
	switch($rand){
		case 1:
			page_header("Höhle");
			output('`n`c`sSpinne`c`n`nDu betrittst die Höhle und bemerkst, das dass Leuchten verschwunden ist. Vor dir tropft von der Decke eine komische Flüssigkeit herunter. Automatisch schaust du nach oben, in die rot leuchtenden Augen einer riesigen Spinne. Sie springt auf den Boden und dich greift dich sofort an!`n`n');
			$badguy = array(
				"creaturename" => "`sRiesenspinne`0",
				"creaturelevel" => $session['user']['level'],
				"creatureweapon" => "`sGift und Netzfäden`0",
		...
		case 3:
			...
			$session['user']['turns']--;
			addnews("`s".$session['user']['name']."`s wurde gesichtet, als Sie um Hilfe schreiend aus einer Höhle gerannt kam. Und ja, das Wort `$`bSpinne`b`s kam auch darin vor...");
			$session['user']['specialinc']='';
		break;
		case 4:
			page_header("Höhle");
			...
			addnews("`s".$session['user']['name']."`s wurde gesichtet, als Sie um Hilfe schreiend aus einer Höhle gerannt kam. Und ja, das Wort `$`bSpinnen`b`s kam auch darin vor ... ... sehr oft sogar ... ...");
			$session['user']['specialinc']='';
		break;
	}
//Battle Settings
    ...
    $session['user']['specialinc']='';
        ...
        $_GET['op']='';
		addnews("`s".$session['user']['name']."`$ ist vor einer Spinne weggerannt! ... Ja einer `bSpinne`b ...");
    }else{
	...
	if ($session['user']['charm']>1) $session['user']['charm']-=2;
	if ($session['user']['gold']>0 && $session['user']['gems']==0) $session['user']['gold']*=0.9;
	if ($session['user']['gold']>0 && $session['user']['gems']>0){
	$session['user']['gems']--; 
	$session['user']['gold']*=0.9;
	}
	if ($session['user']['gold']==0 && $session['user']['gems']>2) $session['user']['gems']-=2;
	$session['user']['turns']--;
...
if ($battle) {
    ...
    $session['user']['specialinc']='spinne.php';
        if ($victory){
            output("`s`n`nDu hast die Spinne besiegt! Du bist der Held aller Frauen!!");
            //Navigation
            $badguy=array();
            $session['user']['badguy']='';
            addnews("`s".$session['user']['name']."`$ hat eine `bSpinne`b erschlagen, er ist ein wahrer Frauenheld!");
            addnav('Zurück in den Wald','forest.php');
            if (rand(1,2)==1) {
                $gem_gain = rand(2,3);
                $gold_gain = rand($session['user']['level']10,$session['user']['level']20);
                output("`s `nAls Du dich noch einmal umdrehst findest du `^$gem_gain`% Edelsteine`s
                und `^ $gold_gain Goldstücke.`s`n");
            ...
            addnews("`$".$session['user']['name']."`$ wurde von einer riesiegen `bSpinne`b zerquetscht ... Sowas soll ein Frauenheld sein? Versager!");
