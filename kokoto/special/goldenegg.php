...
if ($_GET['op']=='takeit') {
	...
	if (getsetting("pvp",1)==0) {
		...
		$session['user']['specialinc']='';
	  	...
	  	output('`3Mitten im dichten Wald bemerkst du etwas von oben durch die Blätter funkeln. Es ist nicht die Sonne! In deiner Neugier kommst du näher. Du siehst auf einer starken Astgabel eines gewaltigen Baumes ein riesiges Nest - in dem ein großes goldenes Ei funkelt!`nDa du keine Ahnung hast, welches Tier solche Eier legt und so riesige Nester baut, bist du unschlüssig, ob du schnell nach oben klettern und das Ei mitnehmen sollst, oder es doch lieber dort lassen solltest. Einerseits könnte das Ei magische Kräfte haben, oder zumindest eine reichliche Mahlzeit bieten - andererseits aber könntest du ebenso verlockend und lecker auf die Mutter dieses Eis wirken...');
		...
		if ($owner == $row['acctid']) {
			output('`3Du kommst an eine Stelle im Wald, die dir merkwürdig bekannt vorkommt. Plötzlich hörst du ein lautes Kreischen über dir in der Luft und ein schnell lauter werdendes Rauschen. Es ist die Greifenmutter, die ihr Ei bei dir wittert!! Du schaffst es gerade noch, ihr das Ei entgegenzustrecken, bevor sie über dich herfällt.');
			$lvl = $session['user']['level'];
			...
			output("`3Mitten im dichten Wald entdeckst du auf einer starken Astgabel eines gewaltigen Baumes ein riesiges Nest.`n Du bemerkst, dass es leer ist und schenkst ihm keine weitere Aufmerksamkeit.`n`n `^Das goldene Ei befindet sich zur Zeit im Besitz von ".$row['name']."`^!`n`3Willst du es ".($row['sex']?"ihr":"ihm")." nicht mal abnehmen?`n");
		}
		$session['user']['specialinc']='';
	}
}
