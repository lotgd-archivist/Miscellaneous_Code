...
function filterwords($string)
{
    ...
    foreach ($filtstr as $key => $filtstr1)
        foreach ($filterpost as $filterpost1) 
            if (strlen_c($filtstr) > strlen_c($filterpost1)) 
                if (!strcasecmp(substr_c($filtstr1, 1strlen_c($filterpost1)), $filterpost1)) 
                    $filtstr[$key] = substr_c($filtstr1, 0, strlen_c($filterstr)strlen_c($filterpost1));
...
if($_GET['op']=='yes'){
        ...
        if (!strpos_c($rid, "Riddle")) {
            $sq1 = "SELECT * FROM riddles ORDER BY rand(".mt_rand().")";
        }else{
            $rid = substr_c($rid, 1(strlen_c($rid)6));    // 6 letters in "Riddle"
        ...
        allownav('forest.php?op=yes&subop=answer');
    }else{
        $rid = substr_c($session['user']['specialmisc'], 6);
        $sq1 = "SELECT * FROM riddles WHERE id=$rid";
        $result = db_query($sq1) or die(db_error(LINK));
        $riddle = db_fetch_assoc($result);
        ...
        foreach($answer as $key => $answer1) {
            ...
            $answer[$key] = preg_replace_c("/[^[:alpha:]]/"," ",$answer1); 
        ...
        $guess = preg_replace_c("/[^[:alpha:]]/"," ",$guess); 
        ...
        if ($correct) {
            output('`n`6"`@Eidechsen und Kaulquappen!! Du hast es!`6", tobt er, `n`6"`@Oh na gut. Hier hast du deinen dämlichen Preis.`6"`n`n');
            ...
            output('`n`6Der merkwürdige Gnom gackert vor Freude und tanzt um dich herum. Du fühlst dich ziemlich albern dabei, als dieser verrückte Gnom um dich herum tänzelt wie eine Fee, `deswegen machst du dich leise aus dem Staub, solange der Gnom abgelenkt ist. Irgendwie fühlst du dich jetzt weniger als ein Held - mit diesem spöttischen Gelächter in den Ohren. ');
            ...
            switch ($rand){
                ...
                case '3':
					...
					$gold = mt_rand(1, $session['user']['level']10);
                ...
                case '5':
                    ...
                    break;
                case '6':
                    output("Was wird wohl ".($session['user']['sex']?"Tharagon":"Ivalaine")." davon halten?");
    ...
    output('`n`6Du fürchtest dich lächerlich zu machen und lehnst seine Herausforderung ab. Er war sowieso etwas gruselig. `n`6Der merkwürdige Gnom kichert hysterisch und verschwindet im Wald.');
    $session['user']['specialinc']='';
    $session['user']['specialmisc']='';
}else{
    output('`6`nEin kurzer kleiner Gnom mit Blättern in den Haaren hockt neben einem kleinen Baum. Er grinst und kichert hinter einer seiner fettigen Hände. Für einen Moment sieht es so aus, als ob er in den Wald wegkrabbeln will, aber dann grinst er dich an.`n`n `6"`@Ich werde dir einen Gefallen tun,`6" sagt er, "`@wenn du mein Rätsel lösen kannst!`6"`n`n `6Vorübergehend verfällt er in unkontrolliertes Kichern, fasst sich wieder für einen Moment und fährt fort.`n`n `6"`@Aber wenn du falsch liegen solltest, mein Gefallen es wird sein!`6"`n `6`nNimmst du die Herausforderung an?`n`n');
    output("<a href=forest.php?op=yes>Ja</a>`n<a href=forest.php?op=no>Nein</a>`n", true);

    addnav('Ja','forest.php?op=yes');
    addnav('Nein','forest.php?op=no');
    allownav('forest.php?op=yes');
