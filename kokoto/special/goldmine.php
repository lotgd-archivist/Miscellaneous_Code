...
if ($hashorse) {
	...
	if (mt_rand(1,10) == 1) $horsecanenter=0;
	if ($horscanenter) {
...
if ($_GET['op']=='no'){
	...
	if ($session['user']['turns']<=0) {
		...
		if (mt_rand(1, 100) > $horsecanenter) {
		...
		switch ($rand){
		  ...
		  case '6': case '7': case '8':case '9': case '10':
			$gold = mt_rand($session['user']['level']5, $session['user']['level']20);
	      ...
	      case '11': case '12': case '13': case '14': case '15':
		    $gems = mt_rand(1, $session['user']['level']71);
		  ...
		  case '16': case '17': case '18':
			$gold = mt_rand($session['user']['level']10, $session['user']['level']40);
		   	$gems = mt_rand(1, $session['user']['level']31);
			...
			break;
		  case '19': case '20':
		  	output('`2Nach einer Menge harter Arbeit glaubst du, einen `&riesigen`2 Edelstein und etwas `6Gold`2 gesehen zu haben`2.`n`2Begierig auf Reichtum lehnst du dich zurück und schmetterst mit der  Spitzhacke darauf ein. `2Denn je fester du zuschlägst, umso schneller wird es erledigt sein....`n`7Unglücklicherweise bist `bdu`b erledigt.`nDein übereifriges Schlagen verursacht einen schweren Höhleneinsturz.`n');
...
if ($session['user']['hashorse']>0){
			...
			if ($dead) {
				if (mt_rand(1,100) < $horsecandie) $horsedead = 1;
			}
}
			$session['user']['specialinc']='';
			if ($dead == 1) {
...
if ($session['user']['hashorse']>0){
				...
				output('Wenigstens hast du dabei etwas über Bergbau gelernt. Du bekommst '.$exp.' Erfahrungspunkte.`n Du verlierst all dein Gold und die Hälfte deiner Edelsteine.`n`n`3Du kannst morgen weiterspielen`n');
	...
	output('`2In den tiefen des Waldes hast du eine alte verlassene Mine gefunden. Es liegt sogar noch Goldgräberausrüstung in der Nähe herum.`n`n`^Als du dich etwas umsiehst, bemerkst du, dass das hier eine Menge Arbeit wäre. So viel, dass du auf jeden Fall einen Waldkampf verlieren wirst, wenn du dein Glück versuchen willst.`n`n`^Ausserdem stellst du fest, dass durchaus die Gefahr von gelegentlichen Einstürzen in der Mine besteht.`n`n');
	addnav('Nach Gold und Edelsteinen suchen','forest.php?op=mine');
	addnav('Zurück in den Wald','forest.php?op=no');
	allownav('forest.php?op=mine');
