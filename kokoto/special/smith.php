...
if ($_GET['op']=='none'){
    ...
    $session['user']['specialinc']="";
    $previously_upgraded   = strpos_c($session['user'][$_GET['op']]," +1")!==false ? true : false;
    $previously_downgraded = strpos_c($session['user'][$_GET['op']]," -1")!==false ? true : false;
    ...
    if ($previously_upgraded){
        ...
        $session['user'][$_GET['op']] = str_replace_c(" -1","",$session['user'][$_GET['op']]);
    }else{
        $session['user']['gems']--;
        $r = mt_rand(1,100);
        if ($r<30){
    ...
    output('`7Du stapfst vorsichtig durchs Unterholz, als du einen stämmigen Mann mit einem schweren Hammer in dern Hand bemerkst. Sicher, daß er keine Bedrohung für dich darstellt, näherst du dich ihm und sagst: "`&Hey du!`7".`n`n "`6Mein Name ist Smiythy.`7", antwortet er. `n`n"`&Was?`7" fragst du und lässt dir deine Verwunderung anmerken. `n`n"`6Smiythe, das ist mein Name.  Ich bin ein Schmied.  Smiythe der Schmied werde ich von einigen genannt.  Und ich würde mich freuen, dir meine Schmiedekünste für eine geringe Gebühr anbieten zu dürfen.`7 `n`n"`6Für nur 1 Edelstein kann ich versuchen, deine Rüstung oder deine Waffe zu verbessern.  Aber ich muss dich warnen: Obwohl ich der beste Schmied hier in der Gegend bin,  mache ich trotzdem hin und wieder Fehler und kann nicht immer für beste Qualität garantieren.`7"');
