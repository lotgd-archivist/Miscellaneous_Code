...
if ($_GET['op']=='climbtree'){
    ...
    if (db_num_rows($result) >0)// User hat schon Schmuck
    {
        ...
        if ($session['user']['gold'] > 0){
            $session['user']['specialinc']='jewelrymaker.php';
            output('`@Der Elf nimmt all Dein Gold und spricht einen Zauber darüber. Es verwandelt sich...`n`n`6in ein wunderschönes `&Etwas `6was Du leider nicht identifizieren kannst. Aber schön ist es. Irgendwie.`n`n`@Du nimmst das Gebilde und staunst eine Weile darüber. Dann steckst Du es ein. Vielleicht gibt Dir ja ein Händler was dafür...');
            // Goldwert randomisieren und Edelsteinwert randomisieren
            $goldvalue = mt_rand(1, $session['user']['gold']  2);
            $gemvalue = mt_rand(0,2);
