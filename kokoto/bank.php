...
addcommentary();
page_header("Die alte Bank von Kokoto");
output("`^`c`bDie alte Stadtbank`b`c`6");
if ($_GET['op']==''){
  
  output('Ein kleiner Mann in einem makellosen Anzug mit Lesebrille grüßt dich.`n`n "`5Hallo guter Mann,`6" grüßt du zurück, "`5Kann ich meinen Kontostand an diesem wunderschönen Tag einsehen?`6"`n`n Der Bankier murmelt "`3Hmm, '.$session['user']['name'].'`3, mal sehen.....`6" während er die Seiten in seinem Buch sorgfältig überfliegt.  ');
	if ($session['user']['goldinbank']>=0){
		output('"`3Aah ja, hier ist es. Du hast  `^'.$session['user']['goldinbank'].' Gold`3 bei unserer renommierten Bank.  Kann ich sonst noch etwas für dich tun?`6"');
	}else{
		output('"`3Aah ja, hier ist es.  Du hast `&Schulden`3 in Höhe von `^'.abs($session['user']['goldinbank']).' Gold`3 bei unserer renommierten Bank.  Kann ich sonst noch etwas für dich tun?`6"`n`n(`iSchulden verfallen durch einen Drachenkill nicht!`i)');
	}
	output('`n`n`6In der Bank:`n');
viewcommentary("bank","`6Auf `$ Rollenspiel`6 gerechte Kommentare Achten!",10);
	...
	if ($session['user']['goldinbank']>=0){
		...
		output("Du musst mindestens `^$minfer`6 Gold überweisen.`nDu kannst nicht mehr als insgesamt `^$maxout`6 Gold überweisen.");
		if ($session['user']['amountouttoday'] > 0) {
			output("(Du hast heute schon `^{$session['user']['amountouttoday']}`6 Gold überwiesen.)`n`n");
		} else output("`n`n");
		output("<form action='bank.php?op=transfer2' method='POST'>Wieviel ü<u>b</u>erweisen: <input name='amount' id='amount' accesskey='b' width='5'>`n A<u>n</u>: <input name='to' accesskey='n'> (Unvollständige Namen werden automatisch ergänzt. Du wirst nochmal zum Bestätigen aufgefordert).`n <input type='submit' class='button' value='Vorschau'></form> <script language='javascript'>document.getElementById('amount').focus();</script>",true);
		allownav('bank.php?op=transfer2');
	}else{
	...
	$string="%";
	for ($x=0;$x<strlen_c($_POST['to']);$x++){
		$string .= substr_c($_POST['to'],$x,1)."%";
	}
	$sql = "SELECT name,login FROM accounts WHERE name LIKE '".mysql_real_escape_string($string)."'";
	$result = db_query($sql);
	...
	if (db_num_rows($result)==1){
		...
		allownav('bank.php?op=transfer3');
		...
		output("<form action='bank.php?op=transfer2' method='POST'>Wieviel ü<u>b</u>erweisen: <input name='amount' id='amount' accesskey='b' width='5' value='$amt'>`n A<u>n</u>: <input name='to' accesskey='n' value='". $_POST['to'] . "'> (Unvollständige Namen werden automatisch ergänzt. Du wirst nochmal zum Bestätigen aufgefordert).`n <input type='submit' class='button' value='Vorschau'></form> <script language='javascript'>document.getElementById('amount').focus();</script>",true);
		allownav('bank.php?op=transfer2');
		...
		for ($i=0;$i<db_num_rows($result);$i++){
			...
			output("<option value=\"".htmlspecialchars($row['login'])."\">".preg_replace_c("'[`].'","",$row['name'])."</option>",true);
		}
	...
	if (($session['user']['gold']>=100 || $session['user']['gold']$session['user']['goldinbank']>=100) && $session['user']['gems']>0){
		...
		output("<form action='bank.php?op=gemtrans2' method='POST'>Einen Edelstein versenden a<u>n</u>: <input name='to' accesskey='n'> (Unvollständige Namen werden automatisch ergänzt. Du wirst nochmal zum Bestätigen aufgefordert).`n <input type='submit' class='button' value='Vorschau'></form>",true);
		allownav("bank.php?op=gemtrans2");
	...
	if (db_num_rows($result)==1){
		...
		allownav("bank.php?op=gemtrans3");
	}elseif(db_num_rows($result)>100){
		output('Der Bankier schaut dich überfordert an und schlägt dir vor, deine Suche vielleicht etwas mehr einzuengen, indem du den Namen genauer festlegst.`n`n');
		output("<form action='bank.php?op=gemtrans2' method='POST'>Versende einen Edelstein a<u>n</u>: <input name='to' accesskey='n' value='". $_POST['to'] . "'> (Unvollständige Namen werden automatisch ergänzt. Du wirst nochmal zum Bestätigen aufgefordert).`n<input type='submit' class='button' value='Vorschau'></form>",true);
		allownav("bank.php?op=gemtrans2");
	}elseif(db_num_rows($result)>1){
		output("<form action='bank.php?op=gemtrans3' method='POST'>",true);
...
//		} else if ($session[user][emailaddress]==$row[emailaddress] && $row[emailaddress]){
  ...
  allownav("bank.php?op=depositfinish");
	...
	if ($session['user']['reputation']<35){
		...
		output("<script language='javascript'>document.getElementById('input').focus();</script>",true);
	  	allownav('bank.php?op=withdrawfinish');
	}
}else if($_GET['op']=='withdraw'){
  	output("<form action='bank.php?op=withdrawfinish' method='POST'>Du hast ".$session['user']['goldinbank']." Gold bei der Bank.`n `^Wieviel a<u>b</u>heben? <input id='input' name='amount' width=5 accesskey='b'> <input type='submit' class='button' value='Abheben'>`n`iGib 0 oder gar nichts ein, um alles abzuheben.`i</form> <script language='javascript'>document.getElementById('input').focus();</script>",true);
  	allownav('bank.php?op=withdrawfinish');
	...
	if ($_POST['amount']>$session['user']['goldinbank'] && $_POST['borrow']=='') {
		output("`\$FEHLER: Nicht genug auf dem Konto.`^`n`nNachdem du darüber informiert wurdest, dass du `&".$session['user']['goldinbank']."`^ Gold auf dem Konto hast, erklärst du dem Männlein mit der Lesebrille, dass du gerne `&".$_POST['amount']."`^ davon abheben würdest. `n`nDer Bankier schaut dich bedauernd an und erklärt dir die Grundlagen der Mathematik. Nach einer Weile verstehst du deinen Fehler und würdest es gerne nochmal versuchen.");
