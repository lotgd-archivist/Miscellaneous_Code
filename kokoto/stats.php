...
output("`b`%Für existierende Accounts:`b`n `@Accounts insgesamt: `^".number_format($row['a'])."`n `@Treffer insgesamt: `^".number_format($row['c'])."`n `@Seitengenerierungszeit insgesamt: `^".dhms($row['t'])."`n `@Seitengenerierungsgröße insgesamt: `^".number_format($row['s'])."b`n `@Durchschnittliche Seitengenerierungszeit: `^".dhms($row['t']$row['c'],true)."`n `@Durchschnittliche Seitengröße: `^".number_format($row['s']$row['c'])."`n `n`%`bTop Referers:`b`0`n");
rawoutput("<table border='0' cellpadding='2' cellspacing='1' bgcolor='#999999'> <tr class='trhead'><td><b>Name</b></td><td><b>Referrals</b></td></tr>");
...
rawoutput('</table>');
$sql = "SELECT count(*) AS c, substr_cing(laston,1,10) AS d FROM accounts GROUP BY d DESC ORDER BY d DESC";
$result = db_query($sql);
