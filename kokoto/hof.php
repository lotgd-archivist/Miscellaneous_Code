...
page_header("Ruhmeshalle von Kokoto");
checkday();
...
$sql = "SELECT count(acctid) AS c FROM accounts WHERE locked=0 AND superuser<4";
if ($op == "kills") {
	$sql = "SELECT count(acctid) AS c FROM accounts WHERE locked=0 AND superuser<4 AND dragonkills>0";
} elseif ($op == "days") {
	$sql = "SELECT count(acctid) AS c FROM accounts WHERE locked=0 AND superuser<4 AND dragonkills>0 AND bestdragonage>0";
}
...
if ($session['user']['live']==0) addnav("Luzifer's Lieblinge","hof.php?op=grave&subop=$subop&page=$page");
...
if ($session['user']['alive']){
	addnav("Zurück zum Dorf","village.php?op=hauptsrasse");
}else{
...
function display_table($title, $sql, $none=false, $foot=false, $data_header=false, $tag=false){
	...
	rawoutput('<table cellspacing="0" cellpadding="2" align="center"><tr class="trhead">');
	...
	if (db_num_rows($result)==0){
		$size = ($data_header === false) ? 2 : 2count($data_header);
		//echo $size;
...
$racesel = "CASE race 
WHEN 1 THEN '`2Troll`0' 
WHEN 2 THEN '`^Elf`0' 
WHEN 3 THEN '`&Mensch`0' 
WHEN 4 THEN '`#Zwerg`0' 
WHEN 5 THEN '`5Echse`0'
WHEN 6 THEN '`1Dunkelelf`0'
WHEN 7 THEN '`$ Vampir`0'
WHEN 8 THEN '`4Dämon`0'
WHEN 9 THEN '`PGottheit`0'
WHEN 10 THEN '`&Engel`0'
WHEN 11 THEN '`^Feenwesen`0'
WHEN 12 THEN '`tLykanthrop`0'
WHEN 13 THEN '`SUntot`0'
WHEN 14 THEN '`sOrk`0'
WHEN 15 THEN '`PNymphe`0'
ELSE '`7Unbekannt`0' END";

if ($_GET['op']=="money"){
	$sql = "SELECT name,(CAST((goldinbank+gold) AS SIGNED)+round((((rand()*10)-5)/100)*CAST((goldinbank+gold) AS SIGNED))) AS data1 FROM accounts WHERE locked=0 AND superuser<4 ORDER BY data1 $order, level $order, experience $order, acctid $order LIMIT $limit";
	...
	display_table($title, $sql, false, $foot, $headers, $tags);
        ...
        $sql = "SELECT name,monsterkills AS data1,dragonkills AS data2 FROM accounts WHERE locked=0 AND superuser<4 ORDER BY monsterkills $order, dragonkills $order, acctid $order LIMIT $limit";
        $adverb = "meisten";
        if ($_GET['subop'] == "least") $adverb = "wenigsten";
        $title = "Die Helden mit den $adverb Monsterkills in diesem Land";
        $headers = array("Monsterkills","Drachenkills");
        display_table($title, $sql, false, false, $headers, false);
} elseif ($_GET['op'] == "gems") {
	$sql = "SELECT name FROM accounts WHERE locked=0 AND superuser<4 ORDER BY gems $order, level $order, experience $order, acctid $order LIMIT $limit";
	if ($_GET['subop'] == "least") $adverb = "wenigsten";
	else $adverb = "meisten";
	$title = "Die Krieger mit den $adverb Edelsteinen";
	display_table($title, $sql);
} elseif ($_GET['op']=="charm"){
	$sql = "SELECT name,$sexsel AS data1,$racesel AS data2 FROM accounts WHERE locked=0 AND superuser<4 ORDER BY charm $order, level $order, experience $order, acctid $order LIMIT $limit";
	$adverb = "schönsten";
	...
	$sql = "SELECT name,level AS data2 ,$racesel as data1 FROM accounts WHERE locked=0 AND superuser<4 ORDER BY maxhitpoints $order, level $order, experience $order, acctid $order LIMIT $limit";
	$adverb = "stärksten";
	...
	$sql = "SELECT name,punch AS data1,$racesel AS data2 FROM accounts WHERE locked=0 AND superuser<4 ORDER BY data1 $order, level $order, experience $order, acctid $order LIMIT $limit";
	$adverb = "härtesten";
    ...
    $sql = "SELECT name,wächterkills AS data1,dragonkills AS data2 FROM accounts WHERE locked=0 AND superuser<4 ORDER BY wächterkills $order, dragonkills $order, acctid $order LIMIT $limit"; 
    $adverb = "meisten"; 
    if ($_GET['subop'] == "least") $adverb = "wenigsten"; 
	...
	$sql = "SELECT name,level AS data1 FROM accounts WHERE locked=0 AND superuser<4 ORDER BY resurrections $order, level $order, experience $order, acctid $order LIMIT $limit";
	...
	$sql = "SELECT name,deathpower,location,loggedin,laston,alive FROM accounts WHERE locked=0 AND superuser<4 ORDER BY deathpower $order, level $order, experience $order, acctid $order LIMIT $limit";
	$adverb = "fleissigste";
	if ($_GET['subop'] == "least") $adverb = "faulste";
	$title = "Luzifer's $adverb Krieger";
	output("`c`b`^$title`0`b `7(Seite $page: $from-$to)`0`c`n");
	...
	$sql = "SELECT name,battlepoints AS data1,dragonkills AS data2 FROM accounts WHERE locked=0 AND superuser<4 ORDER BY battlepoints $order, dragonkills $order, acctid $order LIMIT $limit";
	$adverb = "besten";
	...
	$sql = "SELECT acctid,name,marriedto FROM accounts WHERE sex=0 AND ehe=1 ORDER BY acctid DESC";
	output("`c`b`&Heldenpaare dieser Welt`b`c`n`c<table cellspacing=0 cellpadding=2 align='center'><tr><td><img src=\"images/female.gif\">`b Name`b</td><td></td><td><img src=\"images/male.gif\">`b Name`b</td></tr>",true);
	$result = db_query($sql);
	if (db_num_rows($result)==0){
	...
	for ($i=0;$i<db_num_rows($result);$i++){
		...
		$result2 = db_query($sql2);
  		...
  		output("<tr class='".($i2?"trlight":"trdark")."'><td>`&&nbsp;".$row2['name']."&nbsp; `0</td><td>`)  &nbsp;und &nbsp;&nbsp; `0</td><td>`&",true);
