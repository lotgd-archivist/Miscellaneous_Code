...
page_header("Raymore's Tisch");
output("<span style='color: #9900FF'>`c`bRaymore's Tisch`b`c</span>",true);

if ($_GET['op']=="list"){
	output('Dag fischt ein kleines, ledergebundenes Buch unter seinem Mantel hervor, blättert es zu einer bestimmten Seite durch und hält es dir zum Lesen hin.`n`n `c`bDie Kopfgeldliste`b`c`n');
	$sql = "SELECT name,alive,sex,level,laston,loggedin,bounty,location FROM accounts WHERE bounty>0 ORDER BY bounty DESC";
	$result = db_query($sql);
rawoutput("<table border=0 cellpadding=2 cellspacing=1 bgcolor='#999999'><tr class='trhead'><td><b>Kopfgeld</b></td><td><b>Level</b> </td><td><b>Name</b></td><td><b>Ort</b></td><td><b>Geschlecht</b></td><td><b>Zuletzt online</b></tr>");
	for($i=0;$i<db_num_rows($result);$i++){
		$row = db_fetch_assoc($result);
		output("<tr class='".($i2?"trdark":"trlight")."'><td>",true);
		output("`^{$row['bounty']}`0");
		rawoutput('</td><td>');
		output("`^{$row['level']}`0");
		rawoutput('</td><td>');
...
rawoutput('</td></tr>');
	...
	if ($session['user']['bounties'] >= getsetting("maxbounties",5)) {
		...
		output('Raymore blickt zu dir auf und rückt seine Pfeife mit den Zähnen zurecht.`n"`7So, wen willst\'n tot sehen? Du sollst aber wissen, dass wir keine Kinder killn, deswegen muss dein Opfer mindestens Level '. getsetting("bountylevel",3) .' sein und der Preis darf nicht zu hoch sein. Außerdem dürfen die Opfer nicht zu oft getroffen werdn. Also wer in meinem Buch nicht gelistet is, kann nicht zum Abschuss freigegeben werdn! Wir betreiben hier kein Schlachthaus, sondern\'n ... Unternehmen. Ich verlang '. getsetting("bountyfee",10) .'% Bearbeitungsgebühren für jeden Namen, den ich auf die Liste setzn soll."`n`n');
		output("<form action='dag.php?op=finalize' method='POST'>`2Zielperson: <input name='contractname'>`n `2Betrag aussetzen: <input name='amount' id='amount' width='5'>`n`n <input type='submit' class='button' value='Vertrag abschlie&szlig;en'></form>",true);
		allownav("dag.php?op=finalize");
	...
	if ($_GET['subfinal']==1){
		$sql = "SELECT acctid,name,login,level,locked,age,dragonkills,experience,bounty,pvpflag,lastip,emailaddress,reputation,uniqueid  FROM accounts WHERE name='".mysql_real_escape_string(rawurldecode(stripslashes($_POST['contractname'])))."' AND locked=0";
		//output($sql);
	}else{
		...
		$sql = "SELECT acctid,name,login,level,locked,age,dragonkills,experience,bounty,pvpflag,lastip,emailaddress,reputation,uniqueid FROM accounts WHERE name LIKE '".mysql_real_escape_string($name)."' AND locked=0";
	}
	$result = db_query($sql);
	if (db_num_rows($result) == 0) {
		output("Raymore sagt höhnisch lachend: `7\"Es gibt nicht einen den ich mit so einem Namen kenne. Vielleicht kommst' wieder, wenn du 'n echtes Opfer hast.\"");
	} elseif(db_num_rows($result) > 100) {
		output("Raymore kratzt sich verwirrt am Kopf. `7\"Du beschreibst hier fast die Hälfte der Stadt, du Narr. Warum gibst du mir jetzt nicht mal ne genauere Beschreibung?\"");
	} elseif(db_num_rows($result) > 1) {
		output("Raymore durchsucht seine Liste für einen Moment. `7\"Da sind ein paar, die du meinen könntest. Wer genau soll's denn sein?\"`n");
		output("<form action='dag.php?op=finalize&subfinal=1' method='POST'>",true);
		output("`2Zielperson: <select name='contractname'>",true);
		for ($i=0;$i<db_num_rows($result);$i++){
			$row = db_fetch_assoc($result);
			output("<option value=\"".rawurlencode($row['name'])."\">".preg_replace_c("'[`].'","",$row['name'])."</option>",true);
		}
		output("</select>`n`n `2Betrag aussetzen: <input name='amount' id='amount' width='5' value='{$_POST['amount']}'>`n`n <input type='submit' class='button' value='Vertrag abschlie&szlig;en'></form>",true);
		allownav("dag.php?op=finalize&subfinal=1");
	} else {
		...
		if ($row['locked']) {
			...
			output("Raymore schlägt sich brüllend lachend auf die Schenkel: `7\"Du willst n Kopfgeld auf dich selbst aussetzen? Ich helf doch keinem Selbstmörder!\"");
		} elseif ($row['level'] < getsetting("bountylevel",3)) {
			output("Raymore starrt dich ärgerlich an: `7\"Hab ich dir nicht gesagt, dass ich kein Meuchler bin? Das ist kein Opfer, das ein Kopfgeld wert wäre. Jetzt geh mir aus den Augen!\"");
 	...
 	//	} elseif ($session['user']['lastip'] == $row['lastip'] || ($session['user']['emailaddress'] == $row['emailaddress'] && $row[emailaddress])){
			...
			$fee = getsetting("bountyfee",10);
			if ($amt < $min) {
				output("Raymore blickt finster: `7\"Glaubst im Ernst, ich arbeite für so nen Hungerlohn? Denk ma drüber nach und komm wieder, wenn du bereit bist hartes Bares zu bezahlen. Für dein Opfer brauchste mindestens " . $min . " Gold, damit's meine Zeit wert is.\"");
			} elseif ($session['user']['gold'] <round($amt1.1,0)) {
				output("Raymore schaut dich finster an: `7\"Du hast nicht genug Gold für diesen Vertrag. Wenn du meine Zeit so vergeudest, sollte ich stattdessen vielleicht n Kopfgeld auf DICH aussetzen!\"");
			} elseif ($amt  $row['bounty'] > $max) {
				output("Raymore schaut auf den Berg Münzen und lässt ihn unberührt liegen. `7\"Ich werde diesen Vertrag ablehnen. Das is viel mehr, als `^{$row['name']} `7Wert is und das weißt du. Ich bin kein verdammter Auftragskiller. N Kopfgeld von {$row['bounty']} is schon auf diesen Kopf ausgesetzt. Ich wär bereit, es auf $max zu erhöhen, nach meinen $fee% Bearbeitungsgebühren natürlich\"`n`n");
			} else {
				output("Du schiebst die Münzen zu Raymore, der sie flink einstreicht. `7\"Ich werd mir nur meine $fee% Gebühr einbehalten. Ich werd die Nachricht verbreiten, dass sich jemand um `^{$row['name']} `7kümmern soll. Hab Geduld und hab ein Auge auf die News.");
				$session['user']['bounties']++;
				$session['user']['donation']+=1;
				$cost = round($amt(1($fee100)),0);
	...
	output("Du schlenderst rüber zu Raymore, der es nichtmal für nötig hält, zu dir aufzuschauen. Er nimmt einen langen Zug aus seiner Pfeife.`n`7\"Du willst wohl wissn, ob n Preis auf deinen Kopf ausgesetzt is, richtig?\"`n`n");
...
if ($_GET['op'] != '')
	addnav("Rede mit Raymore", "dag.php");
