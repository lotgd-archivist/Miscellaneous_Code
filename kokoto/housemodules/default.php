...
function module_show_default() {
   ...
   /* content_show begin */
	...
	if ($row['description']!='') output("`0".$row['description']."`2`n");
	...
	// show commentary
	output("`n`^Schatzkammer-System:`n");
viewcommentary("house-".$session['user']['specialmisc']['houseid']."-treasure",'Y',10);
output("`n`n`n`2Du hörst die Mitbewohner reden:`n`n");
	...
	$sql = "SELECT items.id,items.hvalue,items.owner,accounts.acctid AS aid,accounts.name AS besitzer FROM items LEFT JOIN accounts ON accounts.acctid=items.owner WHERE value1={$session['user']['specialmisc']['houseid']} AND class='Schlüssel' ORDER BY items.id ASC";
	...
	$sql = 'SELECT COUNT(acctid) AS num FROM accounts
				LEFT JOIN items ON items.hvalue > 0 AND items.value1!='.$session['user']['specialmisc']['houseid'].' AND items.class="Schlüssel" AND items.owner='.$row['owner'].'
