...
function module_show_description() {
   ...
   /* content_show begin */
	...
	if (empty($_POST['desc'])) {
		...
		output("`2Hier kannst du die Beschreibung für dein Haus ändern.`n`nDie aktuelle Beschreibung lautet: `0".$row['description']."`0`n<form action=\"houses.php?op=drin&act=desc\" method='POST'>`n`2Gib eine Beschreibung für dein Haus ein:`n<input name='desc' maxlength='750' size='50'>`n<input type='submit' class='button' value='Abschicken'>",true);
		allownav('houses.php?op=drin&act=desc');
	}else{
		output("`2Die Beschreibung wurde geändert.`n`0".stripslashes(mysql_real_escape_string($_POST['desc']))."`2");
		$sql = "UPDATE houses SET description='".mysql_real_escape_string($_POST['desc'])."' WHERE houseid=".$session['user']['specialmisc']['houseid'];
		db_query($sql);
	}
