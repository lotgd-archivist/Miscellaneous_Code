...
* verschönert by Tidus (www.kokoto.de) ^^
*/ 
...
if ($_GET['op']=='speak'){ 
	$zufall = mt_rand(1,8); 
    	output('`@`b`cOld Drawls Stammtisch`c`b`0`n`n `@Du hast es gewagt und Old Drawl angesprochen. Langsam dreht der alte Mann seinen Kopf zu Dir herum und schaut Dich durchdringend aus seinen alten Augen an. Dir kommt es so vor als wären sie gelb. Als er zu sprechen beginnt wird Dir klar, woher sein Name kommt. Schleppend setzt er an:`n`n'); 
    	if ($zufall!=7){ 
        		output('"`GMeein Sooohn, was stööörst Du meiiiineee Ruuuuuheeee? Saaag was Duuu voooon mirrrr willlst unnnd daaann laaass miiiich innn Ruuuuheeee. Fooollgeendee Aaktiooneennn kann iiich Diir anbiiieteeenn. Abeeerrrr giiiib acht - irrgeendwiiieee haaabbeee iiicchhhh maanchmaaal meeiinnee Kräääftteee niiicht meeeehr iimmeeerr uunterrr Kooontroolleee.`@"'); 
        		addnav('Old Drawl Aktionen'); 
        		addnav('3x Goldmine','olddrawl.php?op=do&action=goldmine'); 
        		addnav('Lotterie spielen','lottery.php'); 
        	}else{ 
        		output('"`GMeein Sooohn, was stööörst Du meiiiineee Ruuuuuheeee? Haabeenn Diir dieee Waarnungennn niiicht gerreicht? Muußteeest Duu uuunbeeeddinngt meeiiiinee Ruuheeestööörenn? Icchhh haabee voon solcheeen Abstauuuubernn wiiee Diiir diee Naseee volll!!`@"`n`nOld Drawl macht eine Faust, holt aus und'); 
        		switch(mt_rand(1,5)){ 
            			case '1': 
                		output('trifft Dich mitten im Gesicht, so daß eine häßliche Narbe entstanden ist. Die Wucht schleudert Dich bis an den Tresen zurück. `n`n`@Du hast `42 Charmpunkte`@ verloren.'); 
                		$session['user']['charm']-=2; 
			if ($session['user']['charm']<=0) $session['user']['charm']=0;
               			break; 
            			case '2': 
                		output('trifft Dich am Körper und die Wucht schleudert Dich bis an den Tresen zurück. `n`n`@Du hast fast alle deine Lebenspunkte verloren.');
    	...
    	if ($_GET['action']=='goldmine'){ 
        		output('`@`b`cOld Drawls Stammtisch`c`b`0`n`n `@Für die Aktion `^3 mal Goldmine im Wald `@verlangt Old Drawl `42 `@Edelsteine. Aber achte darauf, daß sie nach wie vor einstürzen kann und es keine Garantie für eine erfolgreiche Suche gibt. Außerdem verlierst Du nach wie vor jeweils einen Waldkampf`n`n `@Willst Du ihm die 2 Edelsteine geben?'); 
