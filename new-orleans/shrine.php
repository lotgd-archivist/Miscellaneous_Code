...
page_header("St. Louis Cathedral - Schrein des Ramius");
...
addnav("St. Louis Cathedral","jacksonsquare.php?op=cathedral");
...
        $count++;
    }
    output("`c`b`4Du hast ".$session['user']['deathpower']." Gefallen.`b`c`0`n");
    if (!$count) output("`n Damit kannst du hier nichts anfangen.`0");
...
            $sql = "UPDATE accounts SET alive=1,lasthit='".date("Y-m-d H:i:s",strtotime(date("c")."-".(86500/getsetting("daysperday",4))." seconds"))."' WHERE acctid='{$row['acctid']}'";
            db_query($sql);
