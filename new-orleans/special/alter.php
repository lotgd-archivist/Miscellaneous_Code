...
if ($_GET['op'] == ""){
    output("`@Du stolperst auf eine Lichtung und bemerkst einen Altar mit 5 Seiten vor dir. Auf jeder Seite liegt ein anderer Gegenstand. Du siehst `#einen Dolch, `\$einen Schädel,`% einen juwelenbesetzten Stab, `^ein Rechenbrett `7und ein schlicht aussehendes Buch. `@In der Mitte über dem Altar befindet sich ein `&Kristallblitz.`n`n");
    output("  `@Du weißt, dass es dich Zeit für einen ganzen Waldkampf kosten wird, einen der Gegenstände näher zu untersuchen. `n`n`n");
    addnav("i?Nimm den Dolch","forest.php?op=dagger");
    addnav("m?Nimm den Schädel","forest.php?op=skull");
    addnav("d?Nimm den Stab","forest.php?op=wand");
    addnav("s?Nimm das Rechenbrett","forest.php?op=abacus");
    addnav("a?Nimm das Buch","forest.php?op=book");
    addnav("e?Nimm den Kristallblitz","forest.php?op=bolt");
...
  $session[user][turns]--; 
    if (e_rand(0,1)==0){
      output("`#Du greifst nach dem Schädel. Vor deinen Augen löst sich der Schädel auf und du fühlst eine Energiewelle in deinen Körper strömen!`n`n  `&Du erhältst 10 zusätzliche Anwendungen der Dunklen Künste.`n`n`#Aber du bist auch etwas traurig, denn diese Kraft wird morgen wieder verschwunden sein.");
        $session[user][darkartuses] = $session[user][darkartuses] + 10;
    }else{
    output("`#Du greifst nach dem Schädel. Vor deinen Augen löst sich der Schädel auf und du fühlst eine Energiewelle in deinen Körper strömen!`n`n  `&Du erhältst 3 Levels in Dunklen Künsten!");
...
            addnews("`&".$session['user']['name']." `9wurde von den Göttern niedergeschmettert, da ".($session['user']['sex']?"sie":"er")." `9von Gier zerfressen war!`0");
        }
    $session[user][specialinc]="";
