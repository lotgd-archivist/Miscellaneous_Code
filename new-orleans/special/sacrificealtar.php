...
            if (strtolower(substr($session[user][name],-1))=="s") addnews("`&".$session[user][name]." `j' Körper wurde auf einem Altar in den Wäldern gefunden."); 
            else addnews("`&".$session[user][name]."`j's Körper wurde auf einem Altar in den Wäldern gefunden.");  
            break; 
        case 4: 
        case 5: 
...
            addnav("Tägliche News","news.php"); 
            if (strtolower(substr($session[user][name],-1))=="s") addnews("`&".$session[user][name]."`j's Überbleibsel wurden verkohlt auf einem Altar gefunden."); 
            else addnews("`&".$session[user][name]."`j's Überbleibsel wurden verkohlt auf einem Altar gefunden."); 
            break; 
        case 6: 
        case 7: 
        case 8: 
...
                        $session[user][donation]+=1; 
                        addnav("Tägliche News","news.php"); 
                        if (strtolower(substr($session[user][name],-1))=="s") addnews("`&".$session[user][name]."`j's Körper wurde gefunden... angeknabbert von Hasen!"); 
                        else addnews("`&".$session[user][name]."`j's Körper wurde gefunden... angeknabbert von Hasen!"); 
                        break; 
                    case 2: 
                    case 3: 
                    case 4: 
...
                        output("weiß, dass Du ein hartes Leben hinter Dir hast, also erhältst Du ein Geschenk von mir.`@\"`n`n"); 
...
                        output("siehst Du eine wunderschöne Frau vor Dir stehen.`n`n"); 
                        output("\"`#".($session[user][sex]?"Meine geliebte Tochter":"Mein geliebter Sohn").",`@\" sagt sie, \"`# ich danke Dir für das Geschenk der Rosen. Ich "); 
                        output("weiß, dass Du ein hartes Leben hinter Dir hast, also erhältst Du ein Geschenk von uns.`@\"`n`n"); 
...
                        addnav("Tägliche News","news.php"); 
                        $session['user']['donation']+=1; 
                        if (strtolower(substr($session[user][name],-1))=="s") addnews("`&".$session[user][name]."`j's Waffen wurden bei einer Riesenpflanze gefunden, aber mehr konnte nicht herausgefunden werden."); 
                        else addnews("`&".$session[user][name]."`j's Waffen wurden bei einer Riesenpflanze gefunden, aber mehr konnte nicht herausgefunden werden."); 
                        break; 
                    case 2: 
                    case 3: 
                    case 4: 
...
                        output("Sie gibt Dir etwas, das wie ein leckerer Brotlaib aussieht und motiviert Dich, es zu essen. Da Du nicht unhöflich sein willst, "); 
                        output("nimmst Du das Brot in den Mund und isst es. Auf einmal fühlst Du Dich so, als ob sich mehr Wissen "); 
...
                switch(e_rand(1,5)){ 
                    case 1: 
                        output("siehst eine Göttin, die missbilligend auf Dein Geschenk schaut. Plötzlich dreht sie sich zu Dir und ihre Wut bricht aus. "); 
...
                        $session[user][gold] = 0; 
                        addnav("Tägliche News","news.php"); 
                        if (strtolower(substr($session[user][name],-1))=="s") addnews("`&".$session[user][name]."`j's Kopf wurde gefunden... auf einem Speer in der Nähe eines Altars für die Götter."); 
                        else addnews("`&".$session[user][name]."`j's Kopf wurde gefunden... auf einem Speer in der Nähe eines Altars für die Götter"); 
                        break; 
                    case 2: 
                    case 3: 
                    case 4: 
...
            $session[user][gold] = 0; 
            addnav("Tägliche News","news.php"); 
            if (strtolower(substr($session[user][name],-1))=="s") addnews("`&".$session[user][name]."s`j Überreste waren nicht sehr schön, als sie gefunden wurden..."); 
            else addnews("`&".$session[user][name]."'s `jÜberreste waren nicht sehr schön, als sie gefunden wurden..."); 
        break; 
        case 2: 
        case 3: 
        case 4: 
...
            } 
            output("sprichst Du ein Gebet für den Geist des toten `^".$badguyname."`@ aus. Du drehst Dich um und wäscht Deine Hände in "); 
...
            $session[user][gold] = 0; 
            addnav("Tägliche News","news.php"); 
            addnews("`jDer verkohlte Körper von `&".$session[user][name]." `jwurde irgendwo im Wald gefunden."); 
            break;                                                 
    } 
} 
if ($_GET[op]=="run"){ 
...
        addhistory(1); 
        addnews("`&".$session[user][name]."`j wurde im Wald getötet von`9 $badguy[creaturename]`n$taunt"); 
        $session[user][alive]=false; 
