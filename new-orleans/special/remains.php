...
    $tname = $row2[name];
    output("`GDu stoplerst über etwas. Es sind die skelettierten Überreste dessen, was einst ein Abenteuer wie du gewesen sein könnte. Tatsächlich deutet vieles darauf hin, dass dies die Überreste von `%$tname`G sind.");
...
        addnav("g?Begraben (3 Waldkämpfe)","forest.php?op=bury");
    }
...
        addnews("`&".$session[user][name]."`G kam dem Tod näher, als ".($session[user][sex]?"ihr":"ihm")." lieb war.");
    } else {
        fightnav(true,true);
    }
}
