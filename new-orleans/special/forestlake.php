...
        output("`@Trotzdem macht die Ruine irgendwie einen romantischen Eindruck, und Du kannst Dir vorstellen, dass man von oben - sofern die Treppen noch vorhanden sein sollten - bestimmt einen großartigen Ausblick hat. Wollt ihr euch den Turm nicht mal näher anschauen?`n");
        output("`n`n`n`n`n`n`n`n`n`n`$@`@}-,-´-");
...
        output("`@Ihr beobachtet, wie der Fluss sich fröhlich in das Tal schlängelt, in den See fließt, und sich mit kleinen Seitenläufen vereint in der Ferne verliert...`n`n");
...
                    { output("`n`n`^Während Du langsam wieder zu Bewusstsein kommst, merkst Du, dass Du Dich nicht mehr daran erinnern kannst, was Du in den letzten Tagen getan hast.");
                      $session[user][experience]=0;
                } else
                    { output("`n`n`^Während Du langsam wieder zu Bewusstsein kommst, wird Dir bewusst, was für ein Trottel Du doch bist - durch diese dumme Aktion hast Du $exp Erfahrungspunkte verloren!");
                      $session[user][experience]-=$exp;
                }
              break;
              case 3:
...
        $db_query("INSERT INTO items (name,owner,class,value1,gold,gems,description) VALUES ('Ring des Lebens',".$session[user][acctid].",'Schmuck',$mhp,100,$mhp,'Dieser schöne Ring aus dem See im Wald gibt dir $mhp Lebenspunkte')");
        $db_query("INSERT INTO items (name,owner,class,value1,gold,gems,description) VALUES ('Ring des Lebens',$loverID,'Schmuck',0,100,0,'Diesen schönen Ring hat dir ".$session[user][login]."`0 am See im Wald geschenkt.')");
              break;
              case 4:
                output("`n`n`^Während Du untertauchst, siehst Du etwas am Boden des Sees schimmern. Du tauchst hinab und greifst danach. Dabei rutscht der Ring auf Deinen Finger, und Du fühlst Dich plötzlich schwächer. Zurück an der Oberfläche merkst Du zudem, dass Du den Ring nicht wieder abbekommst!");
                $mhp = e_rand(1,round($session[user][level]/3));
...
              break;
              case 2:
                output("`&Während des Gespräches bemerkst Du immer wieder, wie toll $lover doch aussiehst, und schämst Dich, dass Du Dich heute morgen nicht sorgfältiger frisch gemacht hast.`n`n"); break;
                $session[user][charm]--;
              break;
              case 3:
...
                    { $session[user][gems] = 0;
                      output("`&Du bist so in das Gespräch vertieft, dass Du gar nicht merkst, wie Dir alle Deine Edelsteine aus der Tasche purzeln und im tiefen Gras verschwinden!`n`n"); break;
                } else
                    { $session[user][gems] -= $gems;
                      output("`&Du bist so in das Gespräch vertieft, dass Du gar nicht merkst, wie Dir $gems Edelsteine aus der Tasche purzeln und im tiefen Gras verschwinden!`n`n"); break;
                }
              break;
        }
        viewcommentary($chat,"Flüstern",10,"flüstert zärtlich");
