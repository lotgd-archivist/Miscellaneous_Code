...
if ($_GET[op]==""){
  output("`3Ein alter Mann hält dich im Wald an und fragt dich: \"`9Wie würde es dir gefallen, ein kleines Ratespielchen ");
    output("mit mir zu spielen?`3\" Da du Leute wie ihn kennst, weißt du, dass er auf einem kleinen Wetteinsatz bestehen wird, wenn du dich darauf einlässt. ");
...
        if ($bet<=0){
            output("`3\"`9Ich denke mir eine Zahl aus und du hast 6 Versuche, diese Zahl zwischen 1 und 100 zu erraten. Ich werde dir immer sagen, ob dein Versuch zu hoch oder zu niedrig war.`3\"`n`n");
            output("`3\"`9Wie hoch ist ein Einsatz, ".($session[user][sex]?"junge Dame":"junger Mann")."?`3\"");
...
          output("`3Der alte Mann streckt seinen Stock aus und klopft damit deinen Golsbeutel ab. \"`9Ich glaub nicht, dass da `^$bet`! Gold drin ist!`3\", erklärt er.`n`n");
...
                  if ($try == 1) {
                        output("`3\"`9UNGLAUBLICH`3\", schreit der alte Mann, \"`9Du hast meine Zahl mit nur `^einem Versuch`! erraten! Nun, ich gratuliere dir. Ich bin stark beeindruckt. Es ist gerade so, als ob du meine Gedanken lesen könntest.`3\" Er schaut dich misstrauisch eine Weile an und überlegt, ob er sich mit deinem Gewinn einfach aus dem Staub machen soll, erinnert sich dann aber an deine scheinbaren geistigen Kräfte und händigt dir deine `^$bet`3 Gold aus.");
                    } else {
                        output("`3\"`9AAAH!!!!`3\", schreit der alte Mann, \"`9Du hast die Zahl mit nur $try Versuchen erraten!  Es war `^".$session[user][specialmisc]."`!!!  Nun, ich gratuliere dir , ");
...
                      output("`3Der Mann gluckst vor Freude: \"`9Die Zahl war `^".$session[user][specialmisc]."`!.`3\" Als der ehrenwerte Bürger, der du bist ");
...
                          $try--;
                          output("`3Der Alte lacht: \"`9Das ist wie einem Baby ein Schwert abzunehmen, wenn du wirklich glaubst, $_POST[guess] ist zwischen 1 und 100!`3\"`n\"`9Du hast noch `^".(6-$try)."`9 Versuche übrig.`3\"`n");
                        } else if ((int)$_POST[guess]>$session[user][specialmisc]){
                          output("`3\"`9Nee, nicht `^".(int)$_POST[guess]."`9, meine Zahl ist kleiner als das!  Das war Versuch `^$try`!.`3\"`n`n");
                        }else{
                          output("`3\"`9Nee, nicht `^".(int)$_POST[guess]."`9, meine Zahl ist größer als das!  Das war Versuch `^$try`!.`3\"`n`n");
                        }
...
        }
    }else{
      output("`3Der alte Mann streckt seinen Stock aus und klopft deinen Goldbeutel ab. \"`9Leer?!?! Wie kannst du etwas setzen ohne Gold??`3\", brüllt er.");
        output("   Damit dreht er sich mit einem HARUMPF um und verschwindet im Unterholz.");
        //addnav("Zurück in den Wald","forest.php");
        $session[user][specialinc]="";
    }
}else if($_GET[op]=="no"){
...
    output("weil du so oder so gewonnen hättest. Jawohl, hätte definitiv keine Chance dieser alte Kerl...");
