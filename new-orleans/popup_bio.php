...
$tattoos = array( //Der Array fÃ¼r die Tattoos
...
    4 => ("Nauthiz"), //DEF +2
    5 => ("Loa Ogoun"), //ATK +3
    6 => ("der DrudenfuÃŸ"), //DEF +3
    7 => ("das Auge des Horus"), //ATK +4
    8 => ("Ankh"), //DEF +4
    9 => ("der SchlÃ¼ssel des Salomon") //spezielle Tiere 2
);
...
output("<td width='100'><a href='popup_bio.php?op=veribio&char=" . $row['acctid'] . "'>Ã¼18-Biografie</a></td>", true);
...
output("<td width='100'><a href='popup_bio.php?op=gb&char=" . $row['acctid'] . "'>GÃ¤stebuch</a></td>", true);
output("<td width='100'><a href='popup_bio.php?op=spass&char=" . $row['acctid'] . "'>Out of Character</a></td>", true);
output("<td width='100'><a href='popup_bio.php?op=perso&char=" . $row['acctid'] . "'>PersÃ¶nliches</a></td>", true);
...
                        <label for='bioavaname' >KÃ¼nstler/Model des Avatars: </label>
...
            ", true);
        
        // Plot-Chars von Andras fÃ¼r New Orleans.
...
        output("`GGeschlecht: `7" . ($row['sex'] ? "Weiblich" : "MÃ¤nnlich") . "`n");
        output("`GGesinnung: `7" . $gesinnung[$row['gesinnung']] . "`n");
        if ($row['fsk'] == 0)
            output("`GVolljÃ¤hrig: `7Keine Angabe`n");
        if ($row['fsk'] == 1)
            output("`GVolljÃ¤hrig: `7Nein`n");
        if ($row['fsk'] == 2)
            output("`GVolljÃ¤hrig: `7Ja`n");
...
            output("`n`n`b#c0c0c0FamiliÃ¤res Umfeld:`0`b`n");
        if ($row['char_parents'] != "")
...
        if ($row['char_bro'] != "")
            output("`GBrÃ¼der: `7" . $row['char_bro'] . "`n");
        if ($row['char_sis'] != "")
...
        if ($row['char_affairs'] != "")
            output("`GAffÃ¤ren: `7" . $row['char_affairs'] . "`n");
...
            <div id='familyBox'>
                <fieldset>
                    <legend> FamiliÃ¤res Umfeld </legend>
...
                                        <label for='bioBruder'>BrÃ¼der: </label><input type='text' name='bioBruder' value='" . escapeTags($row['char_bro']) . "' size='40'><br />
...
                    <label for='bioAffaeren'>AffÃ¤ren: </label><input type='text' name='bioAffaeren' value='" . escapeTags($row['char_affairs']) . "' size='40'><br />
...
                output("(Der Bauch ist schÃ¶n rundlich.)");
...
            output("`rDer Chara ist ein SklavenhÃ¤ndler!`0");
        }
...
        if ($row['schmuck'] == 0)
            output("`GRingtrÃ¤ger: `7Nein`n");
        if ($row['schmuck'] == 1)
            output("`GRingtrÃ¤ger: `7Ja`n");
        if ($row['jobid'])
...
        
        output("`n`n`b#c0c0c0Verbindung:`0`b`n");
        /* Gildenaddon by Eliwood fÃ¼r Eliwoods Gilden */
        if ($row['memberid'] > 0) {
...
        
        output("<table border='0'><tr><td>", true);
        output("`n`n`b#c0c0c0Gemeinschaft:`0`b`n");
        
        if ($row['c_memberid'] > 0) {
...
        if ($row['bio_size'])
            output("`GGrÃ¶ÃŸe: `7" . $row['bio_size'] . "`n", true);
        if ($row['bio_frisur'])
...
            output("`GRÃ¼stung: `7" . $row['armor'] . "`n", true);
...
                    <label for='bioSize'>GrÃ¶ÃŸe: </label><input type='text' name='bioSize' value='" . escapeTags($row['bio_size']) . "' size='40'><br />
...
                output("`n`1GroÃŸe Liga: `7kein Preis");
            if ($row['chocgrossprice'] == "platzeins")
                output("`n`1GroÃŸe Liga: `7<img src='/logd/images/system/chocobos/platz1.gif'/>", true);
            if ($row['chocgrossprice'] == "platzzwei")
                output("`n`1GroÃŸe Liga: `7<img src='/logd/images/system/chocobos/platz2.gif'/>", true);
            if ($row['chocgrossprice'] == "platzdrei")
                output("`n`1GroÃŸe Liga: `7<img src='/logd/images/system/chocobos/platz3.gif'/>", true);
        }
        
...
        $charmsteps = array( 'sehr hÃ¤sslich' => 30, 'hÃ¤sslich' => 100, 'unschÃ¶n' => 150, 'durchschnittlich schÃ¶n' => 300, 'schÃ¶n' => 500, 'sehr schÃ¶n' => 750, 'unbeschreiblich schÃ¶n' => 1000, 'StadtschÃ¶nheit' => 6000);
        $c_i = 0;
        $c_name = $c_val = 0;
        $c_nxtname = '';
        //note by Salator: Ich weiÃŸ nicht warum, aber mit der auf Atrahor installierten PHP-Version mÃ¼ssen die Zeilen mit next weg
...
        $bio = $row['bio'];
        
        // VerÃ¤ndern, ob man FSK18 Inhalte sehen oder nicht sehen will
...
        // ist die User, dem die Bio gehÃ¶rt Ã¼berhaupt als volljÃ¤hrig eingetragen?
        if ($row['fsk'] == 2) {
            
...
                
                // Wenn ja und der aktuelle User ist Ã¼ber 18, Link zum Ein-/Ausstellen der FSK18 Inhalte anzeigen
...
                
                // Wenn mehr [/FSK18] Tags vorhanden sind als [FSK18] Tags, dann [/FSK18] Tags hinzufÃ¼gen
                if ($difference > 0) {
...
                <fieldset>
                    <legend> Bio </legend>
                    <textarea name='biofield' cols='110' rows='50'>" . $bio . "</textarea>
                </fieldset>
            </div>
            ", true);
        
...
    case "gb":
        $char = $_GET['char'];
        output("`n<font face='Andalus'><font size=6>#303030G#3d3d3dÃ¤#4a4a4as#575757t#646464e#717171b#7e7e7eu#8b8b8bc#989898h #a5a5a5v#b2b2b2o#c0c0c0n`0 " . $row['name'] . "`7</font face></font size>", true);
        if ($session['user']['loggedin'])
...
            } else {
                output("`c`b`4Noch keine EintrÃ¤ge`0`b`c");
            }
        }
        output("`n`n`n`n");
        break;
...
            db_query($sql);
            systemmail($row['acctid'], "`^GÃ¤stebucheintrag!`0", "`&" . $session['user']['name'] . "`3 hat dir einen Eintrag in deinem GÃ¤stebuch hinterlassen.");
...
    
    case "perso":
        output("`n`G<font face='Andalus'><font size=6>#4a4a4aP#515151e#595959r#616161s#696969Ã¶#707070n#787878l#808080i#888888c#909090h#949494e #989898A#9c9c9cn#a1a1a1g#a5a5a5a#aaaaaab#aeaeaee#b2b2b2n #b7b7b7v#bbbbbbo#c0c0c0n`0 $row[name]`7</font face></font size>", true);
        if ($session['user']['loggedin'])
            output(" <a href=\"popup_mail.php?op=write&to={$row['login']}\" target=\"_blank\" onClick=\"" . popup("popup_mail.php?op=write&to={$row['login']}") . ";return false;\"><img src='images/Assos/Icons/newsroll.png' width='16' height='16' alt='Mail schreiben' border='0'></a>", true);
        
        if (($row['acctid'] == $session['user']['acctid'] && $session['user']['acctid'] != 4) || $session['user']['superuser'] > 2)
...
            output("`GGeschlecht: `7MÃ¤nnlich`n");
...
        }
        output("`GÃœber mich: `7" . $personal_about . "`n");
        
        if ($row['fsk'] == 0)
            output("`GVolljÃ¤hrig: `7Keine Angabe`n");
        if ($row['fsk'] == 1)
...
                <div id='personalBox'>
                    <fieldset>
                        <legend> PersÃ¶nliche Informationen </legend>
...
            output("<option name='personalSex' value=1>MÃ¤nnlich</option>", true);
...
            output("<option name='personalSex' value=1 selected>MÃ¤nnlich</option>", true);
            output("<option name='personalSex' value=2>Weiblich</option>", true);
...
                        <label for='personalAbout' >Ãœber mich: </label> <textarea name='personalAbout' cols='30' rows='10'>" . escapeTags($row['personal_about']) . "</textarea>
                    </fieldset>
                </div>
                ", true);
        
...
    
    case "veribio":
        output("`n`G<font face='Andalus'><font size=6>#303030Ãœ#383838b#404040e#494949r #5151511#5a5a5a8 #626262B#6b6b6bi#737373o#7c7c7cg#848484r#8d8d8da#959595f#9e9e9ei#a6a6a6e #afafafv#b7b7b7o#c0c0c0n`0 $row[name]`7</font face></font size>", true);
        if ($session['user']['loggedin'])
            output(" <a href=\"popup_mail.php?op=write&to={$row['login']}\" target=\"_blank\" onClick=\"" . popup("popup_mail.php?op=write&to={$row['login']}") . ";return false;\"><img src='images/Assos/Icons/newsroll.png' width='16' height='16' alt='Mail schreiben' border='0'></a>", true);
        
        if (($row['acctid'] == $session['user']['acctid'] && $session['user']['acctid'] != 4) || $session['user']['superuser'] > 2)
...
        
        if ($row['fsk'] != 2) {
            output("`c`b`r Der gewÃ¤hlte Charakter ist nicht verifiziert und besitzt daher eine solche Bio nicht.`b`c`n`n");
...
                    if ($_POST['pass1'] != $_POST['pass2'])
                        $session['alert_msg'] = '`$Die angegebenen PasswÃ¶rter stimmen nicht Ã¼berein!';
...
            } else
                $session['alert_msg'] = '`$Du kannst mit dem Testchara nicht das Passwort und die Email Ã¤ndern.';
            
            
            reset($_POST);
            $no_prefs = array(
...
                'email' => 'Email Ã¤ndern',
                "Darstellungsoptionen fÃ¼r die Bio,title",
                "show_no_title" => "Standard-Systemtitel anzeigen?,bool",
                "show_no_fight" => "Kampfinformationen anzeigen?,bool",
                "use_n" => "in der Bio AbsÃ¤tze mit Enter statt mit ``n machen?,bool",
                "Allgemein Einstellungen,title",
                "directspeach" => "Direkte Rede automatisch einfÃ¤rben?,bool",
                //"nosounds"             => "Die Sounds deaktivieren?,bool",
                "otset" => "OOC Chat,enum,0,Einwohnerliste,1,Obere Navigation,2,Beides",
                "flist" => "Freundesliste anzeigen?,toolbool,Die Freundesliste ist identisch mit dem Adressbuch in deinen YoMs. FÃ¼ge dort Freunde hinzu und diese werden auch in der Freundesliste sichtbar. Die Freundesliste befindet sich unter den Charakterwerten[KOMMA] Ã¼berhalb der Hier anwesend Liste[KOMMA] falls diese aktiviert wurde.",
                "ulist" => "'Wer ist hier' anzeigen?,toolbool,Diese Liste erscheint unter den Charakterwerten[KOMMA] bzw. unter der Freundesliste[KOMMA] falls diese aktiviert wurde. Sie zeigt an[KOMMA] welcher Spieler sich noch am selben Ort/Platz befindet wie Du.<br /><i>Achtung: Im Wohnviertel oder den Magischen Orten mÃ¼ssen die anderen angezeigten Spieler sich nicht im gleichen Raum/Ort wie Du aufhalten!</i>",
                "log" => "Changelog anzeigen?,toolbool,Der Changelog zeigt die VerÃ¤nderungen am Server an. Er wird bei fast jeder noch so kleinen Arbeit am Server durch die Techadmins aktualisiert. Falls Du wissen willst[KOMMA] was an Eassos gearbeitet wurde[KOMMA] stelle dies hier auf ja. Der Link dafÃ¼r wird ganz unten bei deinen Charakterwerten sichtbar.",
                "list_type" => "Einwohnerlisten Stil:,enum,0,alt,1,neu",
                "Farb- und Kommentareinstellungen,title",
                "commenttalkcolor" => "Standardfarbe bei GesprÃ¤chen",
                "commentemotecolor" => "Standardfarbe bei Emotes (/me)",
                "comlim" => "Angezeigte Kommentare auf PlÃ¤tzen`n(`bStandart 10`b),int",
                "yom_sig" => "Signatur fÃ¼r YoM-Nachrichten",
                "Schnelleingaben,tooltitle,Schnelleingaben werden unter dem Kommentarfeld nach einem Klick auf Tags zeigen angezeigt. Hier kannst Du lÃ¤ngere WÃ¶rter (auch mit Farbe) hinterlegen. Z.B. den farbigen Namen eines Partners oder eines Begleiters.",
...
                "Info-Meldung (KÃ¤mpferliste),title",
                "info" => "Informationsmeldung fÃ¼r die KÃ¤mpferliste:,textarea,44,10",
...
            
            /*
            Bio Backupsystem von Andras fÃ¼r Saint-Omar
            */
            
            // ÃœberprÃ¼fe, ob es bereits ein Backup gibt
...
                                </tr><tr>
                                    ' . ($time_dif > $month ? '<tr><td colspan="2" style="text-align: center">`i`$Dein Backup ist Ã¼ber einen Monat alt. Du solltest einmal ein neues anlegen.`0 </td></tr><tr>' : '') . '
                                    <td colspan="2" style="text-align: center"> ' . createLink('neues Backup erzeugen', 'popup_bio.php?op=create_backup&char=' . $char, false, 'Achtung, Dein bisheriges Backup wird Ã¼berschrieben, wenn du ein neues anfertigst! Fortfahren?') . ' </td>
                                </tr><tr>
...
    
    
    //Gott, ist das hÃ¤sslich. -_-
    //Warum zum Teufel wird der Wert erst in ne extra-Variable geschrieben, bevor er gespeichert wird? Ã´o (Nashyan)
    case "savebio":
        $player = $_GET['user'];
        $char   = $_GET['char'];
        
...
            if($multilists)
            {
                output('Das System hat erkannt, dass bereits '.count($multilists).' vorhandene Liste'.(count($multilists) > 1 ? 'n' : '').' mit Dir existieren. Willst Du eine dieser Listen Ã¼bernehmen? `n`n');
                
...
                                    <div style="float: right">#808080[ '.createLink('Diese Liste Ã¼bernehmen', 'popup_bio.php?op=insert_multilist&list_id='.$multilist['multi_group'].'&char='.$session['user']['acctid'], false, 'Willst Du wirklich '.stripColors($multilist['owner']).'s Liste fÃ¼r diesen Char Ã¼bernehmen?').' #808080] </div>
                                </span> `n
                                <span style="cursor: pointer;" class="multis_toggler">DazugehÃ¶rige Chars:</span> `n`n', true);
                    
...
                        Falls Du welche besitzt, kannst Du diese aus der Liste unten auswÃ¤hlen oder einen Multi manuell suchen. `n`n`n');
        }
        else
        {
            $n = count($multis);
...
        if($n)
        {
            output('`n`n`bDas System hat `i'.$n.'`i neue'.($n > 1 ? '' : 'n').' Multi'.($n > 1 ? 's' : '').' von Dir gefunden. Du kannst diese nun mit eingabe des Passwortes verifizieren und zu Deiner Liste hinzufÃ¼gen.`b`n`n
                    <table cellpadding="10">
                        <tr class="trhead">
                            <td> `b#000000acctid`b </td>
                            <td> `b#000000Name`b </td>
...
                            <td> '.createLink('hinzufÃ¼gen', 'popup_bio.php?op=confirm_multi&user_id='.$new_multi['acctid'].'&char='.$session['user']['acctid']).' </td>
                        </tr>', true);
            }
            
...
                Sollte der gewÃ¼nschte Multi nicht in der List weiter oben angezeigt werden, liegt das wahrscheinlich daran, dass Du ihn schon lÃ¤nger nicht mehr eingeloggt hast, den Browser gewechselt hast und eine andere Emailadresse fÃ¼r ihn verwendet hast. `n
...
            
            if($n == 0)
                output('Keine Ergebnisse fÃ¼r Deine Suche gefunden.');
            
            elseif($n > 50)
                output('Mehr als 50 Spieler mit Deiner Eingabe gefunden. PrÃ¤zisiere Deine Eingabe!');
            
            else
            {
...
                                <td> '.$row['acctid'].' </td>
                                <td> '.$row['name'].' </td>
                                <td> '.createLink('hinzufÃ¼gen', 'popup_bio.php?op=confirm_multi&user_id='.$row['acctid'].'&char='.$session['user']['acctid']).' </td>
                            </tr>', true);
                }
                
                output('</table>', true);
...
        include './lib/include/multi_funcs.php';
        
        // Keine Listen-ID Ã¼bermittelt? Da brechen wir ab...
...
            output('Es wurden '.++$i.' Chars als Deine Multis hinzugefÃ¼gt.`n');
            
            db_query($prepare_sql);
        }
        else 
        {
            output('Du wurdest in der ausgewÃ¤hlten Liste gar nicht gefunden... Hmmm... komisch... wie kommst Du denn dann Ã¼berhaupt hier her?');
            $prepare_sql = '';
        }
        break;
    
...
        {
            $row = db_fetch_assoc($result);
            output('#808080Du willst '.$row['name'].'#808080 als einen Deiner Multis bestÃ¤tigen. Gib hierfÃ¼r bitte das zu '.$row['name'].' #808080gehÃ¶rige Passwort ein! `n`n
                    <center>
                    <form action="popup_bio.php?op=add_multi&user_id='.$_GET['user_id'].'&char='.$session['user']['acctid'].'" method="POST">
                        <input type="password" name="pw" autofocus> <input type="submit" class="button" value="HinzufÃ¼gen">
                    </form></center>', true);
        }
        else
            output('Es wurde kein Account mit der ID '.$_GET['user_id'].' gefunden...`n
                    Komisch... wie kommst Du hier denn Ã¼berhaupt hin?');
        break;
    
    case 'add_multi':
        if(!isset($_GET['user_id']))
...
                
                output('#FF0000Du hast das Passwort fÃ¼nf Mal falsch eingegeben! Dein Account wurde vorÃ¼bergehend von der Multiliste gesperrt. Das Adminteam wurde benachrichtigt.');
                
...
                global_Log($session['user']['name'].'#808080 ('.$session['user']['acctid'].') hat fÃ¼nf Mal das falsche Passwort eingegeben, um den Char mit der `bID '.$_GET['user_id'].'`b ('.$user_row['name'].'#808080) zu seinen Multis hinzuzufÃ¼gen.');
                
                $sql     = 'SELECT acctid FROM accounts WHERE superuser > 0';
                $result = db_query($sql);
                while($admin = db_fetch_assoc($result))
                {
                    $message = $session['user']['name'].'#808080 ('.$session['user']['acctid'].') hat fÃ¼nf Mal das falsche Passwort eingegeben, um den Char mit der `bID '.$_GET['user_id'].'`b('.$user_row['name'].'#808080)  zu seinen Multis hinzuzufÃ¼gen.';
