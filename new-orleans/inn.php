...
page_header("Algiers Basar - Urzeitschenke");

$session['user']['standort'] = "Algiers Basar - Urzeitschenke";
...
switch($_GET['op']){
    default:
        addnav("aktualisieren","inn.php");
        
        addnav("Was machst du?");
                addnav("Arbeiten","work.php");
                addnav("An den Tisch","inn_rooms.php?op=hauptraum");
                addnav("An den Tresen","inn_rooms.php?op=tresen");
        addnav("Spreche mit Cedrik dem Wirt","inn.php?op=bartender");
...
        addnav("zum Basar","mythalgiers_basar.php");
        addnav("Myth Algiers","mythalgiers.php");
...
        
        output("`c`úDas aktuelle Thema in der Schenke:`c");
        
        switch (e_rand(1,10)){
            case 1:
                output("`c`àDrachen.`0`c");
            break;
            
            case 2:
                output("`c`xSeth`à.`0`c");
            break;
            
            case 3:
                output("`c`xViolet`à.`0`c");
            break;
            
            case 4:
                output("`c`àleckeres Ale.`0`c");
            break;
            
            case 5:
                output("`c`àZwergenweitwurf.`0`c");
            break;
            
            case 6:
                output("`c`àdas elementare Zerwürfnis des Seins.`0`c");
            break;
            
            case 7:
                output("`c`àHäufig gestellte Fragen.`0`c");
            break;
...
        }
    
    output("`c`n`íDie Uhr am Kamin zeigt `Û".getgametime()."`0`í.`c");
...
        output("`c`éAm schwarzen Brett neben der Tür ist nicht eine einzige Nachricht zu sehen.`c");
    }else{
        output("`c`éAm schwarzen Brett neben der Tür flattern einige Nachrichten im Luftzug:`c");
        for ($i=0;$i<db_num_rows($result);$i++){
            $row = db_fetch_assoc($result);
            output("`c`n`n<a href=\"popup_mail.php?op=write&to=".rawurlencode($row['login'])."\" target=\"_blank\" onClick=\"".popup("popup_mail.php?op=write&to=".rawurlencode($row['login'])."").";return false;\"><img src='images/newscroll.GIF' width='16' height='16' alt='Mail schreiben' border='0'></a>`c",true);
            output("`c`& $row[name]`&:`n`^$row[message]`0`c");
            if ($row[acctid]==$session[user][acctid]){
                output("`c[<a href='inn.php?op=msgboard&act=del'>entfernen</a>]`c",true);
                addnav("","inn.php?op=msgboard&act=del");
            }
        }
    }

                viewcommentary("lachendeswildschwein","Hinzufügen:`0");
...
            if(get_special_val('seenlover') == 0){
...
<img src='images/Astaros/Der_Strand.jpg' width='100%' height='100%'></td>
<td width='10'></td>
</tr>
</table>");
        viewcommentary("inn_raum","Unterhalte dich:",20);
    break;
    
    case "bar":
        output("
...
<tr>
<td width='10'></td><td align='center'>
<img src='images/Astaros/.jpg' width='100%' height='100%'></td>

<td width='10'></td>
</tr>
</table>");
        viewcommentary("inn_tresen","Sprich mit dem Wirt:",20);
    break;
    
    case "bartender":
        if (getsetting("paidales",0)<=1 || get_special_val('gotfreeale') >= 2) {
...
                addnav("Ale (`^".(get_special_val('gotfreeale')>=2?"$alecost`0 Gold":"schon bezahlt`0").")","inn.php?op=bartender&act=ale");
                output("`nEs stehen noch $amt frisch gefüllte und schon bezahlte Krüge mit Ale vor Cedrik.");
                if (get_special_val('gotfreeale')>=2) output(" Leider hattest du dein Frei-Ale für heute schon und du wirst selbst bezahlen müssen.");
            }
...
                } else if (abs(get_special_val('gotfreeale')-2)==1){
...
                            $freeales = get_special_val('gotfreeale') + 2;
                            set_special_val('gotfreeale', $freeales);
                        }
