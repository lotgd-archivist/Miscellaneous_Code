...
// hint: use search&replace ;) replace 'logout' with your module's (internal) name.
function module_getinfo_logout() {
    $info = array(
                    'modulename'=>'logout',         // internal name; use letters, numbers and underscores only!
...
function module_uninstall_logout() {
...
function module_build_logout($houseid) {
...
function module_destroy_logout($houseid) {
    // this is only needed if 'built_in' in module_info() is set to 0
...
        $sql = "UPDATE items SET hvalue=".$session['user']['specialmisc']['houseid']." WHERE value1=".(int)$session['user']['specialmisc']['houseid']." AND owner=".$session['user']['acctid']." AND class='SchlÃ¼ssel'";
