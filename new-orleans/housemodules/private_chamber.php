...
            addnav("Einladung zurÃ¼ckziehen","houses.php?op=drin&act=privoutvite");
        }
...
            
            viewcommentary("house_private-".$session['user']['specialmisc']['houseid'],($session['user']['sex']?"Deinem":"Deiner")." Liebsten zuflÃ¼stern:");

        // MÃ¶bel
        $sql = "SELECT * FROM items WHERE class='MÃ¶bel' AND value1=".$session['user']['specialmisc']['houseid']." AND value2=5 AND name!='' ORDER BY id ASC";
...
        }else{ //jeder andere
            output("`Ã”Du rÃ¼ttelst ein wenig an der TÃ¼r - verschlossen.`n Tja, in den PrivatgemÃ¤chern hast du eben nichts zu suchen.`0");
        }
        addnav("ZurÃ¼ck");
...
                    if ($row2[acctid]==$row[owner]) output(" (EigentÃ¼mer)`n");
                    output("`c");
                }
                $amt=$row2[acctid];
            }
...
            $result = db_query($sql) or die(db_error(LINK));
       
            $sql = "SELECT owner FROM items WHERE value1=$row[houseid] AND class='SchlÃ¼ssel' ORDER BY value2 ASC";
            $result = db_query($sql) or die(db_error(LINK));
...
                    $sql = "INSERT INTO commentary (postdate,section,author,comment) VALUES (now(),'house_tresor-".$session['user']['specialmisc']['houseid']."',".$session['user']['acctid'].",'".$session['user']['name']."`0 lÃ¤d ".$row2['name']." `0einen.')";
                    db_query($sql) or die(db_error(LINK));
                }
            }
        }
...
            output("</select>`n`n",true);
            output("<input type='submit' class='button' value='Einladung zurÃ¼ckziehen'></form>",true);
...
            $result2 = db_query($sql);
            $row2  = db_fetch_assoc($result2);
            output("`2Du ziehst die Einladung an `&$row2[name]`2 zurÃ¼ck.`n");

            systemmail($row2['acctid'],"`@Einladung zurÃ¼ckgezogen!`0","`&{$session['user']['name']}`2 hat seine Einladung fÃ¼r das Privatgemach im Haus Nummer `b$row[houseid]`b ($row[housename]`2) zurÃ¼ckgezogen.");

            $sql = "DELETE FROM items WHERE owner=$row2[acctid] AND class='PrivEinladung' AND value1=$row[houseid]";
            db_query($sql);

            $sql = "INSERT INTO commentary (postdate,section,author,comment) VALUES (now(),'house_tresor-".$session['user']['specialmisc']['houseid']."',".$session['user']['acctid'].",'".$session['user']['name']."`0 zieht die Einladug an ".$row2['name']." `0zurÃ¼ck.')";
            db_query($sql) or die(db_error(LINK));
...
        output("`c`)Mit wem mÃ¶chtest du eine Liebesnacht verbringen?`0`c`n`n",true);
        
...
                $sex_info = "`i(geschÃ¼tzt)`Ã¬";
            }else{
                $sex_info = "`i(ungeschÃ¼tzt)`Ã¬";
            }
        }
...
                    output("<td>`c`i`&Du wartest noch auf deinen Partner`i`n".$sex_info."`c`n`c<a href='houses.php?op=drin&act=abort&partner=".$row_sex_check['partner_b']."' onClick=\"return confirm('Willst du die Anfrage wirklich zurÃ¼ckziehen?');\">`4lieber doch nicht`0</a>`c</td></tr>",true);
...
                    output("<td>`c`i`&Du wartest noch auf deinen Partner`i`n".$sex_info."`c`n`c<a href='houses.php?op=drin&act=abort&partner=".$row_sex_check['partner_a']."' onClick=\"return confirm('Willst du die Anfrage wirklich zurÃ¼ckziehen?');\">`4lieber doch nicht`0</a>`c</td></tr>",true);
                    addnav("","houses.php?op=drin&act=abort&partner=".$row_sex_check['partner_a']."");
                }else{
                    if($row_sex_check['type'] == 0){
                        output("<td>(geschÃ¼tzter Sex) [<a href='houses.php?op=drin&act=safe&partner=".$sex_checke_partner."'>`2Annehmen`0</a>] || [<a href='houses.php?op=drin&act=no&partner=".$sex_checke_partner."'>`4Ablehnen`0</a>]</td></tr>",true);
                        
                        addnav("","houses.php?op=drin&act=safe&partner=".$sex_checke_partner."");
                    }else{
                        output("<td>(ungeschÃ¼tzter Sex) [<a href='houses.php?op=drin&act=kind&partner=".$sex_checke_partner."'>`2Annehmen`0</a>] || [<a href='houses.php?op=drin&act=no&partner=".$sex_checke_partner."'>`4Ablehnen`0</a>]</td></tr>",true);
                        
...
                }
            }else{
                output("<td>`c[<a href='houses.php?op=drin&act=doit&type=0&partner=".$row_ehepartner['acctid']."'>GeschÃ¼tzt</a>] || [<a href='houses.php?op=drin&act=doit&type=1&partner=".$row_ehepartner['acctid']."'>UngeschÃ¼tzt</a>]`c</td></tr>",true);
                
...
        
        if($session['user']['house'] == $session['user']['specialmisc']['houseid']){
            output("<tr class='trhead'><td colspan='2'>`c`bMÃ¶gliche Partner:`b`c</td><tr>",true);
...
                            output("<td>`c`i`&Du wartest noch auf deinen Partner`i`n".$sex_info."`c`n`c<a href='houses.php?op=drin&act=abort&partner=".$row_sex_check2['partner_b']."' onClick=\"return confirm('Willst du die Anfrage wirklich zurÃ¼ckziehen?');\">`4lieber doch nicht`0</a>`c</td></tr>",true);
...
                            output("<td>`c`i`&Du wartest noch auf deinen Partner`i`n".$sex_info."`c`n`c<a href='houses.php?op=drin&act=abort&partner=".$row_sex_check2['partner_a']."' onClick=\"return confirm('Willst du die Anfrage wirklich zurÃ¼ckziehen?');\">`4lieber doch nicht`0</a>`c</td></tr>",true);
                            addnav("","houses.php?op=drin&act=abort&partner=".$row_sex_check2['partner_a']."");
                        }else{
                            if($row_sex_check['type'] == 0){
                                output("<td>(geschÃ¼tzter Sex) [<a href='houses.php?op=drin&act=safe&partner=".$sex_checke_partner2."'>`2Annehmen`0</a>] || [<a href='houses.php?op=drin&act=no&partner=".$sex_checke_partner2."'>`4Ablehnen`0</a>]</td></tr>",true);
                                
                                addnav("","houses.php?op=drin&act=safe&partner=".$sex_checke_partner2."");
                            }else{
                                output("<td>(ungeschÃ¼tzter Sex) [<a href='houses.php?op=drin&act=kind&partner=".$sex_checke_partner2."'>`2Annehmen`0</a>] || [<a href='houses.php?op=drin&act=no&partner=".$sex_checke_partner2."'>`4Ablehnen`0</a>]</td></tr>",true);
                                
...
                        }
                    }else{
                        output("<td>`c[<a href='houses.php?op=drin&act=doit&type=0&partner=".$row_partner['acctid']."'>GeschÃ¼tzt</a>] || [<a href='houses.php?op=drin&act=doit&type=1&partner=".$row_partner['acctid']."'>UngeschÃ¼tzt</a>]`c</td></tr>",true);
                        
...
                    output("<td>`c`i`&Du wartest noch auf deinen Partner`i`n".$sex_info."`c`n`c<a href='houses.php?op=drin&act=abort&partner=".$row_sex_check3['partner_b']."' onClick=\"return confirm('Willst du die Anfrage wirklich zurÃ¼ckziehen?');\">`4lieber doch nicht`0</a>`c</td></tr>",true);
...
                    output("<td>`c`i`&Du wartest noch auf deinen Partner`i`n".$sex_info."`c`n`c<a href='houses.php?op=drin&act=abort&partner=".$row_sex_check3['partner_a']."' onClick=\"return confirm('Willst du die Anfrage wirklich zurÃ¼ckziehen?');\">`4lieber doch nicht`0</a>`c</td></tr>",true);
                    addnav("","houses.php?op=drin&act=abort&partner=".$row_sex_check3['partner_a']."");
                }else{
                    if($row_sex_check3['type'] == 0){
                        output("<td>(geschÃ¼tzter Sex) [<a href='houses.php?op=drin&act=safe&partner=".$row_partner2['acctid']."'>`2Annehmen`0</a>] || [<a href='houses.php?op=drin&act=no&partner=".$row_partner2['acctid']."'>`4Ablehnen`0</a>]</td></tr>",true);
                        addnav("","houses.php?op=drin&act=safe&partner=".$row_partner2['acctid']."");
                    }else{
                        output("<td>(ungeschÃ¼tzter Sex) [<a href='houses.php?op=drin&act=kind&partner=".$row_partner2['acctid']."'>`2Annehmen`0</a>] || [<a href='houses.php?op=drin&act=no&partner=".$row_partner2['acctid']."'>`4Ablehnen`0</a>]</td></tr>",true);
...
                }
            }else{
                output("<td>`c[<a href='houses.php?op=drin&act=doit&type=0&partner=".$row_partner2['acctid']."'>GeschÃ¼tzt</a>] || [<a href='houses.php?op=drin&act=doit&type=1&partner=".$row_partner2['acctid']."'>UngeschÃ¼tzt</a>]`c</td></tr>",true);
                    
...
                db_query($sql_sex_promt) or die(db_error(LINK));
            }
            systemmail($row_partner['acctid'],"`2Ein paar schÃ¶ne Stunden?`0","`&{$session['user']['name']}`2 mÃ¶chte mit dir ein paar schÃ¶ne Stunden im Haus (Nummer: ".$row['houseid']." ($row[housename]`2) verbringen. Hast du nicht auch Lust?`0");
        }else{
...
            systemmail($row_partner['acctid'],"`2Ein paar schÃ¶ne Stunden?`0","`&{$session['user']['name']}`2 mÃ¶chte mit dir ein paar schÃ¶ne Stunden im Haus (Nummer: ".$row['houseid']." ($row[housename]`2) verbringen. Hast du nicht auch Lust?`0");
        }
        output("`qDu mÃ¶chtest mit `&".$row_partner['name']."`q ein paar schÃ¶ne Stunden verbringen und machst mehr als eindeutige Angebote. Ob `&".$row_partner['name']."`q darauf eingehen wird?");
        
        addnav("ZurÃ¼ck");
        addnav("zum Privatgemach","houses.php?op=drin&module=".$moduleid."");
...
            $text = "`q eigentlich nur kurz in euren SchlafgemÃ¤chern vorbei schaut, ziehst Du ihn ins Bett und ihr liebt euch den ganzen restlichen Tag heiÃŸ und innig.`nIhr bleibt schmusend noch ein wenig liegen ...`0";
        }else if($row_partner['sex'] == 1){
            $text = "`q eigentlich nur kurz in euren SchlafgemÃ¤chern vorbei schaut, ziehst Du sie ins Bett und ihr liebt euch den ganzen restlichen Tag heiÃŸ und innig.`nIhr bleibt schmusend noch ein wenig liegen ...`0";
        }
        output("`qAls `0".$row_partner['name']." ".$text);
        
        if($row_partner['sex'] == 1){
            $text_mail = "`q eigentlich nur kurz in euren SchlafgemÃ¤chern vorbei schaut, ziehst Du ihn ins Bett und ihr liebt euch den ganzen restlichen Tag heiÃŸ und innig.`nIhr bleibt schmusend noch ein wenig liegen ...`0";
        }else if($row_partner['sex'] == 0){
            $text_mail = "`q eigentlich nur kurz in euren SchlafgemÃ¤chern vorbei schaut, ziehst Du sie ins Bett und ihr liebt euch den ganzen restlichen Tag heiÃŸ und innig.`nIhr bleibt schmusend noch ein wenig liegen ...`0";
        }
...
                {
                    $text = "`q Als ".$row_partner['name']." eigentlich nur kurz in euren SchlafgemÃ¤chern vorbei schaut, ziehst Du sie ins Bett und ihr liebt euch den ganzen restlichen Tag heiÃŸ und innig.`n
                             Ihr bleibt schmusend noch ein wenig liegen. Doch irgendwie habt ihr das GefÃ¼hl, es kÃ¶nnte etwas geschehen sein und du hast den Eindruck, dass ".$row_partner['name']."`0 schwanger sein kÃ¶nnte ....`0";
                }
                else
                {
                    $text = "`q eigentlich nur kurz in euren SchlafgemÃ¤chern vorbei schaut, ziehst Du ihn ins Bett und ihr liebt euch den ganzen restlichen Tag heiÃŸ und innig.`n
                             Ihr bleibt schmusend noch ein wenig liegen.  Doch irgendwie habt ihr das GefÃ¼hl, es kÃ¶nnte etwas geschehen sein und dir dÃ¤mmert, dass du etwas unterm Herzen tragen kÃ¶nntest .... `0";
                }
                output("`qAls `0".$row_partner['name']." ".$text);
                
                
                if($session['user']['sex'] == 0)
                {
                    $text_mail = "`q Als ".$row_parter['name']." eigentlich nur kurz in euren SchlafgemÃ¤chern vorbei schaut, ziehst Du ihn ins Bett und ihr liebt euch den ganzen restlichen Tag heiÃŸ und innig.`n
                                  Ihr bleibt schmusend noch ein wenig liegen. Doch irgendwie habt ihr das GefÃ¼hl, es kÃ¶nnte etwas geschehen sein und du hast den Eindruck, dass du etwas unterm Herzen tragen kÃ¶nntest .... `0";
                }
                else
                {
                    $text_mail = "`qAls du eigentlich nur kurz in eurem Schlafgemach vorbeischaust, ziehst Du sie ins Bett und ihr liebt euch den ganzen restlichen Tag heiÃŸ und innig.`n
                                  Ihr bleibt schmusend noch ein wenig liegen. Doch irgendwie habt ihr das GefÃ¼hl, es kÃ¶nnte etwas geschehen sein und du hast den Eindruck, dass ".$session['user']['name']."`0 schwanger sein kÃ¶nnte ....`0";
                }
                
                systemmail($row_partner['acctid'],"`2Ein aufregendes Abenteuer.`0",$text_mail);
            }else{
                if($row_partner['sex'] == 0){
                    $text = "`q eigentlich nur kurz in euren SchlafgemÃ¤chern vorbei schaut, ziehst Du ihn ins Bett und ihr liebt euch den ganzen restlichen Tag heiÃŸ und innig.`nIhr bleibt schmusend noch ein wenig liegen. Doch irgendwie habt ihr das GefÃ¼hl, es kÃ¶nnte etwas geschehen sein .... `0";
                }else if($row_partner['sex'] == 1){
                    $text = "`q eigentlich nur kurz in euren SchlafgemÃ¤chern vorbei schaut, ziehst Du sie ins Bett und ihr liebt euch den ganzen restlichen Tag heiÃŸ und innig.`nIhr bleibt schmusend noch ein wenig liegen. Doch irgendwie habt ihr das GefÃ¼hl, es kÃ¶nnte etwas geschehen sein ....`0";
                }
                output("`qAls `0".$row_partner['name']." ".$text);
            
                if($row_partner['sex'] == 1){
                    $text_mail = "`q eigentlich nur kurz in euren SchlafgemÃ¤chern vorbei schaut, ziehst Du ihn ins Bett und ihr liebt euch den ganzen restlichen Tag heiÃŸ und innig.`nIhr bleibt schmusend noch ein wenig liegen. Doch irgendwie habt ihr das GefÃ¼hl, es kÃ¶nnte etwas geschehen sein .... `0";
                }else if($row_partner['sex'] == 0){
                    $text_mail = "`q eigentlich nur kurz in euren SchlafgemÃ¤chern vorbei schaut, ziehst Du sie ins Bett und ihr liebt euch den ganzen restlichen Tag heiÃŸ und innig.`nIhr bleibt schmusend noch ein wenig liegen. Doch irgendwie habt ihr das GefÃ¼hl, es kÃ¶nnte etwas geschehen sein ....`0";
                }
                
                systemmail($row_partner['acctid'],"`2Ein aufregendes Abenteuer.`0","`qAls `0".$session['user']['name']." ".$text_mail);
            }
...
            
        if($row_partner['acctid'] < $session['user']['acctid']){
            systemmail($row_partner['acctid'],"`4Keine schÃ¶nen Stunden.`0","`&{$session['user']['name']}`4 hat dein Angebot, mit dir ein paar schÃ¶ne Stunden im Haus verbringen, abgelehnt.`0");
        }else{
            systemmail($row_partner['acctid'],"`4Keine schÃ¶nen Stunden.`0","`&{$session['user']['name']}`4 hat dein Angebot, mit dir ein paar schÃ¶ne Stunden im Haus verbringen, abgelehnt.`0");
        }
            
...
            
        if($row_partner['acctid'] < $session['user']['acctid']){
            systemmail($row_partner['acctid'],"`4Doch keine schÃ¶nen Stunden.`0","`&{$session['user']['name']}`4 hat sein Anfrage, mit dir ein paar schÃ¶ne Stunden im Haus verbringen, zurÃ¼ckgenommen.`0");
        }else{
            systemmail($row_partner['acctid'],"`4Doch keine schÃ¶nen Stunden.`0","`&{$session['user']['name']}`4 hat sein Anfrage, mit dir ein paar schÃ¶ne Stunden im Haus verbringen, zurÃ¼ckgenommen.`0");
        }
            
