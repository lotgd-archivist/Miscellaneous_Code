...
        //global_log('Es wurden `b'.db_affected_rows().' Posts`b gelÃ¶scht!', 'setnewday.php');
    
    $sql = "DELETE FROM news WHERE newsdate<'".date("Y-m-d H:i:s",strtotime(date("c")."-".getsetting("expirecontent",180)." days"))."'";
    db_query($sql);
    //if(db_affected_rows() > 0)
        //global_log('Es wurden `b'.db_affected_rows().' NewseintrÃ¤ge`b gelÃ¶scht!', 'setnewday.php');
}


//if(db_affected_rows() > 0)
    //global_log('Es wurden `b'.db_affected_rows().' YOMs`b gelÃ¶scht!', 'setnewday.php');
...
                    switch(e_rand(1,4)){
                        case 1:
                            $clouds = "DrÃ¼ckend heiÃŸ und schwÃ¼l";
                        break;
                  
                        case 2:
...
                  
                        case 2:
                            $clouds = "StÃ¼rmisch mit vereinzelten Regenschauern";
                        break;
                  
                        case 3:
                            $clouds = "Orkanartige BÃ¶en";
                        break;
                  
                        case 4:
...
// Adventsspecial fÃ¼r MerydiÃ¢ .. dies ist auf die reale Zeit bezogen, vom 1.12. bis 24.12., jeden Tag gibt es Geschenke
// Auch fÃ¼r anderes nutzbar ^^
...
        db_query($sql);
        
        // ArenakÃ¤mpfe in die der User verwickelt ist, zurÃ¼cksetzen.
        $sql = 'DELETE FROM 
                        pvp 
                WHERE 
                        acctid2 = '.$row['acctid'].' 
...
        global_log($row['name'].' `0angeschrieben, dass der Charakter bald gelÃ¶scht wird. Haus, Items, ArenakÃ¤mpfe und Ehepartner zurÃ¼ckgesetzt. Letzte AktivitÃ¤t des Charas: '.date('d.m.Y', StrToTime($row['laston'])), 'setnewday.php');
    }
}
...
    
    
    // Alle Spieler aus der DB holen, die sonst noch gelÃ¶scht werden mÃ¼ssen
    // LÃ¶schgrenze auf Omar nach Ã¼ber 60 Tagen inaktivitÃ¤t, egal welches Level und wie viele DKs
    $sql = 'SELECT
                    acctid,
                    name,
                    laston
...
    
    if(!empty($del_accs)) {
        global_log('Insgesamt `b'.count($del_accs).'`b Chara'.(count($del_accs)>1?'s':'').' gelÃ¶scht!', 'setnewday.php');
        
...
                savesetting("hasegg", "0");
            
            // User aus der accounts Tabelle lÃ¶schen
            $sql = 'DELETE FROM 
                            accounts 
                    WHERE 
                            acctid = '.$acctid;
...
            db_query($sql);
            
            // ArenakÃ¤mpfe, an denen der Spieler beteiligt ist, lÃ¶schen
            $sql = 'DELETE FROM 
                            pvp 
                    WHERE 
                            acctid2 = '.$acctid.'
...
            db_query($sql);
            
            // Selber erstellten RP Ort lÃ¶schen
            $sql = 'DELETE FROM 
                            rporte 
                    WHERE 
                            acctid = '.$acctid;
...
            db_query($sql);
            
            global_log('`b'.$values['name'].'`b`0 als '.($values['type']=='oldacc'?'normalen Chara gelÃ¶scht':'Chara, der sich nie einloggte, gelÃ¶scht').'. Letzter Login am '.date('d.m.Y', StrToTime($values['laston'])), 'setnewday.php');
        }
    }
            
    
