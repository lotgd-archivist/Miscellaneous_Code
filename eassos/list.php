...
  } else if ($_GET['ret'] == 'assos') {
   addnav("nach Eassos","eassos.php");
  } else if ($_GET['ret'] == 'deassos') {
   addnav("nach Dark Eassos","deassos.php");
  } else if ($_GET['ret'] == 'bootsanleger') {
   addnav("zum Bootsanleger","bootsanleger.php");
...
    output("<td align='center'><img src='images/Assos/neu/Bewohnerliste.png' width='100%' height='100%'></td>",true);
}else{
    output("<td align='center'><img src='images/Assos/neu/Bewohnerliste.png' width='100%' height='100%'></td>",true);
}
...
",true);
output("`c#c0c0c0 Folgende Bewohner von Eassos sind online!`n
        Hier findest du jeden, der in unserem Land wohnt.`n
        Außerdem kannst du dich hier über deine Mitbürger informieren und ihren Status einsehen.`0`c`n`n");
...
        if ($prefs['info2'] == "1") {
            output("`iInfo`i: " . html_entity_decode(stripslashes($prefs['info'])) . "`0");
        }
        output("</div>",true);
...
    output("<table border='0' align='center' cellpadding='2' cellspacing='1' bgcolor=#000000>",true);
...
        } else {
            if ($session['user']['loggedin']) {
                $charbio = "<a href='popup_bio.php?char=" . rawurlencode($row['login']) . "'  onmouseover=\"TagToTip('" . $row['acctid'] . "',TITLEBGCOLOR,'#550000',BGCOLOR,'#050505',FONTCOLOR,'#EEEEEE' ,BORDERWIDTH,2,BORDERCOLOR,'#880000',TITLE,'Kurzinformationen über " . preg_replace("/#[a-f0-9]{6}|[`]./i","",$row['name']) . "`0')\" onmouseout=\"UnTip()\" onClick=\"" . popup_bio("popup_bio.php?char=" . rawurlencode($row['login']) . "") . ";return false;\">" . $row['name'] . "`0</a>";
            } else {
                $charbio = $row['name'] . "`0";
            }
        }
...
output("`n`n`n`n`n
<table border='0' align='center' cellspacing='1' cellpadding='1' bgcolor='#1d1d1d'>
    <table border='0' align='center' cellspacing='2' cellpadding='2' bgcolor='#1d1d1d'>
    <tr><td>
      <table border='0' cellspacing='3' cellpadding='3' bgcolor='#050505'>
        <tr>
