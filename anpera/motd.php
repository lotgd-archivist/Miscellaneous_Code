...
            output("<input type='submit' class='button' value='Hinzuf&uuml;gen'></form>",true);
...
                motditem($row['motdtitle'].($session['user']['superuser']>=3?"[<a href='motd.php?op=del&id=$row[motditem]' onClick=\"return confirm('Bist du sicher, dass dieser Eintrag gelÃ¶scht werden soll?');\">Del</a>]":"")."`n[`i`b{$row['motddate']}`b`i]",$row['motdbody']);

            }else{

                pollitem($row['motditem'],$row['motdtitle'].($session['user']['superuser']>=3?"[<a href='motd.php?op=del&id=$row[motditem]' onClick=\"return confirm('Bist du sicher, dass dieser Eintrag gelÃ¶scht werden soll?');\">Del</a>]":"")."`n[`i`b{$row['motddate']}`b`i]",$row['motdbody']);
...
    motditem("Beta!","Diese Seite ist im BETA Status! Ich bastel daran herum, wenn ich Zeit habe. Auch Ã„nderungen von offizieller Seite (MightyE) werden hier Ã¼bernommen. Das ist KEIN Freibrief zum Ausnutzen von Bugs, sondern alle Spieler (Teilnehmer am Beta-Test) sind verpflichtet, gefundene Fehler zu melden! WÃ¼nsche und Anregungen werden ebenfalls jederzeit gern angenommen. :-)");
