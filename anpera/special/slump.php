...
output("`6PlÃ¶tzlich stÃ¼rzt du - waren es deine etwas ungeschickten FÃ¼ÃŸe, oder doch ein Ast?`n");
...
    output("`^Zum GlÃ¼ck hast du kein Gold, welches dir verloren gehen kÃ¶nnte.`n`n`0");



// falls selbst juengster spieler oder drachenkÃ¤mpfer

// oder kein juengster spieler und drachenkaempfer in den settings

}else if (($newplayer.$newdk == "")||($session['user']['name'] == $rownew['name'])||($session['user']['name'] == $rowdk['name'])||($rowdk['name']=="" && $rownew['name']=="")){

    output("`^Schnell sammelst du dein Gold wieder ein und gehst deinen Weg weiter.`n`n`0");



}else if($rownew['name'] == ""){ //kein juengster spieler - nur juengster drachenkaempfer

//    output("`n`^es gibt keinen juengsten spieler`0`n");

    output("`6Schnell willst du das Gold wieder einsammeln, doch bei ein paar GoldstÃ¼cken ist jemand schneller.`n`n");

    output("`3Du denkst: \"`#Der hat doch gerade den `@grÃ¼nen Drachen `#erfolgreich bekÃ¤mpft");

    output(" - wie war der Name noch gleich?`n");

    output("Ah,`0 $rowdk[name] `#muss es gewesen sein.`3\"`n`n`0");

    output("`6Jetzt ist es zu spÃ¤t, so flink er beim Aufheben war, so flink ist er auch verschwunden.`0`n`n");
...
    $mailmessage = "`^".$session['user']['name']."`2 stÃ¼rzte im Wald Ã¼ber einen Ast und verlor dabei ".($session['user']['sex']?"ihr":"sein")." Gold.`n`n"

        ."Ein paar GoldstÃ¼cke rollten dir dabei vor die FÃ¼ÃŸe, die du aufgehoben und behalten hast. "

        ."Die gefundenen `^ $lost `2GoldstÃ¼cke hast du direkt zur Bank gebracht.`0";
...
    output("`^Du hast wenigstens noch $save GoldstÃ¼cke retten kÃ¶nnen.`0`n`n");
...
}else if($rowdk['name'] == ""){ //kein juengster drachenkaempfer nur juengster spieler
...
    output("und der andere hatte eben den `@grÃ¼nen Drachen `6erfolgreich bekÃ¤mpft");

    output("- wie waren ihre Namen noch gleich?`n`n`0");

    output($rownew['name']."`# und `0".$rowdk['name']."`# mÃ¼ssen es gewesen sein.`3\"`6, denkst du. `n`n"); //name des juengsten spielers und drachentoeters

    output("Jetzt ist es zu spÃ¤t, so flink sie beim Aufheben waren, so flink sind sie auch verschwunden.`n`n`0");
...
    $mailmessage1 = "`^".$session['user']['name']."`2 stÃ¼rzte im Wald Ã¼ber einen Ast und verlor dabei ".($session['user']['sex']?"ihr":"sein")." Gold.`n`n"

        ."Ein paar GoldstÃ¼cke rollten dir dabei vor die FÃ¼ÃŸe, die du aufgehoben und behalten hast. "

        ."Die gefundenen `^ $dkgain `2GoldstÃ¼cke hast du direkt zur Bank gebracht.`0";

     systemmail($rowdk['acctid'],"`2Du hast Gold im Wald gefunden",$mailmessage1);

    $mailmessage2 = "`^".$session['user']['name']."`2 stÃ¼rzte im Wald Ã¼ber einen Ast und verlor dabei ".($session['user']['sex']?"ihr":"sein")." Gold.`n`n"

        ."Ein paar GoldstÃ¼cke rollten dir dabei vor die FÃ¼ÃŸe, die du aufgehoben und behalten hast. "

        ."Die gefundenen `^ $newgain `2GoldstÃ¼cke hast du direkt zur Bank gebracht.`0";
...
output("und erhÃ¤ltst `& $reward `@Erfahrungspunkte.`0");
