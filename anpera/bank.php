...
      if ($session['user']['race']==6) output("Er weist dich mit einem kurzen Griff an seinen Hals noch darauf hin, dass dies keine Blutbank ist.`n");
...
        output("<form action='bank.php?op=transfer2' method='POST'>Wieviel &uuml;<u>b</u>erweisen: <input name='amount' id='amount' accesskey='b' width='5'>`n",true);

        output("A<u>n</u>: <input name='to' accesskey='n'> (Unvollst&auml;ndige Namen werden automatisch erg&auml;nzt. Du wirst nochmal zum Best&auml;tigen aufgefordert).`n",true);
...
        output("<input type='hidden' name='to' value='".HTMLEntities($row['login'])."'><input type='hidden' name='amount' value='$amt'><input type='submit' class='button' value='&Uuml;berweisung abschlie&szlig;en'></form>",true);
...
        output("A<u>n</u>: <input name='to' accesskey='n' value='". $_POST['to'] . "'> (Unvollst&auml;ndige Namen werden automatisch erg&auml;nzt. Du wirst nochmal zum Best&auml;tigen aufgefordert).`n",true);
...
        output("`6&Uuml;berweise `^$amt`6 an <select name='to' class='input'>",true);
...
        output("</select><input type='hidden' name='amount' value='$amt'><input type='submit' class='button' value='&Uuml;berweisung abschlie&szlig;en'></form>",true);
...
        $sql = "SELECT name,acctid,level,transferredtoday,lastip,emailaddress,uniqueid FROM accounts WHERE login='".addslashes(stripslashes($_POST['to']))."'";
...
            $row['transferredtoday']=(int)getvalue($row['acctid'],"transferredtoday");
...
                updateuser($row['acctid'],array('goldinbank'=>"+$amt",'transferredtoday'=>'+1'));
...
        output("<form action='bank.php?op=gemtrans2' method='POST'>Einen Edelstein versenden a<u>n</u>: <input name='to' accesskey='n'> (Unvollst&auml;ndige Namen werden automatisch erg&auml;nzt. Du wirst nochmal zum Best&auml;tigen aufgefordert).`n",true);

        output("<input type='submit' class='button' value='Vorschau'></form>",true);

        addnav("","bank.php?op=gemtrans2");

    }elseif ($session['user']['gold']+$session['user']['goldinbank']<100){
...
        output("<input type='hidden' name='to' value='".HTMLEntities($row['login'])."'><input type='submit' class='button' value='Versand abschlie&szlig;en'></form>",true);
...
        output("<form action='bank.php?op=gemtrans2' method='POST'>Versende einen Edelstein a<u>n</u>: <input name='to' accesskey='n' value='". $_POST['to'] . "'> (Unvollst&auml;ndige Namen werden automatisch erg&auml;nzt. Du wirst nochmal zum Best&auml;tigen aufgefordert).`n",true);
...
        output("</select><input type='submit' class='button' value='Versand abschlie&szlig;en'></form>",true);
...
            updateuser($row['acctid'],array('gems'=>'+1','transferredtoday'=>'+1'));
...
}elseif($_GET['op']=="deposit"){

    output("<form action='bank.php?op=depositfinish' method='POST'>Du hast ".($session['user']['goldinbank']>=0?"ein Guthaben von":"Schulden in H&ouml;he von")." ".abs($session['user']['goldinbank'])." Gold bei der Bank.`n",true);

    output("`^Wie <u>v</u>iel ".($session[user][goldinbank]>=0?"einzahlen":"zur&uuml;ckzahlen").":  <input id='input' name='amount' width=5 accesskey='v'> <input type='submit' class='button' value='Einzahlen'>`n`iGib 0 oder gar nichts ein, um alles einzuzahlen.`i</form>",true);
...
          output("<form action='bank.php?op=withdrawfinish' method='POST'>Du hast ".($session['user']['goldinbank']>=0?"ein Guthaben von":"Schulden in H&ouml;he von")." ".abs($session['user']['goldinbank'])." Gold bei der Bank.`n",true);
...
    }elseif($_POST['amount']>$session['user']['goldinbank']){
