...
    $sql = "SELECT petitionid FROM petitions WHERE status=2 AND date<'".date("Y-m-d H:i:s",strtotime(date('Y-m-d H:i:s')." -7 days"))."'";

    $result = db_query($sql);

    while ($row = db_fetch_assoc($result)) db_query('DELETE FROM petitionmail WHERE petitionid="'.$row['petitionid'].'"');

    $sql = "DELETE FROM petitions WHERE status=2 AND date<'".date("Y-m-d H:i:s",strtotime(date("Y-m-d H:i:s")." -7 days"))."'";
...
        output("<tr class='".($i%2?"trlight":"trdark")."'><td>{$row['petitionid']}</td><td>[<a href='viewpetition.php?op=view&id={$row['petitionid']}'>Anzeigen</a>|<a href='viewpetition.php?op=del&id={$row['petitionid']}' onClick='return confirm(\"Diese Anfrage wirklich l&ouml;schen?\");'>Del</a>|<a href='viewpetition.php?setstat=0&id={$row['petitionid']}'>Ungelesen</a>|<a href='viewpetition.php?setstat=1&id={$row['petitionid']}'>Gelesen</a>|<a href='viewpetition.php?setstat=2&id={$row['petitionid']}'>Geschlossen</a>]</td>",true);
...
    output("`i(Geschlossene Anfragen werden nach 7 Tagen automatisch gelÃ¶scht)`i");

    output("`n`bSchlÃ¼ssel:`b`nUngelesen: Niemand arbeitet bisher an diesem Problem.

    `nGelesen: Es arbeitet jemand an diesem Problem.

    `nGeschlossen: Diese Anfrage wurde bearbeitet. Es sollte keine weitere Arbeit mehr nÃ¶tig sein.`n`n
...
    Wenn du ein Problem nicht lÃ¶sen kannst, markiere die Anfrage wieder als ungelesen, damit

    ein anderer dem Spieler helfen kann.`n

    Wenn eine Anfrage erfolgreich bearbeitet wurde, markiere sie als geschlossen. Sie wird nach 7 Tagen dann automatisch gelÃ¶scht.");
...
    viewcommentary("pet-{$_GET['id']}","HinzufÃ¼gen",200);
