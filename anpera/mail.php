...
        $session['message'] = "`\$`bDu kannst 0 Nachrichten nicht lÃ¶schen! Was das heiÃŸt? Du hast \"Markierte lÃ¶schen\" geklickt, aber es sind keine Nachrichten markiert. Was fÃ¼r eine Welt ist das nur, in der Leute KnÃ¶pfe ohne Funktion drÃ¼cken?!?`b`0";
...
output("<table border='0' cellspacing='0' cellpadding='0'><tr><td height='21' width='193' align='left'><a href='mail.php' class='motd'>Inbox</a></td><td width='193' align='center'><a href='mail.php?op=address' class='motd'>Mail schreiben</a></td><td width='193' align='right'><a href='mail.php?op=outbox' class='motd'>Outbox</a></td></tr></table>`n`n",true);
...
// Naja, der Code ist ja eher von MightyE ^^
...
    $sql = "SELECT mail.subject,mail.messageid,accounts.name,mail.msgfrom,mail.seen,mail.sent, petitionmail.petitionid FROM mail LEFT JOIN petitionmail USING(messageid) INNER JOIN accounts ON accounts.acctid=mail.msgto WHERE mail.msgfrom=\"".$session['user']['acctid']."\" ORDER BY mail.sent";
...
        output("`n`iMails bleiben nur so lange im Postausgang, bis der `bEmpfÃ¤nger`b sie aus seinem Posteingang lÃ¶scht!`i");

       }else{

            output("`iDu hast keine Mails verschickt, oder der EmpfÃ¤nger hat sie schon alle gelÃ¶scht. Wie schade.`i");
...
        output("`b`2EmpfÃ¤nger:`b `^{$row['name']}`n");
...
        output("Konnte den EmpfÃ¤nger nicht finden. Bitte versuche es nochmal.`n");
...
        output("<input type='submit' class='button' value='Markierte l&ouml;schen'>",true);
...
    output("`n`n`iDu hast ".db_num_rows($result)." Nachrichten in deiner Mailbox`nDu kannst hÃ¶chstens ".getsetting('inboxlimit',50)." Nachrichten hier speichern.`nNachrichten werden nach ".getsetting("oldmail",14)." Tagen gelÃ¶scht.");
...
        output("<table align='center' border='0' cellspacing='0' cellpadding='0'><tr><td height='21' width='200'><a href='mail.php?op=write&replyto={$row['messageid']}' class='motd'>Antworten</a></td><td width='200'><a href='mail.php?op=del&id={$row['messageid']}' class='motd'>L&ouml;schen</a></td></tr></table>",true);
...
    output("`b`2EmpfÃ¤nger:`b`n");

    output("`2<u>A</u>n: <input name='to' accesskey='a'> <input type='submit' class='button' value='Suchen'></form>",true);
