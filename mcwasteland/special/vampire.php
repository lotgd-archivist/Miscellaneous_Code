...
    output("`n`n`7Ein bÃ¶sartiges Wesen manifestiert sich vor dir. Du erzitterst aus Furcht vor dieser uralten Macht, die jetzt zu dir spricht: ",true);
    output("\"`\$Sterblicher, ich spÃ¼re viel Lebenskraft in dir. Da ich alt werde, schwindet mein Verlangen zu jagen. Im Austausch ",true);
    output("fÃ¼r ein kleines bisschen deiner permanenten Lebenskraft gewÃ¤hre ich dir KrÃ¤fte ausserhalb deiner Vorstellungskraft.`7\"",true);
    output(" Erst jetzt erkennst du, dass du einem Vampir gegenÃ¼berstehst, der auf deine Entscheidung wartet.",true);
...
        output("Ausgehungert und vom Geruch deiner enormen Lebenskraft fast wahnsinnig Ã¼berwÃ¤ltigt dich ein Vampir auf deiner Flucht und saugt dich aus.",true);
...
            output("`nDu fÃ¼hlst dich schlapp und verlierst einen Waldkampf.",true);
            $session['user']['turns']--;
        }
        addnews("`%".$session['user']['name']."`7 hatte im Wald eine folgenschwere Begegnung mit einem Vampir.");
    }else{
        output("`n`7Du verlÃ¤sst diesen verfluchten Ort so schnell du kannst.",true);
        if (getsetting("limithp",0)>0 && $session['user']['charm']>250){
            $session['user']['charm']=250;
            output(" Dennoch hast du das unangenehme, kalte GefÃ¼hl, als ob dir der Vampir etwas unwiederbringlich genommen hÃ¤tte.",true);
        }
    }
...
        output("`n`7Der Vampir schaut dich an und meint schliesslich, dass deine Lebenskraft nicht ausreicht um ihn zu sÃ¤ttigen. Er lÃ¤sst dich ",true);
        output("unangetastet und ohne Belohnung ziehen.");
    }else{
        $session['user']['maxhitpoints'] -= $lifecost;
        if($session['user']['hitpoints']>$session['user']['maxhitpoints']) $session['user']['hitpoints']=$session['user']['maxhitpoints'];
        output("`n`n`7Du erschauderst, als der Vampir seine ZÃ¤hne in deinem Hals versenkt. Du fÃ¼hlst deine Lebenskraft durch die Wunde ",true);
        output("in den Vampir fliessen. Im Gegenzug dafÃ¼r spricht der Vampir einen fluchÃ¤hnlichen Zauber Ã¼ber dich.`n`n`@",true);
        if($_GET['op']=="str"){
            $session['user']['attack']++;
            output("Dein Angriffwert erhÃ¶ht sich vorÃ¼bergehend um `^1`@ und du verlierst `\$".$lifecost." `@permanente Lebenspunkte.",true);
        }else if($_GET['op']=="def"){
            $session['user']['defence']++;
            output("Deine Verteidigung erhÃ¶ht sich vorÃ¼bergehend um `^1`@ und du verlierst`\$".$lifecost." `@permanente Lebenspunkte.",true);
...
    output("`n`n`7Du stehst vor einem verschlungenen Pfad. Ein dunkler Nebel umgibt dich und du fÃ¼hlst ein kaltes ",true);
