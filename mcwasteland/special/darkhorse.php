...
case "tavern":
    checkday();
    output("Du stehst in der NÃ¤he des Eingangs und beobachtest das Geschehen vor dir. WÃ¤hrend es in den meisten Tavernen ");
    output("laut und rauh zugeht, ist diese hier erstaunlich ruhig und fast leer. In einer Ecke spielt ein alter Mann mit ");
    output("einigen WÃ¼rfeln. Du bemerkst, dass Abenteurer, die diesen Platz gefunden haben, Botschaften in die Tische ");
    output("geritzt haben. Hinter der Theke humpelt ein alter Mann herum und poliert GlÃ¤ser - ");
    output("obwohl niemand hier zu sein scheint, der sie benutzen wÃ¼rde.`nSeitlich in einer Nische siehst du einen rundlichen Kerl hinter einem knisternden Grill stehen. ");
    
...
    addnav("ZurÃ¼ck zur Taverne","forest.php?op=tavern");
    break;
case "grill":
    addnav("ZurÃ¼ck zur Taverne","forest.php?op=tavern");
...
            switch(e_rand(1,2)){ 
                case 1:
                output("`@Das hat gut getan! Du fÃ¼hlst dich erfrischt. Du erhÃ¤ltst einen Waldkampf und deine Wunden heilen etwas.`n`n"); 
...
                break;
                case 2:
                output("`@Das hat gut getan! Aber jetzt fÃ¼hlst du dich voll. Du verlierst einen Waldkampf aber deine Wunden heilen etwas.`n`n"); 
                $session['user']['turns']-=1; 
                $session['user']['hitpoints'] += e_rand(2,5*$session['user']['level']);
                if ($session['user']['hitpoints'] > $session['user']['maxhitpoints']) $session['user']['hitpoints'] = $session['user']['maxhitpoints'];
                $session['user']['gold']-=$scost;
...
            addnav("ZurÃ¼ck zum Grill","forest.php?op=grill");
        }else {
...
            switch(e_rand(1,3)){ 
                case 1:
                output("`@Das hat gut getan! Du fÃ¼hlst dich erfrischt. Du erhÃ¤ltst einen Waldkampf.`n`n"); 
                $session['user']['turns']+=1;
                $session['user']['gold']-=$ccost;
                break;
                case 2:
                output("`@Das hat gut getan! Aber jetzt fÃ¼hlst du dich voll. Du verlierst einen Waldkampf.`n`n"); 
                $session['user']['turns']-=1; 
                $session['user']['gold']-=$ccost;
                break;
                case 3:
...
        }
    }else{
        output("`7Der Geruch von KÃ¤se und Steaks steigt dir in die Nase, als du nÃ¤her kommst. Beim Lesen des MenÃ¼s lÃ¤uft dir das Wasser im Mund zusammen.`n`n");
        addnav("MenÃ¼");
        addnav("Cheeseburger (`^".$ccost." Gold`0)","forest.php?op=grill&what=cheese");
        addnav("Steak im BrÃ¶tchen (`^".$scost." Gold`0)","forest.php?op=grill&what=steak");
...
        output("\"`7Schag, wasch kann ich fÃ¼r dich tun, ".($session['user']['sex']?"MÃ¤dchen":"mein Schohn")."?`0\"  spricht der Zahnlose. ");
        output(" \"`7Schehe die WÃ¼nsche deineschgleichen nicht alltschuoft hier.`0\"");
        addnav("Etwas Ã¼ber die Feinde lernen","forest.php?op=bartender&what=enemies");
...
        output("\"`7Schoscho, du willscht alscho etwasch Ã¼ber dasch goldene Ei wischen.`nNun, dasch ischt eine uralte Legende. Esch heischt, wer ein goldnesch Ei beschitscht, kann dem Tod entkommen. ");
        output("Auscherdem scholl dieschesch Ei der SchlÃ¼schel tschu einer Heilerin namensch Golinda schein. Ich glaube ja nicht daran.");
...
            output("`0\" Er beginnt zu flÃ¼stern: \"`7Aber esch geht dasch GerÃ¼cht um, dasch `^".$row['name']." `7genau dieschesch Ei gefunden haben scholl. Wenn du mich fragscht, ich wÃ¼rde ".$row['name']." `7schogar tÃ¶ten und ihm dasch Ei wegnehmen, ");
            output(" um dasch herauschtschufinden, wenn ich kÃ¶nnte...");
        }
        output("`0\"");
        if ($session['user']['acctid']==getsetting("hasegg",0)){
            output("`n`nDu ziehst dich zurÃ¼ck, ohne den Mann in Versuchung zu bringen, dir das Ei wegnehmen zu wollen. An ");
...
              output("Der alte Mann stÃ¼tzt sich auf die Theke. \"`%Scho, du willscht alscho wasch Ã¼ber Farben wischen?`0\"");
                output("  Du willst gerade antworten, als du gerade noch merkst, daÃŸ das eine rhetorische Frage war.  ");
                output("Er fÃ¤hrt fort: \"`%Um farbig tschu sprechen, mache folgendesch. Zuerscht benutsche das &#0096; Tscheichen ",true);
...
        if ($_GET['who']==""){
            output("\"`7Scho, du willscht alscho etwasch Ã¼ber deine Feinde erfahren, ja? Ãœber wen willscht du wasch wischen? Nun? Schag schon! Esch koschtet nur `^100`7 Gold pro Information Ã¼ber eine Perschon.`0\"");
...
                if ($max > 100) {
                    output("`n`n\"`7Hey, wasch glaubscht du tuscht du hier? Dasch schind tschu viele Namen. Ich werd dir nur Ã¼ber ein paar davon wasch ertschÃ¤hlen.`0`n");
                    $max = 100;
                }
...
                $sql = "SELECT acctid,name,alive,location,maxhitpoints,gold,sex,level,weapon,armor,attack,defence,race,charm FROM accounts WHERE login='".$_GET['who']."'";
                $result = db_query($sql) or die(db_error(LINK));
...
                    output("\"`7Nun ... mal schehen wasch ich Ã¼ber ".str_replace("s","sch",$row['name'])."`7 weisch...`0\"`n`n");
...
                    output("`4`bRÃ¼stung:`b`6 ".$row['armor']."`n");
...
                    if ($amt==$row['charm']) { output("dir schehr Ã¤hnlich.`n");}
                    else if ($amt-10>$row['charm']) { output("`bviel`b hÃ¤schlicher alsch du!`n");}
                    else if ($amt>$row['charm']) { output("hÃ¤schlicher alsch du.`n");}
                    else if ($amt+10<$row['charm']) { output("`bviel`b schÃ¶ner alsch du!`n");}
                    else {output("schÃ¶ner alsch du.`n");}
                    $session['user']['gold']-=100;
                    //debuglog("spent 100 gold to learn about an enemy");
                    if ($session['user']['gold']>150){
                        output("`7Wenn du nochmal `^150`7 Gold drauflegscht, schag ich dir schogar, wo schich ".$row['name']."`7 aufhÃ¤lt.`0\"");
                        addnav("Aufenthaltsort (`^150`0 Gold)","forest.php?op=bartender&what=where&who=".$row['acctid']);
                    }
                    
                }else{
                    output("\"`7HÃ¤? Ich kenne niemanden mit dieschem Namen.`0\"");
                }
            }else{
                output("\"`7Nun ... mal schehen wasch ich Ã¼ber Geitschkragen wie dich allesch weisch...`0\"`n`n");
                output("`4`bName:`b`6 Ohnemo Snixlos`n");
                output("`4`bLevel:`b`6 Ganz unten`n");
                output("`4`bLebenspunkte:`b`6 MÃ¶glicherweische mehr alsch du`n");
...
                output("`4`bRÃ¼stung:`b`6 Kleidschamer alsch deine`n");
                output("`4`bAngriffswert:`b`6 DrÃ¶lf Milliarden`n");
                output("`4`bVerteidigung:`b`6 Super Duper`n");
            }
        }
    }else if($_GET['what']=="where"){
        $sql="SELECT acctid,name,location,loggedin,laston,alive,housekey FROM accounts WHERE acctid='".$_GET['who']."'";
        $result = db_query($sql) or die(db_error(LINK));
        $row = db_fetch_assoc($result);
...
        if ($row['location']==2){
            $sql="SELECT hvalue FROM items WHERE class='SchlÃ¼ssel' AND owner='".$row['acctid']."' AND hvalue>'0'";
            $result = db_query($sql) or die(db_error(LINK));
...
    case "":
        checkday();
        output("Der alte Mann schaut mit tiefliegenden und hohlen Augen zu dir auf. Seine roten Augen lassen darauf schlieÃŸen, dass er erst kÃ¼rzlich geweint hat,");
...
        output("`n`n`nSag, wÃ¼rdest du einem alten Mann den Gefallen tun und ihn versuchen lassen, etwas von dem Gold von dir zurÃ¼ckzugewinnen? Ich kenne mehrere");
        output(" Spiele!`0\"");
        $session['user']['specialinc']="darkhorse.php";
        $session['user']['specialmisc']="";
        addnav("W?WÃ¼rfelspiel","forest.php?op=oldman&game=dice");
        addnav("S?Steinchenspiel","forest.php?op=oldman&game=stones");
        addnav("GlÃ¼cksspiel","stonesgame.php");
    
        addnav("ZurÃ¼ck zur Taverne","forest.php?op=tavern");
        break;
    case "stones":
...
        if ($stones['side']==""){
            output("`3Der alte Mann erklÃ¤rt sein Spiel: \"`7Ich habe einen Beutel mit 6 roten und 10 blauen Steinen darin. Du kannst zwischen 'gleiches Paar' und 'ungleiches Paar' wÃ¤hlen. Ich werde");
...
            $stones['oldman']=0;
        }elseif ($stones['bet']==0){
            output("`3\"`7".($stones['side']=="likepair"?"Gleiches Paar fÃ¼r dich, dann ist ein ungleiches ":"Ungleiches Paar fÃ¼r dich, dann ist ein gleiches ")." Paar fÃ¼r mich!");
...
                output("`3Der alte Mann streckt seinen Stock aus und klopft damit deinen Goldbeutel ab. \"`!Ich glaub nicht, daÃŸ da `^$bet`! Gold drin ist!`3\", erklÃ¤rt er.`n`n");
...
                output("`n`nVerlegen kehrst du zur Taverne zurÃ¼ck.");
                addnav("ZurÃ¼ck zur Taverne","forest.php?op=tavern");
            }else{
                if ($_POST['guess']!==NULL){
                    $try = (int)$_GET['try'];
...
                            output("und denke ich werde jetzt besser gehen... `3\" Er steht auf und will zur TÃ¼r gehen, doch mit einem sanften Schlag mit ".$session['user']['weapon']);
                            output(" drÃ¼ckst du ihn zurÃ¼ck auf seinen Stuhl. Du hilfst dem Mann dabei, dir die `^".$bet."`3 GoldmÃ¼nzen zu geben, die er dir schuldet.");
                        }
...
                            output("gibst du dem Mann die `^".$bet."`3 GoldmÃ¼nzen, die du ihm schuldest, bereit, ihn zu verlassen.");
                            $session['user']['specialinc']="darkhorse.php";
                            $session['user']['gold']-=$bet;
...
                            }else{
                                output("`3\"`!Nop, nicht `^".(int)$_POST['guess']."`!, meine Zahl ist grÃ¶ÃŸer als das!  Das war dein `^".$try."`!ter Versuch.`3\"`n`n");
                            }
...
            output("  Damit wendet er sich wieder seinen WÃ¼rfeln zu. Seinen Ã„rger hat er offensichtlich schon wieder vergessen.");
            addnav("ZurÃ¼ck zur Taverne","forest.php?op=tavern");
            //$session['user']['specialinc']="darkhorse.php";
        }
        break;
...
            $bet = abs((int)$_GET['bet'] + (int)$_POST['bet']);
            if ($bet<=0){
                output("`3\"`!Du wÃ¼rfelst und wÃ¤hlst dann, ob du die Zahl behalten willst, oder ob du nochmal wÃ¼rfeln willst. Du hast insgesamt 3 Versuche. ");
                output(" Danach, oder nachdem du eine Zahl behalten hast, werde ich das selbe machen.  ");
                output("Wer am Ende die hÃ¶here Zahl gewÃ¼rfelt und behalten hat, gewinnt. Bei Gleichstand ");
...
                    $try=$_GET['try'];
                    $try++;
                    output("Du wÃ¼rfelst deinen ".($try==1?"ersten":($try==2?"zweiten":"dritten"))." Versuch und es erscheint die `b".$session['user']['specialmisc']."`b`n`n");
                    output("`3Du hast `^".$bet."`3 Gold gesetzt.  Was machst du?");
                    addnav("Behalten","forest.php?op=oldman&game=dice&what=keep&bet=$bet");
                    if ($try<3) addnav("Neu wÃ¼rfeln","forest.php?op=oldman&game=dice&what=pass&try=$try&bet=$bet");
                }else{
                    output("Dein endgÃ¼ltiger Wurf war `b".$session['user']['specialmisc']."`b, der alte Mann wird jetzt versuchen, das zu Ã¼berbieten:`n`n");
                    $r = e_rand(1,6);
...
                    }else{
                        $r = e_rand(1,6);
                        output("Der alte Mann wÃ¼rfelt nochmal und bekommt eine $r...`n");
...
                    }
                    if ($r>$session['user']['specialmisc']){
                        output("`n\"`7Juuhuu! Ich wusste, dass deinesgleichen niemals Ã¼ber meinesgleichen siegen kann!`0\", ruft der Mann, wÃ¤hrend du ihm gibst, was du ihm schuldest: `^$bet`0 Gold.");
                        $session['user']['gold']-=$bet;
                        //debuglog("lost $bet gold at dice");
                    }else{
                        if ($r==$session['user']['specialmisc']){
                            output("`n\"`7Tja... nun, sieht so aus, als ob wir ein Unentschieden hÃ¤tten.`0\"");
                        }else{
                            output("`n\"`7Waaaaaah!! Wie konnte jemand wie du mich besiegen?!?!?`0\" schreit der alte Mann, wÃ¤hrend er dir gibt, was er dir schuldet.");
                            $session['user']['gold']+=$bet;
...
case "leave":
case "leaveleave":
    output("Du verlÃ¤sst die Taverne und lÃ¤ufst durch das dichte Blattwerk. Der seltsame Nebel ");
    output("ist wieder aufgezogen und verwirrt deine Sinne. Als du den Nebel verlÃ¤ÃŸt, befindest du dich wieder im Wald an einer ");
...
default:
    checkday();
    output("Eine Baumgruppe in der NÃ¤he kommt dir bekannt vor... Du bist dir sicher, diesen Platz schoneinmal gesehen zu haben.  ");
    output("Als du nÃ¤her kommst, steigt ein seltsamer Nebel auf. Dein GedÃ¤chtnis spielt dir Streiche ");
    output("und du bist dir plÃ¶tzlich gar nicht mehr sicher, wie du hier her gekommen bist. ");
...
    output("eine HolzhÃ¼tte, bei der Rauch aus dem Kamin aufsteigt. Auf einem Schild Ã¼ber der TÃ¼r steht: \"`7Dark Horse Taverne`0\".");
