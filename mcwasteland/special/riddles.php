...
    //        if (count((is_countable($filtstr)?$filtstr:[])) > count($filterpost1)) 
    //            if (!strcasecmp(substr($filtstr1, -1*count($filterpost1)), $filterpost1)) 
    //                $filtstr[$key] = substr($filtstr1, 0, count($filterstr)-count($filterpost1));
...
if ($_GET['op']==""){
    output("`6`nEin kurzer kleiner Gnom mit BlÃ¤ttern in den Haaren hockt neben einem kleinen Baum. Er grinst und kichert hinter einer seiner fettigen HÃ¤nde.FÃ¼r einen Moment sieht es so aus, als ob er in den Wald wegkrabbeln will, aber dann grinst er dich an.`n`n`6\"`@Ich werde dir einen Gefallen tun,`6\" sagt er, \"`@wenn du mein RÃ¤tsel lÃ¶sen kannst!`6\"`n`n`6VorÃ¼bergehend verfÃ¤llt er in unkontrolliertes Kichern, fasst sich wieder fÃ¼r einen Moment und fÃ¤hrt fort.`n`n");

    output("`6\"`@Aber wenn du falsch liegen solltest, mein Gefallen es wird sein!`6\"`n`6`nNimmst du die Herausforderung an?`n`n");
...
        output("`6Vor Freude kichernd stellt er sein RÃ¤tsel:`n`n`6\"`@".$riddle['riddle']."`6\"`n`n`6Was meinst du?");
...
        if ($correct) {
            output("`n`6\"`@Eidechsen und Kaulquappen!! Du hast es!`6\", tobt er,`n`6\"`@Oh na gut. Hier hast du deinen dÃ¤mlichen Preis.`6\"`n`n");
...
                case 8:
                case 9:
                    output("Er macht ein Hokus Pokus und dreht sich um. Nach diesem Schauspiel fÃ¼hlst du dich bereit fÃ¼r den Kampf!`n`n`^Du erhÃ¤ltst einen zusÃ¤tzlichen Waldkampf!");
                    $session['user']['turns']++;
                    break;
                case 10:
                    output("Er schaut dir tief in die Augen, dann zieht er dir krÃ¤ftig eins Ã¼ber die RÃ¼be.  ");
                    if ($session['user']['specialty']) {
                        output("Als du wieder zu dir kommst, fÃ¼hlst du dich ein klein wenig geschickter.`n`#");
                        increment_specialty();
                    }else{
                        output("Das war eine Lektion in SpaÃŸ.`n`n`^Du erhÃ¤ltst ein paar Erfahrungspunkte!");
...
            output("`n`6Der merkwÃ¼rdige Gnom gackert vor Freude und tanzt um dich herum. Du fÃ¼hlst dich ziemlich albern dabei, als dieser verrÃ¼ckte Gnom um dich herum tÃ¤nzelt wie eine Fee,`deswegen machst du dich leise aus dem Staub, solange der Gnom abgelenkt ist. Irgendwie fÃ¼hlst du dich jetzt weniger als ein Held - mit diesem spÃ¶ttischen GelÃ¤chter in den Ohren. ");
...
                case 2:
                case 3:
                    output("Nicht viel spÃ¤ter stellst du fest, dass dir auch etwas Gold fehlt...`n`n`^Du hast Gold verloren!");
...
                case 4:
                case 5:
                    output("Du glaubst nicht, dass du sofort wieder einem Gegner gegenÃ¼ber treten kannst.`n`n`^Du verlierst einen Waldkampf!");
                    if ($session['user']['turns']>0) $session['user']['turns']--;
                    break;
                case 6:
                    output("Was wird wohl ".($session['user']['sex']?"Seth":"Violet")." davon halten?`n`n`^Du verlierst einen Charmepunkt!");
...
    output("`n`6Du fÃ¼rchtest dich lÃ¤cherlich zu machen und lehnst seine Herausforderung ab. Er war sowieso etwas gruselig.`n`6Der merkwÃ¼rdige Gnom kichert hysterisch und verschwindet im Wald.");
    $session['user']['specialinc']="";
    $session['user']['specialmisc']="";
}
?>
