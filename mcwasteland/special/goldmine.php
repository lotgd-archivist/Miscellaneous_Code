...
    output("`2In den tiefen des Waldes hast du eine alte verlassene Mine gefunden. Es liegt sogar noch GoldgrÃ¤berausrÃ¼stung in der NÃ¤he herum.`n`n`^Als du dich etwas umsiehst, bemerkst du, dass das hier eine Menge Arbeit wÃ¤re. So viel, dass du auf jeden Fall einen Waldkampf verlieren wirst, wenn du dein GlÃ¼ck versuchen willst.`n`n`^Ausserdem stellst du fest, dass durchaus die Gefahr von gelegentlichen EinstÃ¼rzen in der Mine besteht.`n`n");
    addnav("Nach Gold und Edelsteinen suchen","forest.php?op=mine"); 
    addnav("ZurÃ¼ck in den Wald","forest.php?op=no");
...
    output("`2Nop, fÃ¼r so einen langsamen Weg an Gold und Edelsteine zu kommen hast du keine Zeit. Also verlÃ¤sst du die alte Mine...`n"); 
    $session['user']['specialinc']="";
 } elseif ($_GET['op']=="mine") {
    $session['user']['specialinc']="goldmine.php"; 
    if ($session['user']['turns']<=0) {
        output("`2Du bist zu mÃ¼de um weiter zu buddeln...`n");
        $session['user']['specialinc']="";
    } else {
...
            } else {
                output("`&Du siehst, dass der Eingang zu Mine zu klein fÃ¼r dein ".$playermount['mountname']."`&, ist und du bindest das Tier am Eingang fest.`n");
            }
        }}
        output("`2Du hebst die AusrÃ¼stung auf und fÃ¤ngst an nach Gold und Edelsteinen zu graben...`n`n");
        $rand = e_rand(1,20);
        switch ($rand){

          case 1:case 2:case 3:case 4: case 5:
            output("`2Nach einigen Stunden harter Arbeit hast du nur ein paar wertlose Steine und einen SchÃ¤del gefunden...`n`n`^Beim Graben verlierst du einen Waldkampf.`n`n");
...
            output("`^Du hast die Hauptader entdeckt!`n`n`^Nach einigen Stunden harter Arbeit findest du ".$gems." Edelstein".($gems==1?"":"e")." und ".$gold." Gold!`n`n");
            $session['user']['gems'] += $gems;
            $session['user']['gold'] += $gold;
...
            break;
          case 19: case 20:
              output("`2Nach einer Menge harter Arbeit glaubst du, einen `&riesigen`2 Edelstein und etwas `6Gold`2 gesehen zu haben`2.`n`2Begierig auf Reichtum lehnst du dich zurÃ¼ck und schmetterst mit der  Spitzhacke darauf ein. `2Denn je fester du zuschlÃ¤gst, umso schneller wird es erledigt sein....`n`7UnglÃ¼cklicherweise bist `bdu`b erledigt.`nDein Ã¼bereifriges Schlagen verursacht einen schweren HÃ¶hleneinsturz.`n");
...
            $session['user']['specialinc']="";
            if ($dead == 1) {
                output("Du wurdest unter einer Tonne Felsen zerquetscht.`n`nVielleicht wird der nÃ¤chste Abenteurer deine Knochen bergen und ordentlich begraben.`n");
                if ($session['user']['hashorse']>0){
                    if ($horsedead) {
...
                    } else {
                        if ($horsecanenter) {
                            output("Dein ".$playermount['mountname']." hat es geschafft zu entkommen. Du weisst, dass das Tier darauf trainiert ist, zum Dorf zurÃ¼ckzukehren.`n");
                        } else {
                            output("Zum GlÃ¼ck hast du dein(e/n) ".$playermount['mountname']." draussen angebunden. Du weisst, dass das Tier darauf trainiert ist, zum Dorf zurÃ¼ckzukehren.`n");
                        }
                    }
                }
                $exp=$session['user']['experience']*0.1;    // Originalwert 0.6
                output("Wenigstens hast du dabei etwas Ã¼ber Bergbau gelernt. Du bekommst ".$exp." Erfahrungspunkte.`nDu verlierst all dein Gold und die HÃ¤lfte deiner Edelsteine.`n`n`3Du kannst morgen weiterspielen`n");
...
            }else{
                if ($session['user']['race'] == 4) {
                    output("GlÃ¼cklicherweise lassen dich deine ZwergenfÃ¤higkeiten unverletzt entkommen.`n");
                } elseif ($horsesave) {
...
                } else {
                    output("Durch pures GlÃ¼ck kannst du der Mine unverletzt entkommen!`n");
                }
