...
    $tname = $row2['name'];
    output("`GDu stoplerst Ã¼ber etwas. Es sind die skelettierten Ãœberreste dessen, was einst ein Abenteuer wie du gewesen sein kÃ¶nnte. TatsÃ¤chlich deutes vieles darauf hin, dass dies die Ãœberreste von `%".$tname."`G sind. Die sterblichen Ãœberreste scheinen unberÃ¼hrt und was immer ".($row2['sex']?"sie":"er")." fÃ¼r SchÃ¤tze dabei gehabt haben mag, kÃ¶nnte immer noch dort sein.`n`nWas wirst du tun?`n`n");
...
    //check_align();
    output("`GDu verbringst einen groÃŸen Teil des Tages damit, diesem gefallenen Abenteuerer ein Grab zu Schaufeln und ihm ein ordentliches BegrÃ¤bnis zukommen zu lassen. Du verzichtest auf Reichtum, wenn du ihn stehlen mÃ¼sstest - erst Recht von Toten.`n`n`nAuÃŸerdem willst du nicht die Rache der Toten hinter dir wissen, zusÃ¤tzlich zu den Kreaturen des Waldes.`n`nAls du endlich mit dem Grab fertig bist, erscheint eine kleine Fee vor dir. \"`#Das war sehr edel, was du getan hast. Ich werde dich dafÃ¼r belohnen.`G\"`n`n");
    $reward = rand(1,12);
    switch ($reward) {
        case 1:
        case 2:
...
            break;
        default:
            output("Du fÃ¼hlst dich gut bei dem, was du getan hast.`nDu bekommst mehr Charme!");
...
            output("Die Leiche zu plÃ¼ndern hat sich bezahlt gemacht! Du hast ".$gem_gain." ".$gemword." und ".$gold_gain." GoldstÃ¼cke gefunden.`n`n");
...
        case 3:
        case 4:
            output("Du durchsuchst die Leiche nach Reichtum, aber alles, was du finden kannst, sind vermoderte Lumpen und verrostete Waffen. Vielleicht hast du bei der nÃ¤chsten Leiche mehr GlÃ¼ck.`n`n");
            $session['user']['specialinc']="";
            //debuglog("looted a corpse and found nothing");
            break;
        default:
            output("Als du die Leiche nach ReichtÃ¼mern durchwÃ¼hlst, lenkt eine plÃ¶tzliche Bewegung deine Aufmerksamkeit auf sich. Du machst einen Satz zurÃ¼ck und siehst, wie sich dieses tote Skelett aufrappelt. Seine leeren AugenhÃ¶hlen staren dich mit einem roten GlÃ¼hen an. Wenn es reden kÃ¶nnte, wÃ¼rde es dich sicher verfluchen, aber schon das kranke SchleifgerÃ¤usch von Knochen auf Knochen lÃ¤sst es dir eiskalt den RÃ¼cken runterlaufen.`n`n");
            $badguy = array(
...
        $badguy=array();
        $session['user']['badguy']="";
        output("`n`GNach einem heftigen Kampf hast du den Skelettkrieger besiegt. Du hoffst, dass er vielleicht jetzt endlich seine ewige Ruhe finden wird.`n`nOder wenigstens, bis der nÃ¤chste Abenteurer auf der Suche nach dem schnellen Geld Ã¼ber seine Leiche stolpert.`n`n");
...
            $gemword = "Edelsteine";
            if ($gem_gain == 1) $gemword="Edelstein";
            output("Nach deinem Sieg nimmst du dir, was die deiner Meinung nach zusteht. Du findest ".$gem_gain." ".$gemword." und ".$gold_gain." GoldstÃ¼cke.`n`n");
            $session['user']['gems']+=$gem_gain;
            $session['user']['gold']+=$gold_gain;
        }else{
...
        output("`n`GDu wurdest vom Skelettkrieger besiegt! Doch anstatt im Totenreich aufzuwachen, bist du immer noch am Leben!`n`nDu verlierst 2% deiner Erfahrung".($session['user']['gems']?" und vermisst plÃ¶tzlich einen deiner Edelsteine":"").".`nAuÃŸerdem scheint ein schwerer Fluch vom Skelettkrieger auf dich Ã¼bergesprungen zu sein. Ob das seine Todesursache war? ");
