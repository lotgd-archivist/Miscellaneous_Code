...
output("`b`%FÃ¼r existierende Accounts:`b`n",true);
...
output("`@SeitengenerierungsgrÃ¶ÃŸe insgesamt: `^".number_format($row['s'])."b`n");
output("`@Durchschnittliche Seitengenerierungszeit: `^".dhms($row['t']/$row['c'],true)."`n");
output("`@Durchschnittliche SeitengrÃ¶ÃŸe: `^".number_format($row['s']/$row['c'])."`n",true);
...
output("<table border='0' cellpadding='0' cellspacing='5'>",true);
$class="trlight";
$odate=date("Y-m-d");
$j=0;
for ($i=0;$i<db_num_rows($result);$i++){
...
        output("<tr class='$class'><td>".date("Y-m-d",strtotime("$odate -$x days"))."</td><td>0</td><td>".$cumul."</td></tr>",true);
    }
//    if ($j%7==0) $class=($class=="trlight"?"trdark":"trlight");
//    $j++;
    $class=(date("W",strtotime($row['d']))%2?"trlight":"trdark");
    $cumul+=$row['c'];
    output("<tr class='".$class."'><td>".$row['d']."</td><td><img src='images/trans.gif' width='".$row['c']."' border='1' height='5'>".$row['c']."</td><td>".$cumul."</td></tr>",true);
    $odate = $row['d'];
}
output("</table>",true);
page_footer();
