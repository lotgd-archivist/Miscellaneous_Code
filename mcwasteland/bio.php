...
checkday();
$result = db_query("SELECT login,name,level,sex,dress,title,specialty,hashorse,acctid,age,marriedto,pvpflag,charisma,resurrections,bio,dragonkills,race,avatar,housekey,punch,reputation,birthday FROM accounts WHERE login='$_GET[char]'");
$row = db_fetch_assoc($result);
$row['login'] = rawurlencode($row['login']);

page_header("Charakter Biographie: ".preg_replace("'[`].'","",$row['name']));
$specialty=array(0=>"nicht spezifiziert","Dunkle KÃ¼nste","Mystische KrÃ¤fte","Diebeskunst","Kampfkunst ","Spirituelle KrÃ¤fte","Naturkraft");
//$horses=array(0=>"None","Pony","Gelding","Stallion");
output("`^Biographie fÃ¼r ".$row['name'],true);
...
output("`^Geschlecht: `@".($row['sex']?"Weiblich":"MÃ¤nnlich")."`n",true);
if($row['dress']>0){
    $dress = db_fetch_assoc(db_query("SELECT name,description FROM items WHERE id='".$row['dress']."'"));
    output("`^Kleidung: `@".$dress['name']." (".$dress['description'].")`n");
}else{
    output("`^Kleidung: `@`iAbgenutzte Bauernkleidung`i`n");
}
output("`^Spezialgebiet: `@".$specialty[$row['specialty']]."`n",true);
...
output("`^Tier: `@".$mount['mountname']."`n",true);
