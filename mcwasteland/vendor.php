...
sourceCopy("WanderhÃ¤ndler","anpera","anpera.net");
sourceVer("2004","");
...
            addnav("Etwas anderes kaufen","vendor.php?op=buy");
        }else if (db_num_rows(db_query("SELECT id FROM items WHERE name='".$row['name']."' AND owner='".$session['user']['acctid']."' AND class='MÃ¶bel'"))>0){
...
            for ($i=0;$i<db_num_rows($result);$i++){
                  $row = db_fetch_assoc($result);
                if($row['class']=='Kleidung' && $session['user']['dress']==$row['id']){
                }else{
                    $bgcolor=($i%2==1?"trlight":"trdark");
