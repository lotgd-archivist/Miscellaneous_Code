...
        $taunt = str_replace("%X","Scharfe ZÃ¤hne",$taunt);
        $taunt = str_replace("%W","Grosse grÃ¼ne Ratte",$taunt);
        $taunt = str_replace("%w","JoeBloe",$taunt);
        output("Vorschau: $taunt`0`n`n");
    }
    $output.="Spott: <input name='taunt' value='".$row['taunt']."' size='70'><br>";
    output("`nDie folgenden Codes werden unterstÃ¼tzt (GroÃŸ- und Kleinschriebung wird unterschieden):`n");
...
        output("<tr>",true);
        output("<td>",true);
        output("[<a href='taunt.php?op=edit&tauntid=".$row['tauntid']."'>Bearbeiten</a>|<a href='taunt.php?op=del&tauntid=".$row['tauntid']."' onClick='return confirm(\"Diesen Eintrag wirklich lÃ¶schen?\");'>LÃ¶schen</a>]",true);
...
    output("</table>",true);
    addnav("Spott hinzufÃ¼gen","taunt.php?op=edit");
}
page_footer();
?>
