...
*  (18.04.16) Anzahl gelesen und Cheatschutz bei ZÃ¤hlung
...
  gelesen INT(10) UNSIGNED DEFAULT '1' NOT NULL,

  lastreadby INT(10) UNSIGNED DEFAULT '1' NOT NULL,
...
output("`c`b`9Editor fÃ¼r Bibirs Bibliothek`0`b`c`n`n");
...
    addnav("Zur ThemenÃ¼bersicht","sulib.php");
...
    output("Alle BÃ¼cher zum Thema: ".$row['theme']."`0`n`n");

    $sql = "SELECT bookid, title, author, listorder, gelesen FROM lib_books
...
    output("<table cellpadding=2 cellspacing=1 bgcolor='#999999' align='center'><tr class='trhead'><td>Titel</td><td>Autor</td><td>Gelesen</td><td>Sortierung</td></tr>",true);

    if (db_num_rows($result)==0) {

        output("<tr class='trdark'><td colspan=3>Es sind keine BÃ¼cher vorhanden</td></tr>",true);
...
        //output("Thema wurde geÃ¤ndert.`n`n");
...
        output("Hier kann das Thema geÃ¤ndert werden.");
...
        output("Es sind keine BÃ¼cher zu diesem Thema vorhanden, das Thema wird gelÃ¶scht.");
...
        output("Es sind ".$row['anz']." BÃ¼cher vorhanden, was soll mit diesen passieren?`n`n");

        output("<form action=\"sulib.php?op=del_theme2&themeid=".$_GET['themeid']."\" method='POST'>

        <input type='radio' name='del' value='del_choice'>ebenfalls lÃ¶schen`n
...
        <input type='submit' class='button' value='LÃ¶schen'></form>",true);
...
        output("BÃ¼cher und Thema gelÃ¶scht.");

    }

    elseif ($_POST['del']=="other_theme") {

        output("Folgende BÃ¼cher einem anderen Thema zuordnen:`n");
...
        output("`nWelches Thema sollen die BÃ¼cher nun haben?");
...
        output("Thema-ID der BÃ¼cher entfernt und Thema gelÃ¶scht.");
...
    output("BÃ¼cher neu zugeordnet und das Thema gelÃ¶scht.");
...
        output("Dieses Buch ist jetzt fÃ¼r alle in der Bibliothek einsehbar.");

        $sql = "UPDATE lib_books SET activated='1' WHERE bookid=".$_GET['id'];

        $result = db_query($sql);

        $sql = 'SELECT acctid, title, author FROM lib_books WHERE bookid='.$_GET['id'];
...
        systemmail($row['acctid'],'Dein Buch wurde angenommen!','`0Das Buch mit dem Titel "'.$row['title'].'`0", das du eingereicht hattest, wurde in die Bibliothek aufgenommen. Du bekommst dafÃ¼r '.$dpts4book.' Donationpoints.');

        addnews("`qDas Buch '`Q".$row['title']."`q' von `Q".$row['author']."`q steht ab sofort in der Bibliothek zur VerfÃ¼gung.`0");

    }

    output("Das sind die BÃ¼cher, die eingereicht aber noch nicht freigegeben wurden:`n`n");
...
        output("<tr class='trdark'><td colspan=5>Es gibt keine BÃ¼cher, die aktiviert werden mÃ¼ÃŸten.</td></tr>",true);
...
        <a href=\"sulib.php?op=del_post&id=".$row['bookid']."\">LÃ¶schen</a></td>",true);
...
    output("Dieses Buch ist fÃ¼r immer verschwunden - und das ist auch gut so.");
...
    addnav("Weitere neue BÃ¼cher","sulib.php?op=new_books");
...
    output("<input type='submit' class='button' name='del' value='LÃ¶schen'></form>",true);
...
    addnav("Zu neuen BÃ¼chern","sulib.php?op=new_books");

    addnav("Zu BÃ¼chern ohne Thema","sulib.php?op=no_theme_books");
...
        output("<tr class='trdark'><td colspan=4>Es gibt keine BÃ¼cher, die einem Thema zugeordnet werden mÃ¼ÃŸten.</td></tr>",true);
...
        output("<tr class='$bgclass'><td><a href=\"sulib.php?op=del_post&id=".$row['bookid']."\">LÃ¶schen</a></td>",true);
...
    output("Ãœbersicht der Themen`n`n");

    output('<form action="sulib.php?saveorder=1" method="post">',true);

    addnav('','sulib.php?saveorder=1');

    output("<table cellpadding=2 cellspacing=1 bgcolor='#999999' align='center'><tr class='trhead'><td>Option</td><td>ID</td><td>Thema</td><td>Anzahl BÃ¼cher</td><td>Sortierung</td></tr>",true);
...
            <a href=\"sulib.php?op=del_theme&themeid=".$row['themeid']."\">LÃ¶schen</a></td><td>".$row['themeid']."</td><td>
...
    addnav("Neue BÃ¼cher ansehen","sulib.php?op=new_books");

    addnav("BÃ¼cher ohne Thema","sulib.php?op=no_theme_books");
