...
      output("`4Du machst dich auf in die Felder, wo einige unwissende Krieger schlafen.`n`nDu hast noch `^".$session['user']['playerfights']."`4 PvP KÃ¤mpfe Ã¼brig fÃ¼r heute.");
...
    (laston < '".date("Y-m-d H:i:s",strtotime(date("Y-m-d H:i:s")." -".getsetting("LOGINTIMEOUT",900)." seconds"))."' OR loggedin=0) AND
...
            output("`\$Uuuups:`4 Dieser Krieger ist gerade anderweitig ... beschÃ¤ftigt. Du wirst etwas auf deine Chance warten mÃ¼ssen! $row[pvpflag] : $pvptimeout");

        }elseif ($session['user']['dragonkills'] >($row['dragonkills']+5) && $row[location]!=2){

            output("`\$MÃ¶Ã¶Ã¶Ã¶p:`4 Dieser Gegner ist unter deiner WÃ¼rde!");
...
              if (strtotime($row[laston]) > strtotime(date("Y-m-d H:i:s")." -".getsetting("LOGINTIMEOUT",900)." sec") && $row[loggedin]){
...
                              output("`4Du bist zu mÃ¼de, um heute einen weiteren Kampf mit einem Krieger zu riskieren.");
...
          output("`\$Fehler:`4 Dieser Krieger wurde nicht gefunden. Darf ich fragen, wie du Ã¼berhaupt hier her gekommen bist?");
...
      output("Deine Ehre verbietet es dir, deine besonderen FÃ¤higkeiten einzusetzen.");
...
                output("`#AuÃŸerdem erhÃ¤ltst du das Kopfgeld in HÃ¶he von `^$badguy[creaturebounty]`# Gold!`n");
...
              output("`#*** Durch die hohe Schwierigkeit des Kampfes erhÃ¤ltst du zusÃ¤tzlich `^$expbonus`# Erfahrungspunkte!`n");
...
                addnews("`4".$session['user']['name']."`3 verdient `4{$badguy['creaturebounty']} Gold`3 fÃ¼r den Kopf von `4{$badguy['creaturename']}`3!");
...
                ." `n`n%o hatte anfangs `^".$badguy['playerstarthp']."`2 Lebenspunkte und kurz bevor du gestorben bist, hatte %o noch `^".$session['user']['hitpoints']."`2 Lebenspunkte Ã¼brig."

                ." `n`nDu hast `\$".(round(getsetting("pvpdeflose",5)-$xplossfactor*getsetting("pvpdeflose",5)))."%`2 deiner Erfahrungspunkte (etwa $lostexp Punkte) und `^".$badguy[creaturegold]."`2 Gold verloren. Dein Angreifer kassierte ausserdem das Kopfgeld in HÃ¶he von `^".$badguy[creaturebounty]." `2Gold ein."

                ." `n`nGlaubst du nicht auch, es ist Zeit dich zu rÃ¤chen?";
...
            addnav("ZurÃ¼ck zum Wohnviertel","houses.php?op=einbruch");
...
            systemmail($badguy['acctid'],"`2Du warst in $killedin`2 erfolgreich! ","`^".$session['user']['name']."`2 hat dich in $killedin`2 angegriffen, aber du hast gewonnen!`n`nDafÃ¼r hast du `^".round($session['user'][experience]*getsetting("pvpdefgain",10)/100,0)."`2 Erfahrungspunkte und `^".$session['user'][gold]."`2 Gold erhalten!");

            addnews("`%".$session['user']['name']."`5 wurde bei ".($session['user'][sex]?"ihrem":"seinem")."`5 Angriff auf`% $badguy[creaturename] `5  in $killedin `5getÃ¶tet.`n$taunt");
