...
output("`b`cBlusprings Trainingslager fÃ¼r Krieger`c`b",true);
...
    
    if ($_GET['op']==""){
        output("Der Klang von Kampf umfÃ¤ngt dich. Das Geklirr von Waffen in mÃ¤chtigen KÃ¤mpfen lÃ¤sst dein Kriegerherz hÃ¶her schlagen. ",true);
        output("Das alte GebÃ¤ude hinter dem Ãœbungsplatz ist die groÃŸe Akademie der geheimen KÃ¼nste.",true);
...
            output("Du bist der Meinung, dass du heute vielleicht schon genug von deinem Meister hast. Die Lektion, die du heute gelernt hast, hÃ¤lt dich davon ab, dich nochmal so bereitwillig ",true);
            output("einer derartigen DemÃ¼tigung zu unterwerfen.");
            addnav("ZurÃ¼ck zum Dorf","village.php");
            addnav("Warchilds Akademie","academy.php");
        }else{
            if(!$session['user']['prefs']['nosounds']){
                //output("<embed src='media/bigbong.wav' width='10' height='10' autostart='true' loop='false' hidden='true' volume='50'>",true);
                output("<audio autoplay preload=\"auto\" id=\"audioLoop\"><source src=\"media/bigbong.mp3\" type=\"audio/mp3\">Your browser does not support the audio element.</audio><script>var vid = document.getElementById(\"audioLoop\");vid.volume = 0.3;</script> ",true);
            }
...
                    output("Mit einem Wirbelsturm aus SchlÃ¤gen schlÃ¤gst du deinen Meister nieder.`n",true);
                }
            }else{
                output("Du machst dich mit ".$session['user']['weapon']." und ".$session['user']['armor']." bereit und nÃ¤herst dich Meister `^".$master['creaturename']."`0.`n`nEine kleine Menge Zuschauer ",true);
                output("hat sich versammelt und du bemerkst das Grinsen in ihren Gesichtern. Aber du fÃ¼hlst dich selbstsicher. Du verneigst dich vor `^".$master['creaturename']."`0 und fÃ¼hrst ",true);
                output("einen perfekten Drehangriff aus, nur um zu bemerken, dass du NICHTS in den HÃ¤nden hast!  `^".$master['creaturename']."`0 steht vor dir - mit deiner Waffe in der Hand.  ",true);
                output("Kleinlaut nimmst du ".$session['user']['weapon']." entgegen und schleichst unter dem schallenden GelÃ¤chter der Zuschauer vom Trainingsplatz.",true);
                addnav("ZurÃ¼ck zum Dorf","village.php");
                addnav("Warchilds Akademie","academy.php");
                $session['user']['seenmaster']=1;
            }
...
        output("Furchtsam nÃ¤herst du dich `^".$master['creaturename']."`0, um ihn zu fragen, ob du bereits in der selben Klasse wie er kÃ¤mpfst.",true);
        if($session['user']['experience']>=$exprequired){
            output("`n`n`^".$master['creaturename']."`0 sagt: \"Gee, deine Muskeln werden ja grÃ¶ÃŸer als meine...\"",true);
        }else{
            output("`n`n`^".$master['creaturename']."`0  stellt fest, dass du noch mindestens `%".($exprequired-$session['user']['experience'])."`0 Erfahrungspunkte mehr brauchst, bevor du bereit bist, ihn zu einem Kampf herauszufordern.",true);
        }
        if ($session['user']['reputation']>20) output("`nAu&szligerdem ist ".$master['creaturename']." von deinem ausgezeichneten Ruf begeistert.",true);
        if ($session['user']['reputation']<-20) output("`n".$master['creaturename']." zeigt sich sehr enttÃ¤uscht von deinem Verhalten als KÃ¤mpfer in der Welt.",true);
        addnav("Warchilds Akademie","academy.php");
        addnav("Meister befragen","train.php?op=question");
        addnav("Meister herausfordern","train.php?op=challenge");
/*
...
        Kampf von dir und dein eigener Stolz hindert dich daran, seine Forderung abzulehnen.",true);
        if ($session['user']['hitpoints']<$session['user']['maxhitpoints']){
            output("`n`nAls fairer KÃ¤mpfer gibt dir dein Meister vor dem Kampf einen Heiltrank.",true);
...
            output("`&Dein Stolz verbietet es dir, wÃ¤hrend des Kampfes Gebrauch von deinen besonderen FÃ¤higkeiten zu machen!`0",true);
        }
...
            if(!$session['user']['prefs']['nosounds']){
                //output("<embed src='media/cheer.wav' width='10' height='10' autostart='true' loop='false' hidden='true' volume='50'>",true);
                output("<audio autoplay preload=\"auto\" id=\"audioLoop\"><source src=\"media/cheer.mp3\" type=\"audio/mp3\">Your browser does not support the audio element.</audio><script>var vid = document.getElementById(\"audioLoop\");vid.volume = 0.3;</script> ",true);
            }
            $session['user']['level']++;
...
            }else{
                output("Keiner im Land ist mÃ¤chtiger als du!`n",true);
            }
...
    output("Du bummelst Ã¼ber den Ãœbungsplatz. JÃ¼ngere Krieger drÃ¤ngen sich zusammen und deuten auf dich, als du vorÃ¼ber lÃ¤ufst.  ",true);
    output("Du kennst diesen Platz gut. Bluspring grÃ¼ÃŸt dich und du gibst ihr einen starken HÃ¤ndedruck. AuÃŸer Erinnerungen ",true);
...
    output("zum Dorf zurÃ¼ckkehrst.",true);
    addnav("Warchilds Akademie","academy.php");
    addnav("ZurÃ¼ck zum Dorf","village.php");
}
page_footer();
