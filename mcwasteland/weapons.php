...
if ($_GET['op']==""){
  output("`9MightyE `7steht hinter einem Ladentisch und scheint dir nur wenig Interesse entgegen zu bringen, als du eintrittst. ");
...
    output("`n`n`9MightyE`7  nickt dir schlieÃŸlich zu und wÃ¼nscht sich, wÃ¤hrend er seinen Spitzbart streichelt, ");
...
    $result = db_query($sql) or die(db_error(LINK));
    output("`7Du schlenderst durch den Laden und tust dein Bestes, so auszusehen, als ob du wÃ¼ÃŸtest, was die meisten dieser Objekte machen. ");
    output("`9MightyE`7 schaut dich an und sagt \"`#Ich gebe dir `^".$tradeinvalue."`# ");
...
          output("`9MightyE`7 schaut dich eine Sekunde lang verwirrt an und kommt zu dem Schluss, dass du ein paar SchlÃ¤ge zuviel auf den Kopf bekommen hast. SchlieÃŸlich nickt er und grinst.");
...
                    output(" `bGlÃ¼ck gehabt!`b  `9MightyE`5 war gerade durch irgendwas am Fenster abgelenkt. Aber nochmal passiert ihm das nicht! Stolz auf deine ");
...
                    output(" `bGlÃ¼ck gehabt!`b `9MightyE`5 war gerade durch irgendwas am Fenster abgelenkt. Aber nochmal wird ihm das nicht passieren! Stolz auf deine ");
                    output("fette Beute stolzierst du Ã¼ber den Dorfplatz - bis dir jemand mitteilt, dass dir da noch ein Preisschild herumbaumelt...`nDu verlierst einen Charmepunkt!");
                    $session['user']['weapon'] = $row['weaponname'];
                    $session['user']['attack']-=$session['user']['weapondmg'];
                    $session['user']['weapondmg'] = $row['damage'];
...
                      output("WÃ¤hrend du wartest, bis `9MightyE`7 in eine andere Richtung schaut, nÃ¤herst du dich vorsichtig dem `5".$row['weaponname']."`7 und nimmst es leise vom Regal. ");
                    output("Deiner fetten Beute gewiss drehst du dich leise, vorsichtig, wie ein Ninja, zur TÃ¼r, nur um zu entdecken, ");
                    output("dass `9MightyE`7 drohend in der TÃ¼r steht und dir den Weg abschneidet. Du versuchst einen Flugtritt. Mitten im Flug hÃ¶rst du das \"SCHING\" eines Schwerts, ");
                    output("das seine Scheide verlÃ¤sst.... dein FuÃŸ ist weg. Du landest auf dem Beinstumpf und `9MightyE`7 steht immer noch im Torbogen, das Schwert ohne Gebrauchsspuren wieder im  Halfter und mit ");
...
                    output("`b`&Du wurdest von `9MightyE`& umgebracht!!!`n");
...
                    addnews("`%".$session[user][name]."`5 wurde beim Versuch, in `9MightyE`5's Waffenladen zu stehlen, niedergemetzelt.");
                }
...
                if ($klau==1){ // Fall nur fÃ¼r Diebe
                    output("`5Mit den Fertigkeiten eines erfahrenen Diebes tauschst du `%".$row['weaponname']."`5 gegen `%".$session['user']['weapon']."`5 aus und verlÃ¤sst frÃ¶hlich pfeifend den Laden. ");
                    output(" `bGlÃ¼ck gehabt!`b  `9MightyE`5 war gerade durch irgendwas am Fenster abgelenkt. Aber irgendwann wird er den Diebstahl bemerken und in Zukunft wesentlich besser aufpassen! Stolz auf deine ");
                    output("fette Beute stolzierst du Ã¼ber den Dorfplatz - bis dir jemand mitteilt, dass dir da noch ein Preisschild herumbaumelt...`nDu verlierst einen Charmepunkt!");
                    $session['user']['weapon'] = $row['weaponname'];
                    $session['user']['attack']-=$session['user']['weapondmg'];
                    $session['user']['weapondmg'] = $row['damage'];
...
                } else if ($klau==2 || $klau==3) { // Diebstahl gelingt perfekt
                    output("`5Da dir das nÃ¶tige Kleingold fehlt, grapschst du dir `%".$row['weaponname']."`5 und tauschst `%".$session['user']['weapon']."`5 unauffÃ¤llig dagegen aus. ");
                    output(" `bGlÃ¼ck gehabt!`b `9MightyE`5 war gerade durch irgendwas am Fenster abgelenkt. Aber irgendwann wird er den Diebstahl bemerken und in Zukunft besser aufpassen. Stolz auf deine ");
                    output("fette Beute stolzierst du Ã¼ber den Dorfplatz - bis dir jemand mitteilt, dass dir da noch ein Preisschild herumbaumelt...`nDu verlierst einen Charmepunkt!");
                    $session['user']['weapon'] = $row['weaponname'];
                    $session['user']['attack']-=$session['user']['weapondmg'];
                    $session['user']['weapondmg'] = $row['damage'];
...
            }
        }else{
            output("`9MightyE`7 nimmt dein `5".$session['user']['weapon']."`7 stellt es aus und hÃ¤ngt sofort ein neues Preisschild dran. ");
...
            $session['user']['attack']+=$session['user']['weapondmg'];
            $session['user']['weaponvalue'] = $row['value'];
            output("`n`nIm Gegenzug hÃ¤ndigt er dir ein glÃ¤nzendes, neues `5".$row['weaponname']."`7 aus, das du probeweise im Raum schwingst. Dabei schlÃ¤gst du `9MightyE`7 fast den Kopf ab. ");
