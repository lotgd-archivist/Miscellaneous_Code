...
addcommentary();
session_write_close();
popup_header("LoGD Nachricht des Tages (MoTD)");
...
            if ($row['motdtype']==0){
                motditem($row['motdtitle'].($session['user']['superuser']>=3?"[<a href='motd.php?op=del&id=".$row['motditem']."' onClick=\"return confirm('Bist du sicher, dass dieser Eintrag gelÃ¶scht werden soll?');\">LÃ¶schen</a>]":"")."`n[`i`b".$row['motddate']."`b`i]",$row['motdbody']);
            }else{
                pollitem($row['motditem'],$row['motdtitle'].($session['user']['superuser']>=3?"[<a href='motd.php?op=del&id=".$row['motditem']."' onClick=\"return confirm('Bist du sicher, dass dieser Eintrag gelÃ¶scht werden soll?');\">LÃ¶schen</a>]":"")."`n[`i`b".$row['motddate']."`b`i]",$row['motdbody']);
            }
        }
    }
    output("`&");
