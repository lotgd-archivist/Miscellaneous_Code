...
    $sql = "SELECT acctid,name,login,alive,location,sex,level,laston,loggedin,lastip,uniqueid,race FROM accounts WHERE locked=0 AND loggedin=1 AND laston>'".date("Y-m-d H:i:s",(date("U")-getsetting("LOGINTIMEOUT",900)))."' ORDER BY level DESC, dragonkills DESC, login ASC";
...
    output("<form action='list.php?op=search' method='POST'>Nach Name suchen: <input name='name'><input type='submit' class='button' value='Suchen' maxlength='50'></form>",true);
...
output("<tr class='trhead'><td><b>Level</b></td><td><b>Name</b></td><td><b>Rasse</b></td><td><b><img src=\"images/female.png\">/<img src=\"images/male.png\"></b></td><td><b>Ort</b></td><td><b>Status</b></td><td><b>Zuletzt da</b></tr>",true);
...
    output($row['alive']?"`9Lebt`0":"`4Tot`0");
