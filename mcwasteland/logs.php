...
        if (db_num_rows($result)>$ppp) addnav("NÃ¤chste Seite","logs.php?op=mail&in=".$_GET['in']."&limit=".($page+1));

        $output.="<table align='center'><tr><td><b>Datum</b></td><td><b>Absender</b></td><td><b>Betreff</b></td></tr>";

        for ($i=0;$i<db_num_rows($result);$i++){

            $row = db_fetch_assoc($result);

            $output.="<tr><td>".$row['sent']."</td><td>".$row['absender']."</td><td>".$row['subject']."</td></tr><tr><td colspan='3'>".$row['body']."</td></tr>";
...
        addnav("ZurÃ¼ck","logs.php?op=mail");
...
        if (db_num_rows($result)>$ppp) addnav("NÃ¤chste Seite","logs.php?op=mail&out=".$_GET['out']."&limit=".($page+1));

        $output.="<table align='center'><tr><td><b>Datum</b></td><td><b>Empf&auml;nger</b></td><td><b>Betreff</b></td></tr>";

        for ($i=0;$i<db_num_rows($result);$i++){

            $row = db_fetch_assoc($result);

            $output.="<tr><td>".$row['sent']."</td><td>".$row['empfaenger']."</td><td>".$row['subject']."</td></tr><tr><td colspan='3'>".$row['body']."</td></tr>";
...
        if (db_num_rows($result)>$ppp) addnav("NÃ¤chste Seite","logs.php?op=mail&limit=".($page+1)."&order=$sort&subop=".$_GET['subop']);

                $output.="<table align='center'><tr><td><b><a href='logs.php?op=mail&limit=$page&order=sent&subop=".$_GET['subop']."'>Datum</a></b></td><td><b><a href='logs.php?op=mail&limit=$page&order=msgfrom&subop=".$_GET['subop']."'>Absender</a></b></td><td><b><a href='logs.php?op=mail&limit=$page&order=msgto&subop=".$_GET['subop']."'>Empf&auml;nger</a></b></td><td><b>Betreff</b></td></tr>";
...
            if($row['msgfrom']==0) $row2['login']="`^System`0";
...
        addnav("ZurÃ¼ck","logs.php");

        output("`n`iUm das Postfach eines Spielers zu sehen, auf seinen Namen unter \"EmpfÃ¤nger\" klicken.`nUm zu sehen, was ein Spieler gesendet hat, auf seinen Namen unter \"Absender\" klicken.`i");
...
        output("Fehlgeschlagene Logins (".($_GET['limit']*$ppp)."-".($_GET['limit']*$ppp+$ppp).") fÃ¼r ID ".$_GET['id']."`n`n");

        if (db_num_rows($result)>$ppp) addnav("NÃ¤chste Seite","logs.php?op=faillog&id=".$_GET['id']."&limit=".($page+1)."&order=".$sort."&pw=".$_GET['pw']);
...
        addnav("ZurÃ¼ck","logs.php?op=faillog&order=".$_GET['order']);
...
        if ($_GET['pw'] && $i>=3) output("`n`iDie Anzeige der falsch eingegebenen PasswÃ¶rter dient dazu, um festzustellen, wo PasswÃ¶rter geraten werden und wo nur ein Tippfehler vorliegt.`i");
...
        if (db_num_rows($result)>$ppp) addnav("NÃ¤chste Seite","logs.php?op=faillog&ip=".$_GET['ip']."&limit=".($page+1)."&pw=".$_GET['pw']);
...
        if (db_num_rows($result)>$ppp) addnav("NÃ¤chste Seite","logs.php?op=faillog&limit=".($page+1)."&order=$sort");
...
            output('Sperre fÃ¼r die IP '.implode(', ',$ips).'`n');
...
            output('Sperre fÃ¼r die ID '.implode(', ',$ids).'`n');
...
        output("Grund fÃ¼r die Verbannung: <input name='reason' value=\"Ã„rger mich nicht.\">`n",true);
...
                if (substr($_SERVER['REMOTE_ADDR'],0,strlen($that)) == $that){
...
        output('User '.implode(', ',$delnames).' lÃ¶schen? ');
...
        output('BegrÃ¼ndung auswÃ¤hlen: ');

        output('<select name="stdreason" size="1">',true);

        output('<option value="0">--- keine BegrÃ¼ndung angeben ---</option>',true);
...
            output('<option value="'.$row['reasonid'].'">'.htmlentities($row['reason'], ENT_COMPAT | ENT_HTML401,"ISO-8859-1").'</option>',true);
...
        output('BegrÃ¼ndung speichern`n');

        output('</option>',true);



        output('<input type="Submit" value="lÃ¶schen">',true);
...
        foreach ($userid AS $that) {

    $sql = "SELECT name from accounts WHERE acctid='{$that}'";

    $res = db_query($sql);

    // inventar und haus lÃ¶schen und partner und ei freigeben

    if ($that==getsetting("hasegg",0)) savesetting("hasegg","0");

        $sql = "UPDATE items SET owner=0 WHERE owner=$that";

    db_query($sql);

    $sql = "UPDATE houses SET owner=0,status=3 WHERE owner=$that AND status=1";

    db_query($sql);

    $sql = "UPDATE houses SET owner=0,status=4 WHERE owner=$that AND status=0";

    db_query($sql);

    $sql = "UPDATE accounts SET charisma=0,marriedto=0 WHERE marriedto=$that";

    db_query($sql);

    $sql = "DELETE FROM pvp WHERE acctid2=$that OR acctid1=$that";

    db_query($sql) or die(db_error(LINK));

    $sql = "DELETE FROM accounts WHERE acctid='$that'";
...
    $sql = 'SELECT acctid,name,lastip,uniqueid,dragonkills,level,activated FROM accounts WHERE (lastip IN (-1'.$in_ip.') OR uniqueid IN (-1'.$in_id.')) AND locked="0" AND activated="1" ORDER BY dragonkills ASC, level ASC';
...
                        <input type="submit" name="deleteuser" value="lÃ¶schen">
...
    $output.="<table align='center'><tr><td><b>Datum</b></td><td><b>Acctid</b></td><td><b>Name</b></td><td><b>IP</b></td></tr>";

    for ($i=0;$i<db_num_rows($result);$i++){

        $row = db_fetch_assoc($result);

        $output.="<tr><td>".$row['date']."</td><td>".$row['acctid']."</td><td>".$row['absender']."</td><td>".$row['ip']."</td></tr>";

    }

    $output.="</table>";

    output("`n`nDie 5 letzten Systemmails:`n`n");
...
    $output.="<table align='center'><tr><td><b>Datum</b></td><td><b>EmpfÃ¤nger</b></td><td><b>Betreff</b></td></tr>";

    for ($i=0;$i<db_num_rows($result);$i++){

        $row = db_fetch_assoc($result);

        $output.="<tr><td>".$row['sent']."</td><td>".$row['empfaenger']."</td><td>".$row['subject']."</td></tr>";
...
//$output.="`n<div align='right'>";

//output("`)2004 by anpera & Chaosmaker");
