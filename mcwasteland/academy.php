...
sourceCopy("Die Akademie der geheimen KÃ¼nste","Kriegskind/Warchild","warchild@gmx.org");

sourceVer("February/March 2004","v0.961dt");
...
$cost_medium =round((($akt_magiclevel + 1)* 60)-$session['user']['reputation']);

$cost_high = round((($akt_magiclevel + 1) * 75)-$session['user']['reputation']);
...
            $sql="SELECT * FROM items WHERE ".$gebrauchte."class='Zaub.Prot' AND gold<='".$session['user']['gold']."' AND gems<='".$session['user']['gems']."' ORDER BY class DESC,name ASC LIMIT ".$limit;
...
    }elseif ($session['user']['dragonkills']> $min_dk){
