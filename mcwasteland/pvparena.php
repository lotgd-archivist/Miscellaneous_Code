...
    $output.="<table align='center' border='0' cellpadding='2' cellspacing='0' class='vitalinfo'><tr><td class='charhead' colspan='4'>";

    output("`^`bVital Info`b fÃ¼r diesen Kampf");

    $output.="</td></tr><tr><td class='charinfo'>";
...
        $output.="</td><td class='charinfo'>";

        output("`&Lebenspunkte: `^".$row['hp1']."/`0".$row['maxhp1']);

        $output.="</td><td class='charinfo'>";

        output("`&Angriff: `^`b$atk`b");

        $output.="</td><td class='charinfo'>";

        output("`&Verteidigung: `^`b$def`b");

        $output.="</td></tr><tr><td class='charinfo' colspan='2'>";

        output("`&Waffe: `^".$row['weapon1']);

        $output.="</td><td class='charinfo' colspan='2'>";

        output("`&RÃ¼stung: `^".$row['armor1']);

        $output.="</td></tr><tr><td colspan='4' class='charinfo'>";
...
        output("`&Lebenspunkte: `^".$row['hp2']."/`0".$row['maxhp2']);
...
        output("`&Waffe: `^".$row['weapon2']);

        $output.="</td><td class='charinfo' colspan='2'>";

        output("`&RÃ¼stung: `^".$row['armor2']);

        $output.="</td></tr><tr><td class='charinfo' colspan='4'>";

        output("`&Aktionen:`n".$buffs,true);
...
function arenanav($row,$nav=true){ // Navigation during fight
...
         $goodguy = array("name"=>$row['name1'],"level"=>$row['lvl1'],"hitpoints"=>$row['hp1'],"maxhitpoints"=>$row['maxhp1'],"attack"=>$row['att1'],"defense"=>$row['def1'],"weapon"=>$row['weapon1'],"armor"=>$row['armor1'],"darkartuses"=>$row['darkartuses1'],"magicuses"=>$row['magicuses1'],"thieveryuses"=>$row['thieveryuses1'],"warriorsartuses"=>$row['warriorsartuses1'],"priestsartuses"=>$row['priestsartuses1'],"rangersartuses"=>$row['rangersartuses1'],"bufflist"=>$row['bufflist1']);
...
         $goodguy = array("name"=>$row['name2'],"level"=>$row['lvl2'],"hitpoints"=>$row['hp2'],"maxhitpoints"=>$row['maxhp2'],"attack"=>$row['att2'],"defense"=>$row['def2'],"weapon"=>$row['weapon2'],"armor"=>$row['armor2'],"darkartuses"=>$row['darkartuses2'],"magicuses"=>$row['magicuses2'],"thieveryuses"=>$row['thieveryuses2'],"warriorsartuses"=>$row['warriorsartuses2'],"priestsartuses"=>$row['priestsartuses2'],"rangersartuses"=>$row['rangersartuses2'],"bufflist"=>$row['bufflist2']);
...
        addnav("KÃ¤mpfen","pvparena.php?op=fight&act=fight");

        addnav("`bBesondere FÃ¤higkeiten`b");

        if ($goodguy['darkartuses']>0) {

            addnav("`\$Dunkle KÃ¼nste`0", "");
...
        if ($goodguy['warriorsartuses']>0) {

            addnav("`qKampfkunst`0", "");

            addnav("`q&#149; Wuchtschlag`7 (1/".$goodguy['warriorsartuses'].")`0","pvparena.php?op=fight&skill=WA&l=1",true);

        }

        if ($goodguy['warriorsartuses']>1)

            addnav("`q&#149; Blutende Wunde`7 (2/".$goodguy['warriorsartuses'].")`0","pvparena.php?op=fight&skill=WA&l=2",true);

        if ($goodguy['warriorsartuses']>2)

            addnav("`q&#149; Schilddeckung`7 (3/".$goodguy['warriorsartuses'].")`0","pvparena.php?op=fight&skill=WA&l=3",true);

        if ($goodguy['warriorsartuses']>4)

            addnav("`q&#149; Klingentanz`7 (5/".$goodguy['warriorsartuses'].")`0","pvparena.php?op=fight&skill=WA&l=5",true);



        if ($goodguy['priestsartuses']>0) {

            addnav("`#Spirituelle KrÃ¤fte`0","");

            addnav("`#&#149; Heilaura`7 (1/".$goodguy['priestsartuses'].")`0","pvparena.php?op=fight&skill=PA&l=1",true);

        }

        if ($goodguy['priestsartuses']>1)

            addnav("`#&#149; Waffe segnen`7 (2/".$goodguy['priestsartuses'].")`0","pvparena.php?op=fight&skill=PA&l=2",true);

        if ($goodguy['priestsartuses']>2)

            addnav("`#&#149; Schutzschild`7 (3/".$goodguy['priestsartuses'].")`0","pvparena.php?op=fight&skill=PA&l=3",true);

        if ($goodguy['priestsartuses']>4)

            addnav("`#&#149; GÃ¶ttlicher Blitz`7 (5/".$goodguy['priestsartuses'].")`0","pvparena.php?op=fight&skill=PA&l=5",true);



        if ($goodguy['rangersartuses']>0) {

            addnav("`@Naturkraft`0", "");

            addnav("`@&#149; Laub werfen`7 (1/".$goodguy['rangersartuses'].")`0","pvparena.php?op=fight&skill=RA&l=1",true);

        }

        if ($goodguy['rangersartuses']>1)

            addnav("`@&#149; KrÃ¤uterverband`7 (2/".$goodguy['rangersartuses'].")`0","pvparena.php?op=fight&skill=RA&l=2",true);

        if ($goodguy['rangersartuses']>2)

            addnav("`@&#149; Falle legen`7 (3/".$goodguy['rangersartuses'].")`0","pvparena.php?op=fight&skill=RA&l=3",true);

        if ($goodguy['rangersartuses']>4)

            addnav("`@&#149; KrÃ¤fte der Natur`7 (5/".$goodguy['rangersartuses'].")`0","pvparena.php?op=fight&skill=RA&l=5",true);

// Ende neue Fertigkeiten
...
            $msg = str_replace("{weapon}", $session['user']['weapon'], $msg);
...
        output("`6Wen willst du zu einem Duell mit allen FÃ¤higkeiten herausfordern? Die ArenagebÃ¼hr kostet dich `^$cost `6Gold. Dein Gegner wird nicht unvorbereitet zustimmen.`nDu kannst heute noch `4".$session[user][playerfights]."`6 mal gegen einen anderen Krieger antreten.`n`n");
...
        if (db_num_rows($result)>$ppp) addnav("NÃ¤chste Seite","pvparena.php?op=challenge&limit=".($page+1)."&search=$_POST[search]");
...
          addnav("ZurÃ¼ck zur Arena","pvparena.php");
...
        output("`6Der ArenawÃ¤rter lacht sich darÃ¼ber kaputt, dass ein so kleiner SchwÃ¤chling wie du in der Arena kÃ¤mpfen will. Vielleicht solltest du wirklich erst etwas mehr Kampferfahrung sammeln.");
...
                    $sql = "INSERT INTO pvp (acctid1,acctid2,name1,name2,lvl1,lvl2,hp1,maxhp1,att1,def1,weapon1,armor1,darkartuses1,magicuses1,thieveryuses1,warriorsartuses1,priestsartuses1,rangersartuses1,bufflist1,bufflist2,turn)

                    VALUES (".$session['user']['acctid'].",{$row['acctid']},'".addslashes($session['user']['name'])."','".addslashes($row['name'])."',".$session['user']['level'].",{$row['level']},".$session['user']['hitpoints'].",".$session['user']['maxhitpoints'].",".$session['user']['attack'].",".$session['user']['defence'].",'".addslashes($session['user']['weapon'])."','".addslashes($session['user']['armor'])."',".$session['user']['darkartuses'].",".$session['user']['magicuses'].",".$session['user']['thieveryuses'].",".$session['user']['warriorsartuses'].",".$session['user']['priestsartuses'].",".$session['user']['rangersartuses'].",'".addslashes($session['user']['bufflist'])."','',0)";

                    db_query($sql) or die(db_error(LINK));

                    if (db_affected_rows(LINK)<=0) redirect("pvparena.php");

                    output("`6Du hast`4 $row[name] `6zu einem Duell herausgefordert und wartest nun auf ".($row[sex]?"ihre":"seine")." Antwort. Du kÃ¶nntest $row[name]`6 den Kampf ");

                    output(" schmackhafter machen, indem du ".($row[sex]?"ihr":"ihm")." die ArenagebÃ¼hr von  `^".($row[level]*20)."`6 Gold Ã¼berweist.`n");
...
            output("`4Du hast nicht genug Gold dabei, um die ArenagebÃ¼hr zu bezahlen. Mit rotem Kopf ziehst du ab.");
...
    output("`6Beim Anblick deines Gegners $row[name] `6wird dir Angst und Bang. Mit ein paar sehr dÃ¼rftigen Ausreden wie \"nicht genug Gold\" lehnst du die Herausforderung ab und verlÃ¤sst schnell die Arena.");

    systemmail($row[acctid],"`2Herausforderung abgelehnt","`2".$session[user][name]."`2 hat deine Herausforderung abgelehnt. Vielleicht solltest du ".($session[user][sex]?"ihr":"ihm")." etwas fÃ¼r den Kampf anbieten - falls ".($session[user][sex]?"sie":"er")." dich besiegen kann.");
...
        output("`4Du kannst dir die Arena NutzungsgebÃ¼hr von `^$cost`4 Gold nicht leisten.");
...
    output("`6Du bist es Leid, auf $row[name]`6 zu warten und ziehst deine Herausforderung zurÃ¼ck. Die ArenagebÃ¼hr bekommst aber trotz langer Verhandlungen mit der Arena-Leitung nicht zurÃ¼ck.`n");
...
    systemmail($row[acctid],"`2Herausforderung zurÃ¼ckgezogen","`2".$session[user][name]."`2 hat seine Herausforderung zurÃ¼ckgezogen.");
...
                    "startmsg"=>"`n`^Du fÃ¤ngst an zu regenerieren!`n`n",

                    "name"=>"`%Regeneration",

                    "rounds"=>5,

                    "wearoff"=>"Deine Regeneration hat aufgehÃ¶rt",

                    "regen"=>$goodguy['level'],

                    "effectmsg"=>"Du regenerierst um {damage} Punkte.",

                    "effectnodmgmsg"=>"Du bist vÃ¶llig gesund.",
...
                    "wearoff"=>"Die erdene Faust zerfÃ¤llt zu staub.",
...
                    "startmsg"=>"`n`^Deine Waffe glÃ¼ht in einem Ã¼berirdischen Schein.`n`n",
...
                    "effectnodmgmsg"=>"Du fÃ¼hlst ein Prickeln, als deine Waffe versucht, deinen vollstÃ¤ndig gesunden KÃ¶rper zu heilen.",
...
                    "effectmsg"=>"{badguy}`)wird von einem Blitzbogen aus deiner Haut mit `^{damage}`) Schadenspunkten zurÃ¼ckgeworfen.",
...
                "startmsg"=>"`nDu legst deine Stirn in Falten und beschwÃ¶rst die Elemente.  Eine kleine Flamme erscheint. {badguy} zÃ¼ndet sich eine Zigarette daran an, dankt dir und stÃ¼rzt sich wieder auf dich.`n`n",
...
                    "startmsg"=>"`n`\$Du rufst die Geister der Toten und skelettartige HÃ¤nde zerren an {badguy} aus den Tiefen ihrer GrÃ¤ber.`n`n",

                    "name"=>"`\$Skelettdiener",

                    "rounds"=>5,

                    "wearoff"=>"Deine Skelettdiener zerbrÃ¶ckeln zu staub.",
...
                    "effectmsg"=>"Du stÃ¶ÃŸt eine Nadel in die {badguy}-Puppe und machst damit `^{damage}`) Schadenspunkte!",
...
                    "startmsg"=>"`n`\$Du streckst deine Hand aus und {badguy} `\$fÃ¤ngt an, aus den Ohren zu bluten.`n`n",
...
                "startmsg"=>"`nErschÃ¶pft versuchst du deine dunkelste Magie: einen schlechten Witz.  {badguy} schaut dich nachdenklich eine Minute lang an. Endlich versteht er den Witz und stÃ¼rzt sich lachend wieder auf dich.`n`n",
...
                    "wearoff"=>"Dein Gegner putzt sich die Nase und hÃ¶rt auf zu weinen.",
...
                    "startmsg"=>"`n`^Mit dem Geschick eines erfahrenen Diebs scheinst du zu verschwinden und kannst {badguy} aus einer gÃ¼nstigeren und sichereren Position angreifen.`n`n",
...
                    "startmsg"=>"`n`^Mit deinen FÃ¤higkeiten als Dieb verschwindest du und schiebst {badguy} von hinten eine dÃ¼nne Klinge zwischen die RÃ¼ckenwirbel!`n`n",
...
                "startmsg"=>"`nDu versuchst, {badguy} anzugreifen, indem du deine besten DiebeskÃ¼nste in die Praxis umsetzt - aber du stolperst Ã¼ber deine eigenen FÃ¼sse.`n`n",
...
        if ($goodguy['warriorsartuses'] >= $_GET['l']){
...
                    "effectmsg"=>"`qDu holst zu einem besonders krÃ¤ftigen Schlag aus und machst {damage} Punkte zusÃ¤tzlich Schaden.",

                    "minbadguydamage"=>(1+round($goodguy['level']/2)),

                    "maxbadguydamage"=>(2+$goodguy['level']*3),
...
                $goodguy['bufflist']['wa2'] = array(

                    "startmsg"=>"`n`qDu schlÃ¤gst `%{badguy}`q eine blutende Wunde!`n`n",

                    "name"=>"`qBlutende Wunde",

                    "rounds"=>5,

                    "wearoff"=>"`q{badguy}`qs Wunde hÃ¶rt auf zu bluten.",

                    "minioncount"=>1,

                    "effectmsg"=>"`qBlut sprudelt aus {badguy}`q und macht `^{damage}`q Schaden.",
...
                $goodguy['bufflist']['wa3'] = array(

                    "startmsg"=>"`n`qDu setzt {weapon} effektiv als Schild ein, um {badguy}`qs Angriffe zu blocken.`n`n",
...
                    "wearoff"=>"`qDu gehst wieder zum normalen Kampf Ã¼ber.",
...
                $goodguy['bufflist']['wa5'] = array(

                    "startmsg"=>"`n`qDu startest einen wilden Tanz mit {weapon}`q und gibst dem Gegner kein Ziel mehr.`n`n",

                    "name"=>"`qKlingentanz",

                    "rounds"=>5,

                    "wearoff"=>"`qDir wird schwindlig und du beendest den Tanz.",
...
            $goodguy['warriorsartuses']-=$_GET['l'];

        }else{

            $goodguy['wa0'] = array(

                "startmsg"=>"`nDu hebst deine schwere Waffe Ã¼ber den Kopf - und fÃ¤llst nach hinten um.`n`n",
...
        if ($goodguy['priestsartuses'] >= $_GET['l']){
...
                    "wearoff"=>"`#Deine Aura hat aufgehÃ¶rt.",
...
                    "startmsg"=>"`n`#Du segnest {weapon}`n`n",

                    "name"=>"`#Waffe segnen",

                    "rounds"=>5,

                    "wearoff"=>"`#Die Kraft der Segnung lÃ¤sst nach.",
...
                    "startmsg"=>"`n`#Du zauberst einen blÃ¤ulich schimmernden Schutzschild um dich.`n`n",
...
                    "wearoff"=>"`#Der Schutzschild bricht zusammen.",
...
                    "name"=>"`#GÃ¶ttlicher Blitz",

                    "rounds"=>1,

                    "minioncount"=>1,

                    "effectmsg"=>"`#Du fokusierst deine gÃ¶ttliche Kraft auf {badguy}`#. Du machst {damage} Punkte Schaden und kannst diese Runde nicht verletzt werden.",

                    "minbadguydamage"=>(2+4*$goodguy['level']),

                    "maxbadguydamage"=>(2+$goodguy['level']*7),
...
            $goodguy['priestsartuses']-=$_GET['l'];

        }else{

            $goodguy['bufflist']['pa0'] = array(

                "startmsg"=>"`n`#Zu spÃ¤t bemerkst du, dass du den falschen Gott angerufen hast.`n`n",
...
        if ($goodguy['rangersartuses'] >= $_GET['l']){
...
                $goodguy['bufflist']['ra1']=array(

                    "startmsg"=>"`n`@Du wirfst {badguy}`@ Laub ins Gesicht.`n`n",

                    "name"=>"`@Laub werfen",

                    "rounds"=>5,

                    "wearoff"=>"`@Dein Gegner reibt sich die Augen und kann dich wieder sehen.",

                    "roundmsg"=>"`@{badguy} `@schlÃ¤gt nur blind um sich.",
...
                $goodguy['bufflist']['ra2']=array(

                    "startmsg"=>"`n`@Du legst dir einen KrÃ¤uterverband an.`n`n",

                    "name"=>"`@KrÃ¤uterverband",

                    "rounds"=>7,

                    "wearoff"=>"`@Der KrÃ¤uterverband fÃ¤llt ab.",

                    "regen"=>($goodguy['level']+1),
...
                $goodguy['bufflist']['ra3']=array(

                    "startmsg"=>"`n`@Du bittest {badguy}`@ um etwas Geduld und legst schnell eine Falle - die {badguy}`@ auch sofort auslÃ¶st.`n`n",

                    "name"=>"`@Falle",

                    "rounds"=>5,

                    "wearoff"=>"`@{badguy}`@ hat sich aus der Falle befreit.",

                    "roundmsg"=>"`@{badguy}`@ ist mit deiner Falle beschÃ¤ftigt.",
...
                $goodguy['bufflist']['ra5']=array(

                    "startmsg"=>"`n`@Du rufst die KrÃ¤fte der Natur an und die Kreaturen des Waldes wenden sich gegen {badguy}`@.`n`n",

                    "name"=>"`@KrÃ¤fte der Natur",

                    "rounds"=>4,

                    "wearoff"=>"`@Die Natur beruhigt sich.",

                    "minioncount"=>round($goodguy['level']/2)+1,

                    "maxbadguydamage"=>$goodguy['level'],

                    "effectmsg"=>"`@Eine Wurzel und ein Ast treffen {badguy}`@ mit `^{damage}`@ Schadenspunkten.",

                    "effectnodmgmsg"=>"`@{badguy}`@ weicht dem nÃ¼ssewerfenden EichhÃ¶rnchen aus.",
...
            $goodguy['rangersartuses']-=$_GET['l'];

        }else{

            $goodguy['bufflist']['ra0'] = array(

                "startmsg"=>"`n`@Ein Waldschrat schaut hinter einem Baum hervor, schÃ¼ttelt mit dem Kopf und lÃ¤sst euch weiter kÃ¤mpfen.`n`n",
...
            output("`@`b`c--- DAS WAR DEIN BISHER HÃ„RTESTER SCHLAG! ---`c`b`n");
...
            if ($row[turn]==1) $sql = "UPDATE pvp SET hp1=$goodguy[hitpoints],hp2=$badguy[hitpoints],thieveryuses1=$goodguy[thieveryuses],darkartuses1=$goodguy[darkartuses],magicuses1=$goodguy[magicuses],warriorsartuses1={$goodguy['warriorsartuses']},priestsartuses1={$goodguy['priestsartuses']},rangersartuses1={$goodguy['rangersartuses']},bufflist1='".addslashes($goodguy[bufflist])."',lastmsg='$message',turn=2 WHERE acctid1=".$session[user][acctid]."";

            if ($row[turn]==2) $sql = "UPDATE pvp SET hp1=$badguy[hitpoints],hp2=$goodguy[hitpoints],thieveryuses2=$goodguy[thieveryuses],darkartuses2=$goodguy[darkartuses],magicuses2=$goodguy[magicuses],warriorsartuses2={$goodguy['warriorsartuses']},priestsartuses2={$goodguy['priestsartuses']},rangersartuses2={$goodguy['rangersartuses']},bufflist2='".addslashes($goodguy[bufflist])."',lastmsg='$message',turn=1 WHERE acctid2=".$session[user][acctid]."";
...
            output("`n`&Kurz vor deinem finalen TodesstoÃŸ beendet der ArenawÃ¤rter euren Kampf und erklÃ¤rt dich zum Sieger!`0`n");

            output("`b`\$Du hast $badguy[name] `\$besiegt!`0`b`n");

            output("`#Du bekommst das Preisgeld in HÃ¶he von `^$win`# Gold und deinen gerechten Lohn an Arenakampfpunkten!`n");
...
                ." `n`n%o hatte am Ende noch `^".$goodguy['hitpoints']."`2 Lebenspunkte Ã¼brig."
...
            systemmail($badguy[acctid],"`2 du warst in der Arena erfolgreich! ","`^".$session[user][name]."`2 hat in der Arena verloren!`n`nDafÃ¼r hast du `^$exp`2 Erfahrungspunkte und `^$win`2 Gold erhalten!");
...
        output("`6Euer Kampf wurde vorzeitig beendet. Die NutzungsgebÃ¼hr kommt dem Ausbau der Arena zugute.");
...
    $sql="SELECT * FROM pvp WHERE acctid1={$_GET['kid1']} AND acctid2={$_GET['kid2']}";
...
        output("`2Der Kampf ist vorbei. Es gibt nichts mehr zu sehen.");

    }else{

        output("`%".($row['turn']==1?"{$row['name1']}":"{$row['name2']}")."`2 ist jetzt an der Reihe:");

        output("<ul>",true);

        arenanav($row,$nav=false);

        output("</ul>",true);

        addnav("Aktualisieren","pvparena.php?op=watch&kid1={$_GET['kid1']}&kid2={$_GET['kid2']}");

        addnav("ZurÃ¼ck","pvparena.php");
...
        addnav("Herausforderung zurÃ¼cknehmen","pvparena.php?op=back&id=".$row['acctid2']);
...
        output("`6Du wurdest von `&$row[name1] `6herausgefordert. Wenn du die Herausforderung annimmst, musst du `^$cost`6 Gold ArenagebÃ¼hr bezahlen.`n");
...
        output("`6Du betrittst die groÃŸe Kampfarena in der NÃ¤he des Trainingslagers. Hier kÃ¤mpfen einge der Krieger gegeneinander und Ã¼ben sich in ihren besonderen Fertigkeiten, ");

        output("um herauszufinden, wer von ihnen der Beste ist. Es geht um die Ehre - und um eine gute Platzierung in der KÃ¤mpferliga.`n");
...
            output(" Du beobachtest das bunte Treiben auf dem Platz eine Weile.`nFolgende Krieger kÃ¤mpfen gerade gegeneinander:`n`n");

            $output.="<table border='0' cellpadding='3' cellspacing='0'><tr><td align='center'>";

            output("`bHerausforderer`b");

            $output.="</td><td align='center'>";

            output("`bVerteidiger`b");
...
                $output.="</td><td>".$row['hp1']." : ".$row['hp2']."</td><td>".($visitor==1?"<a href='pvparena.php?op=watch&kid1={$row['acctid1']}&kid2={$row['acctid2']}'>Betrachten</a>":"")." ".($session['user']['superuser']>2?"<a href='pvparena.php?op=del&kid1=$row[acctid1]&kid2=$row[acctid2]'>L&ouml;schen</a>":"")."</td></tr>";

                if ($session['user']['superuser']>2) addnav("","pvparena.php?op=del&kid1=$row[acctid1]&kid2=$row[acctid2]");

                if ($visitor==1) addnav("","pvparena.php?op=watch&kid1={$row['acctid1']}&kid2={$row['acctid2']}");
...
            output("`n`n`6Im Moment laufen keine KÃ¤mpfe zwischen den Helden dieser Welt.`n`n");
