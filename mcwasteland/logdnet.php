...
        if (strtotime($row['lastupdate'])<strtotime(date("Y-m-d H:i:s")." -1 minutes")){
...
    $sql = "SELECT address,description FROM logdnet WHERE lastupdate > '".date("Y-m-d H:i:s",strtotime(date("Y-m-d H:i:s")." -7 days"))."' ORDER BY priority DESC";
...
    output("<tr><td>`@`bServername und Link`b`0</td><td width='200'>`@`bVersion`b`0</td></tr>",true);
...
            output("<tr><td valign='top'><a href='".HTMLEntities($row['address'], ENT_COMPAT | ENT_HTML401,"ISO-8859-1")."' target='_blank'>".stripslashes(HTMLEntities($row['description'], ENT_COMPAT | ENT_HTML401,"ISO-8859-1"))."`0</a></td><td valign='top' width='130'>".HTMLEntities($row['version'], ENT_COMPAT | ENT_HTML401,"ISO-8859-1")."</td></tr>",true);
