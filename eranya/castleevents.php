...
* @version DS-E V/2
* 20.6.2006 Tippfehler (Forum 12.4.06) beseitigt
*/
...
    output("`3Dir wird plötzlich so schwindelig... alles um dich herum dreht sich... als du wieder zu die kommst stehst du... auf dem Stadtplatz!!`n");
    
...
        {
            
            output("`&Ohne lange zu überlegen greifst du an und schlägst ihm deine Waffe direkt ins Herz! Unter lauten Fauchen und Kreisen löst sich der Vampir auf und du findest im Staubhaufen `^4 Edelsteine`&!");
            $session['user']['gems']+=4;
                        
            
            addnav("Puh!","abandoncastle.php?loc=$locale");
...
        addnav("Hallo, Jarcath!","shades.php");
        break;
    case 4 :
...
            addnav("Na toll...","mazemonster.php?op=fight&fiend=minotaur");
            break;
        }
    }
    else
...
        case 1 :
        case 2 :
            output("Ihre Todesschreie werden dich noch sehr lange des Nachts verfolgen! Du fühlt dich nur noch hässlich!");
...
            
            addnav("Auf in den Kampf!","mazemonster.php?op=fight&fiend=devoured_soul");
            
            break;
            
        case 4:
...
    $locale=$_GET['loc'];
    output("<big><big><big>`#Ein Schrein!</big></big></big>`n",true);
    output("`&Du siehst am Rande des Ganges einen kleinen `9Luftschrein`&. Ein Knistern liegt in der Luft als du dich ihm näherst. An der Wand über dem Schrein prangt eine Glyphe. Sie glüht so stark und strahlt eine derartige Hitze aus, dass du meinen könntest, sie sei aus flüssigem Metall!`n`nAuf einem Schild neben der Glyphe steht geschrieben: `4\"Wagemutiger, sei gewarnt! Wer die Glyphe berührt, opfert einen Teil seiner Kraft.\"`n");
    addnav("Was tust du?");
    if ($mark<2)
    {
...
{
    $locale=$_GET['loc'];
    output("`&Mit stark pochendem Herzen trittst du an den Schrein und drückt deinen Arm gegen die Glyphe. Ein grauenvoller Schmerz durchzuckt deinen ganzen Körper als sich die Glyphe in dein Fleisch brennt. Du verlierst 5 permanente Lebenspunkte.`n`n");
    $session['user']['maxhitpoints'] -= 5;
...
        break;
    case 5 :
        output("`&Unter gewaltigen Schmerzen bleibst du dennoch standhaft, und als du deinen Arm von der Glyphe lösen kannst, stellst du ein Mal fest, das sich tief in dein Fleisch gebrannt hat.`n");
