...
<?php
/**
* su_usereditor.php: Zentrales Werkzeug für Superuser, um Spieleraccounts zu bearbeiten und zu verwalten
...
        addnav("Letzten Treffer anzeigen","su_usereditor.php?op=lasthit&userid={$_GET['userid']}",false,true);
...
        }
        addnav('Knappeneditor','su_usereditor.php?op=disciple&userid='.$_GET['userid']);
        addnav('Aktualisieren','su_usereditor.php?op=edit&userid='.$_GET['userid']);
...
output("<form action='su_usereditor.php?op=search' method='POST'>Suche in allen Feldern: <input name='q' id='q'><input type='submit' class='button'></form>",true);
output("<script language='JavaScript'>document.getElementById('q').focus();</script>",true);
addnav("","su_usereditor.php?op=search");
...
        
        addnav('Mechanik');
        addnav("Account-Tabellen abgleichen","su_usereditor.php?op=extratable");
        //addnav("bestdragonage kopieren","su_usereditor.php?op=copydata");
        addnav("Überflüssige Tabellen löschen","su_usereditor.php?op=delextra");
        //addnav("Benutzereditor","su_usereditor.php");
        //if($session['user']['acctid'] == 2310) {addnav('Forum','su_usereditor.php?op=forum_all');}
        addnav('Seiten');
...
        while ($row[count]>0){
                $page++;
                addnav("$page Seite $page","su_usereditor.php?page=".($page-1)."&sort=$_GET[sort]");
                $row[count]-=100;
        }
}
...
                        //$row = db_fetch_assoc($result);
                        //redirect("su_usereditor.php?op=edit&userid=$row[acctid]");
...
                        
                        output($count[0].' Spieler wirklich ausloggen?`n`n'.create_lnk('Ab ins Körbchen!','su_usereditor.php?op=logout_all&act=ok'),true);
                        
                }
                
...
                
                $result3 = db_query("SELECT * FROM account_stats WHERE acctid='$_GET[userid]'") or die(db_error(LINK));
                $row3 = db_fetch_assoc($result3) or die(db_error(LINK));
                
...
                        "ctitlepos"=>"Position des Titels,enum,0,Vor dem Namen,1,Hinter dem Namen",
...
                        "autodelete"=>"Automatische Löschung,enum,0,Ja,1,Nein",
                        
...
                        
                        "rp_status"=>"RP-Status (für Einwohnerliste)",
                        "Werte,title",
...
                        "alive"=>"Lebendig,bool",
                        "deathpower"=>"Gefallen bei Jarcath,int",
...
                        "gems"=>"Edelsteine,int",
                        "gold"=>"Bargold,int",
                        "gemsfach"=>"Schließfach freigeschaltet,enum,0,Nein,1,Ja",
                        "gemsinbank"=>"Edelsteine im Schließfach,int",
...
                        
                        "RP-Daten,title",
                        "comments_rp"=>"RP-Kommentare aktuell,int",
                        "comments_rp_words"=>"RP-Komm. aktuell (Wörter),int",
                        "comments_rp_ges"=>"RP-Kommentare insgesamt,int",
                        "rppoints"=>"gesammelte RP-Punkte,int",
                        "rppoints_spent"=>"ausgegebene RP-Punkte,int",
                                                
...
                                        if ($val>"") {
                                                $sql1.="password = '".get_hash($val)."',";
                                        }
                                }
                                elseif (isset($row2[$key])){
                                        $sql2.="$key = \"$val\",";
                                }
                                elseif (isset($row3[$key])){
                                        $sql3.="$key = \"$val\",";
                                }
        
                        }
...
                        } else {
                                header("Location: su_usereditor.php");
                        }
                
                        exit();
                }
...
                }
                
                $row = array_merge($row,$row2,$row3);
                                
                output("<form action='su_usereditor.php?op=special&userid=$_GET[userid]".($_GET['returnpetition']!=""?"&returnpetition={$_GET['returnpetition']}":"")."' method='POST'>",true);
                addnav("","su_usereditor.php?op=special&userid=$_GET[userid]".($_GET['returnpetition']!=""?"&returnpetition={$_GET['returnpetition']}":"")."");
...
                output("</form>",true);
                        
                output("<form action='su_usereditor.php?op=edit&act=save&userid=$_GET[userid]".($_GET['returnpetition']!=""?"&returnpetition={$_GET['returnpetition']}":"")."' method='POST'>",true);
                addnav("","su_usereditor.php?op=edit&act=save&userid=$_GET[userid]".($_GET['returnpetition']!=""?"&returnpetition={$_GET['returnpetition']}":"")."");
                addnav("","su_usereditor.php?op=edit&userid=$_GET[userid]".($_GET['returnpetition']!=""?"&returnpetition={$_GET['returnpetition']}":"")."");
                
                editnav();
                
                addnav("Usereditor");
...
                        output("<iframe src='su_usereditor.php?op=lasthit&userid={$_GET['userid']}' width='100%' height='400'>Dein Browser muss iframes unterstützen, um die letzte Seite des Users anzeigen zu können. Benutze den Link im Menü stattdessen.</iframe>",true);
                }
                addnav("","su_usereditor.php?op=lasthit&userid={$_GET['userid']}");
                
        break;        // END edit
                
        case 'special':
...
                        $str_lnk = 'su_usereditor.php?op=edit&userid='.$_GET['userid'];
                }
                else{
...
                
                addnav('Zurück zum Useredit','su_usereditor.php?op=edit&userid='.$int_uid);
                
                if(!empty($int_did)) {
                
...
                                                        );
                                                        
                $str_lnk = 'su_usereditor.php?op=disciple&userid='.$int_uid;
                addnav('',$str_lnk);
...
                if( !empty($ret) ){
                        redirect("su_usereditor.php?op=edit2&userid=".$_GET['userid']."&msg=ok");
                }
                else{
                        redirect("su_usereditor.php?op=edit2&userid=".$_GET['userid']."&msg=fail");
                }
                
        break;        // END forum
        
...
                
                addnav("User Info bearbeiten","su_usereditor.php?op=edit&userid=$id");
                
                $result = db_query($sql);
...
        output("<tr>
        <td>Ops</td>
        <td><a href='su_usereditor.php?sort=login'>Login</a></td>
        <td><a href='su_usereditor.php?sort=name'>Name</a></td>
        <td><a href='su_usereditor.php?sort=acctid'>ID</a></td>
        <td><a href='su_usereditor.php?sort=level'>Lev</a></td>
        <td><a href='su_usereditor.php?sort=laston'>Zuletzt da</a></td>
        <td><a href='su_usereditor.php?sort=gentimecount'>Treffer</a></td>
        <td><a href='su_usereditor.php?sort=lastip'>IP</a></td>
        <td><a href='su_usereditor.php?sort=uniqueid'>ID</a></td>
        <td><a href='su_usereditor.php?sort=emailaddress'>E-Mail</a></td>
        </tr>",true);
        addnav("","su_usereditor.php?sort=login");
        addnav("","su_usereditor.php?sort=name");
        addnav("","su_usereditor.php?sort=acctid");
        addnav("","su_usereditor.php?sort=level");
        addnav("","su_usereditor.php?sort=laston");
        addnav("","su_usereditor.php?sort=gentimecount");
        addnav("","su_usereditor.php?sort=lastip");
        addnav("","su_usereditor.php?sort=uniqueid");
        $rn=0;
...
                
                //ADDED LOG OFF HERE
                output("[<a href='su_usereditor.php?op=edit&userid=$row[acctid]'>Edit</a>|"
...
                                "<a href='su_usereditor.php?op=logoff&userid=$row[acctid]'>Log Off</a>|",true);
                addnav("","su_usereditor.php?op=edit&userid=$row[acctid]");
                addnav("","su_usereditor.php?op=setupban&userid=$row[acctid]");
                //ADDED LOG OFF HERE
                addnav("","su_usereditor.php?op=logoff&userid=$row[acctid]");
                
                output("</td><td>",true);
                output($row['login']);
                output("</td><td>",true);
