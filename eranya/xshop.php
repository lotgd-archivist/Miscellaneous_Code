...
<?php
// Yaris' Shop für besondere Kunden
...
page_header("Yaris' Laden für besondere Kunden");
...
    output("`!Yaris`7 winkt dich mit einem Lächeln herein und sagt \"`#Willkommen! Ich gebe dir `^$tradeinvalue`# ");
...
    {
        output("`!Yaris`7 schaut dich eine Sekunde lang verwirrt an und kommt zu dem Schluss, dass du ein paar Schläge zuviel auf den Kopf bekommen hast. Schließlich geleitet er dich nach draussen.");
        addnav("Zurück zum Marktplatz","market.php");
    }
    else
    {
...
        if ($row[value]>($session[user][gold]+$tradeinvalue))
        {
            output("`!Yaris`7 schüttelt den Kopf als du auf eine Waffe deutest, die du dir beim besten Willen nicht leisten kannst. ");
            addnav("Nochmal","xshop.php?op=peruse");
            addnav("Zurück zum Marktplatz","market.php");
        }
        else
        {
            output("`!Yaris`7 nimmt dein `5".$session[user][weapon]."`7 in Zahlung. ");
...
            output("`!Yaris`7 fragt dich : \"`#Soll ich dir was darauf eingravieren?`n");
...
    output("`n`nDer Name deiner Waffe darf 20 Zeichen lang sein, Farbcodes exklusive.`nVermeide es, schwarz zu verwenden, da diese Farbe auf dunklem Hintergrund gar nicht oder nur schlecht angezeigt wird.`n`n");
    $n = $session['user']['weapon'];

    output("Deine Waffe heißt bisher : `n");
    $output.=$n;
    output("`n`n`0Wie soll deine Waffe heißen?`nVorschau: ");
    $output .= js_preview('newname');
    output("`n`n");
    $output.="<form action='xshop.php?op=namepreview' method='POST'><input name='newname' value=\"".HTMLEntities($newname)."\" size=\"30\"> <input type='submit' value='Schaut gut aus!'></form>";
    addnav("","xshop.php?op=namepreview");

    addnav('Zurück zum Laden','xshop.php?op=peruse');
...
    $_POST['newname'] = strip_tags(strip_appoencode($_POST['newname'],2));

    if (strlen(strip_appoencode($_POST['newname'],3))>20)
    {
        $msg.="Der neuer Name ist zu lang, exklusive Farbcodes darf er nicht länger als 20 Zeichen sein.`n";
    }
...
        $output.="<form action='xshop.php?op=namepreview' method='POST'><input name='newname' value=\"".HTMLEntities($regname)."\"size=\"30\"> <input type='submit' value='Vorschau'></form>";
        addnav("","xshop.php?op=namepreview");
    }
}
