...
                $str_where = 'WHERE                 1
                                                                                '.($arr_data['account_id'] > 0         ? 'AND l.actor = "'.$arr_data['account_id'].'"' : '').'
                                                                                '.($arr_data['target_id'] > 0         ? 'AND l.bookid = "'.$arr_data['target_id'].'"' : '').'
                                                                                '.(!empty($arr_data['message'])        ? 'AND l.message LIKE "%'.$arr_data['message'].'%"' : '').'
                                                                                '.(!empty($str_date_from)                ? 'AND l.date >= "'.$str_date_from.'"' : '').'
                                                                                '.(!empty($str_date_to)                        ? 'AND l.date <= "'.$str_date_to.'"' : '');

                $str_count_sql = 'SELECT COUNT(*) AS c FROM lib_funda_log l '.$str_where;

                $str_data_sql = '        SELECT                 l.*, a_actor.login AS actorname, b.title AS targetname, a_actor.acctid AS actorid
                                                        FROM                lib_funda_log l
                                                        LEFT JOIN        accounts a_actor         ON a_actor.acctid = l.actor
                                                        LEFT JOIN        lib_books b         ON b.bookid = l.bookid
                                                        '.$str_where.'
...
if($str_type == 'fundalog' && su_check(SU_RIGHT_EDITORLIBRARY)) {
    addnav('Bibliothek-Editor');
    addnav('B?Zum Bibliothek-Editor','su_library.php?op=theme&themeid=10');
}
...
                
                $str_server_name = getsetting('server_name','Samundar');
                                                                        
                // Ergebnisse zeigen
                while($l = db_fetch_assoc($res)) {
                        
                        if($arr_data['account_id'] && !empty($l['actorname']) && !$bool_useredit_actor && $str_type != 'fundalog') {
                                if(su_check(SU_RIGHT_EDITORUSER)) {
                                        addnav('Usereditor: '.$l['actorname'], 'su_usereditor.php?op=edit&userid='.$arr_data['account_id']);
                                }
...
                        }
                        if($arr_data['target_id'] && !empty($l['targetname']) && !$bool_useredit_target && $str_type != 'fundalog') {
                                if(su_check(SU_RIGHT_EDITORUSER)) {
                                        addnav('Usereditor: '.$l['targetname'], 'su_usereditor.php?op=edit&userid='.$arr_data['target_id']);
                                }
...
                        
                        if($str_type == 'syslog' || $str_type == 'debuglog' || $str_type == 'fundalog') {
                                                                                
                                $str_showfull_addon = '';
...
                                                                                                                        
                                        $str_out .= '<td>'.date('d. m. Y H:i:s',strtotime($l['date'])).': </td>';
                                        $str_out .= '<td align="center">'.(empty($l['actor']) ? '`i`h'.$str_server_name.'`i`0' : $l['actorname'].($arr_data['account_id'] != $l['actorid'] ? '`n'.create_lnk('Accountsuche',$str_lnk_accountid) : '') ).'</td>';
...
                                
                                        $str_lnk_accountid = $str_filename.'?op=search&type='.$str_type.'&account_id='.$l['actorid'].'&ret='.urlencode($_REQUEST['ret']);
                                        $str_lnk_targetid = 'su_library.php?op=edit_post&id='.$l['bookid'];

                                        $l['targetname'] = (empty($l['targetname']) && $l['bookid'] > 0 ? '`$`igelöscht`i`0' : $l['targetname']);
                                        $l['actorname'] = (empty($l['actorname']) && $l['actor'] > 0 ? '`$`igelöscht`i`0' : $l['actorname']);

                                        $str_out .= '<td valign="top">'.date('d. m. Y H:i:s',strtotime($l['date'])).': </td>';
                                        $str_out .= '<td valign="top" align="center">'.$l['actorname'].($arr_data['account_id'] != $l['actorid'] ? '`n'.create_lnk('Accountsuche',$str_lnk_accountid) : '').'</td>';
                                        $str_out .= '<td valign="top">'.$l['targetname'].($l['bookid'] > 0 ? '`n'.create_lnk('Buch anzeigen',$str_lnk_targetid) : '').'</td>';
                                        $str_out .= '<td valign="top"><a name="l'.$l['id'].'"></a>'.stripslashes(nl2br($l['message'])).$str_showfull_addon.'</td>';
                                        
                                break;
                                
                                case 'faillog':
