...
addnav('Aktionen');
addnav('Tier hinzufügen','su_mounts.php?op=add');
...
                }
                output("<tr>",true);
                output("<td>[ <a href='su_mounts.php?op=edit&id={$row['mountid']}'>Bearbeiten</a> |",true);
                addnav("","su_mounts.php?op=edit&id={$row['mountid']}");
                if ($row['mountactive']) {
                        output(" <a href='su_mounts.php?op=del&id={$row['mountid']}'>Deaktivieren</a> ]</td>",true);
                        addnav("","su_mounts.php?op=del&id={$row['mountid']}");
                }else{
                        output(" <a href='su_mounts.php?op=undel&id={$row['mountid']}'>Aktivieren</a> ]</td>",true);
                        addnav("","su_mounts.php?op=undel&id={$row['mountid']}");
                }
...
        global $output;
        output("<form action='su_mounts.php?op=save&id={$mount['mountid']}' method='POST'>",true);
        addnav("","su_mounts.php?op=save&id={$mount['mountid']}");
        $output.="<table>";
        $output.="<tr><td>Tier-Name:</td><td><input name='mount[mountname]' value=\"".htmlentities($mount['mountname'])."\"></td></tr>";
        $output.="<tr><td>Beschreibung für Keldorns Ställe:</td><td><input name='mount[mountdesc]' value=\"".htmlentities($mount['mountdesc'])."\"></td></tr>";
...
        $output.="<tr><td>Zusätzliche Waldkämpfe pro Tagesabschnitt:</td><td><input name='mount[mountforestfights]' value=\"".htmlentities((int)$mount['mountforestfights'])."\" size='5'></td></tr>";
...
        $output.="<tr><td>Nachricht am neuen Tagesabschnitt:</td><td><input name='mount[newday]' value=\"".htmlentities($mount['newday'])."\" size='40'></td></tr>";
...
        $output.="Runden am neuen Tagesabschnitt: <input name='mount[mountbuff][rounds]' value=\"".htmlentities((int)$mount['mountbuff']['rounds'])."\" size='5'><Br/>";
...
        $output.="Lifetap: <input name='mount[mountbuff][lifetap]' value=\"".htmlentities($mount['mountbuff']['lifetap'])."\" size='5'> (Faktor)<Br/>";
        $output.="Lebenskraft-Multiplikator: <input name='mount[mountbuff][liferaise]' value=\"".htmlentities($mount['mountbuff']['liferaise'])."\" size='5'> (Faktor)<Br/>";
