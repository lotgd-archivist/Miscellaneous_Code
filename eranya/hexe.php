...
    output("`!Du erzählst der Hexe davon, dass Marek dir bei deiner Freiale-Politik einen Strich durch die Rechnung macht. Sie nimmt dir deine 350 Gold ab und sagt: \"`%Jaja, der olle Marek. ");
...
        addnav($row['name']." `0entfernen","hexe.php?op=fluch2&id=".$row['id']);
    }
...
        if ($session['user']['turns']>0 && !$session['user']['prefs']['nospecialinc']) {
...
        if ($rowe['gotfreeale'] && $session['user']['gold']>350) addnav("Marek verwirren (350 Gold)","hexe.php?op=freeale");
        output("`!\"");
