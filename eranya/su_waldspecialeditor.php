...
addnav('Files aktualisieren','su_waldspecialeditor.php?op=neu');
addnav('Eigenschaften festlegen','su_waldspecialeditor.php?op=edit');
addnav('Zurück');
...
     Achte darauf, dass mind. ein Waldspecial sehr häufig und ab 0 DKs verfügbar ist!`n`n
     <form action="su_waldspecialeditor.php?op=save" method="POST">';
                addnav('','su_waldspecialeditor.php?op=save');
...
                        $ausgabe.='<tr style="background-color:'.$color[$row['prio']].'; color: #000000; text-align: center;">';
                        $ausgabe.='<td>'.($i+1).'</td>';
                        $ausgabe.="<td><a href='#' target='_blank' onclick='".popup('source.php?url=/special/'.$row['filename']).";return false;'>`~".$row['filename']."</a></td>";
...
        for ($i=0;$i<$count;$i++)
        {
                $sql='UPDATE waldspecial SET prio='.abs((int)$_POST['data'][$i]['prio']).', dk='.abs((int)$_POST['data'][$i]['dk']).', descr="'.db_real_escape_string($_POST['data'][$i]['descr']).'", anzahl='.abs((int)$_POST['data'][$i]['anzahl']).', released='.(int)$_POST['data'][$i]['released'].' WHERE row_id='.(int)$_POST['data'][$i]['row_id'] ;
                db_query($sql);
                        $check= db_error();
                if ($check!='')
                {
