...
// 20.6.2006 Tippfehler(Forum 15.4.06) beseitigt
...
        }
        
        output("`tCedrik nickt und führt dich in einen kleinen Nebenraum. Hier steht nur ein Tisch und 2 Stühle. Der Tisch ist schön hergerichtet, stilecht mit einer langen Kerze. Wunderschöne Tischdecken hängen bis fast zum Boden, edles Geschirr ist darauf plaziert. Im hinteren Teil des Raumes steht eine gemütliche Couch. Von irgendwoher dringt leise Musik an dein Ohr.`n`n");
        
        
        // Namen aus Beschreibung nehmen (um auch Zusatz-Chars zu berücksichtigen)
        $row3['name'] = substr(substr($row1['description'],0,-1),32);
        
        if ($row2['hvalue']==0)
        {
...
        else
        {
            output("`tDu freust dich enorm, als du `^".$row3['name']."`t erblickst. Ihr lächelt euch an und setzt euch an den Tisch. Ihr bekommt zur Begrüßung einen kleinen Aperitiv serviert. Ein hübsch gekleideter Knecht zündet für euch die Kerze an und nimmt eure Wünsche entgegen. Natürlich wird er auch auf ein Zeichen schnell verschwinden und euch beide allein lassen. Falls ihr noch etwas braucht oder mit dem Dinner beginnen wollt, steht ein Glöckchen zum Läuten bereit.`nEr informiert dich, dass du noch `^$essen`t Gerichte und `^$trinken`t Getränke bestellen kannst.`n");
...
    else
    {
        output("`tDu sitzt allein da, weil dein Partner gegangen ist oder die Einladung vernichtet hat. Dir bleibt nichts anderes übrig als jetzt still und leise zu verschwinden.");
        addnav("Gehen","dinner.php?op=gehen");
    }
}
...
    $food=$drink;
    
    output("`tIhr sitzt euch im Kerzenschein gegenüber und schaut euch in die Augen. In diesem Moment der Stille hört ihr auch die leise Musik recht gut. Der Aperitiv hat euch warm werden lassen und eure Wangen sind leicht gerötet. `n`n");
    if ($_GET['m1']<$_GET['m2'])
    {
        $temp = "Dinner_".$_GET['m1']."_".$_GET['m2'];
    }
    else
    {
        $temp = "Dinner_".$_GET['m2']."_".$_GET['m1'];
    }
...
            break;
        }
        addnav($drink2." trinken","dinner.php?op=austrinken&m1=".$_GET['m1']."&m2=".$_GET['m2']."&essen=$essen&trinken=$trinken&what=$drink2&nbr=$nbr");
        addnav($drink2." wegschütten","dinner.php?op=austrinken&subop=weg&m1=".$_GET['m1']."&m2=".$_GET['m2']."&essen=$essen&trinken=$trinken&what=$drink2&nbr=$nbr");
    }
    $food-=$drink;
    if ($food>1)
    {
...
            break;
        }
        addnav($food2." essen","dinner.php?op=aufessen&m1=".$_GET['m1']."&m2=".$_GET['m2']."&essen=$essen&trinken=$trinken&what=$food2&nbr=$nbr");
        addnav($food2." wegwerfen","dinner.php?op=aufessen&subop=weg&m1=".$_GET['m1']."&m2=".$_GET['m2']."&essen=$essen&trinken=$trinken&what=$food2&nbr=$nbr");
    }
    addnav("Sonstiges");
...
        $row2 = item_get(' tpl_id="dineinl" AND owner='.$m2,false);
        // Namen aus Beschreibung nehmen (um auch Zusatz-Chars zu berücksichtigen)
        $who = substr(substr($row2['description'],0,-1),32);
        $row3['name'] = $who;
        
...
        else
        {
            output("`tEr teilt dir mit, dass ".$who."`t noch ein Getränk vor sich stehen hat und doch erst austrinken sollte bevor du eine Neues bestellst.");
        }
    }
    else
    {
...
    if ($m1<$m2)
    {
        $temp = "Dinner_".$m1."_".$m2;
    }
    else
    {
        $temp = "Dinner_".$m2."_".$m1;
    }
    
    $row = item_get(' tpl_id="dineinl" AND owner='.$m1,false);
    $row2 = item_get(' tpl_id="dineinl" AND owner='.$m2,false);
    // Namen aus Beschreibung nehmen (um auch Zusatz-Chars zu berücksichtigen)
    $from = substr(substr($row['description'],0,-1),32);
    $who = substr(substr($row2['description'],0,-1),32);
    $row3['name'] = $who;
    
    if (is_array($row))
...
        
        $sql = "INSERT INTO commentary (postdate,section,author,comment) VALUES (now(),'$temp',".$session['user']['acctid'].",'/x ".$from." `tflüstert dem Knecht etwas ins Ohr und dieser serviert ".addslashes($who)." ".$drink." ')";
...
        else
        {
            output("`tEr teilt dir mit, dass ".$who."`t bereits ein Gericht serviert bekommen hat und der Teller noch auf dem Tisch steht.");
        }
    }
    else
    {
        output("`tEr teilt dir Bedauern mit, dass du kein weiteres Gericht mehr an diesem Abend bestellen kannst.`n");
    }
    addnav("Weiterplaudern","dinner.php?op=reden&m1=$m1&m2=$m2&essen=$essen&trinken=$trinken&drink=$hvalue");
}
...
    
    $row2 = item_get(' tpl_id="dineinl" AND owner='.$m1,false);
    $from = substr(substr($row2['description'],0,-1),32);
    
...
    if ($_GET['subop']=="weg")
    {
        output("`tHeimlich schüttest du $drink2 in eine Topfpflanze in der Nähe.");
    }
    else
    {
        output("`tDu nimmst $drink2 und trinkst es in einem Zug aus.");
    }
    
    if ($m1<$m2)
    {
...
        $temp = "Dinner_".$m2."_".$m1;
    }
    $sql = "INSERT INTO commentary (postdate,section,author,comment) VALUES (now(),'$temp',".$session['user']['acctid'].",'/x ".$from." `tist mit dem Getränk fertig.')";
    db_query($sql) or die(db_error(LINK));
    addnav("Weiterplaudern","dinner.php?op=reden&m1=$m1&m2=$m2&essen=$essen&trinken=$trinken&drink=$hvalue");
}
...
    if ($_GET['subop']=="weg")
    {
        output("`tHeimlich schaufelst du $food2 unter den Tisch.");
    }
    else
    {
        output("`tDu stürzt dich auf deine Portion $food2 und isst gierig auf.");
    }
    
    if ($m1<$m2)
    {
...
        $temp = "Dinner_".$m2."_".$m1;
    }
    $sql = "INSERT INTO commentary (postdate,section,author,comment) VALUES (now(),'$temp',".$session['user']['acctid'].",'/x ".$from." `tist mit dem Essen fertig.')";
    db_query($sql) or die(db_error(LINK));
    addnav("Weiterplaudern","dinner.php?op=reden&m1=$m1&m2=$m2&essen=$essen&trinken=$trinken&drink=$hvalue");
}
...
    }
    
    $row2 = item_get(' tpl_id="dineinl" AND owner='.$session['user']['acctid'],false);
    // Namen aus Beschreibung nehmen (um auch Zusatz-Chars zu berücksichtigen)
    $row3['name'] = substr(substr($row2['description'],0,-1),32);;
    
    // Klickspiel-Flirt nur, wenn angeflirteter Char Hauptchar ist:
...
                $flirtnum=0;
            }
            if (($session['user']['marriedto']==4294967295 || $session['user']['charisma']==4294967295) && ($row['marriedto']==4294967295 || $row['charisma']==4294967295))
            {
                //beide verheiratet
...
                    systemmail($row['acctid'],"`%Flirt!`0","`&".$session['user']['name']."`r hat gerade damit begonnen, dich heftig anzuflirten.");
                }
...
                    systemmail($session['user']['marriedto'],"`$Scheidung!`0","`&{$session['user']['name']}
                    `r wurde gesehen, wie ".($session['user']['sex']?"sie":"er")." mit`&{$row[name]} kichernd im Hinterzimmer der Taverne verschwand und du verlässt ".($session['user']['sex']?"sie":"ihn").".");
...
                            db_query($sql);
                            systemmail($session['user']['marriedto'],"`$Scheidung!`0","`&{$session['user']['name']}
                            `r wurde gesehen, wie ".($session['user']['sex']?"sie":"er")." mit `&{$row[name]} im kichernd ins Hinterzimmer der Taverne verschwand. Du verlässt ".($session['user']['sex']?"sie":"ihn").".");
                        }
                        $session['user']['marriedto']=$row['acctid'];
                        $session['user']['charisma']=1;
                        $session['user']['seenlover']=1;
...
        }
    } else {
        output("`tDu geniesst die Zeit mit ".$row3['name']."`t wirklich und ganz allmählich beginnst du deutliche Zeichen zu senden. Sofort entsteht ein heftiger Flirt und ein angeregtes Gespräch. Ihr lacht und fühlt euch wirklich zueinander hingezogen!`n");
        output("Ihr schaut euch an und begebt euch für eine Weile zusammen auf die Couch.`n");
        systemmail($row['acctid'],"`%Flirt!`0","`&".$session['user']['name']."`r hat gerade damit begonnen, dich heftig anzuflirten.");
        addnav("Etwas anderes tun","dinner.php?op=drin&essen=$essen&trinken=$trinken");
    }
...
        break;
    case 6:
        output("`tDu geniesst die Nähe und die Wärme deines Partners und bekommst Lust auf - mehr!`n");
        break;
    case 8:
...
        break;
    case 10:
        output("`tDu tanzt einfach göttlich und schaffst es, deinem Partner wirklich zu imponieren!`n`@Du erhältst einen Charmpunkt`n`n`t");
        $session['user']['charm']++;
        break;
        default:
        output("Ihr tanzt eine Weile recht eng und lasst euch schließlich erschöpft auf die Couch fallen, um dort ein wenig auszuruhen, bevor ihr euch wieder an den Tisch setzt.");
    }
    addnav("Etwas anderes tun","dinner.php?op=drin&essen=$essen&trinken=$trinken");
}
