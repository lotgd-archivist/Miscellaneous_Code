...
output("`c`b`&Ritterturnier`0`b`c`n`n");
//checkevent();
if ($_GET['op'] == "")
{
...
        }
    }
    output("`7Auf dieser wird der Champion des Ritterturniers angepriesen: ");
    if ($plaque <> "")
    {
        output("$plaque`7.`n");
    }
...
        output("Niemand.`n");
    }
    output("`#Auf der Anmeldetabelle sind alle Ritter aufgelistet, gegen die du antreten kannst.`n");
...
    }
    output("</ul>`n`#Ein Kampf beim Ritterturnier wird dich einen Waldkampf kosten.`n",true);
...
    else
    {
        addnav("Zurück zum Ritterturnier","battlearena.php");
    }
}
if ($_GET['op'] == "rank")
{
...
    {
        output("`# Die Heiler versorgen deine Wunden.");
    }
    if ($session['user']['hitpoints']==$session['user']['maxhitpoints'])
    {
...
            break;
        case 10:
            output("`#Die Menge kommt in Bewegung!`n Einige Zuschauer fallen auf den Kampfplatz, nur um anschließend von einer Wache wieder weggetragen zu werden.`n");
            break;
        case 11:
...
            break;
        case 15:
            output("`#Ein Fan rennt auf den Kampfplatz und im Eifer des Gefechts streifst du ihn und er fliegt von dir dort.`n");
            break;
        }
        /*
        output("`6Moderator: ");
