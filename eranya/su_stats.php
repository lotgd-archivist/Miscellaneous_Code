...
                                                ,'comments_rp_words'=>array('name'=>'RP-Komm. aktuell (Wörter)','field'=>'comments_rp_words','desc'=>'Aktuelle Anwärter auf Rollenspiel-Belohnung (Wörteranzahl):')
                                                ,'comments_rp_ges'=>array('name'=>'RP-Kommentare gesamt','field'=>'comments_rp_ges','desc'=>'Spieler mit meisten abgerechneten RP-Posts:')
                                                ,'rppoints'=>array('name'=>'RP-Punkte','field'=>'rppoints','desc'=>'Spieler mit meisten RP-Punkten:')
...
                addnav('Aktionen');
                addnav('`$Reset',$str_filename.'?op=reset',false,false,false,false);
        }
        
        addnav('Statistiken');
        
...
        addnav('Aktuelle MoTD-Umfragenanalyse', $str_filename . '?op=pollresults' );
        
}
...
                        $int_sound_on += ($arr_prefs['sounds'] ? 1 : 0);
                        $int_textarea += ($arr_prefs['textfeld'] ? 1 : 0);
                        $int_popupbio += ($arr_prefs['popupbio'] ? 1 : 0);
                        $int_savepost += ($arr_prefs['savepost'] ? 1 : 0);
                        $int_bio_noimg += ($arr_prefs['bio_noimg'] ? 1 : 0);
                        $int_editor += ($arr_prefs['editor'] ? 1 : 0);
                        $int_com_anchor += ($arr_prefs['com_anchor'] ? 1 : 0);
                        $int_nospecialinc += ($arr_prefs['nospecialinc'] ? 1 : 0);
                        
                        $int_sum++;
                                
                }
...
                $str_out .= '<tr class="trlight">
                                                <td>EMail bei neuer Hörnchenpost?</td>
                                                <td>`b'.$val.'`b</td>        
                                        </tr>';        
                
...
                                        </tr>';        
                
                $val = number_format($int_sound_on);
                $str_out .= '<tr class="trlight">
                                                <td>Sounds an?</td>
                                                <td>`b'.$val.'`b</td>        
                                        </tr>';

                $val = number_format($int_textarea);
                $str_out .= '<tr class="trlight">
                                                <td>Großes Textfeld?</td>
                                                <td>`b'.$val.'`b</td>
                                        </tr>';

                $val = number_format($int_popupbio);
                $str_out .= '<tr class="trlight">
                                                <td>Popup-Bio?</td>
                                                <td>`b'.$val.'`b</td>
                                        </tr>';
                                        
                $val = number_format($int_savepost);
                $str_out .= '<tr class="trlight">
                                                <td>Kommentar-Zwischenspeicherung?</td>
                                                <td>`b'.$val.'`b</td>
                                        </tr>';

                $val = number_format($int_bio_noimg);
                $str_out .= '<tr class="trlight">
                                                <td>Biobilder deaktiviert?</td>
                                                <td>`b'.$val.'`b</td>
                                        </tr>';

                $val = number_format($int_editor);
                $str_out .= '<tr class="trlight">
                                                <td>HTML-Editor aktiv?</td>
                                                <td>`b'.$val.'`b</td>
                                        </tr>';
                                        
                $val = number_format($int_com_anchor);
                $str_out .= '<tr class="trlight">
                                                <td>Sprung ans Ende des Chatfelds bei Aktualisierung?</td>
                                                <td>`b'.$val.'`b</td>
                                        </tr>';

                $val = number_format($int_nospecialinc);
                $str_out .= '<tr class="trlight">
                                                <td>Waldevents deaktiviert?</td>
                                                <td>`b'.$val.'`b</td>
                                        </tr>';

                // Newsletter-Abo für Rasende Reporter
                $c_gossip = db_fetch_assoc(db_query("SELECT COUNT(getgossip) AS g_anz FROM account_extra_info WHERE getgossip = 1"));
                $str_out .= '<tr class="trlight">
                                                <td>Newsletter für Rasende Reporter abonniert?</td>
                                                <td>'.$c_gossip['g_anz'].'</td>
                                        </tr>';
                // end
...
                $sql = "SELECT resultid,motdtitle,choice,account,login
                               FROM pollresults
                               LEFT JOIN accounts ON account = acctid
                               LEFT JOIN motd USING(motditem)
                                    WHERE motditem = (SELECT MAX(motditem) FROM pollresults)
                                          ORDER BY choice ASC, account ASC";
                $result = db_query($sql);
...
                    $row = db_fetch_assoc($result);
                    if($i == 0) {
                        $str_motdtitle = $row['motdtitle'];
                    }
                    if($int_option != $row['choice']) {
                        $int_option = $row['choice'];
                        $tout .= "<tr class='trhead'><td colspan='2'>Option ".($int_option+1)."</td></tr>";
                    }
                    $tout .= "<tr class='".($i%2 == 1 ? "trlight" : "trdark")."'>
                            <td>`&".$row['account']."</td>
                            <td>`&".$row['login']."</td>
                            </tr>";
                }
                output("`n`n`b`%Wer hat wie abgestimmt bei der aktuellsten MoTD-Umfrage?:`b`n
                        `7(MoTD-Titel: ".$str_motdtitle."`7)`0`n`n
                        <table>
                        <tr class='trhead'><td>AcctID</td><td>Login-Name</td></tr>".$tout."</table>");
