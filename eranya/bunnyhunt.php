...
output('`c`b`&Flauschihasenjagd`0`b`c`n');
...
                $board['caught'] = 0;
                $board['nets'] = $nets;
                $session['user']['pqtemp']=serialize($board);
                $_GET['op']='play';
        }
        else
        {
...
if ($_GET['op']=='play')
{
        $board=unserialize($session['user']['pqtemp']);
        $pick=$_GET['pick'];
        $row = intval($_GET['row']);
        $col = intval($_GET['col']);
...
        <style type="text/css">
        td.bunny{
                background-image:url(./images/bunny/grass.jpg);
                width:42px;
                height:42px;
                text-align:center;
...
        }
        td.strike{
                background-image:url(./images/bunny/marked.jpg);
                width:42px;
                height:42px;
                text-align:center;
...
                addnav('Zurück in den Pub','bunnyhunt.php?op=leave');
        }
        else
        {
...
{
        $caught=$_GET['caught'];
        output('`^Du hast alle Hasen gefangen!
        `n`nJoe übergibt dir dankbar einen `@Futtersack`^ als Belohnung.
        `nDein Ergebnis wird zu deinen bisherigen Leistungen in der Ruhmeshalle addiert!
        `n`nDu verlierst einen Waldkampf.`0');
        $session['user']['turns']--;
        $session['user']['pqtemp']='';
...
        addnav('Rangliste (Ruhmeshalle)','hof.php?op=bunny');
        addnav('Zurück in den Pub','harbor.php?op=pub');
}
...
        $boardcols = sqrt($boardsize);
        
        output('`4Du hast keine Netze mehr übrig und es nicht geschafft, alle Hasen zu erwischen!`0`n',true);
        if ($board['caught']<5)
        {
...
{
        output('`wDu erkundigst dich bei Joe, wie denn die Geschäfte so laufen. Der Mann beginnt nach ein paar
        Belanglosigkeiten frei von seinem Kummer zu berichten: "`XAye, ich habe riesige Probleme mit Hasen! Diese
        garstigen Viecher fressen mir das ganze Feld leer, auf dem ich mein Gemüse anpflanze. Hilf mir, die Hasen zu
        finden und einzufangen, dann will ich dich belohnen.`{"
