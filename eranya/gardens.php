...
        addnav('Sonstiges');
        addnav('Zurück zum Stadtgarten','gardens.php');
}
...
                if ($partner=='')
                {
                        $partner = $session['user']['sex']?'`^Silas`0':'`5Ophelía`0';
                }
...
                {
                        $str_output .= 'Du denkst nochmal über deine Ehe mit '.($session['user']['sex']?'`^Silas`0':'`5Ophelía`0').' nach und überlegst, ob du '.($session['user']['sex']?'ihn':'sie').' in der Kneipe besuchen sollst, oder für wen du diese Ehe aufs Spiel setzen würdest.`n';
                }
                if($session['user']['charme']==4294967295)
                {
                        $str_output .= 'Du überlegst dir, dass du dir mal wieder etwas Zeit für '.($session['user']['sex']?'deinen Mann':'deine Frau').' nehmen solltest. Während du '.($session['user']['sex']?'ihn':'sie').' im Stadtgarten suchst, stellst du aber fest, dass der Rest der '.($session['user']['sex']?'Männer':'Frauen').' hier auch nicht zu verachten ist.`n';
                }
...
                $str_output .= ($session['user']['sex']?"<img src=\"images/male.png\">":"<img src=\"images/female.png\">")."</td><td><b>Name</b></td><td><b>Level</b></td><td><b>Rasse</b></td><td><b>Status</b><td><b>Ops</b></td></tr>";
...
                                $str_output .= '-';
                        }
                        $str_output .= '</td><td>[ '.$str_biolink.' | <a href="gardens.php?act=flirt&name='.rawurlencode($row['login']).'">Flirten</a> ]</td></tr>';
...
                {
                        $str_output .= 'Du wartest im Stadtgarten auf `6'.$row['name'].'`0 und beobachtest '.($session['user']['sex']?'ihn':'sie').' eine Weile. Bei näherer Betrachtung stellst du aber fest, dass Elfen und Zwerge vielleicht doch niemals zusammen passen werden.';
                        $str_output .= ' So verlässt du den Stadtgarten.';
                }
                else if ($session['user']['turns']<1)
                {
                        $str_output .= 'Als `6'.$row['name'].'`0 endlich im Stadtgarten auftaucht, fühlst du dich plötzlich vom vielen Kämpfen so erledigt und geschwächt, dass du es für besser hältst, mit dem Flirten bis morgen zu warten.`nDu hast deine Runden für heute aufgebraucht. ';
                }
...
                {
                        $str_output .= 'Du näherst dich `6'.$row['name'].'`0. Je näher du '.($session['user']['sex']?'ihm':'ihr').' kommst, umso hässlicher kommt '.($session['user']['sex']?'er':'sie').' dir vor. Am Ende wirkt '.($session['user']['sex']?'er':'sie').' so abstoßend auf dich, dass du einfach vorbei zurück in die Stadt läufst.';
                }
                /*else if ((abs($row['charm']-$session['user']['charm'])>25 && $session['user']['charisma']!=4294967295) && ($session['user']['marks']<31))
                {
                        $str_output .= 'Du näherst dich `6'.$row['name'].'`0. Ihr beginnt ein Gespräch, aber irgendwie redet ihr aneinander vorbei. Ein richtiger Flirt entwickelt sich nicht. Du beschließt, es später nochmal zu versuchen und machst dich auf den Weg zurück in die Stadt.';
                }*/
...
                        {
                                $str_output .= '`%Du führst '.($session['user']['sex']?'deinen Mann':'deine Frau').' `6'.$row['name'].'`% in den Stadtgarten aus und ihr nehmt euch etwas Zeit füreinander. ';
...
                                        systemmail($row['acctid'],'`%Gartenflirt!`0','`&'.$session['user']['name'].'`6 hat mit dir einige wunderschöne Momente im Stadtgarten verlebt und dir von Neuem '.($session['user']['sex'] ? 'ihre' : 'seine').' Liebe versichert.');
...
                                db_query($sql);
                                systemmail($row['acctid'],'`%Gartenflirt!`0','`&'.$session['user']['name'].'`6 hat mit dir ein paar wunderschöne Stunden im Stadtgarten verbracht. Ihr habt beide einen Charmepunkt bekommen und haltet euer Geheimnis vor eurem Ehepartner verborgen.');
                        }
                        else
                        {
...
                                                $sql = 'UPDATE accounts SET charm=charm-1 WHERE acctid='.$row['acctid'];
                                                $session['user']['charm']-=1;
                                                systemmail($row['acctid'],'`%Gartenflirt!`0','`&'.$session['user']['name'].'`6 hat mit dir ein paar schöne Stunden im Stadtgarten verbracht. Ihr habt beide einen Charmepunkt VERLOREN, da euer schlechtes Gewissen eurem Ehepartner nicht verborgen blieb.');
                                                db_query($sql);
                                                $session['user']['seenlover']=1;
                                                break;
                                        case 3:
...
                                                $sql = 'UPDATE accounts SET charm=charm+1 WHERE acctid='.$row['acctid'];
                                                $session['user']['charm']+=1;
                                                systemmail($row['acctid'],'`%Gartenflirt!`0','`&'.$session['user']['name'].'`6 hat mit dir ein paar schöne Stunden im Stadtgarten verbracht. Ihr habt beide einen Charmepunkt bekommen und haltet euer Geheimnis vor euren Ehepartnern verborgen.');
                                                db_query($sql);
                                                $session['user']['seenlover']=1;
                                                break;
                                        case 4:
...
                                                        db_query($sql);
                                                        systemmail($session['user']['marriedto'],'`$Scheidung!`0','`6Du hast `&'.$session['user']['name'].'`6 mit `&'.$row['name'].' im Stadtgarten erwischt und verlässt '.($session['user']['sex']?'sie':'ihn').'.`nDir werden `^'.$getgold.'`6 Gold von deinem ehemaligen Ehepartner zugesprochen.');
...
                                                $session['user']['charm']-=1;
                                                addnews('`$'.$session['user']['name'].'`$ wurde beim Flirten mit '.$row['name'].' `$im Stadtgarten von '.($session['user']['sex']?'ihrem Mann':'seiner Frau').' erwischt und ist jetzt wieder solo.');
                                                break;
                                }
                        }
                }
...
                        {
                                $str_output .= '`6'.($session['user']['sex']?'Silas':'Ophelía').' springt aus einem Gebüsch und beschimpft dich aufs Heftigste, als du dich '.$row['name'].' nähern willst. '.($session['user']['sex']?'Er':'Sie').'  beobachtet deine "Gartenarbeit" schon eine ganze Weile!`0`n`n'.($session['user']['sex']?'Silas':'Ophelía').' verlässt dich.`nDu verlierst einen Charmepunkt.';
                                $session['user']['marriedto']=$row['acctid'];
                                $session['user']['charisma']-=1;
                                $session['user']['seenlover']=1;
                                $session['user']['charm']-=1;
                                addnews('`$'.$session['user']['name'].'`$ wurde beim Flirten mit '.$row['name'].' im Stadtgarten von '.($session['user']['sex']?'Silas':'Ophelía').' erwischt und ist jetzt wieder solo.');
                        }
                        else
                        {
...
                                                systemmail($row['acctid'],'`%Gartenflirt!`0','`&'.$session['user']['name'].'`6 hat mit dir ein paar schöne Stunden im Stadtgarten verbracht.');
                                                $session['user']['seenlover']=1;
...
                                                $session['user']['charm']-=1;
                                                addnews('`$'.$session['user']['name'].'`$ wurde beim Flirten im Stadtgarten von '.($session['user']['sex']?'ihrem Mann':'seiner Frau').' erwischt und ist jetzt wieder solo.');
                                                break;
                                }
                        }
                }
...
                        else
                        {
                                $str_output .= '`%Du flirtest mit '.$row['name'].'`% und ihr verbringt einige Zeit gemeinsam im Stadtgarten.';
                                $session['user']['charisma']=1;
                                $session['user']['seenlover']=1;
                        }
                        systemmail($row['acctid'],'`%Gartenflirt!`0','`&.'.$session['user']['name'].'`6 hat mit dir im Stadtgarten geflirtet.');
                        $session['user']['marriedto']=$row['acctid'];
                }
                else
                { // beide unverheiratet
...
                                                $str_output .= 'Ihr seid jetzt offiziell verlobt. Im Götterhaus könnt ihr euch ab sofort trauen lassen.`n`n';
...
                                                systemmail($row['acctid'],'`&Verlobung!`0','`& Du und `&'.$session['user']['name'].'`& habt nach zahlreichen gemeinsamen Flirts im Stadtgarten beschlossen, bald zu heiraten!`nIm Götterhaus könnt ihr euch ab sofort trauen lassen.');
...
                                        systemmail($row['acctid'],'`%Gartenflirt!`0','`&'.$session['user']['name'].'`6 hat mit dir ein paar schöne Stunden im Stadtgarten verbracht. Damit habt ihr '.$flirtnum.' gegenseitige Flirts. Ab dem 5. gemeinsamen Flirt könnt ihr heiraten!');
                                }
                                else
                                {
                                        $session['user']['charisma']+=1;
                                        $session['user']['seenlover']=1;
                                        $session['user']['charm']+=1;
                                        $str_output .= '`%Du erwiderst den Flirt von '.$row['name'].' `%und verbringst einige Zeit mit '.($session['user']['sex']?'ihm':'ihr').' im Stadtgarten.`n';
                                        systemmail($row['acctid'],'`%Gartenflirt!`0','`&'.$session['user']['name'].'`6 erwidert deinen Flirt und hat mit dir ein paar schöne Stunden im Stadtgarten verbracht.');
                                        $str_output .= '`n`n`^Du erhältst einen Charmepunkt.';
                                }
                                $session['user']['marriedto']=$row['acctid'];
                        }
...
                                systemmail($row['acctid'],'`%Gartenflirt!`0','`&'.$session['user']['name'].'`6 hat mit dir schon wieder ein paar schöne Stunden im Stadtgarten verbracht.`nWillst du nicht mal reagieren?');
                        }
                        else
                        {
                                $str_output .= '`%Du flirtest mit '.$row['name'].'`% und ihr verbringt einige Zeit gemeinsam im Stadtgarten.';
...
        output('`c`b`¥D`¢e`pr `gStadtgar`pt`¢e`¥n`b`c`n
                `¥E`¢t`pwas abgelegen vom Zentrum hat die Obrigkeit eine Oase der Ruhe in dieser betriebsamen Handelsstadt erschaffen. Eine mannshohe Steinmauer,
                verziert mit einem vierblättrigen Kleeblatt, umgibt
                die großzügige Gartenanlage, die du durch ein breites, güldenes Tor betrittst. Sofort spürst du, wie du die Anspannung und den Lärm der Stadt
                hinter dir lässt und mit jedem Schritt tiefer in die Stille dieses Ortes eintauchst. Die Blätter der Bäume rascheln leise in einer sanften Böe und
                das Gras wiegt sich hin und her, als tanze es zu einer Melodie. Zwischen üppigen Blumenbeeten schlängelt sich ein silbrig glänzender, fröhlich
                plätschernder Bach entlang, über den sich eine Brücke aus weißem Stein spannt. Etwas entfernt siehst du die Spitze eines Glockenturms zwischen
                Baumwipfeln aufragen. Hier und dort in den natürlichen Nischen der Büsche siehst du das eine oder andere Paar, das sich leise unterhält, sich die
                ewige Liebe schwört oder sich in den Armen liegt. Du spürst, dass eine besonnene Stimmung über diesem Garten liegt, und instinktiv ist dir
                bewusst, dass hier besondere Regeln herrschen, wenn du hier geduldet werden will`¢s`¥t.`n`n');
        viewcommentary('gardens','Hier flüstern',30,'flüstert');

        addnav('Liebesdinge');
        addnav('F?Flirten','gardens.php?op=flirt1');
        addnav('t?Götterhaus','tempel.php');
        addnav('G?Geschenkeladen','newgiftshop.php');

        addnav('Der Stadtgarten');
        //addnav('S?Zur Schaukel','gardens.php?op=swing');
        //addnav('a?Zum Pavillon','pavillon.php');
        addnav('O?Zur Orchideenwiese','orchids.php');
        //addnav('K?Zum Kristallsee','kristallsee.php');
        addnav('Besonderes');
        addnav('B?Blumenbeet','flowers.php');
        addnav('i?Tiefer in den Garten','treeoflife.php');
        addnav('Zurück');
