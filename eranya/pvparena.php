...
        output("`6Wen willst du zu einem Duell mit allen Fähigkeiten herausfordern? Die Anmeldegebühr kostet dich `^$cost `6Gold. Dein Gegner wird nicht unvorbereitet zustimmen.`nDu kannst heute noch `4".$session[user][playerfights]."`6 mal gegen einen anderen Krieger antreten.`n`n");
...
                  output("<tr class='".($i%2?"trlight":"trdark")."'><td>$row[name]</td><td>$row[level]</td><td>".($loggedin?"`#Online`0":"`3Offline`0")."</td><td>[ ".$str_biolink." | <a href='pvparena.php?op=challenge&name=".rawurlencode($row[login])."'>Herausfordern</a> ]</td></tr>",true);
                addnav("","pvparena.php?op=challenge&name=".rawurlencode($row[login]));

        }
        output("</table>",true);
          addnav("Zurück zum Ritterturnier","pvparena.php");
...
        output("`6Der Wachtposten lacht sich darüber kaputt, dass ein so kleiner Schwächling wie du beim Ritterturnier kämpfen will. Vielleicht solltest du wirklich erst etwas mehr Kampferfahrung sammeln.");
        addnav("Zurück zum Ritterturnier","pvparena.php");
...
                    if (db_affected_rows()<=0) redirect("pvparena.php");
...
                    output(" schmackhafter machen, indem du ".($row[sex]?"ihr":"ihm")." die Anmeldegebühr von  `^".($row[level]*20)."`6 Gold überweist.`n");
...
                     systemmail($row[acctid],"`2Du wurdest herausgefordert!","`2".$session[user][name]."`2 (Level ".$session[user][level].") hat dich zu einem Duell beim Ritterturnier herausgefordert. Du kannst diese Herausforderung annehmen oder ablehnen, solange sich dein Level nicht ändert.`nBereite dich gut vor und wage den Kampf erst dann!");
...
              addnav("Zurück zum Ritterturnier","pvparena.php");
        }else{
            output("`4Du hast nicht genug Gold dabei, um die Anmeldegebühr zu bezahlen. Mit rotem Kopf ziehst du ab.");
            addnav("Zurück zum Ritterturnier","pvparena.php");
        }
    }
      addnav("Zurück zum Dorf","village.php");
...
    //$session[user][reputation]--;
    output("`6Beim Anblick deines Gegners $row[name] `6wird dir Angst und Bang. Mit ein paar sehr dürftigen Ausreden wie \"nicht genug Gold\" lehnst du die Herausforderung ab und verlässt schnell das Ritterturnier.");
...
    if($session[user][gold]<$cost){
        output("`4Du kannst dir die Anmeldegebühr von `^$cost`4 Gold nicht leisten.");
          addnav("Zurück zum Dorf","village.php");
    }else if($session[user][playerfights]<=0){
...
      $result = db_query($sql) or die(db_error(LINK));
    $row = db_fetch_assoc($result);
    output("`6Du bist es leid, auf $row[name]`6 zu warten und ziehst deine Herausforderung zurück. Die Anmeldegebühr bekommst du aber trotz langer Verhandlungen nicht zurück.`n");
...
            output("`n`&Kurz vor deinem finalen Todesstoß beendet der Wärter euren Kampf und erklärt dich zum Sieger!`0`n");
            output("`b`\$Du hast $badguy[name] `\$besiegt!`0`b`n");
            output("`#Du bekommst das Preisgeld in Höhe von `^$win`# Gold und deinen gerechten Lohn an Turnierpunkten!`n");
...
            db_query($sql);
            $mailmessage = "`^$goodguy[name]`2 hat dich mit %p `^".$goodguy['weapon']."`2 beim Ritterturnier besiegt!"
...
             systemmail($badguy['acctid'],"`2Du wurdest beim Ritterturnier besiegt",$mailmessage);
            addnews("`\$$goodguy[name]`6 besiegt `\$$badguy[name]`6 beim `8Ritterturnier`6!");
...
            systemmail($badguy[acctid],"`2 du warst in beim Ritterturnier erfolgreich! ","`^".$session[user][name]."`2 hat beim Ritterturnier verloren!`n`nDafür hast du `^$exp`2 Erfahrungspunkte und `^$win`2 Gold erhalten!");
...
            addnews("`\$$badguy[name]`6 besiegt `\$$goodguy[name]`6 beim `8Ritterturnier`6!");
            $sql = "DELETE FROM pvp WHERE acctid1=".$session[user][acctid]." OR acctid2=".$session[user][acctid];
            db_query($sql) or die(db_error(LINK));
        }
    }else{
        output("`6Euer Kampf wurde vorzeitig beendet. Die Anmeldegebühr kommt dem Ausbau des Ritterturniers zugute.");
    }
      addnav("Zurück zum Dorf","village.php");
}else if ($_GET[op]=="del"){
    $sql="DELETE FROM pvp WHERE acctid1=$_GET[kid1] AND acctid2=$_GET[kid2]";
    db_query($sql);
    output("Der Wärter beendet einen langweiligen Kampf.");
    systemmail($_GET[kid1],"`2Dein Turnierkampf wurde beendet!","`@Dein Kampf beim Ritterturnier wurde vom Kampfrichter beendet.");
    systemmail($_GET[kid2],"`2Dein Turnierkampf wurde beendet!","`@Dein Kampf beim Ritterturnier wurde vom Kampfrichter beendet.");

    addnav("Zum Ritterturnier","pvparena.php");
    addnav("Zurück zum Dorf","village.php");
}else if ($_GET[op]==""){
    $sql="SELECT * FROM pvp WHERE acctid1=".$session[user][acctid]." OR acctid2=".$session[user][acctid];
      $result = db_query($sql) or die(db_error(LINK));
...
         $text=1;
        output("`6Da du noch eine Herausforderung mit `&$row[name2] `6offen hast, machst du dich auf in Richtung Ritterturnier, um nach deinem Gegner Ausschau zu halten. Doch der scheint nirgendwo in Sicht zu sein.`n");
        addnav("Herausforderung zurücknehmen","pvparena.php?op=back&id=$row[acctid2]");
        addnav("Ritter herausfordern","battlearena.php"); // ONE arena for TWO things - if installed ;)
         addnav("Zurück zum Dorf","village.php");
        stats($row);
...
        $text=1;
        addnav("Ritter herausfordern","battlearena.php");
         addnav("Zurück zum Dorf","village.php");
    }else if($row[acctid2]==$session[user][acctid] && $row[turn]==0){
        output("`6Du wurdest von `&$row[name1] `6herausgefordert. Wenn du die Herausforderung annimmst, musst du `^$cost`6 Gold Anmeldegebühr bezahlen.`n");
...
        $text=1;
        if (@file_exists("battlearena.php")) addnav("Ritter herausfordern","battlearena.php");
         addnav("Zurück zum Dorf","village.php");
...
            output("\"Stop!\" der Kampfrichter versperrt dir mit seiner Lanze den Weg zur Anmeldung und macht nicht den Eindruck, als wollte er dich vorbeilassen \"Geh erstmal die elende Drachenbrut ausrotten, bevor du hier deine Zeit mit Kämpfen verschwendest!\"`n");
            addnav("Zurück zum Dorf","village.php");
            page_footer();
        }
        else {
...
        checkday();
          addnav("Aktualisieren","pvparena.php");
        output("`6Du betrittst das Ritterturnier in der Nähe des Trainingslagers. Hier kämpfen einge der Krieger gegeneinander und üben sich in ihren besonderen Fertigkeiten, ");
...
          $result = db_query($sql) or die(db_error(LINK));
        if (db_num_rows($result)){
            output(" Du beobachtest das bunte Treiben auf dem Platz eine Weile.`nFolgende Krieger kämpfen gerade gegeneinander:`n`n<table border='0' cellpadding='3' cellspacing='0'><tr><td align='center'>`bHerausforderer`b</td><td align='center'>`bVerteidiger`b</td><td align='center'>`bStand (LP)`b</td>".(su_check(SU_RIGHT_EDITORUSER)?"<td>Ops</td>":"")."</tr>",true);
            for ($i=0;$i<db_num_rows($result);$i++){
                $row = db_fetch_assoc($result);
                output("<tr class='".($i%2?"trlight":"trdark")."'><td>$row[name1]</td><td>$row[name2]</td><td>$row[hp1] : $row[hp2]</td>".(su_check(SU_RIGHT_EDITORUSER)?"<td><a href='pvparena.php?op=del&kid1=$row[acctid1]&kid2=$row[acctid2]'>Löschen</a></td>":"")."</tr>",true);
                if (su_check(SU_RIGHT_EDITORUSER)) addnav("","pvparena.php?op=del&kid1=$row[acctid1]&kid2=$row[acctid2]");
            }
