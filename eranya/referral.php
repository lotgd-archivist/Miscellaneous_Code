...
        if($session['user']['acctid'] == getsetting('demouser_acctid',5))
        {
                output('`$Als Demo-User kannst du keine anderen Spieler werben.`n');
        }
        else
        {
...
                                `0Dadurch wird die Seite wissen, dass du derjenige warst, der ihn hergeschickt hat.
                                Wenn er dann zum ersten Mal den oben angeführten Status erreicht, bekommst du deine Punkte!');
...
        output('`@
        Legend of the Green Dragon ist ein Remake des klassischen BBS Spiels `$Legend of the Red Dragon`@. Es ist ein Multiplayer-Browserspiel, das heißt,
...
        Hier schlüpfst du in die Rolle eines Charakters in einer Fantasy-Welt, in der eine Legende von einem riesigen grünen Drachen die
        Bewohner in Angst und Schrecken versetzt. Nunja, zumindest die meisten. Oder wenigstens ein paar.`n`n
        ');
        output("`2<li>Kämpfe gegen unzählige böse Kreaturen, die die Stadt bedrohen
        <li>Erforsche die Stadt und den Wald und unterhalte dich mit anderen Charakteren
        <li>Entwickle deinen eigenen Charakter und lass ihn neue Bekanntschaften machen
        <li>Finde und vernichte den grünen Drachen, um im Ansehen zu steigen und dir interessante Extras zu erspielen
...
        addnav('Navigation');
        addnav('Charakter erstellen','create_rules.php'.((!empty($arr_user) && $arr_user['acctid'] != getsetting('demouser_acctid',5)) ? '?r='.$arr_user['acctid'] : ''));
