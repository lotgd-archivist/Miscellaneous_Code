...
define('LOCNAME','`4Nachtbar `$"Al Anochecer"`0'); //Name der Bar
...
                                        $session['bufflist']['headache'] = array('name'=>'`$Kopfschmerzen`0','rounds'=>30,'wearoff'=>'`&Deine Kopfschmerzen verschwinden`0','atkmod'=>0.2,'defmod'=>0.2,'roundmsg'=>'Du hast fürchterliche Kopfschmerzen','activate'=>'defense');
                                }
                                else
                                {
...
                                else
                                {
                                        output('`Q'.BARTENDER.' `mschaut in deinen Beutel und lacht laut los `4"Diesen Plunder bietest du hier an? Dafür gibt dir bestenfalls Riyad was!"');
                                }
                        break;
                        }
...
                                        $min_chance=1;
                                        if($session['user']['race']==RACE_VAMPIR || $session['user']['race']==RACE_WERWOLF || $session['user']['race']==RACE_DAEMON) $min_chance+=2;
...
                                        if($session['user']['race']==RACE_MENSCH) $min_chance--; //der Mensch schmeckt am süßesten
...
                                                        break;
                                                case 2: //Schleimtod
                                                        output('`mDu beginnst die Suppe auszulöffeln, die du dir eingebrockt hast, als du eine Veränderung an dir bemerkst. Deine Haut wird ebenso grün und schleimig wie das Zeug in der Schüssel. `n`ÎWenig später kippst du um und findest dich bei `²J`ça`Âr`Îc`Âa`çt`²h `Îwieder.');
                                                        addnews('`6'.$session['user']['name'].'`Î hat sich eingeschleimt und durfte `²J`ça`Âr`Îc`Âa`çt`²h `Îbesuchen.');
                                                        killplayer();
                                                        break;
                                                case 3: //AP Verlust
...
                {
                        output('`mDu tastest in deinem Beutel nach einem Insigniensplitter, während du Veri musterst. Den Splitter hast du mit viel Mühe gesammelt.`n
                                `^Möchtest du ihn wirklich verkaufen?`n`n
                                <a href="tittytwister.php?op=regalia&act=selldo" class="button">Splitter verkaufen</a>');
                        addnav('','tittytwister.php?op=regalia&act=selldo');
                        addnav('Ja, Splitter verkaufen','tittytwister.php?op=regalia&act=sell');
                        addnav('Nein, zurück!','tittytwister.php?op=regalia');
                        $bool_shownav = false;
                }
                elseif($_GET['act']=='selldo')
                {
...
                                {
                                        insertcommentary(1,'/x `7Es wurde beobachtet wie '.$session['user']['name'].'`7 einen Insigniensplitter verhökert hat!','guild-'.$session['user']['guildid']);
                                }
                        }
                        else
                        {
...
                                        {
                                                output('`mDir wird nun auch klar, warum deine Gilde nicht im Ansehen des Herzogs steigt. Offenbar hat der Gnom mit seiner Räuberbande die Ritter des Herzogs überfallen und eure mühsam produzierten Insignien geraubt. Und nun bietet er dir die Splitter zum Verkauf an...');
...
        case 'vampire':
        { //der alte Vampir
                if($session['user']['marks']<32) //mit Blutgott würde es hier immer 1000 Gold geben
                {
                        output('`c`b`ZEin dunkler Gang`b`c
                        `n`TDu stehst vor einem langen, unbefestigten Gang, der scheinbar in einen entlegenen, wesentlich gefährlicheren Teil des Waldes führt. Ein dunkler Nebel zieht herauf, sodass du kaum die Hand vor Augen sehen kannst und du spürst, wie sich ein kaltes Grausen in die Luft erhebt. Noch kannst du gefahrlos umkehren und dich in den warmen Schankraum der Nachtbar setzen. Wagst du es, weiterzugehen?');
                        addnav('Gehe tapfer weiter','paths.php?ziel=vampire');
                }
                else
                {
                        output('`c`b`ZDer alte Vampir`b`c
...
                        addnav($msgdays.' Tagesabschnitte (`^'.$msgprice.'`0 Gold)','tittytwister.php?op=board&act=add1&amt=1');
                        addnav(($msgdays*3).' Tagesabschnitte (`^'.($msgprice*3).'`0 Gold)','tittytwister.php?op=board&act=add1&amt=3');
                        addnav(($msgdays*10).' Tagesabschnitte (`^'.($msgprice*10).'`0 Gold)','tittytwister.php?op=board&act=add1&amt=10');
...
                board_view('tittytwist',su_check(SU_RIGHT_COMMENT)?2:1,
...
                {
                        output('`mZaghaft öffnest du die Tür und erblickst im schummrigen Licht die ü-bels-ten Gestalten. Du bemerkst, dass dieser Ort nichts für dich ist und drehst schnell wieder um.
                        `n`nVielleicht willst du ja wiederkommen wenn du etwas größer bist?');
                }
                elseif ($time < OPENTIME && $time > CLOSETIME && !su_check(SU_RIGHT_DEBUG)) //nur nachts geöffnet
                {
                          output('`c`b'.LOCNAME.'`b`c
                        `n`(Dieses Lokal hat geöffnet vom Abendrot bis zum Morgengrauen.
                        `n`mSo steht es groß über der Tür. Tatsächlich ist hier gerade überhaupt nichts los, nur `Q'.BARTENDER.' `mräumt die Trümmer der letzten Nacht weg und ein paar Besoffene liegen noch in den Ecken.
                        `nEinige Stammgäste unterhalten sich vor der Tür:`n`n');
...
                        else
                        {
                                output('`mJemand, der `²J`ça`Âr`Îc`Âa`çt`²h`m verdammt ähnlich sieht, rempelt dich an und
...
                `nDer Schenkenbesitzer `Q'.BARTENDER.'`m '.(e_rand(1,3)==1?'entleert währenddessen eine weitere Flasche seines selbstgebrannten Schnapses in die Gläser.':'scheint davon aber nur gelangweilt zu sein und schenkt niemanden sonderlich viel Aufmerksamkeit.').'`n`n');
...
                }
                else if ($session['user']['superuser']>0)
                {
...
        }
        addnav('v?Bar verlassen','alley.php');
        if(!$_GET['op'])
        {
