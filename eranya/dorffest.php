...
addcommentary();
checkday();
//output('`c`b`&Das Stadtfest`0`b`c`n`n');
if (!isset($session))
{
        exit();
}
...
                killplayer(50,5,0,'');
                addnews($session['user']['name'].' hat '.($session['user']['sex']?'ihren':'seinen').' zarten Elfenkörper auf dem Stadtfest mit zuviel Ale zugrunde gerichtet.');
...
                                output('Du kannst morgen wieder spielen.');
                                killplayer(50,5,0,'');
                                addnews($session['user']['name'].' starb an einer Überdosis Ale auf dem Stadtfest');
                                addnav('Tägliche News','news.php');
                                page_footer();
                                
                }
...
/*// redirect wichtig wegen Postlöschung
if(getsetting ('lastparty',0)<=time())
{
    redirect('dorffest.php?op=nofestival');
}*/
...
    if(getsetting ('lastparty',0)>time())
    {
        output('`c`b`&Das Stadtfest`b`c`n
        `2Du trittst auf die große, bunt geschmückte Festwiese, die auf allen Seiten von Bäumen gesäumt wird.
...
        `n`2 Geradezu spielt Silas mit einer kleinen Band ein paar stimmige Lieder. Es wird ausgelassen getanzt und es liesse sich
...
        leckeres Ale trinken. Irgendwie klasse, dass der Stadtrat alle alkoholischen Getränke auf Kosten der Steuern übernimmt!');
        if($session['user']['marriedto']=='4294967295')
        {
                $str_mate = ($session['user']['sex']==0)?'Silas':'Ophelía';
...
{
        addnav('Losstand');
        output('`c`b`&Das Stadtfest`0`b`c`n');
...
        $query_result = db_query("Select name, login, sex from accounts where acctid = $loverID LIMIT 1");
        $arr_mate = db_fetch_assoc($query_result);
        output("`2 Als du inmitten der tanzenden Menge $arr_mate[login] `2erspähst, macht dein Herz einen Sprung! Voller Freude
...
        befindet euch ein Stück tief im friedlichen Wald direkt am Stadtrand. Leicht könnt ihr noch den Klängen der Musik lauschen,
...
        output('`i`7(Info: Dieser RP-Ort ist nur während des Stadtfestes erreichbar.)`i`n`n');
}

else if ($_GET['op']=='dance')
{
        output('`c`b`&Das Stadtfest`0`b`c`n
        `2Du trittst auf die rappelvolle Tanzfläche und willst dem anderen Geschlecht mal so richtig zeigen was Sache ist!`n
...
                        output('`2 Schon kommt Marek mit einem riesigen Tablett auf dich zu und ehe du dich versiehst, hast du wieder etwas zu trinken in der Hand!');
...
                        
                default:
                        output("`nDu lässt dir das Essen munden. Und dann auch noch so preiswert... So ein Stadtfest ist schon etwas feines.");
        }
...
else if($_GET['op'] == 'nofestival')
{
        if (strlen(getsetting('festwiese_text',0)) >= 15)
        {
                output('`2'.getsetting('festwiese_text',0).'`n`n');
        }
        else
        {
                output('`c`b`&Die Festwiese`b`c`n
                        `2Friedlich liegt sie vor dir, die großflächige Festwiese. An manchen Tagen dient sie den Einwohnern
                        der Stadt als zentraler Ort für ihre Festlichkeiten, doch heute findet keine der Feiern statt, weshalb die
                        Wiese auch großteils verlassen daliegt - von ein paar einzelnen Grüppchen abgesehen, die im Gras
                        zusammensitzen und plaudern.`n`n');
        }
        viewcommentary('party_main','Hinzufügen',15,'sagt');
        if ($session['user']['superuser'] > 0)
        {
                addnav('Admin');
                addnav('Text anpassen','dorffest.php?op=fw_changetext');
                if (strlen(getsetting('festwiese_text',0)) >= 15) {addnav('Text auf Standard zurücksetzen','dorffest.php?op=fw_changetext&act=clear');}
        }
        addnav('Zurück');
        addnav('Zurück in die Stadt','village.php');
}
else if ($_GET['op'] == 'fw_changetext')
{
        if ($_GET['act'] == 'save')
        {
                if (strlen(trim($_POST['fw_text'])) >= 15)
                {
                        savesetting('festwiese_text',$_POST['fw_text']);
                        redirect('dorffest.php?op=nofestival');
                }
                else
                {
                        output('`4Fehler! Eingegebener Text muss mindestens 15 Zeichen lang sein.');
                        addnav('Nochmal','dorffest.php?op=fw_changetext');
                }
        }
        elseif ($_GET['act'] == 'clear')
        {
                savesetting('festwiese_text','0');
                redirect('dorffest.php?op=nofestival');
        }
        else
        {
                // Vorschau
                rawoutput(js_preview('fw_text').'<br /><br />');
                $form = array('fw_text'=>'`2Text eingeben:`n,textarea,45,8');
                $data = array('fw_text'=>getsetting('festwiese_text',0));
                output("<form action='dorffest.php?op=fw_changetext&act=save' method='POST'>",true);
                showform($form,$data,false,'Speichern');
                output("</form>",true);
                addnav('','dorffest.php?op=fw_changetext&act=save');
                addnav('Zurück');
                addnav('Zurück','dorffest.php?op=nofestival');
        }
}
