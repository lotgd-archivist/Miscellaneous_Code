...
        }
        redirect($str_filename.'?type='.$_GET['type'].'&ret='.urlencode($str_ret));
        
    break;
    
    // Export-Maske
...
        }
        else{
            $str_out .= '<tr><td colspan="5">`i'.$arr_res['count'].' Einträge gesamt vorhanden, '.$int_found.' angezeigt!`i</td></tr>';
                        
            $i_end = ord('Z');
            
...
                            ['.create_lnk('Del',$str_filename.'?op=del_bl&type='.$int_type.'&id='.$b['id'].'&ret='.urlencode($str_ret),true,false,'Diesen Blacklisteintrag wirklich aufheben?').']
                        </td>';
