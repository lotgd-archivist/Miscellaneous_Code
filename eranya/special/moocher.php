...
Für deinen Mut wird dich jedoch Jarcath belohnen.`0");
...
    $sql = "SELECT name,level,alive,race,location FROM accounts WHERE locked=0 AND location=0 AND alive=1 AND race='".$session['user']['race']."' AND loggedin=0 AND laston>'".date("Y-m-d H:i:s",strtotime(date("r")."-".getsetting("LOGINTIMEOUT",900)." seconds"))."' ORDER BY level DESC";
