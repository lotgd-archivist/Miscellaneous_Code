...
        output("`t\"Wem willst du dieses Paket schicken?\" `9`0");
...
                        $sql = "SELECT login,name,level,sex,acctid FROM accounts WHERE $search locked=0 AND acctid<>".$session['user']['acctid']." ORDER BY (acctid='".$session['user']['marriedto']."') DESC,login,level LIMIT $limit";
                        $result = db_query($sql);
...
                                                <input type="text" name="search"> <input type="submit" value="neue Suche"> 
                                        </form>');
                }
...
            $gefallen = 15;
            $effekt = "Du erhältst $gefallen Gefallen bei Jarcath.";
...
    output("`n`t\"".($session['user']['sex']?"Gute Frau":"Guter Mann").", was haltet Ihr davon, einem lieben, hübschen Menschen eine kleine
...
    Knochenstücke, die Jarcath faszinierten und demjenigen einige Gefallen bei ihm einbrachten. Andererseits ist
    da auch noch ein Amulett.`nDoch es ist natürlich nicht kostenlos.`n");
    $cost = $lvlcost * $session['user']['level'];
    output("Wenn dir `^".$cost." Goldstücke `tnicht zu viel sind, werd ich für dich das Päckchen wählen und ausliefern. Aber nur an hübsche Leute!\"`n");
    output("`n`@Was machst du? (Bedenke: Die Person muss mindestens zwei Charmepunkte gesammelt haben, um ein Geschenk erhalten zu können.)`n`0");
