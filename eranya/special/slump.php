...
$sql="SELECT acctid,name,goldinbank,sex FROM accounts WHERE name like '".addslashes($newplayer)."' LIMIT 1";
$result = db_query($sql) or die(db_error(LINK));
$rownew = db_fetch_assoc($result);

$sql="SELECT acctid,name,goldinbank,sex FROM accounts WHERE name like '".addslashes($newdk)."' LIMIT 1";
$result = db_query($sql) or die(db_error(LINK));
...
    output("`3Du denkst: \"`#".($rowdk['sex'] ? 'Die' : 'Der')." hat doch gerade den `@grünen Drachen `#erfolgreich bekämpft");
    output(" - wie war der Name noch gleich?`n");
    output("Ah,`0 $rowdk[name] `#muss es gewesen sein.`3\"`n`n`0");
    output("`6Jetzt ist es zu spät, so flink ".($rowdk['sex'] ? 'sie' : 'er')." beim Aufheben war, so flink ist ".($rowdk['sex'] ? 'sie' : 'er')." auch verschwunden.`0`n`n");
    //goldverteilung
...
    output("`6Schnell willst du das Gold wieder einsammeln, doch bei ein paar Goldstücken ist jemand schneller.`n`n");
    output("`3Du denkst: \"`#".($rowdk['sex'] ? 'Die' : 'Der')." ist doch gerade neu in ".getsetting('townname','Atrahor'));
    output(" - wie war der Name noch gleich?`n");
...
{
    output("`6Schnell willst du das Gold wieder einsammeln, doch bei ein paar Goldstücken ist jemand schneller.`n`n");
    output("Den einen Dieb hast du doch gerade erst neu hier in ".getsetting('townname','Atrahor')." gesehen, ");
    output("und der andere hatte eben den `@grünen Drachen `6erfolgreich bekämpft");
    output("- wie waren ihre Namen noch gleich?`n`n`0");
    output($rownew['name']."`# und `0".$rowdk['name']."`# müssen es gewesen sein.`3\"`6, denkst du. `n`n");
    //name des juengsten spielers und drachentoeters
