...
            output("".$c." Du gibst `&Síochara".$c." einen Edelstein und sie überreicht dir einen Zettel aus Pergament mit Anweisungen, wie du deine Fertigkeiten steigern kannst.`n`n");
            output("Du studierst den Zettel, zerreisst ihn und futterst ihn auf, damit Ungläubige nicht an die Information gelangen können. `n`n`&Síochara".$c." seufzt... \"`&Du hättest ihn nicht ");
            output("zu essen brauchen... Naja, jetzt verschwinde von hier!".$c."\"`#");
            increment_specialty();
            $session['user']['gems']--;
            //debuglog("gave 1 gem to Foilwench");
        }else{
            output("".$c." Du überreichst deinen imaginären Edelstein. `&Síochara".$c." starrt dich verdutzt an. \"`&Komm wieder, wenn du einen `bechten`b Edelstein hast, du Dummkopf.".$c."\"`n`n");
            output("\"`#Dummkopf?".$c."\"`n`n");
            output("Damit schmeisst `&Síochara".$c." dich endgültig raus.");
        }    
        $session['user']['specialinc']="";
        //addnav("Zurück zum Wald", "forest.php");
    }else if($_GET['op']=="dont"){
        output("".$c." Du informierst `&Síochara".$c." darüber, dass sie sich ihren Reichtum selbst verdienen sollte. Dann stampfst du davon.");
        $session['user']['specialinc']="";
...
        output("\"`&Sei gegrüsst, ".$session['user']['name'].", ich bin `&Síochara".$c.", Meister in allem.".$c."\"`n`n\"`#Meister in allem?".$c."\" fragst du nach.`n`n");
