...
if( empty($session['comsendmail_section']) ) {
...
$str_section = $session['comsendmail_section'];
...
// Zeit warten
$str_min_interval = time() - $int_lasttime;
if($str_min_interval < COMMENTMAIL_MIN_INTERVAL && $session['user']['superuser'] == 0) {
    
    output('`n`$`bDu mußt noch ca. '.($str_min_interval/60).' warten, ehe du erneut Kommentare von diesem RP-Ort versenden darfst.`b`0`n');
    popup_footer();
    exit;
    
}
...
                        
$str_out .= '<script language="javascript">window.resizeTo(800,600);</script>
        `&`n`n';
...
$str_where = ' WHERE         section="'.addslashes($str_section).'" AND self=1';
...
    {
        $str_where .= ' AND commentid NOT IN ('.db_real_escape_string($_POST['exclude']).') ';
    }
}
                
...
    
    $str_out .= '<div style="height:300px;overflow: auto;padding: 2px;text-align: justify;padding-right:20px;">';
                                    
...
        $c['comment'] = closetags($c['comment'],'`c`i`b');
        
        $str_out .= $c['comment']
...
        }
        
        $str_comments .= '<div align="justify">'.$c['comment'].'</div>';
        
    }
                                    
...
    
    $str_subject_add = substr(stripslashes($_POST['subject']),0,80);
    
    $str_css = '* {
                    padding:0px;
                    margin:0px;
...
                `&Zeitraum: '.date('d.m.Y H:i',$int_start_time).' bis '.date('d.m.Y H:i',$int_end_time).'<br /><br />
                <hr /><br />
            ';

    $body .= $str_comments;
    
    $body = appoencode($body,true,true);
    
    $body .= '</body></html>';
...
    $mailbody .= "Rollenspielarchivierung ".(!empty($str_subject_add) ? ' ('.$str_subject_add.')' : '')." vom ".date('d. m. Y',time())."\n";
    
...
    $arr_file['type']         = 'Content-type: text/html; charset=ISO-8859-15';
    
        
    send_mail($session['user']['emailaddress'],'Rollenspiel aus '.$str_townname.(!empty($str_subject_add) ? ' ('.$str_subject_add.')' : ''),$mailbody,$headers,null,$maili,null,null,array($arr_file));
    //mail($session['user']['emailaddress'],'Rollenspiel aus '.$str_townname.(!empty($str_subject_add) ? ' ('.$str_subject_add.')' : ''),$mailbody,$headers);
    
    //debuglog('versandte Kommentare an EMail (RP-Ort: '.$str_section.')');
    
...
            <a href="javascript:void(0);" onclick="window.close();" class="button">Fenster schließen</a>`n`n');
    popup_footer(false);
    exit();
}
// END Senden
        
// Status
$str_selected = '<div align="justify">`&Beginn: '.(!empty($str_start_comment) ? $str_start_comment.'`n[ <a href="'.$str_filename.'?from=0">Verwerfen</a> ]' : '`iNoch nicht gewählt`i').'`n
...
                    dass du maximal '.$int_comments_max.' Kommentare auf einmal versenden kannst und mindestens '.(COMMENTMAIL_MIN_INTERVAL/60).' Minuten zwischen zwei Sendungen liegen müssen!`n`n'
                .$str_selected.'</div>'                            
                .$str_out;
        
}
...
                    `c<form method="POST" action="'.$str_filename.'?send=1" onsubmit="var e = document.getElementsByName(\'ex[]\');for(i=0;i<e.length;i++){document.getElementById(\'exclude\').value+=(e[i].checked?\',\'+e[i].value:\'\');}">
