...
// Navigation
addnav("Module");
addnav("Module anzeigen",($_GET['op']==""?"":"su_spec.php"));
addnav("Module installieren",($_GET['op']=="install"?"":"su_spec.php?op=install"));
...
                        output(($row['active']==1?"`&[`^<a href='su_spec.php?op=deactivate&filename=".$row['filename']."'>Deaktivieren</a>`&]":"`&[<a href='su_spec.php?op=activate&filename=".$row['filename']."'>`@Aktivieren</a>`&]")." | [`^<a href='su_spec.php?op=uninstall&filename=".$row['filename']."' onClick=\"return confirm('".$info['specname']." wirklich deinstallieren? Wenn du ".$info['specname']." deinstallierst, müssen alle Benutzer mit dieser Specialty ihre Specialty neu wählen.')\";>Deinstallieren</a>`&]",true);
                        rawoutput("</td></tr>");
                        rawoutput("<tr bgcolor=''><td colspan='4'></td></tr>");
                        output('`c');
                        addnav('','su_spec.php?op=activate&filename='.$row['filename']);
                        addnav('','su_spec.php?op=deactivate&filename='.$row['filename']);
                        addnav('','su_spec.php?op=uninstall&filename='.$row['filename']);
                }
                rawoutput("</table>");
        }
...
                output("`2Kategorie: `^".$info['category']);
                rawoutput("</td><td colspan='2'>");
                output("`&[`^<a href='su_spec.php?op=install&filename=".$info['filename']."'>Installieren</a>`&]",true);
                rawoutput("</td></tr>");
                rawoutput("<tr bgcolor=''><td colspan='4'></td></tr>");
                addnav('','su_spec.php?op=install&filename='.$info['filename']);
