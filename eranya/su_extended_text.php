...
        else
        {
                addnav('Entfernen (komplett)',$str_filename.'?op=uninstall',false,false,false,false,true,'Texte wirklich entfernen?');
        }
}
...
                        $output .= '</select>&nbsp;
                                                 und / oder Suche nach ID: <input type="text" maxlength="50" size="20" name="search" value="'.$str_search.'" />
                                                 &nbsp;<input type="button" onClick="parent.location=document.getElementById(\'category\').options[document.getElementById(\'category\').selectedIndex].value" value="Go!">
                        </form>';
                        //End writing a list of categories to the top of the screen

                        if($str_category != '')
                        {
                                $str_sql_get_texts = 'SELECT id,SUBSTRING(text,1,100) as text,author,category,tags,subcategory FROM extended_text WHERE category="'.$str_category.'" ORDER BY id ASC';
                        }
                        elseif(!empty($str_search)) {
                                $str_sql_get_texts = 'SELECT id,SUBSTRING(text,1,100) as text,author,category,tags,subcategory FROM extended_text WHERE id LIKE "%'.addslashes($str_search).'%" ORDER BY id ASC';
                        }
                        else
                        {
                                $str_sql_get_texts = 'SELECT id,SUBSTRING(text,1,100) as text,author,category,tags,subcategory FROM extended_text ORDER BY id ASC';
                        }
...
                                $str_text = appoencode(closetags(htmlentities(stripslashes($arr_result['text'])),'`c`b`i')).'...<br />';
...
                                $output .= '<p><div class="trhead "><b>'.$str_id.'</b>&nbsp;[<a href=# onClick="document.getElementById(\'su_ext_'.$str_id.'\').style.display=\'block\';">+</a>]/[<a href=# onClick="document.getElementById(\'su_ext_'.$str_id.'\').style.display=\'none\';">-</a>]
                                            <br />
                                            <i>'.$str_category.': '.$str_author.'</i>
                                            </div><div id="su_ext_'.$str_id.'" style="display:none;" class="trlight">';
                                output($str_text);
...
                        else
                        {
                                $arr_values = db_fetch_assoc(db_query('SELECT * FROM extended_text WHERE id="'.addslashes($str_id).'"'));
                                $arr_values['text'] = stripslashes($arr_values['text']);
                        }
...
                        $str_author = addslashes($str_author);

                        $str_id = preg_replace('/[^\w_-]/','',$str_id);
                        $str_category = preg_replace('/[^\w_-]/','',$str_category);
                        $str_new_category = preg_replace('/[^\w_-]/','',$str_new_category);
                        $str_subcategory = preg_replace('/[^\w_-]/','',$str_subcategory);
                        $str_new_subcategory = preg_replace('/[^\w_-]/','',$str_new_subcategory);
...
                                        redirect($str_filename.'?op=view&id='.$_POST['id']);
                                }
                                else
                                {
                                        $bool_error = true;
