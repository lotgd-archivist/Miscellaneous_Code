...
                            <td>'.date('d.m.Y H:i:s',strtotime($l['date'])).' / '.getgamedate($l['gamedate']).(strlen($l['gamedate_text']) > 1 ? '`n'.$l['gamedate_text'] : '').'</td>
                            <td>'.$l['aname'].'</td>
                            <td>'.$l['gname'].'</td>
                        </tr>';
            
...
            $arr_entry['msg'] = strip_tags($_POST['msg'],'<font><table><tr><td><br>');
            $arr_entry['gamedate'] = (empty($_POST['gamedate']) ? getsetting('gamedate','') : $_POST['gamedate']);
            $arr_entry['gamedate_text'] = str_replace('`n','',strip_tags($_POST['gamedate_text']));
...
            else {
                $sql = 'UPDATE history 
                        SET gamedate="'.$arr_entry['gamedate'].'",gamedate_text="'.$arr_entry['gamedate_text'].'",msg="'.addslashes(stripslashes($arr_entry['msg'])).'",guildid='.$arr_entry['guildid'].',acctid='.$arr_entry['acctid'].'
                        WHERE id='.$int_id;
                db_query($sql);
            }
            
...
                                'gamedate_text'=>'Spieldatum (Alternativtext),text,255',
                                'msg'=>'Nachricht,textarea,60,4,900');
        
        if($int_id) {
