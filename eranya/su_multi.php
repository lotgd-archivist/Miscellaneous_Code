...
        $str_lnk = 'su_bans.php?op=edit_ban&ids[]='.implode('&ids[]=',$_POST['userid']).'&ret='.urlencode('su_multi.php?op=multi');
...
        $str_lnk = 'su_delete.php?ids[]='.implode('&ids[]=',$_POST['userid']).'&ret='.urlencode('su_multi.php?op=multi');
...
    output('`n');
    
    //Inkognito-Ausnahme für ruhiges Klickspiel:
    $exc_chars = ' and acctid NOT IN (1984) ';

    $in_ip = $in_id = '';
    if ($_GET['searchby']!='id') {

        $sql = 'SELECT lastip FROM accounts WHERE lastip!="" '.$exc_chars.' GROUP BY lastip HAVING COUNT(*) > 1';
...
        $sql = 'SELECT uniqueid FROM accounts WHERE uniqueid!="" '.$exc_chars.' GROUP BY uniqueid HAVING COUNT(*) > 1';
        $result = db_query($sql) or die(db_error(LINK));
        while ($row = db_fetch_assoc($result)) {
            $in_id .= ',"'.$row['uniqueid'].'"';
        }
...
            WHERE (lastip IN (-1'.$in_ip.') OR uniqueid IN (-1'.$in_id.')) '.$exc_chars.' ORDER BY dragonkills ASC, level ASC';
    $result = db_query($sql) or die(db_error(LINK));
    while ($row = db_fetch_assoc($result))
    {
...
    addnav('','su_multi.php?op=multi&searchby='.$_GET['searchby']);

    output('`n`bMultiaccounts`b`nNaaa, wer spielt denn hier noch wen?`n`n');
    output('<form method="POST" action="su_multi.php?op=multi&searchby='.$_GET['searchby'].'">Spieler mit ',true);
...
    output('<option value="5" '.(($minaccs==5)?'selected="selected"':'').'>5</option>',true);
    output('<option value="6" '.(($minaccs==6)?'selected="selected"':'').'>6</option>',true);
...
    if ($_GET['searchby']!='ip') {
        output('<a href="su_multi.php?op=multi&searchby=ip&minaccs='.$minaccs.'">IP</a> ',true);
        addnav('','su_multi.php?op=multi&searchby=ip&minaccs='.$minaccs);
    }
    else
    {
        output('`&`bIP`b`0 ');
    }
    if ($_GET['searchby']!='id') {
        output('<a href="su_multi.php?op=multi&searchby=id&minaccs='.$minaccs.'">ID</a> ',true);
        addnav('','su_multi.php?op=multi&searchby=id&minaccs='.$minaccs);
    }
    else
    {
        output('`&`bID`b`0 ');
...
        output('<a href="su_multi.php?op=multi&searchby=&minaccs='.$minaccs.'">Beidem</a> ',true);
        addnav('','su_multi.php?op=multi&searchby=&minaccs='.$minaccs);
    }
    else
    {
        output('`&`bBeidem`b`0 ');
...
            $counter++;
        }
        output('<form action="su_multi.php?op=multi&searchby='.$_GET['searchby'].'" method="post">',true);
        addnav('','su_multi.php?op=multi&searchby='.$_GET['searchby']);
...
        $linkstr = 'su_multianalyse.php?'.$linkstr;

        output('<tr><td colspan="6" align="left">
                        <input type="submit" name="deleteuser" value="Löschen" class="button">
                        <input type="submit" name="setupban" value="Accounts bannen" class="button">
                        <a href="#" onclick="'.popup_fenster($linkstr).';return false;" class="button">Analyse</a>
                    </td></tr>',true);
...
    addnav('Neu laden');
    addnav('Aktualisieren','su_multi.php?op=multi&searchby='.$_GET['searchby'].'&minaccs='.$minaccs);
    //addnav('Zurück','su_multi.php');
}
else
{
    redirect('su_multi.php?op=multi');
}
addnav('Zurück');
