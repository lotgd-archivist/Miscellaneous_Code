...
page_header("Erik Dinivens Tisch");
output("<span style='color: #9900FF'>",true);
output("`c`bErik Dinivens Tisch`b`c");
...
         if ($session['user']['pvpflag']==PVP_IMMU) output("`0Dag schaut dich fordernd an. \"`7Petersen hat mir erzählt, dass er dich vor Killern schützt - jetzt willst du selber jemanden tot sehen? Du würdest seinen Schutz verlieren, wenn ich jemanden für dich auf die Liste hier setze, ist dir das klar?`0\"`n`n");
    }
...
        $sql = "SELECT acctid,name,login,level,locked,age,dragonkills,pk,experience,bounty,pvpflag,lastip,emailaddress,reputation,uniqueid FROM accounts WHERE name LIKE '".$name."' AND locked=0";
    }
    $result = db_query($sql);
    if (db_num_rows($result) == 0) {
        output("Erik Diniven sagt höhnisch lachend: `7\"Es gibt nicht einen den ich mit so einem Namen kenne. Vielleicht kommst' wieder, wenn du 'n echtes Opfer hast.\"");
    } elseif(db_num_rows($result) > 100) {
        output("Erik Diniven kratzt sich verwirrt am Kopf. `7\"Du beschreibst hier fast die Hälfte der Stadt, du Narr. Warum gibst du mir jetzt nicht mal ne genauere Beschreibung?\"");
    } elseif(db_num_rows($result) > 1) {
        output("Erik Diniven durchsucht seine Liste für einen Moment. `7\"Da sind ein paar, die du meinen könntest. Wer genau soll's denn sein?\"`n");
...
            output("Erik Diniven schlägt sich brüllend lachend auf die Schenkel: `7\"Du willst 'n Kopfgeld auf dich selbst aussetzen? Ich helf doch keinem Selbstmörder!\"");
...
            output("Erik Diniven starrt dich ärgerlich an: `7\"Hab ich dir nicht gesagt, dass ich kein Meuchler bin? Das ist kein Opfer, das ein Kopfgeld wert wäre. Jetzt geh mir aus den Augen!\"");
...
            $fee = getsetting("bountyfee",10);
            if ($amt < $min) {
                output("Erik Diniven blickt finster: `7\"Glaubst im Ernst, ich arbeite für so nen Hungerlohn? Denk ma drüber nach und komm wieder, wenn du bereit bist hartes Bares zu bezahlen. Für dein Opfer brauchste mindestens " . $min . " Gold, damit's meine Zeit wert is.\"");
            } elseif ($session['user']['gold'] <round($amt*1.1,0)) {
                output("Erik Diniven schaut dich finster an: `7\"Du hast nicht genug Gold für diesen Vertrag. Wenn du meine Zeit so vergeudest, sollte ich stattdessen vielleicht n Kopfgeld auf DICH aussetzen!\"");
...
            } else {
                output("Du schiebst die Münzen zu Erik Diniven, der sie flink einstreicht. `7\"Ich werd mir nur meine $fee% Gebühr einbehalten. Ich werd die Nachricht verbreiten, dass sich jemand um `^{$row['name']} `7kümmern soll. Hab Geduld und hab ein Auge auf die News.");
...
                $sql = "UPDATE accounts SET bounty=bounty+$amt WHERE acctid='{$row['acctid']}'";
                db_query($sql);
            }
        }
    }
}else{
    output("Du schlenderst rüber zu Erik Diniven, der es nichtmal für nötig hält, zu dir aufzuschauen. Er nimmt einen langen Zug aus seiner Pfeife.`n");
...
if ($_GET['op'] != '')
    addnav("Rede mit Erik Diniven", "dag.php");
