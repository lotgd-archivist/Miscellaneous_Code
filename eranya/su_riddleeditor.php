...
        addnav("Rätsel hinzufügen","su_riddleeditor.php?op=add");
        output("<table><tr><td>Ops</td><td width='50%'>Rätsel</td><td>Lösung</td></tr>",true);
        addnav("","su_riddleeditor.php");
        for ($i=0;$i<db_num_rows($result);$i++){
            $row = db_fetch_assoc($result);
            output("<tr><td valign='top'> [<a href='su_riddleeditor.php?op=edit&id={$row['id']}'>Edit</a>|".
            "<a href='su_riddleeditor.php?op=del&id={$row['id']}' onClick='return confirm(\"Bist du dir sicher, dass du dieses Rätsel löschen willst?\");'>Del</a>] </td><td>",true);
            addnav("","su_riddleeditor.php?op=edit&id={$row['id']}");
            addnav("","su_riddleeditor.php?op=del&id={$row['id']}");
            output($row['riddle']);
            output("</td><td>",true);
            output($row['answer']);
            output("</td></tr>",true);
