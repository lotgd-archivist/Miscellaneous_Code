...
switch($_GET['op']){
case "theme":
        addnav("Zur Themenübersicht","su_library.php");
...
        $sql = "SELECT bookid, title, author, listorder, recommended FROM lib_books
                        WHERE themeid=".$_GET['themeid']." ORDER BY listorder ASC";
        $result = db_query($sql) or die(db_error(LINK));
        output('<form action="su_library.php?op=theme&amp;themeid='.$_GET['themeid'].'&amp;saveorder=1" method="post">',true);
        addnav('','su_library.php?op=theme&themeid='.$_GET['themeid'].'&saveorder=1');
        output("<table cellpadding=2 cellspacing=1 bgcolor='#999999' align='center'><tr class='trhead'><td>Titel</td><td>Autor</td><td>Empfohlen?</td><td>Sortierung</td></tr>",true);
...
                        output("<tr class='$bgclass'><td><a href=\"su_library.php?op=edit_post&id=".$row['bookid']."\">",true);
                        output($row['title']);
...
                        output("</td></tr>",true);
                        addnav("","su_library.php?op=edit_post&id=".$row['bookid']);
                }
...
                $row = db_fetch_assoc($result);
                $row['maxtheme']++;
                $sql = "INSERT INTO lib_themes (theme,listorder) VALUES ('".str_replace("'","&#39;",$_POST['theme'])."', {$row['maxtheme']})";
                $result = db_query($sql) or die(db_error(LINK));
                output("`\$Neues Thema wurde angelegt.`0`n`n");
        }
        output("<form action=\"su_library.php?op=new_theme&subop=save\" method='POST'>",true);
...
        addnav("","su_library.php?op=new_theme&subop=save");
        output("`n");
...
        addnav("Zur Themenübersicht","su_library.php");
        if ($_GET['subop']=="save") {
                $_POST['theme'] = addslashes(str_replace("'","&#39;",closetags(stripslashes($_POST['theme']),'`i`b`c`H')));
...
                redirect("su_library.php?op=browse");
        }
        else {
...
                $result = db_query($sql) or die(db_error(LINK));
                $row = db_fetch_assoc($result);
                output("<form action=\"su_library.php?op=edit_theme&subop=save&themeid=".$_GET['themeid']."\" method='POST'>
                        Thema:`n<input type='text' name='theme' value='".htmlentities(str_replace("`","``",$row['theme']),ENT_QUOTES)."' maxlength='100' size='80'>`n
                        Beschreibung:`n<textarea name='description' class='input' cols='50' rows='10'>".htmlentities(str_replace("`","``",$row['description']),ENT_QUOTES)."</textarea>`n
                        `n<input type='submit' class='button' value='Speichern'></form>",true);
                addnav("","su_library.php?op=edit_theme&subop=save&themeid=".$_GET['themeid']);
        }
        break;
...
                output("<form action=\"su_library.php?op=del_theme2&themeid=".$_GET['themeid']."\" method='POST'>
...
                addnav("","su_library.php?op=del_theme2&themeid=".$_GET['themeid']);
        }
        break;
...
                $result = db_query($sql) or die(db_error(LINK));
                output("<form action=\"su_library.php?op=del_theme3&old_themeid=".$_GET['themeid']."\" method='POST'>
...
                addnav("","su_library.php?op=del_theme3&old_themeid=".$_GET['themeid']);
        }
        else {
...
                                systemmail($row['acctid'],'`#Dein Buch wurde angenommen!','`3Das Buch mit dem Titel "'.$row['title'].'`3", das du eingereicht hattest, wurde in die Bibliothek aufgenommen. Du bekommst dafür '.$dpts4book.' Donationpoints.'.(!empty($str_comment) ? '`n`#Begründung:`n'.$str_comment : '') );
                                
                                
                        }
                                                                        
...
                        
                        $str_lnk = 'su_library.php?op=new_books&subop=activate&act=ok&id='.$int_id;
                        addnav('',$str_lnk);                        
...
                }
                
                addnav("Weitere neue Bücher","su_library.php?op=new_books");
        }
        elseif ($_GET['subop'] == 'activate_again') {
                $int_id = (int)$_GET['id'];
                output('Dieses Buch ist jetzt wieder für alle in der Bibliothek einsehbar.`n');
                                                                                        
                $sql = "UPDATE lib_books SET activated='1' WHERE bookid=".$int_id;
                $result = db_query($sql);        
        }
        elseif ($_GET['subop'] == 'deactivate') {
                $int_id = (int)$_GET['id'];
                output('Dieses Buch ist nun nicht mehr in der Bibliothek einsehbar, bis es wieder aktiviert wird!`n');
                                                                                        
...
                $sql = "SELECT b.bookid, b.title, t.theme, b.author, b.seen, b.activated 
                        FROM lib_books b
...
                                                        <td>"
                                                                .($row['activated'] == '0' 
                                                                        ? create_lnk('Aktivieren','su_library.php?op=new_books&subop=activate&id='.$row['bookid'])
                                                                        : create_lnk('Erneut Aktivieren','su_library.php?op=new_books&subop=activate_again&id='.$row['bookid'])
                                                                )
                                                                ."|"
                                                                .create_lnk('Löschen','su_library.php?op=del_post&id='.$row['bookid']),true);
                                output("<td>".(!$row['seen'] ? ' `^`b ~ ' : ' - ').$row['bookid'].(!$row['seen'] ? '`0`b ~ ' : ' - ')."</td><td>",true);
                                output($row['theme']);
                                output("`0</td><td><a href=\"su_library.php?op=edit_post&id=".$row['bookid']."\">",true);
                                output($row['title']);
                                output("`0</a></td><td>",true);
                                output($row['author']);
                                output("`0</td></tr>",true);
                                
                                addnav("","su_library.php?op=edit_post&id=".$row['bookid']);        //view
                        }
                }
                
        }
...
                $arr_data = array();
                
                $str_lnk = 'su_library.php?op=del_post&act=ok&id='.$int_id;
                addnav('',$str_lnk);                        
...
                        redirect("su_library.php?op=new_books&subop=activate&id=".$_GET['id']);
                }
                elseif (isset($_POST['deactivate'])) {
                        redirect("su_library.php?op=new_books&subop=deactivate&id=".$_GET['id']);
                }
                elseif (isset($_POST['recommend_on'])) {
                        $sql = 'UPDATE lib_books SET recommended=1 WHERE bookid='.$_GET['id'];
                        db_query($sql);
                        redirect('su_library.php?op=edit_post&id='.$_GET['id']);
                }
...
                        db_query($sql);
                        redirect('su_library.php?op=new_books');
                }
                elseif (isset($_POST['del'])) {
                        redirect("su_library.php?op=del_post&id=".$_GET['id']);
                }
        }
...
        output("<form action=\"su_library.php?op=edit_post&subop=button&id=".$_GET['id']."\" method='POST'>
...
        output("' size='50'>`nAutor: ".$row['author']."`0 hat in dem Buch geschrieben:`n
...
        if ($row['activated']=='0' || $row['activated'] == '2') {
                output("<input type='submit' class='button' name='activate' value='Aktivieren'>",true);
                addnav("Aktivieren","su_library.php?op=new_books&subop=activate&id=".$_GET['id']);
                addnav("Ungelesen markieren","su_library.php?op=edit_post&subop=button&notseen=1&id=".$_GET['id']);
        }
        else {
                output("<input type='submit' class='button' name='deactivate' value='Deaktivieren'>",true);
                addnav("Deaktivieren","su_library.php?op=new_books&subop=deactivate&id=".$_GET['id']);
        }
        if($row['recommended']) {
...
        addnav("","su_library.php?op=edit_post&subop=button&id=".$_GET['id']);
        

        output("Das Buch:`n`n".str_replace("\n",'`n',$row['book']).'`0');

        addnav("Zu neuen Büchern","su_library.php?op=new_books");
        addnav("Zu Büchern ohne Thema","su_library.php?op=no_theme_books");
        if($row['themeid']) {
                addnav("Zum Themenbereich","su_library.php?op=theme&themeid=".$row['themeid']);
        }
        addnav("Zur Themenübersicht","su_library.php");
break;
...
        else while($row = db_fetch_assoc($result)) {
                $bgclass = ($bgclass=='trdark'?'trlight':'trdark');
                output("<tr class='$bgclass'><td><a href=\"su_library.php?op=del_post&id=".$row['bookid']."\">Löschen</a></td>",true);
                output("<td>".$row['bookid']."</td><td><a href=\"su_library.php?op=edit_post&id=".$row['bookid']."\">",true);
                output($row['title']);
                output("`0</a></td><td>",true);
                output($row['author']);
                output("`0</td></tr>",true);
                addnav("","su_library.php?op=del_post&id=".$row['bookid']);        //delete
                addnav("","su_library.php?op=edit_post&id=".$row['bookid']);        //ansehen
        }
break;
...
        output('<form action="su_library.php?saveorder=1" method="post">',true);
        addnav('','su_library.php?saveorder=1');
...
                while ($row = db_fetch_assoc($result)) {
                        $bgclass = ($bgclass=='trdark'?'trlight':'trdark');
                        output("<tr class='$bgclass'><td><a href=\"su_library.php?op=edit_theme&themeid=".$row['themeid']."\">Edit</a> |
                        <a href=\"su_library.php?op=del_theme&themeid=".$row['themeid']."\">Löschen</a></td><td>".$row['themeid']."</td><td>
                        <a href=\"su_library.php?op=theme&themeid=".$row['themeid']."\">",true);
                        output($row['theme']);
...
                        output('</td></tr>',true);
                        output('<tr class='.$bgclass.'><td colspan=5>'.strip_appoencode(substr($row['description'],0,60),2).'...</td></tr>');

                        addnav("","su_library.php?op=edit_theme&themeid=".$row['themeid']);
                        addnav("","su_library.php?op=del_theme&themeid=".$row['themeid']);
                        addnav("","su_library.php?op=theme&themeid=".$row['themeid']);
                }
                $bgclass = ($bgclass=='trdark'?'trlight':'trdark');
                output('<tr class='.$bgclass.'><td colspan="5" style="text-align:right"><input type="submit" class="button" value="Sortierung speichern!" /></td></tr>',true);
        }
        output("</table>",true);
        output('</form>',true);
        addnav("Neues Thema erstellen","su_library.php?op=new_theme");
        addnav("Neue Bücher ansehen","su_library.php?op=new_books");
        addnav("Bücher ohne Thema","su_library.php?op=no_theme_books");
        addnav('F&A-Log');
        addnav('F&A-Log anzeigen','su_logs.php?op=search&type=fundalog');
}
