...
 { output ("Zu dir spricht die dunkle Stimme: `4\"`$".($session['user']['name'])."`4! Höre meine Stimme. Du, " . ($session[user][sex]?"die":"der") . " du auserwählt bist unter den Auserwählten, " . ($session[user][sex]?"die":"der") . " du mächtig bist unter den Mächtigen und " . ($session[user][sex]?"die":"der") . " du in meiner Gunst stehst. Gehe hinaus in die Welt und künde von meiner Herrlichkeit! Sage allen, dass in mir die Ewigkeit ruht! Schare sie um dich, mein Kind, zu meinem Gefallen!\"`n");
...
if ($_GET[op]=="bg4") {
page_header("Der Opferaltar des Blutgottes");
output ("In der festen Überzeugung, dich nicht zum Handlanger irgendwelcher Götter machen zu lassen, löst du den Pakt und der Blutgott wendet sich von dir ab. Du fühlst dich nun freier.");
...
if ($_GET[op]=="bg2") {
 page_header("Der Opferaltar des Blutgottes");
output ("`tDu nimmst den Opferdolch und gibst dem Blutgott ein Zehntel deiner Lebenskraft als Opfer. In Anerkennung dessen trifft dich ein roter Blitz und brennt winzig klein das Zeichen des Blutgottes in deinen Hals, auf dass jeder Vampir erkenne, dass du in der Gunst des Blutgottes stehst!`n");
...
            }
        }else{
                $sql = "SELECT acctid,name,house,housekey FROM accounts WHERE login=\"$_GET[who]\"";
                $result = db_query($sql) or die(db_error(LINK));
...
addnav("Zum goldenen Goblin","thepath.php?op=charta");
...
addnav("Z?Zurück zum Wald","forest.php");
