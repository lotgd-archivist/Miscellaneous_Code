...
        else {
            if(!$applicant && $session['user']['acctid'] != getsetting('demouser_acctid',0)) {
...
    case 'paladin':
        
        dg_show_header('Ritterfestung');
        
        if(!$member) {output('`8Du willst gerade die Feste der herzoglichen Rittergarde betreten, als man dich schroff zurückweist: `&"Zutritt nur für Gildenangehörige!"`n');}    
        else {
                                        
...
                    output('`8Ein kräftiger Ritter packt eine Insignie, wickelt sie sorgfältig in Stoff ein und meint dann zu dir: `&"Ich gratuliere euch, Meister '.$session['user']['login'].', eure Gilde hat eine gute Wahl getroffen! Wir werden das Stück demnächst liefern."');
                    addnav('Zum Lager der Rittergarde','dg_council.php?op=paladin');
                }            
                else {
...
                    }
                    else {
                        output('`8Mit einem gemeinen Lächeln klopft dir der Ritter auf die Schulter. `&"Vielen Dank, aber ich habs mir anders überlegt.."');        
                        addnav('Zum Lager der Rittergarde','dg_council.php?op=paladin');
                    }                
    
                }
...
                }                        
                else {
                    output('`8Verstohlen näherst du dich einer scheinbar höhergestellten Wache und raunst ihr ein Angebot ins Ohr. Nachdenklich wendet der Ritter sich dir zu, seine Miene verheißt nichts Gutes. Du willst schon davonlaufen, als er dir grinsend zuflüstert: `&"'.$bribe_price_points.' Punkte, mein Freund.."`8`n');
                    addnav('Zum Lager der Rittergarde','dg_council.php?op=paladin');
...
                
                output('`8Du näherst dich der Wache und grüßt sie ehrerbietig. Auf deine Frage, welcher Art denn die Stimmung des Herzogs zur Zeit sei, ');
                
                if($king_mood > 90) {
                    output('nickt dir der Ritter freundlich zu: `&"Ausgezeichnet, mein Freund, ausgezeichnet!"');
                }
                else if($king_mood > 70) {
                    output('sieht dich der Ritter kurz an und meint: `&"Der Herzog pflegt gute Beziehungen zu den Gilden '.getsetting('townname','Atrahor').'s!"');
                }
                else if($king_mood > 50) {
                    output('grübelt der Ritter: `&"Nun, der Herzog ist mit den Gilden '.getsetting('townname','Atrahor').'s leidlich zufrieden. Sicherlich könnte es besser sein!"');
                }
                else if($king_mood > 30) {
                    output('sieht dich der Ritter mitleidig an und lacht: `&"Eisig, um es so zu sagen!"');
                }
                else {
                    output('blickt der Ritter starr geradeaus. Als du deine Frage wiederholst, knurrt er: `&"Der Herzog ist äußerst unzufrieden mit den Gilden '.getsetting('townname','Atrahor').'s! Gebt gut auf eure Insignien Acht.."');
                }
                
                addnav('Zum Lager der Rittergarde','dg_council.php?op=paladin');
                
...
            else {
                
                output('`8An die das Gildenviertel umgrenzende Mauer schmiegt sich die Feste der herzoglichen Rittergemeinschaft. Sie wachen im Auftrag des Herzogs über die Gilden, verwalten die Insignien und repräsentieren den Herrscher.`n`nÜberall stehen wohlgerüstete Krieger herum und erfüllen ihre jeweilige Aufgabe. Von ihnen weiß sicherlich auch einer über die Stimmung des Herzogs Bescheid. Vielleicht sind einige auch bereit, bei diesem ein gutes Wort einzulegen..`n`n');
                
                addnav('Nach Stimmung des Herzogs fragen','dg_council.php?op=paladin&subop=ask_mood');
                
                addnav('Ritter bestechen','dg_council.php?op=paladin&subop=bribe_king');
                                                
                if($regalia_left) {
...
        else {
                
            output('`8Sofort stechen dir am prachtvollen und beeindruckenden Gebäude des Gildenrats zwei eicherne Türflügel ins Auge. Hinter jenen, so erfährst du, beraten sich die Gilden über gemeinsame Unternehmungen und - wer weiß - vielleicht auch über die Zukunft der Stadt. Eine Tafel verkündet:`n
                    '.($council_days_left || 1 ? '`n`bDer Gildenrat tagt gerade!`b`n' : '`nNoch `^'.$vote_days_left.'`8 Tage bis zur Einberufung des nächsten Gildenrates.')
...
                        4. Wie steht die Gilde zu offiziellen Autoritäten in '.getsetting('townname','Atrahor').' (Richter, Stadtwache, Priester, Hexen, Stadtrat, Herzog,...)?`n
...
                
                $sql = 'INSERT INTO petitions SET author='.$session['user']['acctid'].',date=NOW(),IP="'.addslashes($session['user']['lastip']).'",ID="'.addslashes($session['user']['uniqueid']).'",title="Gildengründung",body="'.addslashes($str_petition).'"';
                db_query($sql);
                
...
                else {
                    
                    output('`8Stirnrunzelnd weist er dich zurück: `q"Du hast noch nicht genügend Drachen getötet! Komm mit '.$min_dk.' grünen Lindwurmköpfen wieder, dann sehen wir weiter.."');
                    addnav('Zum Gildenviertel','dg_main.php');
                    
                }
                
...
    addnav('Admin');
    addnav('X?Zum Gildeneditor','dg_su.php');
}
