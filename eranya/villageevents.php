...
//Bemerkung: Farbe sollte die gleiche sein wie VILLAGECOLORTEXT in village.php
define('VILLAGEEVENTSCOLORTEXT','`2');
...
if ($event=="")
{
    // $event=e_rand(1,2);   # das Brunnenmonster hat erst mal Pause, bis dafür ein Sinn gefunden wurde
    $event = 1;
}
...
            else
            {
                output(", die dich als Richter zwar nicht festnehmen kann, dich aber an der Hand nimmt und dich zurück auf den Stadtplatz begleitet.");
...
                addnav("In den Kerker","prison.php");
//                $sql="INSERT INTO cases (newsid,newstext,accountid,judgeid,court) VALUES ('', 'User wurde vom Zufallsereignis \"dunkle Gasse\" 3 Tage eingekerkert. Sollte er fliehen und sonst nichts anstehen ist von einer Verurteilung abzusehen.', '".$session['user']['acctid']."', '0', '0')";
...
            break;
        case 4 :
            output("`t`nDu streckst deinen ersten Gegner eiskalt nieder, näherst dich dann dem Zweiten. Auch ihn schickst du schnell zu `²J`ça`Âr`Îc`Âa`çt`²h`t. Doch dann trifft dich eine Klinge genau zwischen die Schulterblätter. Zwar schaffst du es noch einen Dritten zu töten, doch hauchst du selbst nach mehreren weiteren Treffern auch dein Leben aus. Du bist tot und verlierst 5% deiner Erfahrung!");
            $session['user']['experience']*=0.95;
            $session['user']['hitpoints']=0;
...
        
        if ( $_GET['event'] || (
                 getsetting('weather','') == WEATHER_FOGGY && ( ($hour < 10 && $hour > 4) || ($hour < 22 && $hour > 16) ) 
                ))
        {
            
            output("".VILLAGEEVENTSCOLORTEXT."Wie du auf dem Stadtplatz stehst, hörst du plötzlich ein lautes Gurgeln und ein dumpfes Dröhnen aus dem Schacht des Stadtbrunnens. Nahezu unheimlich dringen die Geräusche an dein Ohr und du bekommst eine Gänsehaut.`nAuch bemerkst du, dass es mit einem Mal neblig und kalt zu werden scheint.`nWas wirst du tun ?`0");
         
...
        sleep(5);
                
        insertcommentary($session['user']['acctid'],'/me weicht erschreckt und verunsichert zurück und hält einen deutlichen Abstand zum  Brunnen.`0','village');
        
        redirect("village.php");
    }
    else if ($_GET['op']=="flee2")
    {
        sleep(5);
        
        insertcommentary($session['user']['acctid'],'/me hat es sich wohl doch anders überlegt und entfernt sich rasch vom Brunnen.`0','village');
        
        redirect("village.php");
    }
...
        sleep(5);
                
        insertcommentary($session['user']['acctid'],'/me nähert sich neugierig dem Stadtbrunnen.`0','village');
        
        output("".VILLAGEEVENTSCOLORTEXT."Nahezu bedrohlich wirkt dieser verfluchte Brunnen auf dich, doch hat er auch eine gewisse Anziehungskraft. Du glaubst zu hören wie etwas versucht den Brunnenschacht hinauf zu klettern.`nDeine Neugier ringt mit deiner Angst.`nWas tust du?`0");
...
        case 1 :
        case 2 :
            output("".VILLAGEEVENTSCOLORTEXT."Du ziehst es vor, doch lieber in Sicherheit zu bleiben.`0");
...
            
            output("".VILLAGEEVENTSCOLORTEXT."Du drehst dich um, bereit dich wieder zu entfernen, als plötzlich ein langes, schlankes Tentakel aus dem Brunnen herausschiesst und sich um deinen Hals schlingt.`nDu bekommst kaum Luft und bist wie gelähmt!`nWas nun ?`0");
...
    {
            
        insertcommentary($session['user']['acctid'],'/me ruft verzweifelt mit Armen und Beinen strampend und im Würgegriff des Stranges : "Hilfe! So helft mir doch!".`0','village');
        
        $ops= e_rand(1,2);
        switch ($ops)
        {
        case 1 :
            sleep(5);
            output("".VILLAGEEVENTSCOLORTEXT."Durch das beherzte Eingreifen einiger Helden in deiner Nähe wirst du gerettet und der Strang zieht sich in den Brunnen zurück.`nDu hast Lebenspunkte verloren!");
...
            break;
        case 2 :
            output("".VILLAGEEVENTSCOLORTEXT."Dein Schreien und Wimmern hat dir nicht geholfen. Der Strang zieht dich in den Brunnen herab, wo dich ein sehr unangenehmer Tod erwartet.`0");
...
                addnews("`@".$session['user']['name']."`@ wurde von etwas in den Stadtbrunnen gezogen und starb!");        
                
        killplayer(0);
        
...
        {
        case 1 :
            output("".VILLAGEEVENTSCOLORTEXT."Es gelingt dir dich freizuschlagen und der Strang zieht sich in den Brunnen zurück.`nDu hast Lebenspunkte verloren!`0");
...
        case 2 :
        case 3 :
            output("".VILLAGEEVENTSCOLORTEXT."Du schlägst nach dem Strang und windest dich, allerdings erfolglos. Der Strang zieht dich in den Brunnen herab, wo dich ein sehr unangenehmer Tod erwartet.`0");
            addnav("weiter","villageevents.php?op=die&event=$event");
            break;
        }
    }
...
        
        sleep(5);
        output("".VILLAGEEVENTSCOLORTEXT."Du beugst dich über den Rand des Brunnens um hinein zu schauen und erkennst, dass in der Tiefe ein kleines rotes Lichtlein leutet.`nDu kneifst die Augen zusammen um es besser erkennen zu können, und stellst fest, dass es sich um irgendein Symbol handelt, dass du jedoch noch nie in deinem Leben zuvor gesehen hast.`n`n`0");
        output("<IMG SRC=\"images/symbol.jpg\" align='middle'>",true);
        output("`n`n".VILLAGEEVENTSCOLORTEXT."Als Du dich aufrichtest um zu gehen schnellt plötzlich ein langer tentakelähnlicher Strang aus der Tiefe hinauf und schlingt sich um deinen Hals.`nWas nun?`0");
        addnav("Um Hilfe rufen","villageevents.php?op=helpme&event=$event");
        addnav("Auf den Strang einschlagen","villageevents.php?op=hack&event=$event");
    }
    
