...
        if ($session['user']['imprisoned']>0)
        {
            output("für die nächsten `@".($session['user']['imprisoned'])." ".($session['user']['imprisoned'] == 1 ? "Tagesabschnitt" : "Tagesabschnitte")."`t im Kerker.`n");
        }
        else
        {
            output("auf `bunbestimmte Zeit`b im Kerker.`n Wahrscheinlich hast du `bgegen die Regeln verstoßen`b oder eine `bMail zur Namensänderung ignoriert`b! Öfnne zunächst durch einen Klick auf 'Hörnchenpost' dein `bPostfach`b und seh nach, ob du dort einen Grund findest. Falls nicht, schreib eine `bAnfrage`b.`n`n");
        }
...
        viewcommentary("prison","Flehen:",30,"fleht",false,true,false,false,true);
    }
    
}
else
...
        viewcommentary("prison","Verspotten:",30,"spottet",false,true,false,false,true);
        addnav("Umsehen","prison.php?op=look");
...
{
    output("`&Gegen dich liegt ein Haftbefehl vor.`nWenn du dich jetzt stellst wirst du für ".$row3['sentence']." Tagesabschnitte den Kerker von der anderen Seite kennen lernen.`nWillst du das wirklich tun?");
...
{
    output("`&Du hast hier die Möglichkeit dich deinen Schicksal zu ergeben und die dir auferlegte Strafe abzusitzen.`nDie abgesessenen Tage werden dabei deinem Strafregister hinzugefügt, du darfst den Kerker allerdings nach Ablauf dieses Tagesabschnitts verlassen. Möchtest du das?`n(Dies beschleunigt lediglich die Wartezeit für den Spieler, die eigentliche Haftzeit des Charakters verkürzt sich dadurch nicht!)");
...
    $injail=$days-1;
    output("`&Du setzt dich auf deine Pritsche und lässt die Zeit bis zu deiner Entlassung verstreichen...`nDeinem Strafregister wurden `^$injail`& Tagesabschnitte hinzugefügt.`nMorgen kommst du hier raus!`n");
...
    output("<tr class='trhead'><td align='center'><b>Level</b></td><td align='center'><b>Name</b></td><td align='center'><b>Rasse</b></td><td align='center'><b><img src=\"images/female.png\">/<img src=\"images/male.png\"></b></td><td align='center'><b>Status</b></td><td align='center'><b>Strafe in`nTagesabschnitten</b></tr>",true);
...
        if ($session['user']['loggedin'])
        {
            output("<a href='prison.php?op=free2&char=".$row['acctid']."'>",true);
        }
        if ($session['user']['loggedin'])
        {
...
        }
        output("</td><td>",true);
        output($arr_races[$row['race']]['colname']);
...
        systemmail($row['acctid'],"`\$Kaution bezahlt!`0","`@{$session['user']['name']}
        `& hat die Kaution für dich bezahlt und dich damit aus dem Kerker befreit. Du solltest dich dankbar erweisen!");
        addnews("`#".$session['user']['name']." `&hat `@".$row['name']."`& aus dem Kerker freigekauft!");
        $sql="INSERT INTO commentary (postdate,section,author,comment) VALUES (now(),'prison',".$session['user']['acctid'].",'/me `@bezahlt die Kaution für ".addslashes($row['name'])."`@.')";
        db_query($sql) or die(db_error(LINK));
      }
      else output('Tja, der gute Wille war da, aber jemand anderes war schneller.');  
    }
    
    addnav("Weiter","prison.php");
}
...
                {
                    $str_biolink = "<a href='bio_popup.php?op=pri&char=".rawurlencode($row['login'])."' target='_blank' onClick='".popup_fullsize('bio_popup.php?op=pri&char='.rawurlencode($row['login'])).";return:false;'>";
                }
                else
                {
                    $str_biolink = "<a href='bio.php?op=pri&char=".rawurlencode($row['login'])."&ret=".URLEncode($_SERVER['REQUEST_URI'])."'>";
                    addnav("","bio.php?op=pri&char=".rawurlencode($row['login'])."&ret=".URLEncode($_SERVER['REQUEST_URI']));
                }
            output($str_biolink,true);
        }
        output("`".($row['acctid']==getsetting("hasegg",0)?"^":"&")."$row[name]`0");
        if ($session['user']['loggedin'])
        {
...
    if (e_rand(1,3)==2)
    {
        output("`n`4Die Götter reagieren zornig auf das Ausnutzen der Male. Sie sehen es nicht ein deine kriminellen Machenschaften zu decken und entziehen dir das `^Erdmal`4`nSag nicht, man hätte dich nicht gewarnt...");
        if ($session['user']['marks']>=32)
        {
            $session['user']['marks']=30;
...
            case 3 :
            case 4 :
                output("`&Als der Wärter kommt, um nach dir zu sehen, hältst du deinen hölzernen Napf bereit und versuchst ihn damit niederzuschlagen. Doch leider bist du nicht der Erste, der diese geniale Idee hatte und der Wächter schafft es leicht, dich zu überwältigen.`n");
                if ($session['user']['imprisoned']<5)
                {
                    output("`&Tja... deine Haftstrafe wurde soeben um `#1 Tagesabschnitt`& verlängert");
                    $session['user']['imprisoned']+=1;
                }
                $session['user']['turns']-=1;
