...
        else if ($_GET['act']=='')
        {
            output("`tDu kommst in die Kammer, doch Dodo ist nicht da. Stattdessen findest du einen Zettel mit der Aufschrift `n\"`5Dodo ist mit ".($friend)."`5 zum Spielen rausgegangen.`t\"");
        }
        else
        {
...
        if ($_GET['act']=='')
        {
            output("`tDu durchschreitest eine etwas größere Holztür, die mit Stahlbeschlägen verstärkt wurde. Einige der Beschläge weisen Ausbeulungen auf und das Holz ist an einigen Stellen gerissen oder gesplittert. Du glaubst zu erahnen, was dich dorthinter erwartet.`nMit klopfendem Herzen drückst du die Tür auf und gelangst in einen viereckigen, kahlen Raum.`nUnd dann erblickst du ihn:`5Dodo, den Kuscheldämon`t! Ein Ungetüm von sowohl 2 Metern Größe als auch Breite. Das Geschöpft ist über und über mit purpurnem, langem, zotteligem Fell bedeckt und ein breites, kurzes Horn ragt aus seiner Stirn. Gerade als du reinkommst, sitzt er in der Mitte des Raumes und kaut an einem hölzernen Pferd. Als er dich erblickt, lässt er sein Spielzeug fallen und schaut dich mit einem lieben, treudoofen Blick an.`n`n\"`5Halluu, duuuuu?\"`t, erklingt es.`n`n");
            addnav('Was nun?');
...
                case 1 :
                case 2 :
                    output("`tDoch Dodo hüpft dir entgegen, kurz bevor du zuschlagen willst. Du prallst gegen seinen weichen Bauch und landest auf deinem Allerwertesten.`n\"`5Nochmaaaaal!`t\", ruft dir Dodo zu.`nDu hast durch den Sturz ein paar Lebenspunkte verloren.");
                    $dodo+=20;
...
                    break;
                case 5 :
                    output("`tDu triffst Dodo hart an der Schulter und der Dämon heult laut auf. Dann packt er dich, drückt dich zu Boden und setzt sich auf dich drauf.`n`nDu bist tot und verlierst 8% deiner Erfahrung!");
...
            {
                output('`tDu näherst dich Dodo mit einem Grinsen und kuschelst dich in sein warmes, flauschiges Fell.`n');
                
                switch (e_rand(1,5))
                {
                case 1 :
                case 2 :
                    output('`tDodo grunzt vergnügt und knuddelt eine Weile mit dir.`n"`5Ohhh, das war schöööön!`t", quiekt er.`n');
                    $dodo+=10;
                    break;
                case 3 :
                case 4 :
...
            {
                output("`tDu setzt dich vor Dodo auf den Boden und erzählst ihm eine überaus spannende Heldengeschichte vom großen Bellerophontes.`n");
                switch (e_rand(1,5))
                {
                case 1 :
                case 2 :
                    output("`tDodo lauscht dir ganz gespannt, und als du fertig bist, schauen dich seine riesigen Kulleraugen bewundernd an.`n");
                    $dodo+=25;
                    break;
                case 3 :
                case 4 :
...
                    break;
                case 5 :
                    output("Dodo wirft einen kurzen Blick auf die Puppe und schlägt mit seiner wuchtigen Faust darauf. \"`5Dich mochte ich noch nie!`t\", sagt er grollend. Leider traf er bei dem Schlag auch deine Hand - du verlierst ein paar Lebenspunkte.");
                    $dodo-=10;
                    (int)$session['user']['hitpoints']*=0.95;
                    break;
                }
...
                    break;
                case 5 :
                    output('Dodo wertet deine plötzliche Kitzelattacke als einen ernsten Angriff und schleudert dich wie wild durch den Raum. Er schlägt dich solange gegen Wände und Boden, bis du das Bewusstsein verlierst. Dann setzt er sich auf dich drauf und zerquetscht dich.`n`nDu bist tot und verlierst 8% deiner Erfahrung!');
                    $session['user']['hitpoints']=0;
                    $session['user']['experience'] *= 0.92;
                    addnews('`@'.$session['user']['name'].'`& wurde von `5Dodo, dem Kuscheldämon`& zerquetscht!');
                    addnav('Nein!','shades.php');
...
            {
                output("`tDu schaust Dodo an und fragst ihn: \"`&Dodo, willst du nicht mit nach draußen kommen und da mit mir weiterspielen?`t\"`n`n");
...
                addnav('Spielen','chosenfeats.php?op=dodo&act=play&dodo='.$dodo);
                addnav('Kitzeln','chosenfeats.php?op=dodo&act=tinkle&dodo='.$dodo);
                if (($dodo>=200) || (su_check(SU_RIGHT_DEBUG)))
                {
                    addnav('Dodo mitnehmen','chosenfeats.php?op=dodo&act=takeout&dodo='.$dodo);
                }
                addnav('Dodo allein lassen','thepath.php');
