...
if ($HTTP_GET_VARS['op']==""){
        ...
        foreach ($keys AS $key) {
            ...
            output("`UÜbersicht  über die Jobs`n`^Jobs mit dem Ausbildungszweig 6 werden NUR übers Team vergeben.`0`n`n");
//Beginn der Übersichts Tabelle
    output("<table cellpadding=2 cellspacing=1 bgcolor='#999999' align='center'><tr class='trhead'><td>Optionen</td><td>ID</td><td>Name</td><td>Lohn</td><td>Runden</td><td>Ausbildungszweig</td></tr>",true);
    $sql = "SELECT name,lohn,turns,id,aubid FROM jobs";
    ...
    if (db_num_rows($result)==0) {
        ...
        while ($row = db_fetch_assoc($result)) {
            ...
            output("`0Delete</a></td><td>".$row['id']."</td><td>".$row['name']."</td><td>".$row['lohn']."</td><td>".$row['turns']."<td>".$row['aubid']."",true);
            output('</td></tr>',true);
...
//Die Eingabefelder
    ...
    output("<table><tr><td>Job (Name) </td><td><input name='name' maxlength='50'></td></tr>",true);
    output("<tr><td>Lohn </td><td><input name='lohn' maxlength='6'></td></tr>",true);
...
if ($HTTP_GET_VARS['op']=="editjob"){
    ...
    if ($_GET['subop']=="save") {
        ...
        output("<form action=\"su-job.php?op=editjob&subop=save&id=".$_GET['id']."\" method='POST'>
            Name: <input name='name' value='".htmlentities(str_replace("`","``",$row['name']),ENT_QUOTES)."' maxlength='50' size='30'>`n
            Lohn: <input name='lohn' value='".htmlentities(str_replace("`","``",$row['lohn']),ENT_QUOTES)."' maxlength='6' size='10'>`n
