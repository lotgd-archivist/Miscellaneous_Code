...
//if ($session['user']['hashorse']>0 && $session[user][fedmount]<=2) addnav("f?{$playermount['mountname']} füttern (`^$futtercost`0 Gold)","stableshaus.php?op=futter");
if ($session['user']['petid']>0) addnav("t?{$playerpet['name']} füttern","stableshaus.php?op=futterpet");

if ($_GET[op]==""){
    checkday();
    output("`7Du gehst durch die Tür, auf der Haustiere steht und betrittst einen großen Raum.
    Darin kümmert sich Meruk, ein stämmig wirkender Zwerg, um verschiedene Drachen.
    `n`n
    ...
    kann ich für disch tun? Heb hia de Hausviecha. Unn wenne nix finds, komm wieda wenne meh groß odda meh reichig bis.`7\"");
        ...
        if (count($pet)==0) {
            ...
            output("`n");
            addnav("Dieses Tier kaufen","stableshaus.php?op=buypet&id={$pet['id']}");
        }
    ...
    if (db_num_rows($result)<=0){
        ...
        output("`n");
        addnav("Dieses Tier kaufen","stableshaus.php?op=buymount&id={$mount['mountid']}");
    }
        ...
        if (count($pet)==0) {
            ...
            if (
                ...
                output("`7Meruk schaut dich schief von der Seite an. \"`&Ähm, was gläubst du was du hier machst? Kanns u nich sehen, dass {$pet['name']} `^{$pet['gold']}`& Gold und `%{$pet['gems']}`& Edelsteine kostet?`7\"");
            }
            else {
                $feeddays = getsetting("daysperday",4);
                if ($session['user']['petid']>0) {
                    output("`7Du übergibst dein(e/n) {$playerpet['name']} und bezahlst den Preis für dein neues Tier. Meruk führt ein(e/n) schöne(n/s) neue(n/s) `&{$pet['name']}`7  für dich heraus und gibt dir Futter für $feeddays Tage dazu!`n`n");
                }
                else {
                    output("`7Du bezahlst den Preis für dein neues Tier und Meruk führt ein(e/n) schöne(n/s) neue(n/s) `&{$pet['name']}`7 für dich heraus und gibt dir Futter für $feeddays Tage dazu!`n`n");
                }
                // delete old pet
    ...
    if (db_num_rows($result)<=0){
        ...
        if ( 
            ...
            output("`7Meruk schaut dich schief von der Seite an. \"`&Ähm, was gläubst du was du hier machst? Kanns u nich sehen, dass {$mount['mountname']} `^{$mount['mountcostgold']}`& Gold und `%{$mount['mountcostgems']}`& Edelsteine kostet?`7\"");
        }else{
            if ($session['user']['hashorse']>0){
                output("`7Du übergibst dein(e/n) {$playermount['mountname']} und bezahlst den Preis für dein neues Tier. Meruk führt ein(e/n) schöne(n/s) neue(n/s) `&{$mount['mountname']}`7  für dich heraus!`n`n");
                $session[user][reputation]--;
            }else{
                output("`7Du bezahlst den Preis für dein neues Tier und Meruk führt ein(e/n) schöne(n/s) neue(n/s) `&{$mount['mountname']}`7 für dich heraus!`n`n");
            }
    ...
    $session[user][reputation]-=2;
        ...
        if (empty($_POST['days'])) {
            output('Das Futter kostet `^'.$playerpet['value1'].' Gold`0 pro Tag.`n');
            output('<form action="stableshaus.php?op=futterpet" method="post">',true);
            ...
            output('</form>',true);
            addnav('','stableshaus.php?op=futterpet');
        }
        else {
            $days = (int)$_POST['days'];
            if ($session['user']['gold']>=$playerpet['value1']*$days /*&& $session['user']['gems']>=$playerpet['value2']*$days*/) {
                ...
                output('Meruk nimmt die '.$coststr.' und gibt dir genug Futter, um dein(e/n) '.$playerpet['name'].' die nächsten '.$days.' Tage zu versorgen.`n');
                ...
                $session['user']['petfeed'] = date('Y-m-d H:i:s',$newtime);
                    ...
                    output('`7Du kannst das Futter nicht bezahlen. Meruk weigert sich, dein Tier für dich durchzufüttern.');
            }
        }
    ...
    if ($session[user][gold]>=$futtercost) {
            ...
            output("Dein {$playermount['mountname']} ist satt und rührt das vorgesetzte Futter nicht an. Darum gibt Meruk dir dein Gold zurück.");
            ...
            output("Da aber noch über die Hälfte des Futters übrig ist, gibt dir Meruk 50% Preisnachlass.`nDu bezahlst nur $futtercost Gold.");
            ...
            output("Dein {$playermount['mountname']} ist vollständig regeneriert und du gibst Meruk die $futtercost Gold."); 
            $session[user][reputation]--;
        }
               $session['bufflist']['mount']=$buff;
        $session[user][fedmount]++;
    } else {
        output("`7Du hast nicht genug Gold dabei, um das Futter zu bezahlen. Meruk weigert sich dein Tier für dich durchzufüttern und empfiehlt dir, im Wald nach einer grasbewachsenen Lichtung zu suchen.");
    }
}
if ($session[user][superuser]>=0) {$sql = "SELECT mountname,mountid,mountcategory FROM mounts WHERE mountactive=1 AND mountcategory='Haustiere' ORDER BY mountcategory,mountcostgems,mountcostgold";

}else{ 
$sql = "SELECT mountname,mountid,mountcategory FROM mounts WHERE mountactive=1 AND mountcategory='Haustiere' AND mountdk <= $userdk AND mountdp <= $userdp AND mountcostgold <= $usergold AND mountcostgems <= $usergems ORDER BY mountcategory,mountcostgems,mountcostgold";
}
$result = db_query($sql);
$category="";
for ($i=0;$i<db_num_rows($result);$i++){
    ...
    addnav("Betrachte {$row['mountname']}`0","stableshaus.php?op=examine&id={$row['mountid']}");
}
if ($session['user']['housekey']>0) {
        ...
        if (db_num_rows($result)>0) {
            ...
            while ($row = db_fetch_assoc($result)) {
                addnav("Betrachte {$row['name']}`0",'stableshaus.php?op=examinepet&id='.$row['id']);
            }
        }
    }
if ($session['user']['hashorse']>0){
    output("`n`nMeruk bietet dir `^$repaygold`& Gold und `%$repaygems`& Edelsteine für dein(e/n) {$playermount['mountname']}.");
    addnav("Sonstiges");
    addnav("Verkaufe {$playermount['mountname']}","stableshaus.php?op=sellmount");
}
if ($session['user']['petid']>0) {
        if ($session['user']['hashorse']==0) addnav("Sonstiges");
        output("`n`nMeruk bietet dir `%$petrepaygems`7 Edelsteine für dein(e/n) {$playerpet['name']}.");
        addnav("Verkaufe {$playerpet['name']}","stableshaus.php?op=sellpet");
    }
