...
// output("`b`c`\$IN ÜBERARBEITUNG`b`c`0`n");
    if ($session[user][seenlover]){
        ...
        addnav("","gardens.php?op=flirt1");
          $sql = "SELECT acctid,name,sex,level,crace,login,marriedto,charisma FROM accounts WHERE 
        $search
        (locked=0) AND

        (alive=1) AND 
        (acctid <> ".$session[user][acctid].") AND
        (laston > '".date("Y-m-d H:i:s",strtotime(date("r")."-34600000 sec"))."' OR (charisma=4294967295 AND acctid=".$session[user][marriedto].") )
    ...
    if (db_num_rows($result)>0){
        ...
        //if ($session[user][lastip]==$row[lastip] || ($session[user][emailaddress]==$row[emailaddress] && $row[emailaddress])){
            output("`\$`bDas geht doch nicht!!`b Du kannst doch nicht mit deinen eigenen Charakteren oder deiner eigenen Familie flirten!");
             addnav("Zurück zum Garten","gardens.php");
        //} else if (($session[user][race]=='Elf' && $row[race]=='Zwerg') || ($session[user][race]=='Zwerg' && $row[race]=='Elf')){
    ...
    output("`n`n");
    */
    output("`n`n`c`dDu `Ltr`êittst durch einen Luftstrom und so`Lfo`drt`n
um`Lfä`êngt dich eine Ruhe und ein Friede`Ln, d`den`n
du `Lau`êf ganz Arda noch nie gespürt `Lha`dst.`n
An `Ldi`êesem wundervollen Ort scheine`Ln d`die`n
Gö`Ltt`êer zusammengearbeitet zu haben, de`Lnn `dso`n
et`Lwa`ês wunderschönes kann nicht aus dem Ni`Lch`dts`n
en`Lts`êtanden sein. Wasserfälle, die fröhlich i`Ln d`den`n
Hi`Lmm`êel plätschern und schöne rosa B`Läu`dme,`n
di`Le d`êie Wege säumen und Schatten spe`Lnd`den.`n
We`Lnn `êdu nach oben siehst, kannst du b`Lun`dte`n
Wo`Llk`êen in den unterschiedlichsten Formen s`Leh`den`n
un`Ld d`êie das ganze abrunden. Außerdem liegt `Lhi`der`n
ei`Lne `êwunderschöne, romantische Stimmung i`Ln d`der`n
Lu`Lft. `êOb das wohl an den verliebten Pärchen l`Lie`dgt,`n
di`Le h`êier gleich in Scharen umherschlen`Lde`drn?`c`n`n ");
    ...
    addnav("Der Obstgarten","pilzfee.php");
    addnav("Steintroll","steintroll.php");
