    ...
    if ($_POST[charname] != "") {
        ...
        if (db_num_rows($result) > 0) {
            ...
            if (trim($row[emailaddress]) != "") {
                ...
                mail($row[emailaddress], "LoGD Account Verification", "Jemand von " . $_SERVER['REMOTE_ADDR'] . " hat ein vergessenes Passwort von deinem Accoount angefordert.  Wenn du das warst, ist hier dein" . " Link. Du kannst damit einloggen und dein Passwort im Profil vom Dorfplatz aus einstellen.\n\n" . "Wenn du diese E-Mail nicht angefordert hast, keine Panik! Du hast sie bekommen, sonst niemand." . "\n\n  http://" . $_SERVER['SERVER_NAME'] . $_SERVER['SCRIPT_NAME'] . "?op=val&id=$row[emailvalidation]\n\nDanke für's Spielen!", "From: " . getsetting("gameadminemail", "postmaster@localhost.com"));
        ...
        if (!$blockaccount) {
            ...
            if (db_num_rows($result) > 0) {
                ...
                for ($i = 0; $i <= $user_amount; $i++); {
                    //check usage of name
                    if (strtolower($chatname) == strtolower($users[$i]['userName']))
                        $username_used = 1;
                }
                ...
                if (!$username_used and !$password_error) {
                    ...
                    //Erstellt mit Arda von ' . $shortname . '
$users[' . $next_user . '] = array();
$users[' . $next_user . '][\'userRole\'] = AJAX_CHAT_USER;
$users[' . $next_user . '][\'userName\'] = \'' . $chatname . '\';
$users[' . $next_user . '][\'password\'] = \'' . $HTTP_POST_VARS[pass1] . '\';
...
?>';
                ...
                //ende
                output("`n`nBitte beachte das dein Chat Account syncron zu deinem Game Account ist( ".$shortname." )");
                $title = ($HTTP_POST_VARS[sex] ? "Fremde" : "Fremder");
                ...
                if (db_affected_rows(LINK) <= 0) {
                    ...
                    if ($emailverification != "") {
                        mail($_POST[email], "LoGD Account Verification", "Um deinen LoGD-Account freizuschalten, musst du nur noch auf den folgenden Link klicken.\n\n  http://" . $_SERVER['SERVER_NAME'] . $_SERVER['SCRIPT_NAME'] . "?op=val&id=$emailverification\n\nDanke für's Spielen!", "From: " . getsetting("gameadminemail", "postmaster@localhost.com"));
