...
page_header("Das singende Gasthaus");
output("<span style='color: #9900FF'>",true);
output("`c`b`wD`\$a`Qs `^s`ii`@n`Og`Se`sn`Pd`fe`i `#G`Fa`Ys`kt`Jh`Ma`âu`%s`b`c");
if ($HTTP_GET_VARS[op]=="strolldown"){
        output("Wiedermal bereit für's Abenteuer schlenderst du die Treppen der Schenke runter!  ");
}
if ($HTTP_GET_VARS[op]==""){
        output("`QSchon durch die geschlossene Tür der Schenke hörst du, wie das Haus seine Gäste dazu zu animieren versucht, mit ihm ein weiteres Trinklied anzustimmen. Der große, früher sicher mal von Pfeifentabak und schalem Bier erfüllte Raum ist jetzt luftig und mit hellen Farben eingerichtet. Vehement verbietet sich das Gasthaus jede Art von Qualmerei in seinem Innern und besteht darauf,");
        output("`Qdass alles umgekippte Ale sofort aufgewischt wird, was natürlich viel zusätzliche Arbeit bedeutet.");
}  
if ($HTTP_GET_VARS[op]=="" || $HTTP_GET_VARS[op]=="strolldown"){
        output("  `QDu winkst einigen deiner Kumpels, die sich zu - oder wegen? - den Klängen des Gasthauses betrinken und zwinkerst ".
                                ($session[user][sex]?
                                "`^Seth `Qzu, der versucht, Cedrics strengem Blick zu entkommen, um eventuell nach ein paar Röcken zu haschen.":
                                "`5Violet`Q zu, die ein paar einheimischen Ale serviert.").
                                "  `QDer Barkeeper Cedrik steht hinter seiner Theke und quatscht mit irgendjemandem. Seine Ohren sind mit plüschigen Ohrschützern verdeckt, so dass er eigentlich immer schreit, um sich zu verständigen. Aber so muss er nicht mehr die ganze Zeit dem Gasthaus zuhören. Er selbst sagt, dass dies auf Dauer dem Gehirn schadet. Du kannst nicht genau verstehen "
                                ." `Qwas er schreit, aber es ist irgendwas über ");
        ...
        if (getsetting("pvp",1)) {
                output(" `QDag Durnick sitzt übel gelaunt mit einer kalten Pfeife fest im Mund in der Ecke. Er wird das Rauchverbot dieses agilen Hauses wohl nie verwinden. ");
        }
        output("`n`n`QDas Gasthaus verkündet zu jeder vollen Stunde mit seiner volltönenden, dröhnend-fröhlichen Stimme die Uhrzeit. Dafür blickt es immer wieder auf eine große, gelbe Uhr, die von normalen Wesen wohl kaum gelesen werden kann. Allerdings ist es auch nach wie vor ein Rätsel, wie das Gasthaus ohne Augen etwas lesen kann.
 Auf besondere Nachfrage teilt es natürlich auch die aktuelle mit.
 Momentan ist es  `6".getgametime()."`0.");
    ...
    addnav("Aufträge","quests.php");
        if ( $session['user']['eisenstange']==0) addnav("Eisenstange kaufen","inn.php?op=eisen");
  ...
  switch($HTTP_GET_VARS[op]){
        ...
        if ($_GET[act]=="del"){
                ...
                output("\"`@Du möchtest eine Nachricht am schwarzen Brett hinterlassen, ja? Wie lang soll die Nachricht denn dort zu sehen sein? Aber denk dran: Das sind Arda-Tage.`Q\" fragt dich Cedrik fordernd und nennt die Preise.");
        ...
        if (!$session['user']['prefs']['nosounds']) output("<embed src=\"media/giggle.wav\" width=10 height=10 autostart=true loop=false hidden=true volume=100>",true);
            ...
            output("`n`3Du gibst Cedrik 2 Edelsteine und er gibt dir eine Eisenstange. Doch was willst du eigentlich damit?");
                                   } else {            output("`n`3Du hast leider keine 2 Edelsteine, die du für die Stange bezahlen musst.");
     }
        break;
                case "seth":
                        /*
      ...
      if ($HTTP_GET_VARS[subop]=="hear"){
        ...
        if($session[user][seenbard]){
         ...
         // addnav("Return to the inn","inn.php");
          ...
          switch ($rnd){
            case 0:
              output("`@Grüner Drache`^ ist grün.`n`@Grüner Drache`^ ist wild.`n`@Grünen Phoenix`^ wünsch ich mir gekillt. ");
                                                ...
                                                if ($gemcount>0){
                                                        switch($HTTP_POST_VARS[wish]){
                                                                ...
                                                                case 4:
                                                                        ...
                                                                        break;
                                                                case 5:
                                                                        if ($session['user']['race']=='Troll') $session['user']['attack']--;
