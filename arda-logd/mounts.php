...
addnav("Ein Tier erstellen","mounts.php?op=add");
...
if ($_GET['op']==""){
    $sql = "SELECT * FROM mounts ORDER BY mountcategory, mountcostgems, mountcostgold, mountdp";
    output("<table>",true);
    output("<tr><td>Aktion</td><td>Name</td><td>Kosten</td><td>&nbsp;</td></tr>",true);
    $result = db_query($sql);
    $cat = "";
    for ($i=0;$i<db_num_rows($result);$i++){
        ...
        output("<tr>",true);
        output("<td>[ <a href='mounts.php?op=edit&id={$row['mountid']}'>Editieren</a> |",true);
        ...
        output("<td>{$row['mountcostgems']} Edelsteine, {$row['mountcostgold']} Gold, {$row['mountdp']} DP</td>",true);
...
function mountform($mount){
    ...
    $output.="<table>";
    $output.="<tr><td>Mount PK:</td><td><input name='mount[mountdk]' value=\"".htmlentities($mount['mountdk'])."\"></td></tr>";
    $output.="<tr><td>Mount DP:</td><td><input name='mount[mountdp]' value=\"".htmlentities($mount['mountdp'])."\"></td></tr>";    
