...
if ($_GET[op]=="mail"){
    ...
    elseif ($_GET['act']=='saveban') {
        ...
        foreach ($vals AS $das) {
            ...
            $sql.="\"$_POST[reason]\")";
            if ($_POST[type]=="ip"){
                if (substr($_SERVER['REMOTE_ADDR'],0,strlen($das)) == $das){
                    $sql = "";
    ...
    elseif ($_GET['act']=='dodeleteuser') {
        ...
        foreach ($userid AS $das) {
            deleteuser($das,$_POST['stdreason'],$_POST['reason'],$_POST['savereason']);
        }
        output('`n');
    }
    else output('`n');
    ...
    if ($_GET['searchby']!='ip') {
        $sql = 'SELECT emailaddress FROM accounts WHERE emailaddress!="" GROUP BY emailaddress HAVING COUNT(*) > 1';
        $result = db_query($sql) or die(db_error(LINK));
        while ($row = db_fetch_assoc($result)) {
    ...
    $ip = $id = $users = array();
    $sql = 'SELECT acctid,name,lastip,emailaddress,dragonkills,level FROM accounts WHERE (lastip IN (-1'.$in_ip.') OR emailaddress IN (-1'.$in_id.')) AND locked="0" ORDER BY dragonkills ASC, level ASC';
    $result = db_query($sql) or die(db_error(LINK));
    while ($row = db_fetch_assoc($result)) {
        if ((!isset($id[$row['emailaddress']]) || $_GET['searchby']=='ip') && (!isset($ip[$row['lastip']]) || $_GET['searchby']=='id')) {
            if ($_GET['searchby']!='id') $ip[$row['lastip']] = count($users);
            if ($_GET['searchby']!='ip') $id[$row['emailaddress']] = count($users);
            $users[] = array($row);
        }
        elseif (isset($id[$row['emailaddress']])) {
            $ip[$row['lastip']] = $id[$row['emailaddress']];

            $users[$id[$row['emailaddress']]][] = $row;
        }
        else {
            $id[$row['emailaddress']] = $ip[$row['lastip']];
    ...
    foreach ($users AS $list) {
        ...
        $ips = $ids = $accts = array();
        foreach ($list AS $das) {
            $tmpstr .= ('<tr><td><input type="checkbox" name="userid[]" value="'.$das['acctid'].'"></td>
                            <td>'.$das['name'].'</td>
                            <td>'.$das['lastip'].'</td>
                            <td>'.$das['emailaddress'].'</td>
                            <td>'.$das['dragonkills'].'</td>
                            <td>'.$das['level'].'</td></tr>');
        }
