...
output("<br><br><a href='biopopup_backup.php?char=" . $row[login] . "&op=data' name='Datenblatt'>Datenblatt & Bio</a>", true);
output(" | ");
output("<a href='biopopup_backup.php?char=" . $row[login] . "&op=diary' name='diary'>Tagebuch</a><br>", true);

if ($HTTP_GET_VARS[op] == "diary") {
    ...
    if ($row['bio'] > "") {
        ...
        output("<h2 style='color:#069' >Biografie</h2>", true);
        output("<br>", true);
    ...
    if ($row['biopet'] > "") {
        ...
        output("<h2 style='color:#069'>Tierbiografie</h2>", true);
        output("<br>", true);
