...
if ($HTTP_GET_VARS['op']==""){
addnav("Zurück zum Dorf","univier.php");
...
if ($session['user']['aubid']==1 && $session[user][lektion]<50) { addnav("`5Zum Unterricht","uni.php?op=aub1"); }
if ($session['user']['aubid']==2 && $session[user][lektion]<75) { addnav("`6Zur Bibliothek","uni.php?op=aub2"); }
if ($session['user']['aubid']==3 && $session[user][lektion]<100) { addnav("`@In den Hörsaal","uni.php?op=aub3"); }
...
output("Du betrittst die `8Universität`0, das weitaus grösste Gebäude weit und breit.`n");
output("Sofort als dein Fuß über die Schwelle des Eingangsbogen der grossen Tür tritt`n");
output("kommt dir ein kahlköpfiger, überarbeitet wirkender Proffesor entgegen: \"`^Ähm...was kann ich für sie tun?`0\" ");
...
output("selbst an hohe Berufe wie Anwalt etc. können sie nach dem Studium ihre Berwerbung schicken!!!!!`0\"");
}
}
...
if ($session[user][aubid]==1 && $session[user][lektion]>=50){
...
if ($session[user][aubid]==2 && $session[user][lektion]>=75){
...
if ($session[user]['lektion']>=50){
output("Du hast die Mittlere Reife gemacht!");
$session[user]['lektion']=400;
...
viewcommentary("uni2","Flüstern",15);
}
}
...
if ($session[user][turns]>=2){
output("Du hast eine Lektion gelernt mein Junge..gute Arbeit!");
addnav("Zurück zum Unterricht","uni.php?op=aub2");
$session[user][lektion]+=1;
$session[user][turns]-=2;
} else {
output("Du bist zu müde um heute noch zu Lernen..");
...
if ($session[user][turns]>=2){
output("Du hast eine Lektion gelernt mein Junge..gute Arbeit!");
addnav("Zurück zum Unterricht","uni.php?op=aub3");
$session[user][lektion]+=1;
$session[user][turns]-=2;
} else {
output("Du bist zu müde um heute noch zu Lernen..");
