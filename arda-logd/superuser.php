...
if ($_GET[op]=="checkcommentary"){
        ...
        if ($session[user][superuser]>=2) addnav("K?Aktuelle Kommentare","superuser.php?op=checkcommentary");
    if ($session[user][superuser]>=3) addnav("Orte ansehen","placelist.php?op=viewcom") ;
        addnav("B?SpielerBiografien","bios.php");
        addnav("B?SpielerTierBiografien","petbios.php");
        addnav("Schwarzes Brett","innboard.php");
        if (getsetting("avatare",0)==1) addnav("Avatare","avatars.php");
        if ($session[user][superuser]>=4) addnav("Spendenseite","donators.php");
        if ($session[user][superuser]>=4) addnav("Retitler","retitle.php");
        if ($session[user][superuser]>=4) addnav("Faillog & Mail","logs.php");
        ...
        //if ($session['user']['superuser']>=3) addnav("Schlumpfdingens","multi.php");
        if ($session['user']['superuser']>=4) addnav("Cachecleaner","cachecleaner.php");
    addnav("Todo-Liste","todolist.php");
        addnav("Editoren");
       if ($session[user][login] == "Ellalith" or $session[user][login] == "Kibarashi" or $session[user][login] == "Viconia") {
            addnav("User Editor","user.php");
        }        
    //addnav("User zulassen","usereinlass.php");
    //addnav("Bioeditor","userbio.php");
    addnav("Biobewertungen","biobewertung.php");
        ...
        if ($session[user][superuser]>=3) addnav("Item Editor","itemeditor.php");
        if ($session[user][superuser]>=3) addnav("Rasseneditor","races.php?editorStep=show");
        if ($session[user][superuser]>=3) addnav("Template Editor","templateedit.php");
        if ($session[user][superuser]>=3) addnav("Gildeneditor","gildentool.php");
        ...
        if ($session[user][superuser]>=3) addnav("Wortfilter","badword.php");
        if ($session['user']['superuser']>=3) addnav("Specialeditor","waldspecialeditor.php");
    if ($session[user][superuser]>=3)addnav("Quest Editor","questeditor.php");
        ...
        addnav ("Universität `$ *BETA*","uni.php");
        if ($session[user][superuser]>=4) addnav("`\$Die Stadt verteidigen","dorfangriff.php");
        addnav("Mechanik");

        if ($session[user][superuser]>=4) addnav("Spieleinstellungen","configuration.php");
        addnav("Herführende URLs","referers.php");
        if ($session[user][superuser]>=2) addnav('Massenmails','massmail.php');
        if ($session[user][superuser]>=3) addnav("Statistiken","stats.php");
