...
if ($_GET[op]=="search"){
        $sql = "SELECT acctid FROM accounts WHERE einlass LIKE '0'";
        ...
        if (db_num_rows($result)<=0){
                ...
                //redirect("usereinlass.php?op=edit&userid=$row[acctid]");
                $_GET[op]="";
                $_GET['page']=0;
        }else{
                $_GET[op]="";
...
page_header("User zulassen");
        output("<form action='usereinlass.php?op=search' method='POST'>Suche in allen Feldern: <input name='q' id='q'><input type='submit' class='button'></form>",true);
        output("<script language='JavaScript'>document.getElementById('q').focus();</script>",true);
        addnav("","usereinlass.php?op=search");
addnav("G?Zurück zur Grotte","superuser.php");
addnav("W?Zurück zum Weltlichen","village.php");
//addnav("Verbannung","usereinlass.php?op=setupban");
//addnav("Verbannungen anzeigen/entfernen","usereinlass.php?op=removeban");
//addnav("Benutzereditor","usereinlass.php");
...
while ($row[count]>0){
        $page++;
        addnav("$page Seite $page","usereinlass.php?page=".($page-1)."&sort=$_GET[sort]");
        $row[count]-=100;
}
...
$userinfo = array(
        ...
        "emailaddress"=>"Email Adresse,viewonly",
        //"locked"=>"Account gesperrt,bool",
        //"banoverride"=>"Verbannungen übergehen,bool",
         "einlass"=>"Zulassen,bool",
        //"superuser"=>"Superuser,enum,0,Standard Spieltage pro Kalendertag,1,Stadtwache,2,Biowächter,3,Mod,4,Admin",
...
//        "race"=>"Race,enum,0,Unknown,1,Troll,2,Elf,3,Human,4,Dwarf,5,Echse",
        ...
        //"dragonkills"=>"Phoenixkills,int",
        //"dragonage"=>"Alter beim letzten Phoenixkill,int",
        //"bestdragonage"=>"Jüngstes Alter bei einem Phoenixkill,int",
        //"veri"=>"Verifizierung (0=Nein 1=Ja),viewonly",
        //"bio"=>"Biographie,textarea,50,10",
        //"avatar"=>"Avatar,textarea,50,10",
        /*"Kindersystem,title",
        "ssstatus"=>"Schwanger?,enum,0,Nein,1,Ja",
        "ssmonat"=>"Schwangerschafts Monat (16 Std),int",
...
"schuhe"=>"Schuhe,enum,0,Keine,1,Wanderschuhe,2,Sportschuhe,3,Lederstiefel,4,Drachenleder Stiefel",
        ...
        "dragonpoints"=>"Eingesetzte Phoenixpunkte,viewonly",
...
if ($_GET[op]=="lasthit"){
   ...
   exit();
        ...
        output("<form action='usereinlass.php?op=special&userid=$_GET[userid]".($_GET['returnpetition']!=""?"&returnpetition={$_GET['returnpetition']}":"")."' method='POST'>",true);
        addnav("","usereinlass.php?op=special&userid=$_GET[userid]".($_GET['returnpetition']!=""?"&returnpetition={$_GET['returnpetition']}":"")."");
 ...
 if (!ac_check($row)){
        ...
        addnav("Letzten Treffer anzeigen","usereinlass.php?op=lasthit&userid={$_GET['userid']}",false,true);
        output("<form action='usereinlass.php?op=save&userid=$_GET[userid]".($_GET['returnpetition']!=""?"&returnpetition={$_GET['returnpetition']}":"")."' method='POST'>",true);
        addnav("","usereinlass.php?op=save&userid=$_GET[userid]".($_GET['returnpetition']!=""?"&returnpetition={$_GET['returnpetition']}":"")."");
        addnav("","usereinlass.php?op=edit&userid=$_GET[userid]".($_GET['returnpetition']!=""?"&returnpetition={$_GET['returnpetition']}":"")."");
        addnav("Verbannen","usereinlass.php?op=setupban&userid=$row[acctid]");
        addnav("Debug-Log anzeigen","usereinlass.php?op=debuglog&userid={$_GET['userid']}");
 if (ac_check($row)){
        ...
        output("</form>",true);
        output("<iframe src='usereinlass.php?op=lasthit&userid={$_GET['userid']}' width='100%' height='400'>Dein Browser muss iframes unterstützen, um die letzte Seite des Users anzeigen zu können. Benutze den Link im Menü stattdessen.</iframe>",true);
        addnav("","usereinlass.php?op=lasthit&userid={$_GET['userid']}");
      ...
      fclose($file);
        ...
        db_query($sql);
        if ($_GET['returnpetition']==""){
                // redirect("usereinlass.php?".db_affected_rows());
                redirect("usereinlass.php");
        }else{
        ...
        output("<form action='usereinlass.php?op=saveban' method='POST'>",true);
        ...
        addnav("","usereinlass.php?op=saveban");
        ...
        db_query($sql);
        //output($sql);
        redirect("usereinlass.php?op=removeban");
        ...
        for ($i=0;$i<db_num_rows($result);$i++){
                $row = db_fetch_assoc($result);
                output("<tr class='".($i%2?"trlight":"trdark")."'><td><a href='usereinlass.php?op=delban&ipfilter=".URLEncode($row[ipfilter])."&uniqueid=".URLEncode($row[uniqueid])."'>Bann&nbsp;aufheben</a>",true);
                addnav("","usereinlass.php?op=delban&ipfilter=".URLEncode($row[ipfilter])."&uniqueid=".URLEncode($row[uniqueid]));
                output("</td><td>",true);
                output($row[ipfilter]);
                output($row[uniqueid]);
                output("</td><td>",true);
        ...
        addnav("User Info bearbeiten","usereinlass.php?op=edit&userid=$id");
        $result = db_query($sql);
        $odate = "";
        for ($i=0; $i<db_num_rows($result); $i++) {
                $row = db_fetch_assoc($result);
        ...
        if (isset($_GET['page'])){
                ...
                output("<tr>
                <td>Ops</td>
                <td><a href='usereinlass.php?sort=login'>Login</a></td>

                <td><a href='usereinlass.php?sort=name'>Name</a></td>
                <td><a href='usereinlass.php?sort=level'>Lev</a></td>
                <td><a href='usereinlass.php?sort=laston'>Zuletzt da</a></td>
                <td><a href='usereinlass.php?sort=gentimecount'>Treffer</a></td>
                <td><a href='usereinlass.php?sort=lastip'>IP</a></td>
                <td><a href='usereinlass.php?sort=uniqueid'>ID</a></td>
                <td><a href='usereinlass.php?sort=emailaddress'>E-Mail</a></td>
                </tr>",true);
                addnav("","usereinlass.php?sort=login");
                addnav("","usereinlass.php?sort=name");
                addnav("","usereinlass.php?sort=level");
                addnav("","usereinlass.php?sort=laston");
                addnav("","usereinlass.php?sort=gentimecount");
                addnav("","usereinlass.php?sort=lastip");
                addnav("","usereinlass.php?sort=uniqueid");
                $rn=0;
                for ($i=0;$i<db_num_rows($result);$i++){
                        $row=db_fetch_assoc($result);
                        ...
                        output("<td>",true);
                        output("[<a href='usereinlass.php?op=edit&userid=$row[acctid]'>Edit</a>|",true);
                        addnav("","usereinlass.php?op=edit&userid=$row[acctid]");
                        addnav("","usereinlass.php?op=del&userid=$row[acctid]");
                        addnav("","usereinlass.php?op=setupban&userid=$row[acctid]");
                        addnav("","usereinlass.php?op=debuglog&userid=$row[acctid]");
                        output("</td><td>",true);
                        output($row[login]);
                        output("</td><td>",true);
                        output($row[name]);
