...
/********Armourer********/ 
    ...
    if (strchr($session[user][armor],"High-Grade")){ 
        ...
        $cost = $session[user][armordef] * 400;
...
/********Blacksmith********/ 
    ...
    if (strchr($session[user][weapon],"High-Grade")){ 
        ...
        $cost = $session[user][weapondmg] * 400; 
...
/********Guard Fight********/
    ...
    if (e_rand(1,100) <95){ 
        ...
        output("`#Aber du gibst dir einen Ruck, lässt deine Ängste hinter dir und läufst weiter auf die Burg zu. Als du näher kommst, bemerkst du, daß ein Greif vor dem Tor Wache hält. Du kommst dort an und die mystische Kreatur spricht dich an. `%'Willkommen in der Orkburg! Wenn du hier rein willst, musst du deine Tapferkeit entweder schon beim Kampf mit dem `wdun`Rklen `wPhoe`Rnix`% bewiesen haben, oder du musst mich in einem fairen Kampf besiegen!'"); 
        if ($session['user']['dragonkills']>0){
            output("`n`n`^Da du den Phoenix bereits mindestens 1x gekillt hast, darfst du passieren."); 
