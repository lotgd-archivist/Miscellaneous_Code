...
$balance = getsetting("creaturebalance", 0.31);
...
if ($session[user][superuser]>2 && $HTTP_GET_VARS[specialinc]!=""){
...
//        output(serialize($badguy));
        if ($victory){
                ...
                if ($session['user']['gildenactive']!=0) { $sql = "UPDATE gilden SET gold=gold+100 WHERE gildenid = {$session['user']['memberid']}";
                db_query($sql);
                output("Deine Gilde bekommt 100 Gold.`n"); }
        ...
        $unique1=1; 
        $unique2=1000;
        $uniqueran = e_rand($unique1,$unique2);                
        //find something
                $findit=e_rand(1,27);
                if ($findit == 2) { //gem
...
output("`n`Q`bBeim Durchsuchen von $badguy[creaturename] `Qfindest du das Unique `%$row3[name]`Q!`b`n`n`#"); 
                ...
                //if ($session['user']['gildenactive']!=0) { $sql = "UPDATE gilden SET gold=gold+100 WHERE gildenid = {$session['user']['memberid']}";
db_query($sql);
                //output("Deine Gilde bekommt 100 Gold."); }
