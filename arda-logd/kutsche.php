...
page_header("Kutschenhändler");

if ($_GET[op]=="buy"){ // Wig-Wam Bam
    if (!$_GET[id]){
        $sorti=($_GET[sorti]?"$_GET[sorti]":"class DESC, name");
        output("`qStolz präsentiert dir der Händler `tMortimer`q seinen Wagen. Zu jeder der Kutschen scheint er eine kleine Geschichte zu kennen. Dabei scheint er auffällig oft darauf ");
        ...
        if (!$_GET[limit]){
            ...
            $page=(int)$_GET[limit];
            addnav("Vorherige Waren","kutsche.php?op=buy&sorti=$sorti&limit=".($page-1));
        }
        $limit="".($page*$ppp).",".($ppp+1);
        $sql="SELECT * FROM items WHERE owner=0 AND class='Kutsche.Prot' ORDER BY $sorti ASC LIMIT $limit";
        $result=db_query($sql);
        if (db_num_rows($result)>$ppp) addnav("Mehr Waren","kutsche.php?op=buy&sorti=$sorti&limit=".($page+1));
        if (db_num_rows($result)){
            output("<table border='0' cellpadding='2' cellspacing='2'>",true);
            output("<tr class='trhead'><td>`b<a href='kutsche.php?op=buy&sorti=name&limit=$_GET[limit]'>Name</a>`b</td><td>`b<a href='kutsche.php?op=buy&sorti=".urlencode("gems ASC,gold")."&limit=$_GET[limit]'>Preis</a>`b</td></tr>",true);
            addnav("","kutsche.php?op=buy&sorti=name&limit=$_GET[limit]");
            addnav("","kutsche.php?op=buy&sorti=".urlencode("gems ASC,gold")."&limit=$_GET[limit]");
            //addnav("","kutsche.php?op=buy&sorti=".urlencode("class DESC,name")."&limit=$_GET[limit]");
            for ($i=0;$i<db_num_rows($result);$i++){
                  $row = db_fetch_assoc($result);
                $bgcolor=($i%2==1?"trlight":"trdark");
                output("<tr class='$bgcolor'><td><a href='kutsche.php?op=buy&id=$row[id]'>$row[name]</a></td><td align='right'>`^$row[gold]`0 Gold, `#$row[gems]`0 Edelsteine</td></tr><tr class='$bgcolor'><td colspan='3'>$row[description]</td></tr>",true);
                addnav("","kutsche.php?op=buy&id=$row[id]");
            }
            output("</table>",true);
        
        } else {
            output("`qDa `tMortimer `qheute schon ein gutes Geschäft gemacht hat, will er sich leider nicht von seinen verbliebenen Sachen trennen. Enttäuscht schlenderst du zurück zum Dorfplatz.");
        }
        ...
        if ($session[user][gems]<$row[gems] || $session[user][gold]<$row[gold]){
            ...
            addnav("Etwas anderes kaufen","kutsche.php?op=buy");
        }else if ($row['class']=="Kutsche.Prot" && $session[user][housekey]<=0 ){
            ...
            addnav("Etwas anderes kaufen","kutsche.php?op=buy");
        }else if (db_num_rows(db_query("SELECT id FROM items WHERE name='$row[name]' AND owner=".$session[user][acctid]." AND class='Kutsche'"))>0){
            ...
            if ($row['class']=="Kutsche.Prot") output(" Er ist dir noch kurz beim Transport behilflich, bevor er sich seinem nächsten Kunden zuwendet.");
            addnav("Mehr kaufen","kutsche.php?op=buy");
            $sql="UPDATE items SET owner=".$session[user][acctid]." WHERE id=$_GET[id]";
            // insert SQL for special classes here to reset their values
            if ($row['class']=="Kutsche.Prot") $sql="INSERT INTO items(name,class,owner,value1,gold,gems,description) VALUES ('$row[name]','Kutsche',".$session[user][acctid].",".$session[user][house].",1,".(round($row[gems]/2)).",'$row[description]')";
            $session[user][gold]-=$row[gold];
            $session[user][gems]-=$row[gems];
            db_query($sql);
        }
    }
    addnav("Zurück","kutsche.php");
    ...
    if (!$_GET[id]){
        ...
        $sql="SELECT * FROM items WHERE owner=".$session[user][acctid]." AND (gold>0 OR gems>0) AND class='Kutsche'";
        $result=db_query($sql);
        if (db_num_rows($result)){
            ...
            for ($i=0;$i<db_num_rows($result);$i++){
                  $row = db_fetch_assoc($result);
                $bgcolor=($i%2==1?"trlight":"trdark");
                output("<tr class='$bgcolor'><td><a href='kutsche.php?op=sell&id=$row[id]'>$row[name]</a></td><td align='right'>`^$row[gold]`0 Gold, `#$row[gems]`0 Edelsteine</td></tr><tr class='$bgcolor'><td colspan='2'>$row[description]</td></tr>",true);
                addnav("","kutsche.php?op=sell&id=$row[id]");
            }
            output("</table>",true);
        
        } else {
            output("Du hast aber nichts, was `tMortimer`q interessieren würde. Enttäuscht schlenderst du zurück zum Dorfplatz.");
        }
    }else{ // Hell Raiser
        $sql="SELECT * FROM items WHERE id=$_GET[id]";
        $result=db_query($sql);
        ...
        addnav("Mehr verkaufen","kutsche.php?op=sell");
    ...
    output("`qHeute ist der Kutschenhändler `tMortimer `qwieder im Dorf! Direkt vor `!MightyE`qs Waffenladen hat er seinen Wagen aufgebaut, was MightyE sichtlich missfällt. Da er aber ");
    ...
    addnav("Waren durchstöbern","kutsche.php?op=buy");
    addnav("Etwas verkaufen","kutsche.php?op=sell");
    addnav("Inventar anzeigen","prefs.php?op=inventory&back=kutsche.php");
    addnav("Zurück zum Dorf","zwergenstadt.php");
}
page_footer();
