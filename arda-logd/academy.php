...
$akt_magiclevel = $session[user][$skills[$session[user][specialty]]] + 1;
// man faengt bei 0 an ;o)

$cost_low = (($akt_magiclevel + 1) * 50) - $session['user']['reputation'];
$cost_medium = (($akt_magiclevel + 1) * 60) - $session['user']['reputation'];
//plus ein Edelstein
$cost_high = (($akt_magiclevel + 1) * 75) - $session['user']['reputation'];
//plus 2 Edelsteine

$min_dk = 1;
// wieviele DKs muss ein User haben um eintreten zu dürfen?
...
if (($HTTP_GET_VARS[op1] <> "" && $HTTP_GET_VARS[op2] <> "") && $session['user']['dragonkills'] >= $min_dk && $session[user][seenAcademy] == 0 && $session[user][turns] > 0) {
    ...
    if ($HTTP_GET_VARS[op1] == "enter" && $HTTP_GET_VARS[op2] == "chat") {
        ...
        viewcommentary("academy", "Sprich", 15);
    }

    // check if User has enough gems/gold if he wants to learn
    // 1st Case: STUDY
    if ($HTTP_GET_VARS[op1] == "enter" && $HTTP_GET_VARS[op2] == "study" && $session[user][gold] < $cost_low) {
        output("`$`b`c Das Innere der Akademie`c`b`n`n");
        output("`n`$ Leider kannst Du den geforderten Preis nicht bezahlen.`^");
        addnav("Nochmal nachschauen", "academy.php?op1=enter&op2=0");
    }
    else if ($HTTP_GET_VARS[op1] == "enter" && $HTTP_GET_VARS[op2] == "study" && $session[user][gold] >= $cost_low) {
        // subtract costs
    ...
    // 2nd Case: PRACTICE
    if ($HTTP_GET_VARS[op1] == "enter" && $HTTP_GET_VARS[op2] == "practice" && ($session[user][gold] < $cost_medium || $session[user][gems] < 1)) {
        output("`$`b`c Das Innere der Akademie`c`b`n`n");
        output("`n`$ Leider kannst Du den geforderten Preis nicht bezahlen.`^");
        addnav("Nochmal nachschauen", "academy.php?op1=enter&op2=0");
    }
    else if ($HTTP_GET_VARS[op1] == "enter" && $HTTP_GET_VARS[op2] == "practice" && ($session[user][gold] >= $cost_medium || $session[user][gems] >= 1)) {
        // subtract costs
    ...
    // 3rd Case: WARCHILD
    if ($HTTP_GET_VARS[op1] == "enter" && $HTTP_GET_VARS[op2] == "warchild" && ($session[user][gold] < $cost_high || $session[user][gems] < 2)) {
        output("`$`b`c Das Innere der Akademie`c`b`n`n");
        output("`n`$ Leider kannst Du den geforderten Preis nicht bezahlen.`^");
        addnav("Nochmal nachschauen", "academy.php?op1=enter&op2=0");
    }
    else if ($HTTP_GET_VARS[op1] == "enter" && $HTTP_GET_VARS[op2] == "warchild" && ($session[user][gold] >= $cost_high || $session[user][gems] >= 2)) {
        // subtract costs
    ...
    if ($_GET[action] == "sell") {
        ...
        if (isset($_GET[id])) {
            ...
            if (db_num_rows($result) > $ppp)
                addnav("Mehr Zauber", "academy.php?op1=bringmetolife&actino=buy&limit=" . ($page + 1));
            if (db_num_rows($result)) {
        ...
        output("`vDurch schwere und reich verzierte Holztüren betrittst du den Zauberladen der Akademie. Hier bietet ein älterer Zauberer die Werke verschiedenster Akademie-Magier an, denen es gelungen ist, selbst magisch unbegabten " . $session['user']['race'] . "en wie dir die Anwendung ihrer Zauber zu ermöglichen.");
...
// auf jeden Fall Begrüßung und Einleitung wenn keine Params gesetzt
    ...
    elseif ($session['user']['reputation'] < -5) {
        ...
        if ($session['user']['specialty'] == 1)
            output("den `\$Dunklen Künsten`7");
        if ($session['user']['specialty'] == 2)
            output("den `%Mystischen Kräften`7");
        if ($session['user']['specialty'] == 3)
            output("`^Diebeskünsten`7");
