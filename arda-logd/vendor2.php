...
// The vendor Ülülü sells furniture for houses and buys items found at beaten monsters in the forest.
...
if ($_GET[op]=="buy"){ // Wig-Wam Bam
    if (!$_GET[id]){
        ...
        output("`qStolz präsentiert dir der Händler `tÜlülü`q seinen Wagen. Zu jedem der seltsamen Gegenstände, Artefakte und Zauber scheint er eine kleine Geschichte zu kennen. Dabei scheint er auffällig oft darauf ");
        ...
        if (!$_GET[limit]){
            ...
            $page=(int)$_GET[limit];
            addnav("Vorherige Waren","vendor2.php?op=buy&sorti=$sorti&limit=".($page-1));
        }
        $limit="".($page*$ppp).",".($ppp+1);
        $sql="SELECT * FROM items WHERE owner=0 AND (class='Schwarzmarkt') ORDER BY $sorti ASC LIMIT $limit";
        $result=db_query($sql);
        if (db_num_rows($result)>$ppp) addnav("Mehr Waren","vendor2.php?op=buy&sorti=$sorti&limit=".($page+1));
        if (db_num_rows($result)){
            output("<table border='0' cellpadding='2' cellspacing='2'>",true);
            output("<tr class='trhead'><td>`b<a href='vendor2.php?op=buy&sorti=name&limit=$_GET[limit]'>Name</a>`b</td><td>`b<a href='vendor2.php?op=buy&sorti=".urlencode("gems ASC,gold")."&limit=$_GET[limit]'>Preis</a>`b</td></tr>",true);
            addnav("","vendor2.php?op=buy&sorti=name&limit=$_GET[limit]");
            addnav("","vendor2.php?op=buy&sorti=".urlencode("gems ASC,gold")."&limit=$_GET[limit]");
            //addnav("","vendor2.php?op=buy&sorti=".urlencode("class DESC,name")."&limit=$_GET[limit]");
            for ($i=0;$i<db_num_rows($result);$i++){
                  $row = db_fetch_assoc($result);
                $bgcolor=($i%2==1?"trlight":"trdark");
                output("<tr class='$bgcolor'><td><a href='vendor2.php?op=buy&id=$row[id]'>$row[name]</a></td><td align='right'>`^$row[gold]`0 Gold, `#$row[gems]`0 Edelsteine</td></tr><tr class='$bgcolor'><td colspan='3'>$row[description]</td></tr>",true);
                addnav("","vendor2.php?op=buy&id=$row[id]");
            }
            output("</table>",true);
        
        } else {
            output("`qDa `tÜlülü `qheute schon ein gutes Geschäft gemacht hat, will er sich leider nicht von seinen verbliebenen Sachen trennen. Enttäuscht schlenderst du zurück zum Dorfplatz.");
        }
        ...
        if ($session[user][gems]<$row[gems] || $session[user][gold]<$row[gold]){
            ...
            addnav("Etwas anderes kaufen","vendor2.php?op=buy");
            ...
            addnav("Mehr kaufen","vendor2.php?op=buy");
    ...
    if (!$_GET[id]){
        ...
        $sql="SELECT * FROM items WHERE owner=".$session[user][acctid]." AND (gold>0 OR gems>0) AND class='Schwarzmarkt'";
        $result=db_query($sql);
        if (db_num_rows($result)){
            ...
            for ($i=0;$i<db_num_rows($result);$i++){
                  $row = db_fetch_assoc($result);
                $bgcolor=($i%2==1?"trlight":"trdark");
                output("<tr class='$bgcolor'><td><a href='vendor2.php?op=sell&id=$row[id]'>$row[name]</a></td><td align='right'>`^$row[gold]`0 Gold, `#$row[gems]`0 Edelsteine</td></tr><tr class='$bgcolor'><td colspan='2'>$row[description]</td></tr>",true);
                addnav("","vendor2.php?op=sell&id=$row[id]");
            }
            output("</table>",true);
        
        } else {
            output("Du hast aber nichts, was `tÜlülü`q interessieren würde. Enttäuscht schlenderst du zurück zum Dorfplatz.");
        }
    }else{ // Hell Raiser
        $sql="SELECT * FROM items WHERE id=$_GET[id]";
        $result=db_query($sql);
        ...
        addnav("Mehr verkaufen","vendor2.php?op=sell");
    ...
    output("`qHeute ist der Schwarzmarkthänder `tÜlülü `qwieder im Dorf! Direkt vor `!MightyE`qs Waffenladen hat er seinen Wagen aufgebaut, was MightyE sichtlich missfällt. Da er aber ");
    ...
    addnav("Waren durchstöbern","vendor2.php?op=buy");
    addnav("Etwas verkaufen","vendor2.php?op=sell");
    addnav("Inventar anzeigen","prefs.php?op=inventory&back=vendor2.php");
    addnav("Zurück zum Dorf","zwergenstadt.php");
}
page_footer();
