...
if ($_GET[op]=="search"){
        ...
        if (db_num_rows($result)<=0){
                ...
                //redirect("userbio.php?op=edit&userid=$row[acctid]");
                $_GET[op]="";
                $_GET['page']=0;
        }else{
                $_GET[op]="";
...
page_header("Bio-Editor");
        output("<form action='userbio.php?op=search' method='POST'>Suche in allen Feldern: <input name='q' id='q'><input type='submit' class='button'></form>",true);
        output("<script language='JavaScript'>document.getElementById('q').focus();</script>",true);
        addnav("","userbio.php?op=search");
addnav("G?Zurück zur Grotte","superuser.php");
addnav("W?Zurück zum Weltlichen","village.php");
//addnav("Verbannung","userbio.php?op=setupban");
//addnav("Verbannungen anzeigen/entfernen","userbio.php?op=removeban");
//addnav("Benutzereditor","userbio.php");
...
while ($row[count]>0){
        $page++;
        addnav("$page Seite $page","userbio.php?page=".($page-1)."&sort=$_GET[sort]");
        $row[count]-=100;
}
...
if ($_GET[op]=="lasthit"){
   ...
   exit();
        ...
        output("<form action='userbio.php?op=special&userid=$_GET[userid]".($_GET['returnpetition']!=""?"&returnpetition={$_GET['returnpetition']}":"")."' method='POST'>",true);
        addnav("","userbio.php?op=special&userid=$_GET[userid]".($_GET['returnpetition']!=""?"&returnpetition={$_GET['returnpetition']}":"")."");
 ...
 if (!ac_check($row)){
        ...
        addnav("Letzten Treffer anzeigen","userbio.php?op=lasthit&userid={$_GET['userid']}",false,true);
        output("<form action='userbio.php?op=save&userid=$_GET[userid]".($_GET['returnpetition']!=""?"&returnpetition={$_GET['returnpetition']}":"")."' method='POST'>",true);
        addnav("","userbio.php?op=save&userid=$_GET[userid]".($_GET['returnpetition']!=""?"&returnpetition={$_GET['returnpetition']}":"")."");
        addnav("","userbio.php?op=edit&userid=$_GET[userid]".($_GET['returnpetition']!=""?"&returnpetition={$_GET['returnpetition']}":"")."");
        addnav("Verbannen","userbio.php?op=setupban&userid=$row[acctid]");
        addnav("Debug-Log anzeigen","userbio.php?op=debuglog&userid={$_GET['userid']}");
 if (ac_check($row)){
        ...
        output("</form>",true);
        output("<iframe src='userbio.php?op=lasthit&userid={$_GET['userid']}' width='100%' height='400'>Dein Browser muss iframes unterstützen, um die letzte Seite des Users anzeigen zu können. Benutze den Link im Menü stattdessen.</iframe>",true);
        addnav("","userbio.php?op=lasthit&userid={$_GET['userid']}");
      ...
      fclose($file);
        ...
        db_query($sql);
        if ($_GET['returnpetition']==""){
                // redirect("userbio.php?".db_affected_rows());
                redirect("userbio.php");
        }else{
        ...
        output("<form action='userbio.php?op=saveban' method='POST'>",true);
        ...
        db_query($sql);
        //output($sql);
        redirect("userbio.php?op=removeban");
        ...
        for ($i=0;$i<db_num_rows($result);$i++){
                $row = db_fetch_assoc($result);
                output("<tr class='".($i%2?"trlight":"trdark")."'><td><a href='userbio.php?op=delban&ipfilter=".URLEncode($row[ipfilter])."&uniqueid=".URLEncode($row[uniqueid])."'>Bann&nbsp;aufheben</a>",true);
                addnav("","userbio.php?op=delban&ipfilter=".URLEncode($row[ipfilter])."&uniqueid=".URLEncode($row[uniqueid]));
                output("</td><td>",true);
                output($row[ipfilter]);
                output($row[uniqueid]);
                output("</td><td>",true);
        ...
        addnav("User Info bearbeiten","userbio.php?op=edit&userid=$id");
        $result = db_query($sql);
        $odate = "";
        for ($i=0; $i<db_num_rows($result); $i++) {
                $row = db_fetch_assoc($result);
        ...
        if (isset($_GET['page'])){
                ...
                output("<tr>
                <td>Ops</td>
                <td><a href='userbio.php?sort=login'>Login</a></td>

                <td><a href='userbio.php?sort=name'>Name</a></td>
                <td><a href='userbio.php?sort=level'>Lev</a></td>
                <td><a href='userbio.php?sort=laston'>Zuletzt da</a></td>
                <td><a href='userbio.php?sort=gentimecount'>Treffer</a></td>
                <td><a href='userbio.php?sort=lastip'>IP</a></td>
                <td><a href='userbio.php?sort=uniqueid'>ID</a></td>
                <td><a href='userbio.php?sort=emailaddress'>E-Mail</a></td>
                </tr>",true);
                addnav("","userbio.php?sort=login");
                addnav("","userbio.php?sort=name");
                addnav("","userbio.php?sort=level");
                addnav("","userbio.php?sort=laston");
                addnav("","userbio.php?sort=gentimecount");
                addnav("","userbio.php?sort=lastip");
                addnav("","userbio.php?sort=uniqueid");
                $rn=0;
                for ($i=0;$i<db_num_rows($result);$i++){
                        $row=db_fetch_assoc($result);
                        ...
                        output("<td>",true);
                        output("[<a href='userbio.php?op=edit&userid=$row[acctid]'>Edit</a>|",true);
                        addnav("","userbio.php?op=edit&userid=$row[acctid]");
                        addnav("","userbio.php?op=del&userid=$row[acctid]");
                        addnav("","userbio.php?op=setupban&userid=$row[acctid]");
                        addnav("","userbio.php?op=debuglog&userid=$row[acctid]");
                        output("</td><td>",true);
                        output($row[login]);
                        output("</td><td>",true);
                        output($row[name]);
