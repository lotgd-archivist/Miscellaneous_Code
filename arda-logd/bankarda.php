...
if ($HTTP_GET_VARS[op]==""){
  ...
  output("sorgfältig überfliegt.  ");
    if ($session[user][goldinbank]>=0){
        ...
        output("renommierten Bank.  Kann ich sonst noch etwas für dich tun?`6\"`n`n(`iSchulden verfallen durch einen Phoenixkill nicht!`i)");
    }
    ...
    if ($session[user][goldinbank]>=0){
        ...
        output("<form action='bankarda.php?op=transfer2' method='POST'>Wieviel ü<u>b</u>erweisen: <input name='amount' id='amount' accesskey='b' width='5'>`n",true);
        ...
        addnav("","bankarda.php?op=transfer2");
    }else{
    ...
    if (db_num_rows($result)==1){
        ...
        output("<form action='bankarda.php?op=transfer3' method='POST'>",true);
        ...
        addnav("","bankarda.php?op=transfer3");
        ...
        output("<form action='bankarda.php?op=transfer2' method='POST'>Wieviel ü<u>b</u>erweisen: <input name='amount' id='amount' accesskey='b' width='5' value='$amt'>`n",true);
...
//             } else if ($session[user][emailaddress]==$row[emailaddress] && $row[emailaddress]){
    ...
    if (($session[user][gold]>=100 || $session[user][gold]+$session[user][goldinbank]>=100) && $session[user][gems]>0){
        ...
        output("<form action='bankarda.php?op=gemtrans2' method='POST'>Einen Edelstein versenden a<u>n</u>: <input name='to' accesskey='n'> (Unvollständige Namen werden automatisch ergänzt. Du wirst nochmal zum Bestätigen aufgefordert).`n",true);
        output("<input type='submit' class='button' value='Vorschau'></form>",true);
        addnav("","bankarda.php?op=gemtrans2");
    ...
    if (db_num_rows($result)==1){
        $row = db_fetch_assoc($result);
        output("<form action='bankarda.php?op=gemtrans3' method='POST'>",true);
        ...
        addnav("","bankarda.php?op=gemtrans3");
    }elseif(db_num_rows($result)>100){
        output("Der Bankier schaut dich überfordert an und schlägt dir vor, deine Suche vielleicht etwas mehr einzuengen, indem du den Namen genauer festlegst.`n`n");
        output("<form action='bankarda.php?op=gemtrans2' method='POST'>Versende einen Edelstein a<u>n</u>: <input name='to' accesskey='n' value='". $_POST['to'] . "'> (Unvollständige Namen werden automatisch ergänzt. Du wirst nochmal zum Bestätigen aufgefordert).`n",true);
        output("<input type='submit' class='button' value='Vorschau'></form>",true);
        addnav("","bankarda.php?op=gemtrans2");
    }elseif(db_num_rows($result)>1){
        output("<form action='bankarda.php?op=gemtrans3' method='POST'>",true);
...
//        } else if ($session[user][emailaddress]==$row[emailaddress] && $row[emailaddress]){
  ...
  output("<form action='bankarda.php?op=depositfinish' method='POST'>Du hast ".($session[user][goldinbank]>=0?"ein Guthaben von":"Schulden in Höhe von")." ".abs($session[user][goldinbank])." Gold bei der Bank.`n",true);
  ...
  addnav("","bankarda.php?op=depositfinish");
    ...
    if ($session['user']['reputation']<-35){
        ...
        $maxborrow = $session[user][level]*getsetting("borrowperlevel",20);
          output("<form action='bankarda.php?op=withdrawfinish' method='POST'>Du hast ".($session[user][goldinbank]>=0?"ein Guthaben von":"Schulden in Höhe von")." ".abs($session[user][goldinbank])." Gold bei der Bank.`n",true);
        ...
        output("<script language='javascript'>document.getElementById('input').focus();</script>",true);
          addnav("","bankarda.php?op=withdrawfinish");
    }
}else if($HTTP_GET_VARS[op]=="withdraw"){
      output("<form action='bankarda.php?op=withdrawfinish' method='POST'>Du hast ".$session[user][goldinbank]." Gold bei der Bank.`n",true);
    ...
    if ($_POST[amount]>$session[user][goldinbank] && $_POST[borrow]=="") {
     ...
     output("<form action='bankarda.php?op=gemdrawfinish' method='POST'>Du hast ".$session[user][gemsinbank]." Edelsteine bei der Bank.`n",true);
     output("`^Wieviel a<u>b</u>heben? <input id='input' name='amount' width=5 accesskey='b'> <input type='submit' class='button' value='Abheben'>`n`iGib 0 oder gar nichts ein, um alles abzuheben.`i</form>",true);
   output("<script language='javascript'>document.getElementById('input').focus();</script>",true);
     addnav("","bankarda.php?op=gemdrawfinish");
   ...
   output("<form action='bankarda.php?op=gemdepositfinish' method='POST'>Du hast ".$session[user][gemsinbank]." Edelsteine bei der Bank.`n",true);
   ...
   output("<script language='javascript'>document.getElementById('input').focus();</script>",true);
   addnav("","bankarda.php?op=gemdepositfinish");
...
if ($session[user][goldinbank]>=0){
    addnav("Abheben","bankarda.php?op=withdraw");
    addnav("Einzahlen","bankarda.php?op=deposit");
    if (getsetting("borrowperlevel",20)) addnav("Kredit aufnehmen","bankarda.php?op=borrow");
}else{
    addnav("Schulden begleichen","bankarda.php?op=deposit");
    if (getsetting("borrowperlevel",20)) addnav("Mehr leihen","bankarda.php?op=borrow");
}
/*if ($session[user][gemsinbank]>0) addnav("Edelsteine mitnehmen","bankarda.php?op=gemdraw");
if ($session[user][gems]>0) addnav("Edelsteine einlagern","bankarda.php?op=gemdeposit");
if (getsetting("allowgoldtransfer",1)){
    if ($session[user][level]>=getsetting("mintransferlev",3) || $session[user][dragonkills]>0){
        addnav("Gold überweisen","bankarda.php?op=transfer");
    }
    addnav("Edelstein versenden","bankarda.php?op=gemtrans");
}
*/
page_footer();
