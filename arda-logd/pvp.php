...
if ($HTTP_GET_VARS[op]=="" && $HTTP_GET_VARS[act]!="attack"){
    ...
    addnav("Krieger auflisten","pvp.php?op=list");
      addnav("Zurück zur Klingengasse","klingengasse.php");
}else if ($HTTP_GET_VARS[op]=="list"){
    checkday();
    pvpwarning();
    $days = getsetting("pvpimmunity", 5);
    $exp = getsetting("pvpminexp", 1500);
      $sql = "SELECT name,alive,location,sex,level,laston,loggedin,login,pvpflag,prison FROM accounts WHERE 
    (locked=0) AND 
    (location=0) AND
    (age > $days OR dragonkills > 0 OR pk > 0 OR experience > $exp) AND
    (level >= ".($session[user][level]-5)." AND level <= ".($session[user][level]+5).") AND 
    (alive=1 ) AND
    (specialty>0) AND
    (dragonkills >= ".($session[user][dragonkills]-10).") AND
    ...
    if (db_num_rows($result)>0){
        $row = db_fetch_assoc($result);
        if (abs($session[user][level]-$row[creaturelevel])>10 ){
        ...
        }elseif ($session[user][dragonkills] >($row[dragonkills]+10) ){
...
if ($battle){
    ...
    if ($victory){
        ...
        if ($dksdiff>$mindks){
            ...
            output("`#Davon werden dir `\$$loss `#Erfahrungspunkte abgezogen, weil dein Gegner $dksdiff Phoenixkills weniger als du hat.");
        }
