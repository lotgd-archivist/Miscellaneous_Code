...
if ($_GET[op]=="rebirth1"){
    ...
    if ($what=="true"){
        ...
        output("Du verlierst alle Phoenixpunkte.`n`n`bBist du zu diesem Schritt wirklich bereit?`b");
    ...
    if ($what=="false"){
        ...
        if ($session[user][house]) output("Du verlierst dein Haus.`n");
        if ($session[user][hashorse]) output("Du verlierst dein Tier.`n");
        output("Du kannst alle Phoenixpunkte neu vergeben.`n`n`bBist du zu diesem Schritt wirklich bereit?`b");
        output("`n`n`n<form action='rebirth.php?op=rebirth2&full=$what' method='POST'>",true);
...
// Bad idea for balance...?
        ...
        output("Phoenixkills, Titel, Ehepartner und deine Erinnerung bleiben dir erhalten, jedoch legst du alle anderen weltlichen Besitztümer ab und wirst es sehr schwer haben, dich wieder ");
        output(" an das knallharte Leben mit dem Phoenix zu gewöhnen. Dafür kannst du alle Phoenixpunkte neu vergeben.");
        ...
        output(" Phoenix erschlagen müssen, bevor du den Schrein betreten kannst.`6\"");
    }
    addnav("Zurück zum Club","rock.php");
}
