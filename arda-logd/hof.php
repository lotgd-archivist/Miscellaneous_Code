...
addnav("Bestenlisten");
addnav("Phoenixkills", "hof.php?op=kills&subop=$subop&page=$page");
...
addnav("Vielschreiber", "hof.php?op=rp&subop=$subop&page=$page");
...
addnav("Ältesten", "hof.php?op=age&subop=$subop&page=$page");
...
if ($_GET[op]=="money"){
 ...
 display_table($title, $sql,false,false,$headers,false);
    ...
    $sql = "SELECT name, acctid  FROM accounts WHERE locked=0 ORDER BY acctid $order, level $order, experience $order, acctid $order LIMIT $limit";
    if ($_GET[subop] == "least") $adverb = "ältesten";
    else $adverb = "jüngsten";
    ...
    $sql = "SELECT name FROM accounts WHERE locked=0 ORDER BY rppunkte $order, level $order, experience $order, acctid $order LIMIT $limit";
    if ($_GET[subop] == "least") $adverb = "wenigsten";
    else $adverb = "meisten";
    $title = "Die Krieger mit den $adverb RP-Punkten";
    display_table($title, $sql);
    ...
    $title = "Die $adverb Maler des `5lila Phoenix`^ in diesem Land";
    ...
    $title = "Helden mit den $adverb Phoenixkills";
