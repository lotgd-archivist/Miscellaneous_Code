...
if ($HTTP_GET_VARS[op]=='' && $HTTP_GET_VARS[level]>0){
...
output("<div style='position: absolute; 
    top: 0px; 
    left: 0px;'><img src='./images/sudoku/sudoku.jpg'></div>",true);
...
elseif($HTTP_GET_VARS[op]=='pruefen'){ 
output("Sudoku wird geprüft....`n");
$fehlerfrei=true; 
...
if ($fehlerfrei){$fehler='Gratulation! du hast das Sudoku erfolgreich gelöst!';}
...
}elseif($HTTP_GET_VARS[op]=='verbessern'){
...
output("<div style='position: relative; 
    ...
    left: 0pt; 
    width: 780px; 
    height: 477px;'>",true);       // keine ahnung ob das hier nötig ist... es war halz ZWINGEND nötig bei einem anderen Projekt, somit c&p
output("<div style='position: absolute; 
    top: 0px; 
    left: 0px;'><img src='./images/sudoku/sudoku.jpg'></div>",true);
$ausgabe.="<form action='sudoku2.php?op=pruefen' method='post' name='f1'>";
...
}elseif($HTTP_GET_VARS[op]=='regeln'){
output("<h2>Sudokuregelwerk</h2><h3>
Du musst das Rastergitter so ausfüllen, dass:`n
jede Reihe, `n
jede Spalte, und  `n
alle 3 x 3 Boxen`n
...
",true);
}elseif($HTTP_GET_VARS[op]=='' && $HTTP_GET_VARS[level]==''){
  output("`7Etwas verunsichert was das seltsame Wort `bSudoku`b bedeuten soll gehst du vorsichtig weiter.`n
           ...
           \"`qSei geglüst edles Seinsei... Ich abe hiel eine luustige Spielll fül dich volbeleitet.`n
               Du kannst hiel deinen Velstand trainielen und elfahlenel welden.`n`n
               In ein paal tausend Jahlen welden es dil deine Nachfahlen danken, sobald ihle Kindel das Wolt Pisa hölen.`n`n
               Falls du Flagen zum Spiel hast kannst du natülich im Legelwelk nachlesen.
               `7\"`n
