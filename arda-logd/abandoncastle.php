...
//checkevent();
if ($HTTP_GET_VARS[op] == "" and $HTTP_GET_VARS[loc] == "")
    {
    output("`c`b`&Das Labyrinth`0`b`c`n`n");
    if (($session['user']['dragonkills'] > 3) && ($session[user][turns]>=1))
        {
        output("`2 Du hast dich allen Ernstes in Viconias Labyrinth gewagt. Kaum betrittst du denn ersten Raum, versperrt hinter Dir plötzlich und unerwartet ein Felsblock den Eingang. Erschrocken rennst Du zurück und stemmst Dich dagegen, ");
        ...
        if ($session['user']['hashorse']>0)
            {
            output("Schade, aber Dein {$playermount['mountname']} kann Dich nicht begleiten. Du bist jetzt auf Dich allein gestellt, während es brav vor dem Tempel steht und es sich gutgehn lässt`n");
            }
        ...
        {
        output("Du willst das Labyrinth betreten, aber ein Durchsichtiger Kristall versperrt dir den Weg.`n");
        output("Komm wieder, wenn Du mehr Erfahrung hast Krieger.`n");
        addnav("zurück","victempel.php?op=halle");
        }
    else
        {
        output("In Deinem Zustand willst Du in Viconias Labyrinth? Du bist echt Lebensmüde`n");
...
if ($HTTP_GET_VARS[op] <> ""){
    ...
    if ($HTTP_GET_VARS[op] == "e"){
        $locale+=1;
        redirect("abandoncastle.php?loc=$locale");
    }
}else{
if ($HTTP_GET_VARS[loc] <> ""){
...
switch(e_rand($session['user']['mazeturn'],2500)){
    ...
    case 9:
    case 10:
    output("Viconia meint es gut mit dir!  Du findest einen Edelstein!");
    $session['user']['gems']+=1;
    break;
    case 11:
    case 12:
    ...
    case 19:
    case 20:
    output("Viconia meint es gut mit dir! Du findest 1000 Gold!");
    ...
    case 29:
    case 30:
    output("Viconia meint es gut mit dir! Du findest 900 Gold!");
    ...
    case 39:
    case 40:
    output("Viconia meint es gut mit dir! Du findest 800 Gold!");
    ...
    case 49:
    case 50:
    output("Viconia meint es gut mit dir! Du findest 700 Gold!");
    ...
    case 59:
    case 60:
    output("Viconia meint es gut mit dir! Du findest 600 Gold!");
    ...
    case 69:
    case 70:
    output("Viconia meint es gut mit dir! Du findest 500 Gold!");
    ...
    case 79:
    case 80:
    output("Viconia meint es gut mit dir! Du findest 400 Gold!");
    ...
    case 89:
    case 90:
    output("Viconia meint es gut mit dir! Du findest 300 Gold!");
    ...
    case 99:
    case 100:
    output("Viconia meint es gut mit dir! Du findest 200 Gold!");
    ...
    case 109:
    case 110:
    output("Viconia meint es gut mit dir! Du findest 100 Gold!");
    ...
    case 119:
    case 120:
    output("Viconia meint es gut mit dir! Du findest 50 Gold!");
    ...
    */
    ...
    case 129:
    case 130:
    output("Viconia meint es gut mit dir! Du findest 10 Gold!");
    ...
    addnews("`%".$session[user][name]."`5 versuchte sich an Viconias Labyrinth und füttert jetzt ihre Haustiere.");
    break;
    case 2464:
    case 2465:
    case 2466:
...
if ($navigate<>"z"){
if ($navigate=="x"){
    output("Du fällst ins Bodenlose des Weltraums!");
    $session['user']['hitpoints']=0;
    addnews("`%".$session[user][name]."`5 versuchte sich an Viconias Labyrinth, sie fand ".($session[user][sex]?"ihn":"sie"). "als Vogel besser");
}
if ($navigate=="p"){
...
if ($navigate=="q"){
    ...
    $session['user']['hitpoints']=0;
    addnews("`%".$session[user][name]."`5 versuchte sich an Viconias Labyrinth und füttert jetzt die Fische.");
}
if ($navigate=="r"){
...
if ($navigate=="s"){
    ...
    $session['user']['hitpoints']=0;
    addnews("`%".$session[user][name]."`5versuchte sich an Viconias Labyrinth und füttert jetzt ihre Haustiere.");
}
if ($session['user']['hitpoints'] > 0){
if ($locale=="6"){
...
if ($session['user']['superuser']>=2) addnav("!?Superuser Exit","abandoncastle.php?loc=$exit");
    ...
    addnews("`%".$session[user][name]."`5 hat Viconias Labyrinth überlebt!  Und das in nur ".$session['user']['mazeturn']." Zügen! Ein Liebling der Göttin");
    ...
    addnav("Weiter","victempel.php?op=halle");
