...
*/
        ...
        addnews("`3{$session['user']['name']}`3 wurde von Meister `^{$master['creaturename']}`3 wegen Überheblichkeit gejagt und gestellt.`0");
    }
...
*/
            addnav("Zurück zur Zwergenstadt","zwergenstadt.php");
            addnews("`%".$session[user][name]."`3 hat ".($session[user][sex]?"ihren":"seinen")." Meister `%$badguy[creaturename]`3 an ".($session[user][sex]?"ihrem":"seinem")." `^".ordinal($session[user][age])."`3 Tag besiegt und steigt auf Level `^".$session[user][level]."`3 auf!!`0");
            $badguy=array();
            $session[user][hitpoints] = $session[user][maxhitpoints];
            ...
            if($defeat){
                ...
                addnews("`%".$session[user][name]."`5 hat Meister $badguy[creaturename] herausgefordert und verloren!`n$taunt`0");
