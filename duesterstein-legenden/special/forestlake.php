...
/*
 * modifications by bibir
 * for              www.nicegames.de & www.rabenthal.de
 */
 
/* Installation: 
...
    $chat = "forestlake_".$session[user][acctid]."_".$session[user][marriedto]; 
} else { 
    $chat = "forestlake_".$session[user][marriedto]."_".$session[user][acctid]; 
} 
...
case "": 
    // Zutritt ................................................................... 
    // alle optionen freigeben (noch nicht getaetigt)
    $swim_done = 0;
    $music_done = 0;
    $picknick_done = 0;
    $flirt_done = 0;
    $ride_done = 0;
    $link = "swim=$swim_done&music=$music_done&picknick=$picknick_done&flirt=$flirt_done&ride=$ride_done";
...
            addnav("Folge den Pfeilen","forest.php?op=clearing&".$link); 
        } 
    } else { 
        //output("Aber da Du eh' Single bist, kann dieses Zeichen gar nicht für Dich bestimmt sein`n, und Du ziehst Dich diskret in den Wald zurück.`n`n"); 
        output("Aber da Du eh' Single bist, kann dieses Zeichen gar nicht für Dich bestimmt sein.`n");
        output("Dennoch bist du neugierig und der Drang herauszufinden, was sich hier verstecken könnte, ist nicht gerade gering.`n`n"); 
        addnav("Folge den Pfeilen","forest.php?op=single&".$link); 
    } 
...
    switch($rand){
    case 1:
        output("`@Leider gibts hier überhaupt nichts zu sehen.`0");
        break;
    case 2:
        output("`@Auf einer Lichtung siehst du ".($session[user][sex]?"einen sonderbaren fremden, der":"eine sonderbare fremde, die")." dich anlächelt. ");
        output("`@Irgendetwas erinnert dich an ".($session[user][sex]?"Fexez":"Aphrodite")." - so gehst du furchtlos näher.`n");
        output(($session[user][sex]?"Er":"Sie")." deutet dir, es sich auf der Decke nahe am Ufer des kleinen Sees - der sich hier auf dieser Lichtung erstreckt - gemütlich zu machen.");        addnav("Auf Decke setzen","forest.php?op=single_flirt");
    }
    if ($allowtovillage==TRUE) addnav("Zurück ins Dorf","village.php"); 
    if ($allowtoforest==TRUE) addnav("Zurück in den Wald","forest.php?op=return&subop=single"); 
    $session[user][specialinc] = "forestlake.php"; 
    break; 
case "single_flirt":
    output("`@Du setzt dich auf die Decke und schon läßt ".($session[user][sex]?"er":"sie")." sich neben dich nieder.");
    output("Das Gespräch, was du gerade anfangen wolltest, läßt du bei ".($session[user][sex]?"seinem":"ihrem")." Lächeln und dem unwiderstehlichen Blick sein.`n");
    output(($session[user][sex]?"Er":"Sie")." zeigt auf den Picknick-Korb und du nickst nur. Sogleich werden frische Beeren und andere Köstlichkeiten ausgepackt.");
    output("Während ihr esst, treffen sich eure Blicke immer wieder; ein wunderbar angenehmes Kribbeln macht sich dabei in dir breit.`n");
    output("Nach dem Essen drückt ".($session[user][sex]?"Er":"Sie")." dich sanft auf die Decke und beginnt dich zu massieren - genüßlich schließt du die Augen.`n`n");
    output("Als du die Augen wieder öffnest ist ".($session[user][sex]?"er":"sie")." schon wieder verschwunden, ");
    output("doch du fühlst dich erholt und bereit für einen weitern Waldkampf - außerdem scheint dich das anhaltende Kribbeln zu schützen.");
    $session[user][turns]++;
    if ($session[user][sex] == 1) {
        $buff = array("name"=>"`vSchutz der Verliebtheit","rounds"=>20,"wearoff"=>"`VDu vermisst das Kribbeln!`0","defmod"=>1.2,"roundmsg"=>"`vDie Errinerung an die Zeit mit dem Fremden schützt dich!","activate"=>"defense");
    } else {    
        $buff = array("name"=>"`vSchutz der Verliebtheit","rounds"=>20,"wearoff"=>"`VDu vermisst das Kribbeln!`0","defmod"=>1.2,"roundmsg"=>"`vDie Errinerung an die Zeit mit der Fremden schützt dich!","activate"=>"defense");
    }
...
case "clearing": 
    // alle optionen 
    $swim_done = $HTTP_GET_VARS[swim];
    $music_done =$HTTP_GET_VARS[music];
    $picknick_done = $HTTP_GET_VARS[picknick];
    $flirt_done = $HTTP_GET_VARS[flirt];
    $ride_done = $HTTP_GET_VARS[ride];
    $link = "swim=$swim_done&music=$music_done&picknick=$picknick_done&flirt=$flirt_done&ride=$ride_done";
...
    } 
    output("`n`n`n`n`n`n`n`n`n`n`$@`@}-,-´-"); 
    if ($swim_done==0) addnav("Im See schwimmen","forest.php?op=swim&".$link); 
    if ($music_done==0) addnav("Musizieren","forest.php?op=music&".$link); 
    if ($picknick_done==0) addnav("Picknicken","forest.php?op=picknick&".$link); 
    if ($flirt_done==0) addnav("Plaudern","forest.php?op=flirt&".$link); 
    if ($loverHorseID+$lovedHorseID>0 && $ride_done==0) addnav("Ausreiten","forest.php?op=ride&".$link);
    if ($allowtovillage==TRUE) addnav("Zurück ins Dorf","village.php"); 
...
    $picknick_done = $HTTP_GET_VARS[picknick];
    $flirt_done = $HTTP_GET_VARS[flirt];
    $ride_done = 1; //wurde schon (hier) gewaehlt
    $link = "swim=$swim_done&music=$music_done&picknick=$picknick_done&flirt=$flirt_done&ride=$ride_done";
...
    output("`n`n`n`n`n`n`n`n`n`n`$@`@}-,-´-"); 
    addnav("Turm untersuchen","forest.php?op=towerruin&".$link); 
    addnav("Zurück zur Lichtung","forest.php?op=rideback&".$link); 
    $session[user][specialinc] = "forestlake.php"; 
    break; 
  // Im Turm ................................................................... 
case "towerruin": 
...
    $rand = e_rand(0,1); // Zufällige Ereignisse während des Musizierens. 
    switch($rand) { 
    case 0: 
...
    break; 
case "return": 
    if($HTTP_GET_VARS[subop]==single){
        output("`@Du fühlst dich ein wenig einsam, als du diesen Ort verläßt und beschließt, im Garten nach einem Partner Ausschau zu halten.`0");
    } else {
