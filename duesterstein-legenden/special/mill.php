...
else if ($HTTP_GET_VARS[op]=="cont"){   // zurück in den Wald
...
else if ($HTTP_GET_VARS[op]=="enter"){   // Die Mühle
    $rand = e_rand(1,10);
...
else if ($HTTP_GET_VARS[op]=="cellar"){   // Der Mühlenkeller
...
    `9Hoffentlich hält die Leiter, denn sonst wärst Du wohl gefangen...`n`n`0");
    output("`QUnten ist es still wie in einem Grab.`0 Du hörst nichts ausser Deine
    eigenen Schritte, die vom alten Gemäuer dumpf wiederhallen. Es tropft von
...
else if ($HTTP_GET_VARS[op]=="shelf"){   // Regal
...
else if ($HTTP_GET_VARS[op]=="furtherdown"){   // weiter keller
...
            Zu kehrst ans Tageslicht zurück. Ein bischen mehr hätte es ja sein können....`0");
            $session[user][gold]++;
            break;
        }
    }
