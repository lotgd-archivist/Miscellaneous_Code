...
        output("stösst Du Dein/e/n ".$session[user][weapon]." in Dein Herz. Als sich die Dunkelheit Deiner bemächtigt ");  
        switch(e_rand(1,15)){ 
        case 1: 
        case 2: 
        case 3: 
...
              $plev++; 
                output("`\$Du gehst in ein Gebiet des Waldes, welches Kreaturen aus Deinen Alpträumen enthält, in der Hoffnung das Du ein verletztes findest.`0`n"); 
            } 
...
                        addnav("Tägliche News","news.php");  
                        $session['user']['donation']+=1;  
                        if (strtolower(substr($session[user][name],-1))=="s") addnews($session[user][name]."'s Waffen wurden bei einer Riesenpflanze gefunden, aber mehr konnte nicht herausgefunden werden ".($session[user][sex]?her:him).".");  
                        else addnews($session[user][name]."'s Waffen wurden bei einer Riesenpflanze gefunden, aber mehr konnte nicht herausgefunden werden ".($session[user][sex]?her:him).".");  
                        break;  
                    case 2:  
                    case 3:  
                    case 4:  
...
            output("`n`n`^Du findest `%".$rewardnum."`^ Edelsteine!`n");  
...
                output("`b`c`&~~ Flawless Fight! ~~`\$`n`bDu erhältst einen Extra-Waldkampf!`c`0`n");  
                $session[user][turns]++;  
            }else{  
...
                addnav("`\$Dunkle Pfeile`0", "");  
                addnav("`\$&#149; Skelettkämpfer`7 (1/".$session[user][darkartuses].")`0","$script?op=fight&skill=DA&l=1",true);  
            }  
...
                addnav("`\$&#149; Seele schädigen`7 (5/".$session[user][darkartuses].")`0","$script?op=fight&skill=DA&l=5",true);  
          
            if ($session[user][thieveryuses]>0) {  
                addnav("`^Diebestalente`0","");  
                addnav("`^&#149; Beleidigen`7 (1/".$session[user][thieveryuses].")`0","$script?op=fight&skill=TS&l=1",true);  
            }  
            if ($session[user][thieveryuses]>1)  
                addnav("`^&#149; Giftklinge`7 (2/".$session[user][thieveryuses].")`0","$script?op=fight&skill=TS&l=2",true);  
            if ($session[user][thieveryuses]>2)  
                addnav("`^&#149; Getarnter Angriff`7 (3/".$session[user][thieveryuses].")`0","$script?op=fight&skill=TS&l=3",true);  
