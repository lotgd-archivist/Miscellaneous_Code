...
$racesel = "CASE race WHEN 1 THEN '`2Troll`0' WHEN 2 THEN '`^Elf`0' WHEN 3 THEN '`&Mensch`0' WHEN 4 THEN '`#Zwerg`0' WHEN 5 THEN '`5Ork`0' WHEN 6 THEN '`2Halbling`0' WHEN 7 THEN '`^Echse`0' WHEN 8 THEN '`&Goblin`0' WHEN 9 THEN '`#Kilrath`0' WHEN 10 THEN '`5Arachnia`0' ELSE '`7Unbekannt`0' END"; 
...
    $sql ="SELECT a.name, IF(ISNULL(d.value), a.gems,(a.gems+d.value)) AS data1 FROM accounts a LEFT JOIN depositbox d USING(acctid) WHERE a.acctid = d.acctid OR a.acctid AND locked=0 ORDER BY data1 $order, a.level $order, a.experience $order, a.acctid $order LIMIT $limit"; 
