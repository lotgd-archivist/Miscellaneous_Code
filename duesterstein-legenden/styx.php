...
    $was=e_rand(1,4); 
    output("`8Deine Seele folgt dem Fluss der Toten aufwärts. Du siehst viele Seelen, die im Fluss mitgerissen werden, einige werden auf Booten von toten Fährmännern gefahren und andere versuchen wie du die Flucht."); 
    if($was==2){ 
        output("`8 Schließlich siehst du ein Licht am Horizont, aus dem der Fluss zu entspringen scheint. Eilig bewegst du dich darauf zu."); 
        output("`n`n`#Dir gelingt die Flucht aus dem Totenreich!`n`n`8Erschöpft öffnest du die Augen. Dein Körper benötigt dringend Heilung, wenn du ihn nicht sofort wieder verlieren willst. Wie lange du tot warst, kannst du nicht sagen, aber sehr lange kann es nicht gewesen sein."); 
...
    }else{ 
        output("`8 Die vielen Hände, die aus dem Fluss nach dir greifen, lassen dich nur langsam vorankommen. Schließlich zerren sie dich ganz in den Fluss. Du wirst zurück ins Totenreich geschwemmt - direkt vor `\$Ramius`8' Füsse."); 
...
    page_header("Der Seelenfluss"); 
    if ($_GET[subop]=="favor"){ 
        output("`8Für ein paar mehr Gefallen bei `\$Ramius`8 würdest du sogar deine Seele verkaufen. Ups! Lieber doch nicht. So versprichst du dieser trotteligen Seele"); 
        output(" ein paar deiner Edelsteine, die dir hier unten ja sonst wirklich nichts nützen. Wie du `4Hatetepe`8 die Steinchen übergeben sollst, ist dir in dem Moment egal - dir "); 
        output("wäre es sogar recht, wenn er die Steine nie einfordern würde.`n"); 
        output("`4Hatetepe`8 verspricht, ein gutes Wort für dich bei `\$Ramius`8 einzulegen. Gerade als du ihn fragen willst, was er hier unten überhaupt mit Edelsteinen anfangen will, "); 
        output(" findest du dich auf dem Friedhof wieder..."); 
        $session[user][deathpower]+=10; 
        $session[user][gems]-=2; 
        addnav("Weiter...","graveyard.php"); 
    }elseif ($_GET[subop]=="gem"){ 
        output("`4Hatetepe`8 verspricht dir, einen Edelstein für dich im Dorf bereit zu legen. Gerade als du ihn fragen willst, wie er das schaffen will, "); 
        output(" findest du dich auf dem Friedhof wieder..."); 
        addnav("Weiter...","graveyard.php"); 
        $session[user][gems]++; 
...
            output("`8Du hast keine Gefallen mehr übrig, die du auf diese Weise verspielen könntest. Traurig darüber, dass du wohl gerade deine Chance, "); 
            output(" heute noch aus dem Totenreich zu kommen, verspielt hast, machst du dich auf den Weg zurück zum Friedhof"); 
        }else{ 
            output("`8Du verlierst einen Gefallen und nimmst mit der Welt der Lebenden Kontakt auf.`n Du hast noch `b`4".$session[user][deathpower]."`b`8 Gefallen.`n`n"); 
            addcommentary(); 
            switch($_GET[where]){ 
                case "1": 
...
        $where=e_rand(1,7); 
        if ($what==1){ 
            output("`8\"`!Sei gegrüsst!`8\", spricht dich eine alte Seele an, die schon seit Ewigkeiten hier zu sein scheint, \"`!Ich bin `4Hatetepe`!, "); 
...
            output(" im Reich der Lebenden einen Vorteil, der keiner ist. Also, kann ich dir materiellen oder spirituellen Besitz anbieten oder abknöpfen?`8\""); 
            addnav("Kaufen"); 
...
            addnav("Zum Friedhof","graveyard.php"); 
        }elseif ($what==2){ 
            output("`8Du entdeckst eine Möglichkeit, mit der Welt der Lebenden in Verbindung zu treten! Allerdings wird dich dieses Unternehmen einiges "); 
...
            addnav("Zum Friedhof","graveyard.php"); 
        }else{ 
            output("`8Du entdeckst hier absolut nichts besonderes."); 
            addnav("Zum Friedhof","graveyard.php"); 
        } 
...
        output("`8Du bemerkst einen seltsamen Schimmer und wandelst darauf zu.`nDu hast den `bFluss der Seelen`b gefunden! Jenen merkwürdigen Ort, "); 
