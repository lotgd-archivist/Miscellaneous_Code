...
        if ($session['user']['pvpflag']=="2033-10-06 00:42:00") output("`0Dag schaut Dich an. \"`7Petersen hat mir erzählt, daß Du Dir PvP Immunität gekauft hast und er Dich beschützt - und nun willst Du jemand anderen getötet sehen? Es ist Dir klar, daß Du Deine Immunität verlierst, solltest Du jetzt wirklich ein Kopfgeld aussetzten, oder?`0\"`n`n"); 
        addnav("","dag.php?op=finalize"); 
    } 
}elseif ($HTTP_GET_VARS['op']=="finalize") { 
    //$name = "%" . rawurldecode($_POST['contractname']) . "%"; 
    if ($_GET['subfinal']==1){ 
        $sql = "SELECT acctid,name,login,level,locked,age,dragonkills,pk,experience,bounty,pvpflag,lastip,emailaddress FROM accounts WHERE name='".addslashes(rawurldecode(stripslashes($_POST['contractname'])))."' AND locked=0";
        //output($sql); 
    }else{ 
...
        } 
        $sql = "SELECT acctid,name,login,level,locked,age,dragonkills,pk,experience,bounty,pvpflag,lastip,emailaddress FROM accounts WHERE name LIKE '".addslashes($name)."' AND locked=0";
    } 
    $result = db_query($sql); 
...
        } elseif ($row['pvpflag']=="2013-10-06 00:42:00") {
