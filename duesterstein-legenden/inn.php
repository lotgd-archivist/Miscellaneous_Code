...
if ($session[user][locate]!=6){
    $session[user][locate]=6;
    redirect("inn.php");
}
addcommentary();
checkday();
...
    output("Wieder mal bereit für's Abenteuer schlenderst du die Treppen der Kneipe runter! "); 
}
if ($HTTP_GET_VARS[op]=="" && $tag != "Montag" ){
    output("Du tauchst in eine schummerige Kneipe ab, die du sehr gut kennst. Der stechende Geruch von Pfeifentabak erfüllt "); 
    output("die Luft."); 
}
if ($HTTP_GET_VARS[op]=="" || $HTTP_GET_VARS[op]=="strolldown"){
    if ( $tag == "Montag" ) {
        output("Du stehst zwar im Vorraum der Kneipe, kommst aber in den Gastraum nicht
        hinein. Als Du an heftig an der Tür rüttelst, fällt Dir ein kleines Schild auf:`n`n
        `QMontag Ruhetag!`0`n`n
        Violet, Seth, Cedrick, Dag, alle geniessen heute einen freien Tag. Sogar Old Drawl hat
        sich mürrisch aufgemacht, den Tag im Wald zu geniessen. Du begreifst, dass
        Du hier heute absolut keine Dienste erwarten kannst und beschliesst, morgen wieder zu
        kommen.`0");
       addnav("Zurück zum Dorf","village.php");
    } else if ( $session[user][stone] == 2 ) {
        output("Du stehst im Vorraum der Kneipe und hast $stone[2] in der Tasche. Durch
        seine Kraft bleibt die Tür zum Gastraum für Dich verschlossen!`0");
       addnav("Zurück zum Dorf","village.php");
    } else {
...
            addnav("Aktualisieren","inn.php?op=converse");
            break;
        case "bartender":
        if (getsetting("paidales",0)<=1 || $session[user][gotfreeale]==1) { 
                if ( $wetter == "Warm und sonnig" ) {
                    $alecost = $session[user][level]*11;
                }
                else if ( $wetter == "Heiß und sonnig" ) {
                    $alecost = $session[user][level]*12;
                } else {
...
                if ( $wetter == "Warm und sonnig" ) {
                    output("Du bemerkst, dass das Ale heute etwas teurer ist. Wohl wegen dem warmen Wetter...`n");
                }
                else if ( $wetter == "Heiß und sonnig" ) {
                    output("Du bemerkst, dass die Ale-Preise wegen dem heissen Wetter ganz schön angezogen haben...`n");
                }
...
                                    debuglog("`^Edelsteine`6-hat ".$gemcount." Steine für Charm eingelöst");
                                    break;
                                case 2:
...
                                    debuglog("`^Edelsteine`6-hat ".$gemcount." Steine für Lebensenergie eingelöst");
                                    break;
                                case 3:
                                    if ($session[user][hitpoints]<$session[user][maxhitpoints]) $session[user][hitpoints]=$session[user][maxhitpoints];
                                    $session[user][hitpoints]+=($gemcount*10);
                                                        output("`&Du fühlst dich gesund! `^(Du erhältst vorübergehend mehr Lebenspunkte)"); 
                                    debuglog("`^Edelsteine`6-hat ".$gemcount." Steine für vorübergehende Lebenspunkte eingelöst");
                                    break;
                                case 4:
...
                                    debuglog("`^Edelsteine`6-hat ".$gemcount." Steine für Vergessen eingelöst");
                                    break;
                                case 5:
...
                                        );
                                    }
                                    debuglog("`^Edelsteine`6-hat ".$gemcount." Steine für Transmutation eingelöst");
                                    break;
                            }
                        }else{
...
    output("Der Barkeeper Cedrik schaut Dich an `6\"Junge, meinst Du nicht, daß Du den anderen auch noch den Spaß lassen solltest, eine Runde schmeissen zu können?
        Wir wissen ja auch so, daß Du der Größte bist.\"`0");
}else{ 
...
                    }
                    if ( $tag == "Samstag" || $tag == "Sonntag" ) {
                        output("Du trottest zum Barkeeper und fragst nach einem Zimmer. Er betrachtet dich und sagt: \"Das kostet `\$".$expense."`0 Gold für die Nacht, den Wochenendzuschlag habe ich schon mitgerechnet.\"");
                    } else {
