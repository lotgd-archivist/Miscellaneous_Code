...
Version: 25.10.2004
Modified by Raven     - Superusers Level 3 or higher are able to stop the current Arena Fight
            A systemmail is send to inform both players - no gold will be refund.

            Source modified at several places
            Database modified - add attribut int(11) fightno in table pvp first with auto increment
...
if ($session[user][locate]!=31){
    $session[user][locate]=31;
...
                    addnews("Im Namen seiner Gilde besiegte ".$session['user']['name']." seinen
                    Gegner ".$badguy['creaturename']." in der Arena!");
                } else {
                    // Error
...
                    }
                    $ClanFee = round((($MyClan['PercentOfFightsEarned']['PvP']/100) * $badguy['creaturegold']),0);
                    if ($ClanFee<=0) $ClanFee=(($session['user']['level']*10) * ($MyClan['PercentOfFightsEarned']['PvP']/100)+1);
                    output("`&`nDein Clan fordert seinen Anteil. Du zahlst `^".$ClanFee." Gold `&Tribut.");
...
                        output("`nDu zahlst einen Teil des Tributs direkt von der Bank!");
                    } else {
                        $session['user']['gold']-=$ClanFee;
                    }
                    $MyClan['gold']+=$ClanFee;
                    update_guild_info($MyClan);
                    addnews("Im Namen seines Clans besiegte ".$session['user']['name']." seinen
                    Gegner ".$badguy['creaturename']." in der Arena!");
                } else {
                    // Error
...
        $rowtodel = $HTTP_GET_VARS[num];
        $sql="SELECT * from pvp WHERE fightno=$rowtodel";
        $result=db_query($sql) or die(db_error(LINK));
        $row=db_fetch_assoc($result);
        $sql="DELETE from pvp WHERE fightno=$rowtodel";
        db_query($sql);
        output("Der Kampf zwischen $row[name1] und $row[name2] wurde gelöscht.");
        systemmail($row[acctid1],"`2Dein Arena Kampf wurde gelöscht!","`@Dein Kampf in der Arena mit `4".$row[name2]." `@wurde auf Anforderung gelöscht."); 
        systemmail($row[acctid2],"`2Dein Arena Kampf wurde gelöscht!","`@Dein Kampf in der Arena mit `4".$row[name1]." `@wurde auf Anforderung gelöscht.");
    }else{
        output("Keine Kampfnummer ausgewählt.");
    }
...
        if (db_num_rows($result)){ 
        if ($session['user']['superuser']>=3){
                    output(" Du beobachtest das bunte Treiben auf dem Platz eine Weile.`nFolgende Krieger kämpfen gerade gegeneinander:`n`n<table border='0' cellpadding='3' cellspacing='0'><tr><td align='left'>`bFunktion`b&nbsp;</td><td align='left'>`bHerausforderer`b</td><td align='left'>`bVerteidiger`b</td><td align='left'>`bStand (LP)`b</td></tr>",true); 
            }else{
            output(" Du beobachtest das bunte Treiben auf dem Platz eine Weile.`nFolgende Krieger kämpfen gerade gegeneinander:`n`n<table border='0' cellpadding='3' cellspacing='0'><tr><td align='center'>`bHerausforderer`b</td><td align='center'>`bVerteidiger`b</td><td align='center'>`bStand (LP)`b</td></tr>",true); 
        }
            for ($i=0;$i<db_num_rows($result);$i++){ 
                $row = db_fetch_assoc($result); 
        if ($session['user']['superuser']>=3){
            //output("[ <a href='superuser.php?op=newsdelete&newsid=$row[newsid]&return=".URLEncode($_SERVER['REQUEST_URI'])."'>Del</a> ]&nbsp;",true);
                    output("<tr class='".($i%2?"trlight":"trdark")."'><td>[ <a href='pvparena.php?op=del&num=$row[fightno]&return=".URLEncode($_SERVER['REQUEST_URI'])."'>Löschen</a> ]</td><td>$row[name1]</td><td>$row[name2]</td><td>$row[hp1] : $row[hp2]</td></tr>",true);
            addnav("","pvparena.php?op=del&num=$row[fightno]&return=".URLEncode($_SERVER['REQUEST_URI']));
                }else{
