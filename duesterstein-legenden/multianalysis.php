...
   output("</table>",true);
   // -> debuglog
   $sql = 'SELECT a3.date as date,a3.actor as actor,a3.target as target,a3.message as message,a1.name as actorname,a2.name as targetname 
             FROM debuglog a3 LEFT JOIN accounts as a1 ON a1.acctid=a3.actor 
                           LEFT JOIN accounts as a2 ON a2.acctid=a3.target 
            WHERE actor IN (-1'.$in_acctid.') 
              AND target IN (-1'.$in_acctid.') 
              AND date > "'.date("Y-m-d H:i:s",strtotime($abdatum)).'" 
            ORDER BY a3.date DESC, a3.actor ASC';
   $result = db_query($sql) or die(db_error(LINK));
