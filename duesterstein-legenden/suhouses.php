...
    }
    addnav("Veranda","suhouses.php?op=veranda&id=$_GET[id]");
...
    $sql = "SELECT * FROM items WHERE value1=$row[houseid] AND class='Schlüssel' ORDER BY value2 ASC,id ASC";
    $result = db_query($sql) or die(db_error(LINK));
...
        $row2 = db_fetch_assoc($result2);
        output("<tr><td>`b$i`b</td><td>".($row2[acctid]?"$row2[acctid] ($row2[name])":"0 (`4Verloren`0)")."</td><td>$item[value1]</td><td>$item[value2]</td><td>$item[hvalue]</td><td>",true);
        if ($row2[name]==""){
...
    addnav("Private Kommentare", "suhouses.php?op=comment_private&id=$_GET[id]");
    addnav("Veranda", "suhouses.php?op=veranda&id=$_GET[id]");
...
        db_query($sql);
    }
    viewcommentary("veranda-$_GET[id]","X",100);
    addnav("Zurück zu Haus $_GET[id]","suhouses.php?op=drin&id=$_GET[id]");
    addnav("Private Kommentare", "suhouses.php?op=comment_private&id=$_GET[id]");
    addnav("Allgemeine Kommentare", "suhouses.php?op=comment&id=$_GET[id]");
}else if ($_GET[op]=="comment_private"){
    if ($_GET[subop]=="delete"){
        $sql = "DELETE FROM commentary WHERE commentid='$_GET[commentid]'";
        db_query($sql);
    }
    viewcommentary("private-$_GET[id]","X",100);
    addnav("Zurück zu Haus $_GET[id]","suhouses.php?op=drin&id=$_GET[id]");
    addnav("Allgemeine Kommentare", "suhouses.php?op=comment&id=$_GET[id]");   
    addnav("Veranda", "suhouses.php?op=veranda&id=$_GET[id]"); 
...
    }else{
        $page=(int)$_GET[limit];
        addnav("Vorherige Straße","suhouses.php?limit=".($page-1)."");
    }
