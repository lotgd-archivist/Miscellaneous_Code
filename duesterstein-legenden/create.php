...
$firstday = getsetting("daysalive",0);
...
        $sql = "UPDATE accounts SET emailvalidation='', firstday=".$firstday." WHERE emailvalidation='$_GET[id]' AND emailvalidation!=''";
        db_query($sql);
...
        Deine E-Maill Adresse wurde bestätigt. Dein Login Name ist `^$row[login]`0.  `n`n<input type='submit' class='button' value='Hier klicken zum Einloggen'></form>`n`n"
...
if ($HTTP_GET_VARS[op]==""){
    output("`&`c`bCharacter erstellen`b`c`n");
...
    output("`nWie willst du in dieser Welt heissen? <input name='name' type='text'>`n",true);
