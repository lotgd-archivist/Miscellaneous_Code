...
sicher, dass er kein höheres Level als du selbst hat. Weil in diesem Fall überlebst du den Kampf vielleicht nicht. Denke immer daran, dass du jederzeit versuchen kannst, zu fliehen
...
Das liegt an den Cookies aus Punkt 2 und dem Umstand aus Punkt 3.`n
Wenn du den Internet Explorer 6 verwendest, klicke einfach `iExtras - Internetoptionen - Datenschutz - Bearbeiten`i, trage dort \"`^logd.lordraven.det`@\" als `iZugelassen`i ein.
Beim Internet Explorer 5 klickst du `iExtras - Internetoptionen - Sicherheit - \"Vertrauenswürdige Sites\" - Sites`i und trägst dort die beiden genannten Adressen ein. Alternativ kannst du unter Sicherheit auch einfach `iStandardstufe`i klicken.
`n
",true);
    
}else if ($_GET['op']=="faq"){
...
werden nicht geduldet und der betroffene Charakter sofort gelöscht. Das gilt auch für die Wahl des Avatars. Desweiteren dürfen Titel nicht in den Namen verwendet werden und die Namen 
sollten nach mittelalterlichen Namen oder Namen aus Mythen und Sagen klingen. Ist dies nach Meinung der Admins nicht der Fall, werdet ihr angesprochen und 
darum gebeten, Euch einen anderen Namen auszudenken. Sollte keine Reaktion darauf erfolgen, wird der betroffene Charakter 
gelöscht.`n
`n
`^2. Multi-Accounts`@`n
...
 und es gelten die in Punkt 2 genannten Einschränkungen und Strafen.`n
`n
...
dass er zur kostenlosen Werbeplattform für andere Seiten verkommt. Links in Chat-Areas werden kommentarlos entfernt. Wenn ihr einen Link unbedingt bekannt geben wollt, nutzt dazu
 bitte das Forum.`n
`n
`^7. Umgangston`@`n
...
Wenn du nichst schönes (oder nützliches oder interessantes oder kreatives für die Stimmung des Spiels) zu sagen hast, sag einfach gar nichts.`n
...
NEIN! Uns zu liebe benutze Buchstaben zum Schreiben, vollständige Wörte und verdständliche Grammatik. BITTE!`n
`n
",true);
...
`n
`^6.  Okay, ich hab den alten Mann im Wald gesehen und er hat mich mit seinem hässlichen Stock geschlagen, aber es hiess, ich wäre hässlicher als sein Stock und der Stock hat einen Charmpunkt verloren. Was ist da los?`@`n
...
`n
`^10.  Wie wird man so verdammt attraktiv?`@`n
...
        /*
        mail(getsetting("gameadminemail","logd@lordraven.de"),"LoGD Anfrage",output_array($_POST,"POST:").output_array($session,"Session:"));
...
        check die <a href='petition.php?op=faq'>FAQ</a>.  Anfragen, die das Spielgeschehen betreffen, werden 
