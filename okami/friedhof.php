...
require_once(LIB_PATH.'picture.lib.php');

page_header('Der Friedhof');
$session[user][standort]="Friedhof";
addcommentary();
...
    if($_GET['type']==1)
    {
        $str_out='`kDu gehst einen der Seitenwege entlang, in den Bereich des Friedhofs wo sich die neuesten Gräber befinden. Hier gibt es außer frischen Gräbern nicht viel zu sehen. Zwar entdeckst du auf einigen Gräbern wertvolle Grabbeigaben, du weißt jedoch dass die Götter jene strafen, die es wagen, Dinge von den Gräbern zu entwenden.';
    }
    else
    {
        $str_out='`kDu begibst dich in den Bereich des Friedhofs in dem die jüngsten und unerfahrensten Einwohner '.getsetting('townname','Atrahor').'s begraben sind. Noch so jung, und hatten doch keine Chance im rauhen Alltag...';
    }
...
        `n`k Ehrfürchtig betrittst du die große, reich verzierte Halle. Hier wurde den berühmtesten Kriegern vergangener Zeiten ein Denkmal gesetzt.';
    }
    else  
    {
        $str_out='`c`b`2Folkvang`0`b`c
        `n`k Ehrfürchtig betrittst du die große Halle. Hier wurde den großen Kriegern vergangener Zeiten ein Denkmal gesetzt.';
    }
...
{
    output('`c`b`2Das Mausoleum`0`b`c
    `n`kEhrfurchtsvoll betrittst du das Mausoleum. Es ist im gotischen Spitzbogen-Stil gebaut. Durch die schmalen, bunt verglasten Fenster an den Seitenwänden fällt etwas Tageslicht in den Raum. Geradewegs führt ein Gang an Sitzbänken vorbei direkt zum Altar.
    `nAuf der linken Seite entdeckst du einen kleinen Stand, an dem Blumen verkauft werden.`n`n');
...
{
    output('`kGleich links neben dem Eingang befindet sich ein Stand mit Schnittblumen. Damit verdient sich der Totengräber ein kleines Zubrot.`n Diese Sträuße kannst du auf den Gräbern deiner liebsten Verblichenen verteilen und ihr Grab verschönern.`n');
...
    if(($row['deathpower']<80 || ($row['deathpower']<100 && $row['marks']<CHOOSEN_FULL))
        && ac_check($row) == false 
...
        if ($row['deathpower']==100 || ($row['deathpower']==80 && $row['marks']>=CHOOSEN_FULL))
        {
            output('`$Anubis`k ist gerührt von deiner Liebe zu '.$row['name'].'`k und beschließt, '.($row['sex']?'ihr':'ihm').' noch eine Chance zu geben.`n');
        }
    }
...
    if ($session['graveid'])
    {
        output('`kDu überlegst dir, ob du nicht das Grab vor gemeinen Grabräubern schützen möchtest.`nDazu müsstest du dein Nachtlager hier aufschlagen und bist für jeden angreifbar.`nWillst du das?');
...
        {
            output('`kDu schaust dich noch einmal um. Nein, weit und breit ist niemand zu sehen der dir bei deinem Vorhaben gefährlich werden könnte. ');
...
        unset ($session['graveid']);
    }
    else output('`kIn dem ganzen Durcheinander hast du leider vergessen welches Grab du plündern wolltest.');
    break;
}
...
    else 
    {
        output('`kIn dem ganzen Durcheinander hast du leider vergessen welches Grab du plündern wolltest.');
    }
    break;
}
...
    $row = db_fetch_assoc(db_query($sql));
    // Macht das Sinn? Bei Löschen eines Users werden alle seine Bilder entfernt (talion)
...
        $result = db_query($sql);
        $row = db_fetch_assoc($result);
        output('`kWillst du das Grab von '.$row['name'].'`$ wirklich zerstören?`n`n');
...
        `n`kDu gehst durch ein großes schmiedeeisernes Tor und betrittst den Platz der ewigen Ruhe. Am Ende des Hauptweges, welcher auf beiden Seiten von Gräberreihen flankiert wird, erblickst du einen prunkvollen Säulentempel mit dem Schriftzug
        `0`c`^Den ruhmreichen Helden '.getsetting('townname','Atrahor').'s`0`c
        `k`nMitten auf dem Friedhof steht ein altertümliches Mausoleum, welches vor nicht allzu langer Zeit aufwändig restauriert wurde. Eine geflügelte Figur, deren Augen dir zu folgen scheinen, ziert die Dachspitze. Auf der Gedenktafel über der Tür ist zu lesen:`0
        `c`$Anubis, Herr über den Tod`0`c`n`n');
        viewcommentary("friedhof_main","Hinzufügen",25,"spricht leise");
        addnav('Mausoleum betreten','friedhof.php?op=temple');
        addnav('Anubis\' Gesellschaft');
