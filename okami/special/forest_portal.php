...
        `n'.create_lnk('Das Dorf erkunden.','forest.php?op=explore&what=village',true,true,'',false,'Dorf erkunden',1));
    }
...
        $session['user']['specialmisc']=serialize($backup);
...
                $backup=unserialize($session['user']['specialmisc']);
...
        else
        {
            output('Das `_Dorf`7 zu erkunden ist wohl die beste Möglichkeit, für das Ganze hier eine Lösung zu finden, also machst du dich auf den Weg. ');
...
            case 10:
            { //p) Leiche plündern
                output('Du irrst nun schon eine Weile durch '.($_GET['what']=='forest'?'den `pWald':'das `_Dorf').'`7, als du endlich etwas findest. Doch beruhigt es dich in keiner Weise, die `&Leiche`7 eines anderen Besuchers dieser Welt vor dir liegen zu sehen.
...
                `n`n`6`c<u>'.$result['tpl_name'].'</u>`c`6...als '.$ingred_nr.'. Zutat nehme man '.$ingredient['tpl_name'].'`6, '.$quality[$q].'.`n
...
            output('`n`n`7Jemand hat an eine Hauswand geschmiert: `4'.$arr_apokalypse[$rand].'`0`n`n');
