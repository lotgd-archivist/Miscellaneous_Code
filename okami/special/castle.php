...
    else
    {
        output('`n`n`#Du wirfst einen '.($session['user']['gems']>0?'Edelstein':'glänzenden Kieselstein').' hinein und wünschst dir ');
...
    $cost=$cost*0.9;
    $cost = round($cost,0);
    output('`c`b`#Kalas Beautyshop`0`b`c`n
    `3Die wunderschöne Kala begrüßt dich in ihrem Beautyshop. "`6Ah.. hallo '.$session['user']['name'].'`6.
...
    Du fragst sie, was genau sie damit meint. "`6Also: Heilung dürfte dir klar sein. Mit einer Gesichtsmaske - vorzugsweise aus Gurken von Violet - kann ich dich attraktiver machen. Und wenn du willst, kannst du mir etwas von deiner Schönheit ... überlassen und dafür etwas Erfahrung gewinnen.`3"');
...
    else if ($k_vists>=3)
    {
        output('`3"`6Ja, ich könnte dir noch eine Gurkenmaske machen, aber helfen wird sie dir heute nicht mehr.`3"');
        if($k_vists==3){
