...
$mountbuff = unserialize($session['user']['specialmisc']);
...
    {
        $beute = unserialize($session['user']['pqtemp']);
    }
    else
    {
        $beute = array();
...
                {
                    $beute[] = 'erz';
                    $session['user']['pqtemp'] = serialize($beute);
                }
            }
            break;
...
{
    case 'exit':
        if ($mount_enters==0 && is_array($mountbuff))
        {
...
            addnav('g?Nach Schätzen graben',"forest.php?op=enter&level=0&gallery=0&mount_enters=$mount_enters");
...
        else
        {
            $output_str .= "`n`4Du bist in der Mine gestorben!`nGlücklicherweise hat es dein ".$playermount['mountname']."`4 geschafft aus der Mine zu entkommen und ins Dorf zu laufen.";
...
    {
        $output_str .= "`n`4Du bist zwar in der Mine gestorben, aber dein ".$playermount['mountname']."`4 schafft es vom Eingang der Mine aus allein ins Dorf zurück zu finden.";
        addnews($session['user']['name'].' `^ging in die Goldmine, kam jedoch nie wieder heraus.');
        addnav("Weiter","shades.php");
    }
    else
...
output($output_str);
$session['user']['specialmisc'] = serialize($mountbuff);
?>
