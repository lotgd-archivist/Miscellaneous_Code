...
page_header('Yangs Waffenladen');
output('`c`b`IYangs Waffen`0`b`c`n');
...
        output('Yang lacht aus vollem Halse, als du ihm entgegentrittst und wendet sich dann auch wieder seiner Arbeit zu, nachdem er etwas Unverständliches gemurmelt hat. Was immer es war, es klang nicht sehr freundlich.');
        addnav('Zurück zum Basar','market.php');
    }
    else {
        output('Yang legt seine Sachen bei Seite und mustert dich eindringlich und nickt. Ihr steht euch nun gegenüber. Einige Schaulustige haben sich bereits am Schaufenster versammelt. Noch kannst du weglaufen. Der Kampf kostet dich, egal wie er ausgeht, 100 Donation Punkte und weitere 400, wenn du gewinnst.
        `n`nWenn du gewinnst, wird dir Yang gekaufte Waffen ganz nach deinen Wünschen gravieren.');
...
    }
    $battle=true;
    $badguy = array("creaturename"=>"`#Yang`0","creaturelevel"=>$session['user']['level'],"creatureweapon"=>"Katana","creatureattack"=>$session['user']['attack'],"creaturedefense"=>$session['user']['defence'],"creaturehealth"=>$session['user']['hitpoints'], "diddamage"=>0);
...
    $sql = "SELECT * FROM weapons WHERE level = ".$choicelevel." ORDER BY damage ASC";
    $result = db_query($sql);
    if($rename_weapons)
    {
        output('Mit genügend Entfernung zu den Ställen, um sich vor dem Gestank der Tiere zu schützen, hat sich Yang, der Zwerg, einen Raum gemietet, der zum Basar hin seinen Eingang und ein kleines Schaufenster hat.
...
                 Wenngleich Yang für seine Größe schon einen gewaltigen Bauchumfang hat und auch sein Haupt nicht mehr von viel Haar bedeckt wird, wirkt sein Blick hell und wachsam; verfolgt einen jeden Kunden aufmerksam, um seine Ware zu bewachen.
                 Man kann sich also relativ sicher sein, dass dieser so gemütlich wirkende Mann mit verdorbener Laune kurzen Prozess mit jedem Dieb machen wird...
                 `n`nAls du den Laden betrittst, winkt Yang dich mit einem angedeuteten Lächeln herein. Der Zwerg hat es gewiss nicht nötig, seine Waffen überschwänglich anzupreisen, wo doch fast jeder Krieger in diesem Dorf für den Kampf gegen den grünen Drachen eine seiner Waffen braucht.
...
    else
    {
        $str_out.='Mit genügend Entfernung zu den Ställen, um sich vor dem Gestank der Tiere zu schützen, hat sich Yang, der Zwerg, einen Raum gemietet, der zum Basar hin seinen Eingang und ein kleines Schaufenster hat.
                 Der Raum, in denen er seine Waffen an '.($session['user']['sex']?'die Frau':'den Mann').' bringen will, ist recht überschaulich, dennoch können die vielen ausgestellten Waffen,
                 welche im Schein der Fackeln, die den Raum erhellen, aufblitzen sicherlich für so manch einen begeisterten Blick des ein oder anderen Waffennarren sorgen.`n
                 Wenngleich Yang für seine Größe schon einen gewaltigen Bauchumfang hat und auch sein Haupt nicht mehr von viel Haar bedeckt wird, wirkt sein Blick hell und wachsam; verfolgt einen jeden Kunden aufmerksam, um seine Ware zu bewachen.
                 Man kann sich also relativ sicher sein, dass dieser so gemütlich wirkende Mann mit verdorbener Laune kurzen Prozess mit jedem Dieb machen wird...
                 `n`nAls du den Laden betrittst, winkt Yang dich mit einem angedeuteten Lächeln herein. Der Zwerg hat es gewiss nicht nötig, seine Waffen überschwänglich anzupreisen, wo doch fast jeder Krieger in diesem Dorf für den Kampf gegen den grünen Drachen eine seiner Waffen braucht.
                 Mit einem Blick, der jahrelange Erfahrung erkennen lässt, mustert er deine alte Waffe. Yang merkt allerdings recht schnell, dass du nicht weißt, ob du einfach warten sollst oder die Waffen betrachten kannst. `U"Willkommen in meinem Laden. Schau dich ruhig ein wenig um und such dir eine neue Waffe aus.
...
        uasort($arr_items,
            create_function('$a,$b','return strcasecmp($a["content"]["subclass"],$b["content"]["subclass"]);')
        );
...
    if (!$rename_weapons)
    {
        addnav('Yang zum Kampf herausfordern (500 DP)','weapons.php?op=duel');
    }
    addnav('Zurück zum Basar','market.php');
...
    {
        output('`0Yang schaut dich eine Sekunde lang verwirrt an und kommt zu dem Schluss, dass du ein paar Schläge zuviel auf den Kopf bekommen hast. Schließlich nickt er und grinst.');
...
            {
                output('`0Yang schüttelt nur den Kopf, als du auf eine Waffe deutest, die du dir beim besten Willen nicht leisten kannst. ');
...
                    `bGlück gehabt!`b `0Yang war gerade durch irgendwas am Fenster abgelenkt.
                    Aber nochmal passiert ihm das nicht!
                    Stolz auf deine fette Beute stolzierst du über den Basar - bis dir jemand mitteilt,
...
                    Als du mit deiner Beute über den Basar stolzierst, siehst du aus dem Augenwinkel `4Yang`0 auf dich zurauschen.
                    Er packt dich mit einer Hand an '.$session['user']['armor'].' und zerrt dich mit zur Stadtbank...`n`n
                    Yang zwingt dich mit seinen Händen eng um deinen Hals geschlungen dazu, die
...
                    output('`0Während du wartest, bis Yang in eine andere Richtung schaut, näherst du dich vorsichtig dem `L'.$row['weaponname'].'`0 und nimmst es leise vom Regal.
                    Deiner fetten Beute gewiss drehst du dich leise, vorsichtig, wie ein Ninja, zur Tür, nur um zu entdecken,
                    dass `4Yang`0 drohend in der Tür steht und dir den Weg abschneidet.
...
                    `&Du wurdest von `4Yang`& umgebracht!!!`n
...
                    addnews('`%'.$session['user']['name'].'`5 wurde beim Versuch, in `!Yang`5s Waffenladen zu stehlen, niedergemetzelt.');
                }
                else
...
                output('`0Yang` nimmt dein `L'.$session['user']['weapon'].'`0 in Zahlung.
...
                Yang fragt dich anschließend: `U"Soll ich dir was darauf eingravieren?"`n
...
            else
            {
                output('`0Yang` nimmt dein `L'.$session['user']['weapon'].'`0 stellt es aus und hängt sofort ein neues Preisschild dran.`n
                Im Gegenzug händigt er dir ein glänzendes, neues `L'.$row['weaponname'].'`0 aus, das du probeweise im Raum schwingst.
                Dabei schlägst du Yang beinahe den Kopf ab.
...
    $output.="<form action='weapons.php?op=namepreview' method='POST'><input name='newname' id='newname' value=\"".HTMLEntities($newname)."\" size=\"30\" maxlength=\"30\"> <input type='submit' value='Schaut gut aus!'></form>";
...
    $newname = preg_replace('/[`][c]/','',$newname);
...
        $output.="<form action=\"weapons.php?op=changename\" method='POST'><input type='hidden' name='name' value=\"".HTMLEntities($newname)."\"><input type='submit' value='Ja' class='button'>, meine Waffe heißt nun ".appoencode("{$newname}
...
        $output.="<form action='weapons.php?op=namepreview' method='POST'><input name='newname' value=\"".HTMLEntities($newname)."\"size=\"30\" maxlength=\"30\"> <input type='submit' value='Vorschau'></form>";
        addnav('','weapons.php?op=namepreview');
    }
...
        user_set_aei(array('weaponname'=>$session['user']['weapon']));
...
        $session['user']['badguy']='';
        $battle=false;
        output('`0Bevor du zum letzten Schlag ansetzen kannst hebt Yang eine Hand.`n
...
        addnews('`#'.$session['user']['name'].'`5 hat `!Yang`5 in einem fairen Zweikampf bezwungen.');
        $session['user']['donationspent']+=400;
        user_setConfBit(1,1,$session['user']['rename_weapons']);
...
    else if ($defeat)
    {
        output ('`0Anstatt dich in das Reich des Schlafes zu befördern, reicht Yang dir eine Hand und hilft dir auf. Das war wohl nichts!');
        $session['user']['hitpoints']=$session['user']['maxhitpoints'];
        $battle=false;
        addnews('`%'.$session['user']['name'].'`5 wurde von `!Yang`5 in einem fairen Zweikampf windelweich geschlagen.');
        addnav('Zurück zum Basar','market.php');
    }
    else
    {
