...
page_header('Yings Rüstungen');
output('`c`b`IYings Rüstungen`0`b`c`n');
...
        output('Yings lacht aus vollem Halse, als du ihr entgegentrittst und wendet sich dann auch wieder ihrer Arbeit zu, nachdem sie etwas Unverständliches gemurmelt hat. Was immer es war, es klang nicht sehr freundlich.');
        addnav('Zurück zum Basar','market.php');
    }
    else {
        output('Yings legt ihre Sachen bei Seite und mustert dich eindringlich und nickt. Ihr sitzt euch nun gegenüber. Einige Schaulustige haben sich bereits vor dem Wagen versammelt und Vessa bietet ihre Dienste als Schiedsrichter an - natürlich vollkommen unparteiisch.
        `nNoch kannst du weglaufen. Der Kampf kostet dich, egal wie er ausgeht, 100 Donation Punkte und weitere 400, wenn du gewinnst.
        `n`nWenn du gewinnst, wird dir Yings ihre Rüstungen in deinen Lieblingsfarben färben.');
...
    }
    $battle=true;
    $badguy = array("creaturename"=>"`#Yings`0","creaturelevel"=>$session['user']['level'],"creatureweapon"=>"Gestickte Ornamente","creatureattack"=>$session['user']['attack'],"creaturedefense"=>$session['user']['defence'],"creaturehealth"=>$session['user']['hitpoints'], "diddamage"=>0);
...
        $result = db_query($sql);
        $str_out='`0Unweit von Yangs Waffenladen entfernt wurde ein Zigeunerkarren aufgestellt, der mit zahlreichen bunten Tüchern geschmückt ist und mit seiner Größe besticht. Kleine Glöckchen an jenem Wagen klirren leise im Wind auf und erzeugen angenehme, dezente Klänge.
        Über eine dreistufige Treppe gelangt man in das Innere des Karren, wo es ebenso bunt ist, wie schon außerhalb. Allerdings lässt sich hier noch mehr finden: Zahlreiche Rüstungen werden hier ausgestellt und die Besitzerin, Yings, welche sie verkauft, ist in ein gleichermaßen farbenfrohes Kleid gehüllt, das um die Taille mit einer Art Kordel zusammen gebunden ist.
...
        Dennoch sollte man sich nicht in falscher Sicherheit wiegen, dass diese Waren hier ungeschützt sind, denn der Zwergenstolz von Yang veranlasst ihn dazu, auch für den Schutz Yings zu sorgen &hellip;
...
        if($rename_armor)
        {
            $str_out.='`n`0Als Stammkunde weißt du, dass Yings deine bevorzugte Rüstung auch in '.color_from_name('individuellen Farben').' bereithält.';
        }
...
        {
            //$arr_desc=array('Fundstücke','Einfaches Leder','Holzfällerkleidung','Wolfsfell','Kettenschutz','Drachenkrieger-Kleidung','Bronze-Rüstung','Zwergenrüstung','Zauber-Ringe','Magische Ringe','Yings\' Kreationen','Krimskram','Drachen-Rüstungen','Importware','Yazata-Ware','undefiniert');
...
    {
        $str_out.='Gerade noch rechtzeitig merkst du, dass du noch dein '.$session['user']['armor'].'`0 an hast. Du weißt, dass Yings nicht lange fragt und sich dein Gewand grapscht wenn du etwas Neues kaufst.
...
    if (!$rename_armor)
    {
        addnav('Yings zum Stickwettbewerb herausfordern (500 DP)','armor.php?op=duel');
    }
    addnav('Zurück zum Basar','market.php');
}
...
    {
        output('`0Yings schaut dich ein paar Sekunden verwirrt an, entschließt sich dann aber zu glauben, dass du wohl ein paar Schläge zu viel auf den Kopf bekommen hast und nickt lächelnd.');
...
                    `bGlück gehabt!`b `0Yings ist immer noch in ihre Handarbeit vertieft und hat nichts bemerkt. Aber nochmal wird ihr das nicht passieren! Stolz auf deine fette Beute stolzierst du über den Basar - bis dir jemand mitteilt, dass dir da noch ein Preisschild herumbaumelt...`nDu verlierst einen Charmepunkt!');
...
                    output('`0Du grapschst dir `z'.$row['armorname'].'`0 und tauschst `z'.$session[user][armor].'`z unauffällig dagegen aus. `bGlück gehabt!`b `0Yings ist immer noch in ihre Handarbeit vertieft und hat nichts bemerkt. Aber nochmal wird ihr das nicht passieren! Stolz auf deine fette Beute stolzierst du über den Basar - bis dir jemand mitteilt, dass dir da noch ein Preisschild herumbaumelt...`nDu verlierst einen Charmepunkt!');
...
                    output('`0Du grapschst dir `z'.$row['armorname'].'`0 und tauschst `z'.$session['user']['armor'].'`0 unauffällig dagegen aus. So schnell und unauffällig wie du kannst, verlässt du den Laden. Geschafft! Als du mit deiner Beute über den Basar stolzierst, siehst du aus dem Augenwinkel `4Yings`0 knapp an dir vorbei Richtung Stadtbank laufen. Im Vorbeigehen reißt sie das Preisschild ab, das noch immer von deiner neuen Rüstung baumelt...`n`n');
                    if ($session['user']['goldinbank']<0)
                    {
                        output('`0Da du jedoch schon Schulden bei der Bank hast, bekam Yings von dort nicht, was sie verlangte.`n
                        `0Als du dein(e/n) `z'.$row['armorname'].'`0 stolz auf dem Basar präsentierst, packt dich von hinten `4Yangs`0 starke Hand. Er entreißt dir '.$row['armorname'].' gewaltsam, drückt dir dein(e/n) alte(n/s) '.$session['user']['armor'].' in die Hand und schlägt dich nieder. Er raunzt noch etwas, daß du Glück hast, so arm zu sein, sonst hätte er er dich umgebracht und daß er dich beim nächsten Diebstahl ganz sicher umbringen wird, bevor er in seinen Laden zurück geht, wo bereits ein Kunde wartet.`n
                        Yings wird dir sowas nicht nochmal durchgehen lassen!');
                        $session['user']['hitpoints']=round($session['user']['hitpoints']/2);
                    }
                    else
                    {
                        output('`0Yings hat sich die `^'.($row['value']-$tradeinvalue).' `0Gold, die du ihr schuldest, von der Bank geholt! Sie wird dir sowas nicht nochmal durchgehen lassen.');
...
                    output('`0Du wartest, bis Yings wieder abgelenkt ist. Dann näherst du dich vorsichtig `z'.$row['armorname'].'`0 und lässt die Rüstung leise vom Stapel verschwinden, auf dem sie lag. Deiner Beute sicher drehst du dich um ... nur um festzustellen, dass du dich nicht ganz umdrehen kannst, weil sich zwei Hände fest um deinen Arme schliessen. Du schaust an ihnen herunter und stellst fest, dass diese kurzen, aber muskulösen Arme nur dem Zwerg Yang gehören können. Als du anfängst, eine Erklärung zu stammeln, hörst du nur das Zerschellen einer Vase auf deinem Kopf.
                    `n`nLangsam wird es dunkel um dich, du siehst nur noch, wie Yings etwas entsetzt auf dich starrt, nachdem Yang die Vase geschleudert hat.
                    `n`n`&Du wurdest von `4Yang`& umgebracht!!!
...
                    addnews('`%'.$session['user']['name'].'`5 wurde von `!Yang`5 für den Versuch, bei `#Yings`5 zu stehlen, umgebracht.');
                }
            }
            else
            {
...
                if ($klau==1)
                { // Fall nur für Diebe
                    output('`0Mit den Fertigkeiten eines erfahrenen Diebes tauschst du `z'.$row['armorname'].'`0 gegen `z'.$session['user']['armor'].'`0 aus und verlässt fröhlich pfeifend den Laden. `bGlück gehabt!`b Yings ist immer noch in ihre Handarbeit vertieft und hat nichts bemerkt. Trotzdem wird sie den Diebstahl früher oder später bemerken und in Zukunft besser aufpassen! Stolz auf deine fette Beute stolzierst du über den Basar - bis dir jemand mitteilt, dass dir da noch ein Preisschild herumbaumelt...`nDu verlierst einen Charmepunkt!');
...
                else if ($klau==2 || $klau==3)
                { // Diebstahl gelingt perfekt
                    output('`0Du grapschst dir `z'.$row['armorname'].'`0 und tauschst `z'.$session['user']['armor'].'`0 unauffällig dagegen aus. `bGlück gehabt!`b `0Yings ist immer noch in ihre Handarbeit vertieft und hat nichts bemerkt. Trotzdem wird sie den Diebstahl früher oder später bemerken und in Zukunft besser aufpassen! Stolz auf deine fette Beute stolzierst du über den Basar - bis dir jemand mitteilt, dass dir da noch ein Preisschild herumbaumelt...`nDu verlierst einen Charmepunkt!');
...
                    if ($session['user']['goldinbank']<0)
                    {
                        output('Da du jedoch schon Schulden bei der Bank hast, bekam Yings von dort nicht, was sie verlangte.`n
                        Als du dein(e/n) `z'.$row['armorname'].'`0 stolz auf dem Basar präsentierst, packt dich von hinten `4Yangs`0 starke Hand. Er entreißt dir '.$row['armorname'].' gewaltsam, drückt dir dein(e/n) alte(n/s) '.$session['user']['armor'].' in die Hand und schlägt dich nieder. Er raunzt noch etwas, dass du Glück hast, so arm zu sein, sonst hätte er er dich umgebracht und dass er dich beim nächsten Diebstahl ganz sicher umbringen wird, bevor er in seinen Laden zurück geht, wo bereits ein Kunde wartet.`n
                        Yings wird dich in Zukunft sehr genau im Auge behalten, wenn du ihren Laden betrittst.');
                        $session['user']['hitpoints']=ceil($session['user']['hitpoints']/2);
                    }
                    else
                    {
                        output('`0Yings` hat sich die `^'.($row['value']-$tradeinvalue).' `0Gold, die du ihr schuldest, von der Bank geholt! Sie wird dich in Zukunft sehr genau im Auge behalten, wenn du ihren Laden betrittst.');
                        $session['user']['goldinbank']-=($row['value']-$tradeinvalue);
...
                else
                { // Diebstahl gelingt nicht
                    output('`0Du grapschst dir `z'.$row['armorname'].'`0 und tauschst `z'.$session['user']['armor'].'`0 unauffällig dagegen aus. So schnell und unauffällig wie du kannst verlässt du den Laden. Geschafft! Als du dein(e/n) `z'.$row['armorname'].'`0 stolz auf dem Basar präsentierst, packt dich von hinten `4Yangs`0 starke Hand. Er entreißt dir '.$row['armorname'].' gewaltsam, drückt dir dein(e/n) alte(n/s) '.$session['user']['armor'].' in die Hand und schlägt dich nieder. Er raunzt noch etwas, dass er dich beim nächsten Diebstahl ganz sicher umbringen wird, bevor er in seinen Laden zurück geht, wo bereits ein Kunde wartet.`n');
...
                    else
                    {
                        output('`n`4Yang hat dich so schlimm erwischt, dass eine Narbe bleiben wird.`n
...
        else
        {
            output('`0Yings nimmt dein Gold und sehr zu deiner Überraschung nimmt sie auch dein(e/n) `z'.$session['user']['armor'].'`0, hängt ein Preisschild dran und legt die Rüstung hübsch zu den anderen.
            `nIm Gegenzug händigt sie dir deine wunderbare neue Rüstung `z'.$row['armorname'].'`0 aus.
            `nDu fängst an zu protestieren: `f"Werde ich nicht albern aussehen, wenn ich nichts außer '.$row['armorname'].' trage?" `0Du denkst einen Augenblick darüber nach, dann wird dir klar, dass jeder im Dorf ja dasselbe macht. `f"Na gut. Andere Länder, andere Sitten."`0');
...
        ,'demotiviert'
        ,'demotivierst'
        ,'</span><span class="c36">Yings\' Muster'
...
        $session['user']['badguy']='';
        $battle=false;
        output('`0Bevor du zum letzten Schlag ansetzen kannst hebt Yings eine Hand.`n
...
        addnews('`#'.$session['user']['name'].'`5 hat `!Yings`5 in einem Stick-Wettbewerb bezwungen.');
...
    else if ($defeat)
    {
        output ('`0Als du völlig demotiviert und unter Tränen zusammenbrichst, reicht Yings dir ihre Hand und muntert dich auf. Das war wohl nichts!');
        $session['user']['hitpoints']=$session['user']['maxhitpoints'];
        $battle=false;
        addnews('`%'.$session['user']['name'].'`5 wurde von `!Yings`5 in einem Stick-Wettbewerb völlig demotiviert.');
        addnav('Zurück zum Basar','market.php');
    }
    else
    {
