...
<?php
/*
* author: bibir (logd_bibir [-[at]-] email.de)
*      and Chaosmaker (webmaster [-[at]-] chaosonline.de)
...
page_header("Wolfs Bibliothek");
$session[user][standort]="Bibliothek";
output("`c`b`IWolfs Bibliothek des gesammelten Wissens in ".($books['anz']==1?'einem Band':$books['anz'].' Bänden')."`0`b`c`n");
...
            output("`0Die Bibliothek erstreckt sich in einer großen, steinernen Halle, in der sich unzählige Regale aneinanderreihen. Fast greifbar sind in diesem Gebäude die Stille und das Wissen, welches hier zusammengetragen wird.
...
            So mag auch jedes Kinderherz hier gewiss höher schlagen, wenn man ihnen eine dieser Geschichten vorliest. Gleich am Eingang wurde für die Bibliothekare ein kleiner Schreibtisch aufgestellt, damit sie das Geschehen am besten überblicken können.");
...
// Wortsuch-Game
if (getsetting("wortsuchspiel","0") == 1)
{
    $sentence = getsetting ("wsp_satz","");
    $arr_sentence = explode(" ",$sentence);
    $ttl = count($arr_sentence);
    foreach ($arr_sentence as $key => $val)
    {
        $nmb = $key+1;
        $hint_text = "<b>Wortsuchspiel:</b><br><br>Bravo, du hast den ".($nmb).". Teil des Lösungssatzes gefunden!<br>(".$nmb." von ".$ttl.").";
        $output = preg_replace('#\b('.$val.')\b#i',jslib_hint("\\1",$hint_text,'lotgdHintSweet'),$output);
    }
}
