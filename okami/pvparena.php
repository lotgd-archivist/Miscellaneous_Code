...
            `n
            `2Level: `6'.$badguy['level'].'`0`n
            `2`bErgebnis der letzten Runde:`b`n
            `2'.$badguy['name']."`2's Lebenspunkte: `6".$badguy['hitpoints'].'`0`n
            `2DEINE Lebenspunkte: `6'.$goodguy['hitpoints'].'`0`n
            `n
            <hr />`n
            '.$row['lastmsg'];
        output($str_output);
...
        
        output('`6Wen willst du zu einem Duell mit allen Fähigkeiten herausfordern? Die Arenagebühr kostet dich `^'.$cost.' `6Gold. Dein Gegner wird nicht unvorbereitet zustimmen.`n
...
        `nDu kannst heute noch `4'.$session['user']['playerfights'].'`6 mal gegen einen anderen Krieger antreten.`n`n
...
                    <td>[<a href="pvparena.php?op=challenge&acctid='.$row['acctid'].'">Herrausfordern</a>]</td>
                </tr>';
...
                    output('
                        `6Du hast`4 '.$row['name'].' `6zu einem Duell herausgefordert und 
                        wartest nun auf '.($row['sex']?'ihre':'seine').' Antwort. 
                        Du könntest '.$row['name'].'`6 den Kampf schmackhafter machen, 
                        indem du '.($row['sex']?'ihr':'ihm').' die Arenagebühr 
                        von  `^'.($row['level']*20).'`6 Gold überweist.`n
                    ');
...
                                Wenn '.$row['name'].'`6 sich meldet, bekommst du eine Nachricht.)`i
                        ');
                    systemmail($row['acctid'],'`2Du wurdest herausgefordert!','
                        `2'.$session['user']['name'].'`2 (Level '.$session['user']['level'].') 
                        hat dich zu einem Duell in der Arena 
                        '.($nospec?'`&`bohne Spezialfähigkeiten`b`2 ':'').'herausgefordert. 
                        Du kannst diese Herausforderung in der Arena annehmen oder ablehnen, 
                        solange sich dein Level nicht ändert.`n
                        Bereite dich gut vor und betritt die Arena erst dann!
                    ');
...
    $row = db_fetch_assoc($result);
    output('
        `6Du bist es Leid, auf '.$row['name'].'`6 zu warten und ziehst deine Herausforderung zurück. 
...
        `2Level: `6'.$badguy['level'].'`n
        `2`bBeginn der Runde:`b`n
        `2'.$badguy['name'].'`2\'s Lebenspunkte: `6'.$badguy['hitpoints'].'`n
        `2DEINE Lebenspunkte: `6'.$goodguy['hitpoints'].'`0`n`n';
    }
...
                        `bufflist1`            = '".addslashes($goodguy['bufflist'])."',
                        `lastmsg`            = '".$message."',
                        `turn`                = '2' 
                    WHERE 
                        `acctid1`            = '".$session['user']['acctid']."'
...
                        `bufflist2`            = '".addslashes($goodguy['bufflist'])."',
                        `lastmsg`            = '".$message."',
                        `turn`                = '1' 
                    WHERE 
                        `acctid2`            = '".$session['user']['acctid']."'
...
            $str_output.='`n`&Kurz vor deinem finalen Todesstoß beendet der Arenawärter euren Kampf und erklärt dich zum Sieger!`0`n`b`$Du hast '.$badguy['name'].' `$besiegt!`0`b`n`#Du bekommst das Preisgeld in Höhe von `^'.$win.'`# Gold und deinen gerechten Lohn an Arenakampfpunkten!`nDu bekommst insgesamt `^'.$exp.'`# Erfahrungspunkte!`n`0';
...
            systemmail($badguy['acctid'],'`2Du wurdest in der Arena besiegt',
                '`^'.$goodguy['name'].'`2 hat dich mit '.($session['user']['sex']?'ihrem':'seinem').' 
                `^'.$goodguy['weapon'].'`2 in der Arena besiegt!`n
                `n
...
            systemmail($badguy['acctid'],'`2 du warst in der Arena erfolgreich! ','
                `^'.$session['user']['name'].'`2 hat in der Arena verloren!`n
                `n
...
        $text=1;
        output('
            `6Da du noch eine Herausforderung mit `&'.$row['name2'].' `6offen hast, 
            machst du dich auf in Richtung Arena, um nach deinem Gegner Ausschau zu halten. 
...
    {
        output('
            `6Du wurdest von `&'.$row['name1'].' `6herausgefordert. 
            Wenn du die Herausforderung annimmst, 
            musst du `^'.$cost.'`6 Gold Arenagebühr bezahlen.`n
        ');
...
        {
            output('
                "Stopp!" der Arenawächter versperrt dir mit seiner Lanze den Weg zur Anmeldung und 
...
        addnav('Aktualisieren','pvparena.php');
        $str_output .= '
            `c`b`6Die Arena`0`b`c
            `n`6Du betrittst die große Kampfarena in der Nähe des Trainingslagers. 
            Hier kämpfen einige der Krieger gegeneinander und üben sich in ihren besonderen Fertigkeiten, 
            um herauszufinden, wer von ihnen der Beste ist. Es geht um die Ehre - 
...
                Folgende Krieger kämpfen gerade gegeneinander:`n
                `n`0
...
        else
        {
            $str_output .= '`n`n`6Im Moment laufen keine Kämpfe zwischen den Helden dieser Welt.`n`n';
        }
        output($str_output);
        unset($str_output);
