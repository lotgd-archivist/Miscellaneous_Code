...
$session[user][standort]="Schneider";
...
        <td align="right" valign="top">'.($row['tpl_gold']>0?'<br>`^'.$row['tpl_gold'].'&nbsp;Gold':'').($row['tpl_gems']>0?'<br>`#'.$row['tpl_gems'].'&nbsp;Edelstein'.($row['tpl_gems']>1?'e':''):'').'`0</td>
        </tr>';
    }
    $str_out.='</table>';
...
                if(!strpos($row['tpl_name'],'`0')) $row['tpl_name'].='`0'; //ggf Farbaufhebung anhängen
...
                `n`n"`oEine sehr gute Wahl, '.($session['user']['sex']?($session['user']['dragonkills']>10?'Madame':'junges Fräulein'):'mein Herr').', '.strip_appoencode($row['tpl_description']).'
...
        </tr><tr>
        <td>Beschreibung:</td>
        <td><input type="text" name="desc" id="desc" size="60" maxlength="300" value="'.$desc.'"></td>
        </tr><tr>
        <td>&nbsp;</td>
