...
output('`c`b`GDie Wolkeninsel`0`b`c`n');
$session[user][standort]="Wolkeninsel";
if ($_GET['op']=='')
{
    addnav('Weiter','wolkeninsel.php?op=insel');
    output('`gDu gehst am Ufer des Flusses entlang bis zu der kleinen, weißen Brücke, die zum anderen Ufer führt, welches ständig im Nebel liegt und deshalb vom Garten nicht gesehen werden kann.
    `nVorsichtig gehst du, Schritt für Schritt, über den Steg auf die andere Seite, als sich der Nebel lichtet und du eine Insel mit völlig anderem Wetter inmitten der Wolken erblickst.
...
{
    output('`gIn der Mitte der Insel steht ein kleiner Pavillon, umringt von Bäumen auf einer Wiese, durch die ein sanfter Wind weht und Geschichten erzählt von der Liebe.
...
    `nÜberall blühen Blumen in den schönsten Farben und ein kleines Rinnsal bahnt sich, lustig plätschernd, seinen Weg zum See, während Du hier und da kleine Feen sehen kannst, die sich im lustigen Tanze in der Luft bewegen.`n`n');
