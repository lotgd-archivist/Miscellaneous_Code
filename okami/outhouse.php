...
if ($_GET['op'] == 'pay')
{
    page_header('Versteckter Busch');
    user_set_aei(array('usedouthouse' => 1));
    output('`7Du bezahlst die '.$cost.' Gold an den Klo-Gnom für die Erlaubnis, den verstecketen Busch zu benutzen.`n
    Dies ist das sauberste Busch im ganzen Land!`n
...
    Beinahe rutschst Böschung dahinter runter, während du '.($session['user']['sex']?'in die Hocke gehst':'dich dahinter stellst').'. Der Klopapier-Gnom beobachtet dich bei deinem Geschäft sehr genau.`n
...
            <a href="outhouse.php?op='.($canpay?'pay':'free').'">
            <img src="images/outhouse_clean.jpg" title="Private Toilette" border="0">
            </a></td><td align="right">
...
    else
    {
        output('`2Das Dorf verfügt über 2 Klohäuschen, die wegen der monsterabwehrenden Wirkung des Gestanks etwas ausserhalb im Wald stehen.`n`n');
        switch (e_rand(1,4))
        {
        case 1:
...
        }
        output('`n`n`7Du kehrst in den Wald zurück.`n`n`0');
        forest(true);
    }
}
page_footer();
