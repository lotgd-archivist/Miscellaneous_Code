...
page_header("Das Wolfamt");
$session[user][standort]="Wolfamt";
...
        if (strip_appoencode(getsetting('fuerst',''),3) == $session['user']['login'] || $access_control->su_check(access_control::SU_RIGHT_VIEW_SYMPATHY_VOTES))
        {
            addnav('Fürstliches');
...
        }
        addnav('Zurück');
        addnav('W?zum Wolfamt','dorfamt.php');
        addnav('R?zum Reich','village.php');
        addcommentary();
...
            in der Vergangenheit bisher insgesamt `&'.$int_count.'mal`^ das Fürstenamt bekleidet haben:
            `n`n`0');
            $str_out = '<table>';
            $str_startdate = '';
            $str_msg = '';
...
                    FROM accounts
                    WHERE (locked=0 AND name LIKE "'.addslashes($search).'")
...
    case 'passier1': { //Passierschein A38 für Forum
...
    case 'passier2': { //Passierschein fertig + Eintrag der Userdaten
        include_once(LIB_PATH.'communityinterface.lib.php');
        $aUser = array();
        if(getsetting('ci_direct_pw_convert_possible',false) == true)
        {
            $str_passwd = $session['user']['password'];
        }
        else if (getsetting("ci_std_pw_active",0) == true)
        {
            $str_passwd = getsetting("ci_std_pw",'');
        }
        else if (!empty($_POST['ci_password']))
        {
            if (trim($_POST['ci_password']) == '')
            {
...
        if (user_getconfBit(CHOSEN_FULL,'marks') < CHOSEN_FULL)
        {
            addnav('Steuern');
...
        `n`0`c`^-~-
        `n`2In der Amtskasse befinden sich `^' .number_format(getsetting('amtskasse', 0),0,'',' '). ' `2Goldstücke!`0`c');
