...
$session[user][standort]="Trainingslager";
...
            addnav('V?Zum Virgator','virgator.php?op=eingang');
        }
...
output('`c`b`IBlusprings Trainingslager für Krieger`0`b`c`n');
$sql = 'SELECT * FROM masters WHERE creaturelevel = '.$session['user']['level'];
$result = db_query($sql);
$master['creaturename']='Bluespring';
...
        `nDas alte Gebäude hinter dem Übungsplatz ist die große Akademie der geheimen Künste.
        `n`nDein Meister ist `^'.$master['creaturename'].'`0.');
    }
    else{
        output('Du bummelst über den Übungsplatz. Jüngere Krieger drängen sich zusammen und deuten auf dich, als du vorüber läufst. Du kennst diesen Platz gut. Bluspring grüßt dich und du gibst ihr einen starken Händedruck. Außer Erinnerungen gibt es hier nichts mehr für dich. Du bleibst noch eine Weile und siehst den jungen Kriegern beim Training zu, bevor du zum Platz zurückkehrst.');
    }
    train_navi();
}
        
