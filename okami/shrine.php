...
// Anubis' Shrine by unknown
// found at sourceforge project page
// translation and addons by anpera

require_once "common.php";

page_header("Schrein des Anubis");
$session[user][standort]="Schrein des Anubis";
output("<span style='color: #9900FF'>",true);

addcommentary();
checkday();

output("`c`b`2Schrein des Anubis`0`b`c
...
Die Inschriften verraten dir, daß es dich den dreifachen Aufwand kostet, einen anderen zu erwecken, als wenn du dich selbst von Anubis wiedererwecken lässt.
`n`n Nachdem du dich eine Weile darauf konzentriert hast, kannst du erkennen, daß du ".$session['user']['deathpower']." Gefallen bei `\$Anubis`0 hast.`n");
...
            `&$row[name]`6 hat $row[deathpower] Gefallen bei `\$Anubis`6. Wiedererwecken?
            <input type='hidden' name='to' value=\"".HTMLEntities($row['login'])."\">
...
            systemmail($row['acctid'],"`^Du wurdest mit einem Gebet bedacht","`&".$session['user']['name']."`6 hat dich mit einem Gebet bedacht, welches Anubis veranlasst hat, dir den Weg in die Welt der Lebenden zu ebnen! Du solltest ".($session['user']['sex']?"ihr":"ihm")." dafür dankbar sein.");
            debuglog(' hat an Anubis Schrein wiedererweckt: ',$row['acctid']);
        }
    }
    else
    {
