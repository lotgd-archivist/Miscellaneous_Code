...
{
    addnav('Zurück');
    addnav('P?Zum Platz','village.php');
    addnav('B?Zum Basar','market.php');

}
else
{
    addnav(words_by_sex('Du bist tot [kleine|kleiner]!'));
...
    if ((isset($_POST['search']) && strlen($_POST['search'])>2) || (isset($_GET['search']) && strlen($_GET['search'])>2))
    {
        $newsperpage=15;
...
            FROM news
            WHERE newstext LIKE '%".addslashes(rawurldecode($_GET['search']))."%'
