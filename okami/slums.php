...
        `TGanz am Ende einer Gasse steht abseits der anderen Häuser das wohl trostlosestes Gebäude, dass es in '.getsetting('townname','Atrahor').' gibt.
...
        if ($row['postdate']>=$session['user']['recentcomments']) output('Ein paar Gauner und Ganoven haben dies ebenfalls genutzt und nutzen nun ein paar alte Möbel für ein kleines, Wärme spendendes Lagerfeuer.`n`n');
        else output('Ein paar verstaubte Möbel lassen das Innere nicht ganz so verlassen wie die Fassade wirken, doch man merkt schnell, dass lange niemand hier war.`n`n');
