...
$session[user][standort]="Bank";
output('`c`b`IDie alte Stadtbank`0`b`c`n');
...
        $show_invent = true;
        checkday();
        output('`0Die Empfangshalle der alten Bank ist schlicht und dennoch stilvoll eingerichtet, helle Kacheln säumen die Wände und viele Metallverzierungen wurden in die Möbel eingearbeitet.
...
        Einzelne Schreibtische stehen in der Halle, hinter denen die Angestellten ihren Aufgaben nachgehen, zu denen auch die Betreuung der einzelnen Bürger gehört, die hier ihr Gold oder ihre Edelsteine sicher aufbewahren oder abheben möchten.`n
...
        else if ($rows>100)
        {
            output('`0Der Bankier schaut dich überfordert an und schlägt dir vor, deine Suche vielleicht etwas mehr einzuengen, indem du den Namen genauer festlegst. Es gibt leider zu viele Bürger im Dorf, die bei diesem Namen Gold bekommen würden.`n`n');
...
            output("<script language='javascript'>document.getElementById('amount').focus();</script>");
            addnav('','bank.php?op=transfer2');
        }
        else if ($rows>1)
        {
...
        </form>
        <script language="javascript">document.getElementById("quantity").focus();</script>');
            addnav('','bank.php?op=gemstrans2');
        }
        break;
    }
...
        {
            if($session['user']['prefs']['notall2bank']) //Einstellung: etwas Gold behalten
            {
...
        $max_gemsperday=max(min($dragonkills*10,getsetting('housemaxgemsout',10)) - $rowe['gemsin'],0);
        $max_storage=min(1000,$dragonkills*10);
...
        `nDie tägliche Entnahmemenge beträgt 10 pro vollbrachte Heldentat, jedoch einschließlich Hausschatzentnahmen nicht mehr als '.getsetting('housemaxgemsout',10).'.
...
                break;
            case 13:
                $object_desc = 'Im Dorf gibt es einen Greis, der behauptet mit Tieren sprechen zu können. Dafür benötigt er einen Hörtrichter, da er seit einigen Jahren fast taub ist.';
                break;
            case 14:
...
                break;
            case 28:
                $object_desc = 'Hier meint einer, was dem Dorf noch fehlt sei eine Rennbahn für Hängebauchschweine.';
                break;
            case 29:
