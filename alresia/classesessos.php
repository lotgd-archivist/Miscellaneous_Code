...
class haus {
    ...
    function haus($id,$owner=false) {
            // Abfrage:
        if($owner == false) {
            $sql="SELECT * FROM `housesessos` WHERE houseid=".$id;
        } else {
            $sql="SELECT * FROM `housesessos` WHERE owner=".$id;
        }
        $result=db_query($sql) or die(db_error(LINK));
        $row=db_fetch_assoc($result);
        ...
        $this->textgkammer = $row['gkammerdesc'];
        $this->housepic = $row['housepic'];
    ...
    function levelup() {
        ...
        $sql="UPDATE housesessos SET level = ".$nextlevel." WHERE houseid=".$this->id;
    ...
    function eintragen() {
        ...
        $sql="UPDATE `housesessos` SET owner = ".$this->besitzerid.",status = ".$this->status.", gold = ".$this->gold.", gems = ".$this->gems.", housename = '".$this->name."', description = '".addslashes($this->text)."', gkammerdesc = '".addslashes($this->textgkammer)."', housepic = '".addslashes($this->housepic)."', ausbauten = '".implode(',',$this->ausbauten)."' WHERE houseid = ".$this->id;
        db_query($sql) or die(db_error(LINK));
    }    // Ende Funktion
    ...
    function strongest() {
        ...
        for ($i=0;$i<db_num_rows($result);$i++){
            $row = db_fetch_assoc($result);
            $sql = "SELECT value1 FROM items WHERE value1=".(int)$session['housekey']." AND class='essos' AND owner=$row[acctid] AND hvalue=".(int)$session['housekey']." ORDER BY id";
            $result2 = db_query($sql) or die(db_error(LINK));
            if (db_num_rows($result2)>0 || ((int)$row[housekey]==(int)$session['housekey'] && 0==db_num_rows(db_query("SELECT hvalue FROM items WHERE hvalue<>0 AND class='essos' AND value1<>$session[housekey] AND owner=$row[acctid]")))){
                $athome++;
...
$date2 = date("G:i");
        if (getsetting('activategamedate','0')==1) output("Wir schreiben den `^{$date} im Jahre ".$settings['year']." nach dem letzten Meteoriteneinschlag`2.`n");
        output ("Es ist jetzt `^".$date2."`2 Uhr.`n`n");
        viewcommentary("essos-haus-".$this->id,"Mit Mitbewohnern reden:",30,"sagt");
if (!$session['user']['prefs']['ooc']){output("`n`n");
viewcommentary("oocvillage","`nOOC:`&",10); }

        addnav("Ausgang");
        addnav("Zurück zur Stadt","essos_village.php");            addnav("W?Zurück zum Wohnviertel","housesessos.php");

        addnav("Im Haus");
            addnav("Schatzkammer","housesessos.php?op=drin&go=schatz");
            addnav("Schlafzimmer","housesessos.php?op=drin&go=schlafzimmer");
            addnav("Badezimmer","housesessos.php?op=drin&go=badezimmer");
            addnav("Arbeitszimmer","housesessos.php?op=drin&go=office");
        // Ermittlung der Installierten und Aktiven Hauserweiterungen START
if($session['user']['rlalter']>0)$sql = "SELECT * FROM `zimmeressos` WHERE aktiv=1 AND level < ".$this->level->level." ORDER BY `zimmerid` ASC";
else        $sql = "SELECT * FROM `zimmeressos` WHERE aktiv=1 AND zimmerid<>45 and level < ".$this->level->level." ORDER BY `zimmerid` ASC";
        $result = db_query($sql) or die(db_error(LINK));
        $schalter = false;
        for($i=0;$i<db_num_rows($result);$i++) {
                $row = db_fetch_assoc($result);
...
if($session['user']['rlalter']){ addnav($row['label'],$row['link']);
}
if(!$session['user']['rlalter']){
if($row['zimmerid']!=45)        addnav($row['label'],$row['link']);
}
            }
        }
    ...
    function bio() {
        ...
        output($this->level->name."`2Du riskierst einen Blick durch eines der Fenster");
                ...
                /* Item-Anzeige START */
        $sql="SELECT * FROM items WHERE class='MöbelHeave' AND value1=".$this->id." ORDER BY id ASC";
        $result = db_query($sql) or die(db_error(LINK));
        ...
        $sql="SELECT value1 FROM items WHERE class = 'essos' AND owner = ".$session['user']['acctid']." AND value1 = '".$this->id."' LIMIT 1";
        if(db_num_rows(db_query($sql)) == 1) addnav("Haus betreten","housesessos.php?op=drin&id=".$this->id);
        if($session['user']['housekey'] == 0 && $haus->status >= 2 && $haus->status <= 4) addnav("Haus Kaufen","housesessos.php?op=kaufen&id=".$haus->id);
        addnav("Zurück","housesessos.php");
if($this->housepic){
   $pic_size = @getimagesize($this->housepic);
   $pic_width = $pic_size[0];
    $pic_height = $pic_size[1];
   output("`n`n<img src=\"$this->housepic\" ",true);
   ...
   if ($pic_height > 300) output("height=\"300\" ",true );
    ...
    function hausbauen() {
        ...
        for ($i=1;$i < $basis->keys;$i++){
            $sql = "INSERT INTO items (name,owner,class,value1,value2,gold,gems,description) VALUES ('Schlüssel von essos',".$session['user']['acctid'].",'essos',".$this->id.",$i,0,0,'Schlüssel für Haus Nummer ".$this->id."')";
            db_query($sql);
    ...
    function klauen() {
        ...
        global $session;
        addnav("Zurück zur Stadt","essos_village.php");
        switch(e_rand(1,3)) {
            case 1:
            $getgems=0;
    ...
    function kaufen() {
        ...
        if ($session['user']['acctid']==$this->besitzerid) {
            ...
            $this->gold = 0;
            $this->gems = 0;
        }elseif ($this->status != 3 && $this->status != 4 && $session['user']['gold']< $this->kosten['gold'] || $session['user']['gems'] < $this->kosten['gems']){
...
if ($this->status != 3 && $this->status != 4)            $session['user']['gold'] -= $this->kosten['gold'];
if ($this->status != 3 && $this->status != 4)            $session['user']['gems'] -= $this->kosten['gems'];
            ...
            if ($this->status == 3){
                ...
                $sql = "UPDATE items SET owner=".$session['user']['acctid']." WHERE owner=0 and class='essos' AND value1=".$this->id;
                output("Du steckst ein kleines Fähnchen in den Boden, 'Hier wohne ich' steht darauf, stolz bist du, dass du dieses wundervolle Häuschen umsonst bekommen hast! Bitte bedenke, dass du ein verlassenes Haus gekauft hast, zu dem vielleicht noch andere einen Schlüssel haben!");
                $session['user']['housekey']=$this->id;
            }else if ($this->status == 4){
                    // Bauruine
                $this->status = 0;
                $this->besitzerid = $session['user']['acctid'];
                output("Du steckst ein kleines Fähnchen in den Boden, 'Hier wohne ich' steht darauf, stolz bist du, dass du dieses wundervolle Häuschen umsonst bekommen hast! Bitte bedenke, dass du eine Bauruine gekauft hast, die du erst fertigbauen musst!");
            }else{
                    // Anderer Status(2)
                $this->gold = 0;
                $this->gems = 0;
                $this->status = 1;
                $this->besitzerid = $session['user']['acctid'];
                $sql = "UPDATE items SET owner=".$session['user']['acctid']." WHERE class='essos' AND value1=".$this->id;
                $session['user']['housekey']=$this->id;
            }    // Ende ELSE-IF innen
            db_query($sql);
        }    // Ende ELSE-IF aussen
    ...
    function verkaufsabwicklung() {
        ...
        $sql = "SELECT owner FROM items WHERE value1=".$this->id." AND class='essos' AND owner<>".$session['user']['acctid']." ORDER BY id ASC";
        $result = db_query($sql) or die(db_error(LINK));
        ...
        $sql = "UPDATE items SET owner=".$this->besitzerid." WHERE class='essos' AND value1=".$this->id;
        db_query($sql);
    ...
    function verkauf() {
        ...
        output("`0<form action='housesessos.php?op=verkaufen&act=verkauf' method='POST'>",true);
        ...
        addnav("","housesessos.php?op=verkaufen&act=verkauf");
        addnav("An den Makler","housesessos.php?op=verkaufen&act=makler");
        addnav("W?Zurück zum Wohnviertel","housesessos.php");
        addnav("Zurück zur Stadt","essos_village.php");
        } else {
            $standard = $this->wert(false);
    ...
    function maklerverkauf() {
        ...
        if($_GET['valid']==true) {
            ...
            $this->besitzerid = 0;
            $sql = "UPDATE items SET owner=0 WHERE class='essos' AND value1=".$this->id;
            db_query($sql) or die(db_error(LINK));
            $this->eintragen();
        } else {
            ...
            addnav("Ja, Verkaufen!","housesessos.php?op=verkaufen&act=makler&valid=true");
        }
        addnav("W?Zurück zum Wohnviertel","housesessos.php");
        addnav("Zurück zur Stadt","essos_village.php");
    } // Ende Funktion
...
class collector {
    ...
    function ausgeben() {
        ...
        $ausgabe .="</td><td>\n";
            ...
            $sql = "SELECT * FROM `zimmeressos` ORDER BY `zimmerid`";
            $result = db_query($sql) or die(db_error(LINK));
            if(db_num_rows($result) < 1) {
                ...
                for($i=0;$i< db_num_rows($result);$i++) {
                    ...
                    $zimmer = new zimmer;
                    $zimmer->einlesen($row);
                    $ausgabe .= "<tr class='".$bgcolor."'><td>".$zimmer->id."</td><td>".$zimmer->name."</td><td>".str_replace('housesessos.php?op=drin','',$zimmer->linker)."</td><td>"
                    .$zimmer->label."</td><td>".$zimmer->level."</td><td>".$zimmer->aktiv."</td><td>"
                    .$zimmer->gold."</td><td>".$zimmer->gems."</td><td><a href='nhmasteressos.php?op=zimmeredit&zimmer=".$zimmer->id."'>edit</a></td></tr>\n";
                    addnav("","nhmasteressos.php?op=zimmeredit&zimmer=".$zimmer->id);
                    unset($zimmer);
                }    // Ende FOR
            }    // Ende (ELSE-)IF
            $ausgabe .="</table>\n";
    ...
    function pruefung() {
        ...
        addnav("","nhmasteressos.php?op=weiter");
        $match = false; $ausgabe = "";
        if($input!==false) {
        ...
        if($match) {
            output("<form action='nhmasteressos.php?op=weiter' method='POST'>\n<table><tr><td>Neue Zimmerklasse</td><td>Aktivieren</td></tr>",true);
            output($ausgabe,true);
    ...
    function weiterverarbeitung() {
        ...
        foreach($_POST as $key=>$val) {
            ...
            if($val = "aktiv") {
                ...
                $nav         = $original['nav'];
                $sql = "INSERT INTO `zimmeressos` (name,link,label,level,aktiv,gold,gems) VALUES ('".$key."','housesessos.php?op=drin&go=".$key."','".$key."',
    ...
    function run() {
        ...
        do{ // Suche
            $zeile = fgets($datei);
            $suche = ereg($suchmuster,$zeile,$treffer[$x]);
            if($suche) {
                    // Stringbearbeitung
