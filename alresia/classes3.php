...
class ausbaustufe {
    ...
    function ausbaustufe($level=1) {
        ...
        if($level <= $this->hoechste()) {
                // Abfrage:
            $sql="SELECT * FROM hauslevels3 WHERE level = ".$level;
    ...
    function levels() {
        $sql="SELECT COUNT(level) AS anzahl FROM hauslevels3";
        $result=db_query($sql) or die(db_error(LINK));
        $row=db_fetch_assoc($result);
    ...
    function hoechste() {
        $sql="SELECT MAX(level) AS maximum FROM hauslevels3";
        $result=db_query($sql) or die(db_error(LINK));
        $row=db_fetch_assoc($result);
    ...
    function speichern($aufid=false) {
        ...
        if($aufid <= $this->levels()) {
            $sql = "UPDATE `houselevels3` SET level=".$aufid.", name=".$this->name.", gold=".$this->gold.", gems=".$this->gems.", keys=".$this->keys.", goldchest=".$this->goldchest.", gemschest=".$this->gemschest." WHERE level=".$this->level;
        } else {
            $sql = "INSERT INTO houselevels3 SET level=".$aufid.", name=".$this->name.", gold=".$this->gold.", gems=".$this->gems.", keys=".$this->keys.", goldchest=".$this->goldchest.", gemschest=".$this->gemschest;
        }
...
class haus {
    ...
    function haus($id,$owner=false) {
            // Abfrage:
        if($owner == false) {
            $sql="SELECT * FROM `houses3` WHERE houseid=".$id;
        } else {
            $sql="SELECT * FROM `houses3` WHERE owner=".$id;
        }
        $result=db_query($sql) or die(db_error(LINK));
        $row=db_fetch_assoc($result);
        ...
        if($this->besitzerid > 0) {
            ...
            // Fixe Werte:
        $this->goldkosten = getsetting("baukostengold2",50000);
        $this->gemskosten = getsetting("baukostengems2",30);
            // Sonstiges
    ...
    function wert($level=false) {
        ...
        $sql = "SELECT level, gold, gems FROM hauslevels3 WHERE level <= $level";
        $result = db_query($sql) or die(db_error(LINK));
        ...
        $kosten['basisgold'] = getsetting("baukostengold2",50000);
        $kosten['basisgems'] = getsetting("baukostengems2",50000);
    ...
    function eintragen() {
        ...
        $sql="UPDATE `houses3` SET owner = ".$this->besitzerid.",status = ".$this->status.", gold = ".$this->gold.", gems = ".$this->gems.", housename = '".$this->name."', description = '".addslashes($this->text)."', ausbauten = '".implode(',',$this->ausbauten)."' WHERE houseid = ".$this->id;
        db_query($sql) or die(db_error(LINK));
    }    // Ende Funktion
    ...
    function strongest() {
        ...
        $sql = "SELECT acctid,name,maxhitpoints,defence,attack,level,laston,loggedin,login,hkey2 FROM accounts WHERE
        (locked=0) AND
        (alive=1 AND location=6) AND
        ...
        for ($i=0;$i<db_num_rows($result);$i++){
            $row = db_fetch_assoc($result);
            $sql = "SELECT value1 FROM items WHERE value1=".(int)$session['hkey2']." AND class='Schlüssel3' AND owner=$row[acctid] AND hvalue=".(int)$session['hkey2']." ORDER BY id";
            $result2 = db_query($sql) or die(db_error(LINK));
            if (db_num_rows($result2)>0 || ((int)$row[hkey2]==(int)$session['hkey2'] && 0==db_num_rows(db_query("SELECT hvalue FROM items WHERE hvalue<>0 AND class='Schlüssel3' AND value1<>$session[hkey2] AND owner=$row[acctid]")))){
                $athome++;
    ...
    function flur($text="") {
        ...
        viewcommentary("haus3-".$this->id,"Mit Mitbewohnern reden:",30,"sagt");
        output("`n`n`n`n`%`GOOC-Bereich:`n");
        viewcommentary("ooc");
        addnav("Im Haus");
            addnav("Schatzkammer","nhouses3.php?op=drin&go=schatz");
            addnav("Schlafzimmer","nhouses3.php?op=drin&go=schlafzimmer");
            addnav("Arbeitszimmer","nhouses3.php?op=drin&go=office");
        // Ermittlung der Installierten und Aktiven Hauserweiterungen START
        $sql = "SELECT * FROM `zimmer3` WHERE aktiv=1 AND level < ".$this->level->level." ORDER BY `zimmerid` ASC"; 
        $result = db_query($sql) or die(db_error(LINK));
        $schalter = false;
        for($i=0;$i<db_num_rows($result);$i++) {
                $row = db_fetch_assoc($result);
    ...
    function bio() {
        ...
        $sql="SELECT value1 FROM items WHERE class = 'Schlüssel3' AND owner = ".$session['user']['acctid']." AND value1 = '".$this->id."' LIMIT 1";
        if(db_num_rows(db_query($sql)) == 1) addnav("Haus betreten","nhouses3.php?op=drin&id=".$this->id);
        if($session['user']['hkey2'] == 0 && $haus->status >= 2 && $haus->status <= 4) addnav("Haus Kaufen","nhouses3.php?op=kaufen&id=".$haus->id);
    ...
    function hausbauen() {
        ...
        $this->gold = 0;
        $this->gems = 0;
        $session['user']['hkey2'] = $this->id;
        $this->status = 1;
        addnews("`2".$session['user']['name']."`3 hat das Haus `2".$this->name."`3 fertiggestellt.");
        for ($i=1;$i < $basis->keys;$i++){
            $sql = "INSERT INTO items (name,owner,class,value1,value2,gold,gems,description) VALUES ('Hausschlüssel',".$session['user']['acctid'].",'Schlüssel3',".$this->id.",$i,0,0,'Schlüssel für Haus Nummer ".$this->id."')";
            db_query($sql);
    ...
    function kaufen() {
        ...
        if ($session['user']['acctid']==$this->besitzerid) {
            ...
            if ($this->besitzerid > 0){
                $sql = "UPDATE accounts SET goldinbank=goldinbank+".$this->kosten['gold'].",gems=gems+".$this->kosten['gems'].",house=0,hkey2=0 WHERE acctid=".$this->besitzerid;
                db_query($sql);
            ...
            if ($this->status == 3){
                ...
                $sql = "UPDATE items SET owner=".$session['user']['acctid']." WHERE owner=0 and class='Schlüssel3' AND value1=".$this->id;
                ...
                $this->status = 1;
                $this->besitzerid = $session['user']['acctid'];
                $sql = "UPDATE items SET owner=".$session['user']['acctid']." WHERE class='Schlüssel3' AND value1=".$this->id;
                $session['user']['hkey2']=$this->id;
            }    // Ende ELSE-IF innen
            db_query($sql);
        }    // Ende ELSE-IF aussen
    ...
    function verkaufsabwicklung() {
        ...
        $sql = "SELECT owner FROM items WHERE value1=".$this->id." AND class='Schlüssel3' AND owner<>".$session['user']['acctid']." ORDER BY id ASC";
        $result = db_query($sql) or die(db_error(LINK));
        ...
        $sql = "UPDATE items SET owner=".$this->besitzerid." WHERE class='Schlüssel3' AND value1=".$this->id;
        db_query($sql);
        $this->gold = $_POST['gold'];
        $this->gems = $_POST['gems'];
        $this->status = 2;
        $session['user']['hkey2'] = 0;
    }    // Ende Funktion
    ...
    function verkauf() {
        ...
        addnav("Zurück zum Sonnenplatz","silanna_village.php");
            ...
            if($_POST['gold'] <= ($standard['gold']/2) || $_POST['gems'] <= ($standard['gems']/2)) {// $_POST['gold'] > ($standard['gems']*2) || $_POST['gems'] > ($standard['gems']*2)) {
                ...
                output(" unter euch aufgeteilt und deine Untermieter haben ihre Schlüssel3 abgegeben.");
    ...
    function maklerverkauf() {
        ...
        if($_GET['valid']==true) {
            ...
            $this->besitzerid = 0;
            $sql = "UPDATE items SET owner=0 WHERE class='Schlüssel3' AND value1=".$this->id;
            db_query($sql) or die(db_error(LINK));
            $this->eintragen();
        } else {
            ...
            addnav("Ja, Verkaufen!","nhouses3.php?op=verkaufen&act=makler&valid=true");
        }
        addnav("W?Zurück zum Wohnviertel","nhouses3.php");
        addnav("Zurück zum Sonnenplatz","silanna_village.php");
    } // Ende Funktion
        ...
        $ausgabe .="</td><td>\n";
            ...
            if(db_num_rows($result) < 1) {
                ...
                for($i=0;$i< db_num_rows($result);$i++) {
                    ...
                    $zimmer = new zimmer;
                    $zimmer->einlesen($row);
                    $ausgabe .= "<tr class='".$bgcolor."'><td>".$zimmer->id."</td><td>".$zimmer->name."</td><td>".str_replace('nhouses3.php?op=drin','',$zimmer->linker)."</td><td>"
    ...
    function weiterverarbeitung() {
        ...
        foreach($_POST as $key=>$val) {
            ...
            if($val = "aktiv") {
                ...
                $nav         = $original['nav'];
                $sql = "INSERT INTO `zimmer` (name,link,label,level,aktiv,gold,gems) VALUES ('".$key."','nhouses3.php?op=drin&go=".$key."','".$key."',
