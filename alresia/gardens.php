...
// output("`b`c`\$IN ÜBERARBEITUNG`b`c`0`n");
    if ($session[user][seenlover]){
        ...
        $partner=$row[name];
        if ($partner=="") $partner = $session[user][sex]?"`^Steve`0":"`5Wanda`0";
        ...
        if ($session[user][marriedto]==4294967295) output("Du denkst nochmal über deine Ehe mit ".($session[user][sex]?"`^Steve`0":"`5Wanda`0")." nach und überlegst, ob du ".($session[user][sex]?"ihn":"sie")." in der Kneipe besuchen sollst, oder für wen du diese Ehe aufs Spiel setzen würdest.`n");
        if($session[user][charm]==4294967295) output("Du überlegst dir, dass du dir mal wieder etwas Zeit für deinen Partner nehmen solltest. Während du deinen Ehepartner im Garten suchst, stellst du aber fest, dass der Rest der Leute hier auch nicht zu verachten ist.`n");
        ...
        (laston > '".date("Y-m-d H:i:s",strtotime(date("r")."-9996000 sec"))."' OR (charisma=4294967295 AND acctid=".$session[user][marriedto].") )
    ...
    if (db_num_rows($result)>0){
        ...
        if ($session[user][turns]<1){
            ...
            output("Du näherst dich `6$row[name]`0 und mit dem Charme einer Blattlaus fängst du ein Gespräch an. Schon fast beleidigt dreht sich $row[name] um und stapft davon.`nDu solltest etwas an deiner Ausstrahlung arbeiten...");
        } else if ($row[charm]<=1 && $session[user][charisma]!=4294967295){
            output("Du näherst dich `6$row[name]`0. Je näher du kommst, umso hässlicher kommt $row[name] dir vor. Am Ende wirkt $row[name] so abstoßend auf dich, dass du einfach vorbei zurück ins Dorf läufst.");
            ...
            output("Du entdeckst `6$row[name]`0 im Schatten unter einer Gruppe Bäume und machst dich sofort daran, mit deiner Alefahne ein lallendes Gespräch anzufangen. Als $row[name] überhaupt ");
            output("nicht reagiert und immer noch irgendwie auf den Boden zu starren scheint, willst du dessen Kopf heben - und greifst voll in das Dornengestrüpp vor dir.`n");
            ...
            if ($session[user][marriedto]==$row[acctid] && $session[user][acctid]==$row[marriedto]){
                output("`%Du führst deinen Partner `6$row[name]`% in den Garten aus und ihr nehmt euch etwas Zeit füreinander. ");
                ...
                switch(e_rand(1,4)){
                    ...
                    break;
                    case 4:
                    output(" aber ihr werdet bei eurem Vergnügen von deinem Partner erwischt.`nDie Katastrophe ist komplett.`0`n`nDein Ehepartner verlässt dich"); 
                    if ($session[user][charisma]==4294967295){
                        ...
                        db_query($sql);
                        systemmail($session[user]['marriedto'],"`\$Scheidung!`0","`6Du hast `&{$session['user']['name']}`6 mit `&{$row[name]} im Garten erwischt und verlässt deinen Partner.`nDir werden `^$getgold`6 Gold von deinem ehemaligen Ehepartner zugesprochen.");
                    ...
                    $session['user']['seenlover']=1;
                    systemmail($row['acctid'],"`%Gartenflirt!`0","`&{$session['user']['name']}`6 hat mit dir geflirtet und wurde dabei von dessen Ehepartner erwischt.");
                    $session[user][charm]-=1;
                    addnews("`\$".$session[user][name]."`\$ wurde beim Flirten mit $row[name] `\$im Garten von dessen Ehepartner erwischt und ist jetzt wieder solo.");
                    break;
                }
            }
            ...
            if ($session[user][marriedto]==4294967295 && $session[user][charisma]>=5){
                output("`6".($session[user][sex]?"Steve":"Wanda")." springt aus einem Gebüsch und beschimpft dich aufs Heftigste, als du dich $row[name] nähern willst. ".($session[user][sex]?"Er":"Sie")."  beobachtet deine \"Gartenarbeit\" schon eine ganze Weile!`0`n`n".($session[user][sex]?"Steve":"Wanda")." verlässt dich.`nDu verlierst einen Charmepunkt."); 
                $session[user][marriedto]=$row[acctid];
                $session[user][charisma]-=1;
                $session['user']['seenlover']=1;
                $session[user][charm]-=1;
                addnews("`\$".$session[user][name]."`\$ wurde beim Flirten mit $row[name] im Garten von ".($session[user][sex]?"Steve":"Wanda")." erwischt und ist jetzt wieder solo.");
            } else {
                if ($session[user][acctid]==$row[marriedto]){
                    output("`%Obwohl du verheiratet bist, gehst du auf die Flirtversuche von $row[name] ein. Ihr versteht euch prima und für einen Moment vergisst du deinen Ehepartner.");
                } else {
                    output("`%Obwohl du verheiratet bist, lässt du dich auf einen Flirt ein. Ihr versteht euch prima und für einen Moment vergisst du deinen Ehepartner. ");
                }
                switch(e_rand(1,4)){
                    case 1:
                    case 2:
                    ...
                    break;
                    case 4:
                    output(" Aber dein Partner ruft sich selbst aufs Heftigste ins Gedächtnis zurück!`nDie Katastrophe ist komplett.`0`n`nDein Ehepartner verlässt dich ");
                    if ($session[user][charisma]==4294967295){
                    ...
                    $session[user][charisma]=1;
                    $session['user']['seenlover']=1;
                    systemmail($row['acctid'],"`%Gartenflirt!`0","`&{$session['user']['name']}`6 hat mit dir geflirtet, wurde dabei aber von dessen Ehepartner erwischt.");
                    $session[user][charm]-=1;
                    addnews("`\$".$session[user][name]."`\$ wurde beim Flirten im Garten von dessen Ehepartner erwischt und ist jetzt wieder solo.");
                    break;
                }
            }
