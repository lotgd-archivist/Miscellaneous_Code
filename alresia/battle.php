...
if ($badguy['creaturehealth']>0 && $session['user']['hitpoints']>0) {
    ...
    output("`kDu hast den Gegner `m$badguy[creaturename]`k entdeckt, der sich mit seiner Waffe `m$badguy[creatureweapon]`k auf dich stürzt!`0`n`n");

    if ($session['user']['alive']){
        output("`kLevel: `m$badguy[creaturelevel]`0`n");
    }else{
        output("`kLevel: `mUntoter`0`n");
    }

    output("`k`bBeginn der Runde:`b`n");
    output("`k$badguy[creaturename]`k's ".($session['user']['alive']?"Lebenspunkte":"Seelenpunkte").": `o$badguy[creaturehealth] `0`n");
    output("`kDEINE ".($session['user']['alive']?"Lebenspunkte":"Seelenpunkte").": `o".$session[user][hitpoints]."`0`n");
}
...
if ($session['user']['alive'] && $session['user']['buffbackup']==""){
    if (e_rand(1,6)==2){
        ...
        if ($wetter=="Starker Wind mit vereinzelten Regenschauern"){
            if (e_rand(1,2)==1){
                $session['bufflist']['weather'] = array("name"=>"`oWetter","rounds"=>1,"wearoff"=>"","atkmod"=>0,"roundmsg"=>"`oEin starker Windstoss läßt dich dein Ziel verfehlen.","activate"=>"offense");
            }else{
                $session['bufflist']['weather'] = array("name"=>"`oWetter","rounds"=>1,"wearoff"=>"","badguyatkmod"=>0,"roundmsg"=>"`oEin starker Windstoss hindert {badguy} daran, dich zu treffen.","activate"=>"defense");
            }
        }else if ($wetter=="Schneeregen"){
            if (e_rand(1,2)==1){
                $session['bufflist']['weather'] = array("name"=>"`oWetter","rounds"=>1,"wearoff"=>"","defmod"=>0,"roundmsg"=>"`oDurch den Schneeregen siehst du den Schlag deines Gegners nicht kommen.","activate"=>"defense");
            }else{
                $session['bufflist']['weather'] = array("name"=>"`oWetter","rounds"=>1,"wearoff"=>"","badguydefmod"=>0,"roundmsg"=>"`oDurch den Schneeregen sieht dein Gegner deinen Schlag nicht kommen.","activate"=>"offense");
            }
        }
    }
}
...
$creaturedmg=0;
$selfdmg=0;
if ($count>1 && $session[user][hitpoints]>0 && $badguy[creaturehealth]>0) output("`k`bNächste Runde:`b`n");
