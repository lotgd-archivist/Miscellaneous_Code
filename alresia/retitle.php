...
if ($_GET['op']=="rebuild"){
    ...
    //output("<pre>".HTMLSpecialChars($titles)."</pre>",true);
    //output("<pre>".HTMLSpecialChars(output_array($titles))."</pre>",true);
   ...
   // $sql = "SELECT   name,title,dragonkills,acctid,sex,ctitle  FROM  accounts WHERE dragonkills>0";
    ...
    for ($i=0;$i<db_num_rows($result);$i++){
        ...
        if ($row['ctitle'] == "") {
            ...
            if ($row['title']!=""){
                $n = $row['name'];
                $x = mb_strpos($n,$row['title']);
                if ($x!==false){
                    $regname=mb_substr($n,$x+mb_strlen($row['title']));
                    $row['name'] = mb_substr($n,0,$x).$newtitle.$regname;
                }else{
