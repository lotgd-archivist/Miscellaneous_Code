...
if ($_GET[op]=="buy"){ // Wig-Wam Bam
    if (!$_GET[id]){
        ...
        output("`qStolz präsentiert dir der Händler `tGunther`q seinen Wagen. Zu jedem der seltsamen Gegenstände, Artefakte und Zauber scheint er eine kleine Geschichte zu kennen. Dabei scheint er auffällig oft darauf ");
    ...
    if (!$_GET[id]){
        ...
        if (db_num_rows($result)){
            ...
            output("Du hast aber nichts, was `tGunther`q interessieren würde. Enttäuscht schlenderst du zurück zum Brunnenplatz.");
        }
    }else{ // Hell Raiser
        $sql="SELECT * FROM items WHERE id=$_GET[id]";
        $result=db_query($sql);
    ...
    output("`qHeute ist der Wanderhändler `tGunther `qwieder im Dorf! Direkt am Brunnenplatz hat er seinen Wagen aufgebaut, sehr zum missfallen der feinen Bürger Nuvolas. Da er aber ");
    output(" selbst hin und wieder Handel mit ihnen betreibt, lassen sie ihn widerwillig gewähren.`nNeugierig näherst du dich dem Wagen, um zu sehen, ob der Händler diesmal etwas Interessantes");
