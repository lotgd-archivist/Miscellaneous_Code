...
if ($_POST[name]!=""){
    if ($session[loggedin]){
        ...
        if(0){
            ...
            $result = db_fetch_assoc(db_query("SELECT COUNT(acctid) AS onlinecount FROM accounts WHERE locked=0 AND loggedin=1 AND laston>'".date("Y-m-d H:i:s",strtotime(date("r")."-".getsetting("LOGINTIMEOUT",4900)." seconds"))."'"));
            ...
            if (db_num_rows($result)==1){
                ...
                /*if (mb_strpos($_SERVER['SERVER_NAME'],"logd.mightye.org")!==false && $session['user']['superuser']<1){
                ...
                if ($session[user][emailvalidation]!="" && mb_substr($session['user']['emailvalidation'],0,1)!="x"){
                    $session[user]=array();
