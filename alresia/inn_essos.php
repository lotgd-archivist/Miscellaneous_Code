...
$buff = array("name"=>"`!Schutz der Liebe","rounds"=>60,"wearoff"=>"`!Du vermisst ".($session['user']['sex']?"Kain":"`5Irina`")."!.`0","defmod"=>1.2,"roundmsg"=>"Deine große Liebe lässt dich an deine Sicherheit denken!","activate"=>"defense");

page_header("Zum durstigen Zentaur");
output("<span style='color: #9900FF'>",true);
output("`c`bZum durstigen Zentaur`b`c");
...
if ($_GET[op]=="" || $_GET[op]=="strolldown"){
    output("  Du winkst einigen deiner Kumpels und zwinkerst ".
                ($session[user][sex]?
                "`^Kain`0 zu, der seine Harfe beim Feuer stimmt.":
                "`5Irina`0 zu, die ein paar einheimischen Ale serviert.").
    ...
    if (db_num_rows($result)<=0){
        ...
        for ($i=0;$i<db_num_rows($result);$i++){
            ...
            if ($row[acctid]==$session[user][acctid]){
                output("[<a href='inn_essos.php?op=msgboard&act=del'>entfernen</a>]",true);
                addnav("","inn_essos.php?op=msgboard&act=del");
            }
        }
    }

    addnav("Was machst du?");
    if ($session[user][sex]==0) addnav("V?Flirte mit Irina","inn_essos.php?op=Irina");
    if ($session[user][sex]==1) addnav("V?Quatsche mit Irina","inn_essos.php?op=Irina");
    addnav("S?Rede mit dem Barden Kain","inn_essos.php?op=Kain");
    addnav("Mit Freunden unterhalten","inn_essos.php?op=converse");
    addnav("B?Spreche mit Barkeeper Cedrik","inn_essos.php?op=bartender");
    if (getsetting("pvp",1)) {
    ...
    addnav("n?Zimmer nehmen (Log out)","inn_essos.php?op=room");
  ...
  switch($_GET[op]){
    ...
    if ($_GET[act]=="del"){
        ...
        addnav("Neue Nachricht","inn_essos.php?op=msgboard"); 
        ...
        output("<form action=\"inn_essos.php?op=msgboard&act=add2&amt=$_GET[amt]\" method='POST'>",true);
        ...
        addnav("","inn_essos.php?op=msgboard&act=add2&amt=$_GET[amt]");
        ...
        addnav("$msgdays Tage (`^$msgprice`0 Gold)","inn_essos.php?op=msgboard&act=add1&amt=1");
        addnav("".($msgdays*3)." Tage (`^".($msgprice*3)."`0 Gold)","inn_essos.php?op=msgboard&act=add1&amt=3");
        addnav("".($msgdays*10)." Tage (`^".($msgprice*10)."`0 Gold)","inn_essos.php?op=msgboard&act=add1&amt=10");
      ...
      case "Irina":
            ...
            */
            if ($session[user][sex]==1){
                if ($_GET[act]==""){
                    addnav("Tratsch","inn_essos.php?op=Irina&act=gossip");
                    addnav("Frage, ob dich dein ".$session[user][armor]." dick aussehen lässt","inn_essos.php?op=Irina&act=fat");
                    output("Du gehst rüber zu `5Irina`0 und hilfst ihr dabei, ein paar Ales zu servieren. Als sie alle ausgeteilt sind, ");
                    ...
                    output("Für ein paar Minuten tratschst du mit `5Irina`0 über alles und nichts. Sie bietet dir eine Essiggurke an. ");
                    output("Das liegt in ihrer Natur, da sie früher Gurken angebaut und verkauft hat. Du nimmst an. Nach ein paar Minuten ");
                    output("bemerkst du die brennenden Blicke, die Cedrik immer häufiger in eure Richtung wirft und du beschließt, dass es besser ist, Irina wieder ihre Arbeit machen zu lassen. ");
                }else if($_GET[act]=="fat"){
                    $charm = $session[user][charm]+e_rand(-1,1);
                    output("Irina schaut dich ernst von oben bis unten an. Nur ein echter Freund kann wirklich ehrlich sein und genau deswegen ");
            ...
            if ($session[user][sex]==0){
              ...
              if ($session[user][seenlover]==0){
                    if ($session['user']['marriedto']==4294967295){
                      if (e_rand(1, 4)==1){
                      output("Du gehst rüber zu Irina um sie zu knuddeln und sie auf Gesicht und Hals zu küssen, aber sie brummelt nur etwas ");
                      switch(e_rand(1,4)){
                      case 1:
                      ...
                      output("`n`n`^Du VERLIERST einen Charmepunkt!");
                        ...
                        output("Du und `5Irina`0 nehmt euch etwas Zeit für euch selbst und du verlässt die Schenke zuversichtlich strahlend!");
                    ...
                    $session['user']['seenlover']=1;
                        ...
                        output("Du starrst verträumt durch den Raum auf `5Irina`0, die sich über einen Tisch beugt, ");
                        ...
                        addnav("Flirt");
                        addnav("Zwinkern","inn_essos.php?op=Irina&flirt=1");
                        addnav("Handkuss","inn_essos.php?op=Irina&flirt=2");
                        addnav("Küsschen auf die Lippen","inn_essos.php?op=Irina&flirt=3");
                        addnav("Setze sie auf deinen Schoß","inn_essos.php?op=Irina&flirt=4");
                        addnav("Greif ihr an den Hintern","inn_essos.php?op=Irina&flirt=5");
                        addnav("Trag sie nach oben","inn_essos.php?op=Irina&flirt=6");
                        //if ($session[user][charisma]!=4294967295) addnav("Heirate sie","inn_essos.php?op=Irina&flirt=7");
                    }else{
                      ...
                      switch($_GET[flirt]){
                          case 1:
                              if (e_rand($c,2)>=2){
                                  output("Du zwinkerst `5Irina`0 zu und sie gibt dir ein warmes Lächeln zurück.");
                                    if ($c<4) $c++;
                                }else{
                                  output("Du zwinkerst `5Irina`0 zu, doch sie tut so, als ob sie es nicht bemerkt hätte.");
                                }
                              break;
                          case 2:
                              if (e_rand($c,4)>=4){
                                  output("Selbstsicher schlenderst du Richtung `5Irina`0 durch den Raum. Du nimmst ihre Hand, ");
                                    output("küsst sie sanft und hältst so für einige Sekunden inne. `5Irina`0 ");
                                  ...
                                  output("Selbstsicher schlenderst du Richtung `5Irina`0 durch den Raum und greifst nach ihrer Hand.  ");
                                    output("`n`nAber `5Irina`0 zieht ihre Hand rasch zurück und fragt dich, ob du vielleicht ein Ale haben willst.");
                                }
                              break;
                          case 3:
                              if (e_rand($c,7)>=7){
                                  output("Du lehnst mit deinem Rücken an einer hölzernen Säule und wartest, bis `5Irina`0 in ");
    ...
    if (!$session['user']['prefs']['nosounds']) output("<embed src=\"media/giggle.wav\" width=10 height=10 autostart=true loop=false hidden=true volume=100>",true);    
                                  output("Du sitzt an einem Tisch und lauerst auf deine Gelegenheit. Als `5Irina`0 bei dir vorbei kommt, ");
                          ...
                          case 5:
                              if (e_rand($c,14)>=14){
                                  output("Du wartest, bis `5Irina`0 an dir vorbeirauscht und gibst ihr einen Klaps auf den Hintern. Sie dreht sich um und ");
                          ...
                          case 6:
                              if (e_rand($c,18)>=18){
                                output("Wie ein Wirbelwind braust du durch die Schenke, schnappst dir `5Irina`0, die dir ihre Arme ");
                                    ...
                                    addnews("`@Es wurde beobachtet, wie ".$session[user][name]."`@ und `5Irina`@ gemeinsam die Treppen in der Schenke nach oben gingen.");
                                    if ($session[user][charisma]==4294967295 && e_rand(1,3)==2) {
                                        ...
                                        $row = db_fetch_assoc($result);
                                        $partner=$row[name];
                                        addnews("`\$$partner hat ".$session[user][name]."`\$ wegen eines Seitensprungs mit `5Irina`\$ verlassen!");
                                        ...
                                        db_query($sql);
                                        systemmail($row['acctid'],"`\$Seitensprung!`0","`&{$session['user']['name']}`6 geht mit Irina fremd!`nDas ist Grund genug für dich, die Scheidung einzureichen. Ab sofort bist du wieder solo.`nDu bekommst `^$getgold`6 von seinem Vermögen auf dein Bankkonto.");
                                  ...
                                  output("Wie ein Wirbelwind fegst du durch die Schenke und schnappst nach `5Irina`0. Sie dreht sich um und ");
                                    output("schlägt dir ins Gesicht! \"`%Für was hältst du mich eigentlich?`0\" brüllt sie dich an! ");
                                    if ($c>0) $c--;
                                }
                            /*  break;
                            case 7:
                                output("`5Irina`0 arbeitet fieberhaft, um einige Gäste der Schenke zu bedienen. Du schlenderst zu ihr rüber, ");
                              ...
                              if ($c>=22){
                                  output("`n`nEs ist ein Ausdruck überschäumender Freude. \"`%Ja! Ja, ja, ja!`0\" sagt sie. ");
                                    ...
                                    output("`n`5Irina`0 und du heiraten in der Kirche am Ende der Strasse in ");
                                    output("einer prachtvollen Feier mit vielen rausgeputzten Mädels.");
                                    addnews("`&".$session[user][name]." und `%Irina`& haben heute feierlich den Bund der Ehe geschlossen!!!");
                  ...
                  output("Du denkst, es ist besser, dein Glück mit `5Irina`0 heute nicht mehr herauszufordern.");
                }
            }else{
      ...
      if ($_GET[subop]=="" && $_GET[flirt]==""){
        output("Kain schaut dich erwartungsvoll an.");
        addnav("Fordere Kain auf, dich zu unterhalten","inn_essos.php?op=Kain&subop=hear");
        if ($session[user][sex]==1){
            if ($session['user']['marriedto']==4294967295) {
                addnav("Flirte mit Kain", "inn_essos.php?op=Kain&flirt=1");
            } else {
                addnav("Flirt");
                addnav("Zwinkern","inn_essos.php?op=Kain&flirt=1");
                addnav("Mit den Wimpern klimpern","inn_essos.php?op=Kain&flirt=2");
                addnav("Taschentuch fallenlassen","inn_essos.php?op=Kain&flirt=3");
                addnav("Frage ihn nach einem Drink","inn_essos.php?op=Kain&flirt=4");
                addnav("Küsse ihn geräuschvoll","inn_essos.php?op=Kain&flirt=5");
                addnav("Den Barden komplett verführen","inn_essos.php?op=Kain&flirt=6");
                //if ($session[user][charisma]!=4294967295) addnav("Heirate ihn","inn_essos.php?op=Kain&flirt=7");
            }
        } else {
            addnav("Frage Kain nach seiner Meinung über dein(e/n) ".$session[user][armor],"inn_essos.php?op=Kain&act=armor");
        }
      }
            if ($_GET[act]=="armor"){
                $charm = $session[user][charm]+e_rand(-1,1);
                output("Kain schaut dich ernst von oben bis unten an. Nur wahre Freunde können wirklich ehrlich sein, das ist der Grund, weshalb du ");
      ...
      if ($_GET[subop]=="hear"){
        ...
        if($session[user][seenbard]){
          output("Kain räuspert sich und trinkt einen Schluck Wasser. \"Tut mir Leid, mein Hals ist einfach zu trocken.\"");
         // addnav("Return to the inn","inn_essos.php");
        }else{
          $rnd = e_rand(0,18);
          output("Kain räuspert sich und fängt an:`n`n`^");
    ...
    if (!$session['user']['prefs']['nosounds']) output("<embed src=\"media/ragtime.mid\" width=10 height=10 autostart=true loop=false hidden=true volume=100>",true);    
              ...
              output("`n`n`0Du bist dir nicht ganz sicher, was du davon halten sollst... du gehst lieber wieder weg und denkst, es ist besser, Kain wieder zu besuchen, wenn er sich besser fühlt. ");
            ...
            case 3:
              ...
              output("`n`n`0Du stellst fest, dass er Recht hat, Cedriks Bier ist wirklich eklig. Das dürfte der Grund dafür sein, warum die meisten Gäste sein Ale bevorzugen. Du kannst der Geschichte von Kain nicht wirklich etwas abgewinnnen, aber du findest dafür etwas Gold auf dem Boden!");
              $gain = e_rand(10,50);
              $session[user][gold]+=$gain;
              //debuglog("found $gain gold near Kain");
              break;
            case 4:
    ...
    if ($session[user][race]==4){
        output("Als Zwerg kannst du darüber nicht lachen. Mit grimmigem Gesichtsausdruck, der auch Kains Lachen zu ersticken scheint, schlägst du ihn zu Boden.");
    ...
    if (!$session['user']['prefs']['nosounds']) output("<embed src=\"media/matlock.mid\" width=10 height=10 autostart=true loop=false hidden=true volume=100>",true);
              ...
              output("`n`n`0Du denkst, Kain ist ganz in Ordnung... jetzt willst du los, um irgendwas zu töten. Aus irgendeinem Grund denkst du an Bienen und Fisch.");
              $session[user][turns]++;
              break;
            case 7:
    if (!$session['user']['prefs']['nosounds']) output("<embed src=\"media/burp.wav\" width=10 height=10 autostart=true loop=false hidden=true volume=100>",true);
              output("`0Kain richtet sich auf und scheint sich auf etwas eindrucksvolles vorzubereiten. Dann rülpst er dir laut ins Gesicht. \"`^War das unterhaltsam genug?`0\"");
    ...
    $session['user']['donation']+=1;
            ...
            case 8:
                if ($session['user']['gold'] >= 5) {
                  output("`0\"`^Welches Geräusch macht es, wenn man mit einer Hand klatscht?`0\", fragt Kain. Während du über diese Scherzfrage nachgrübelst, \"befreit\" Kain eine kleine Unterhaltungsgebühr aus deinem Goldsäckchen.");
                  output("`n`nDu verlierst 5 Gold!");
                  $session[user][gold]-=5;
                  //debuglog("lost 5 gold to Kain");
                } else {
                  output("`0\"`^Welches Geräusch macht es, wenn man mit einer Hand klatscht?`0\", fragt Kain. Während du über diese Scherzfrage nachgrübelst, versucht Kain eine kleine Unterhaltungsgebühr aus deinem Goldsäckchen zu befreien, findet aber nicht, was er sich erhofft hat.");
        $session['user']['donation']+=1;
                }
              break;
            case 9:
              output("Welcher Fuss muss immer zittern?`n`nDer Hasenfuss.");
              output("`n`nDu gröhlst und Kain lacht herzlich. Kopfschüttelnd bemerkst du einen Edelstein im Staub.");
              $session[user][gems]++;
              //debuglog("got 1 gem from Kain");
              break;
            case 10:
              output("Kain spielt eine sanfte, aber mitreißende Melodie.");
    if (!$session['user']['prefs']['nosounds']) output("<embed src=\"media/indianajones.mid\" width=10 height=10 autostart=true loop=false hidden=true volume=100>",true);
              ...
              break;
            case 11:
              output("Kain spielt dir ein düsteres Klagelied vor.");
    ...
    if (!$session['user']['prefs']['nosounds']) output("<embed src=\"media/babyphan.mid\" width=10 height=10 autostart=true loop=false hidden=true volume=100>",true);
              ...
              output("`n`n`0Kain singt immer weiter, aber du hast nicht den Wunsch herauszufinden, wie weit Kain zählen kann, deswegen verschwindest du leise. Nach dieser kurzen Rast fühlst du dich erholt.");
                            $session[user][hitpoints]=$session[user][maxhitpoints];
              break;
            case 13:
              output("Es war ein mal eine Dame von der Venus, ihr Körper war geformt wie ein ...");
              if ($session[user][sex]==1){
                output("`n`n`0Kain wird durch einen barschen Schlag ins Gesicht unterbrochen. Du fühlst dich rauflustig und gewinnst einen Waldkampf dazu.");
              }else{
                output("`n`n`0Kain wird durch dein plötzliches lautes Gelächter unterbrochen, das du ausstößt, ohne seinen Reim vollständig gehört haben zu müssen. So angespornt erhältst du einen zusätzlichen Waldkampf.");
              }
              $session[user][turns]++;
              break;
            case 14:
    if (!$session['user']['prefs']['nosounds']) output("<embed src=\"media/knightrider.mid\" width=10 height=10 autostart=true loop=false hidden=true volume=100>",true);
              output("Kain spielt einen stürmischen Schlachtruf für dich, der den Kriegergeist in dir erweckt.");
              output("`n`n`0Du bekommst einen zusätzlichen Waldkampf!");
              $session[user][turns]++;
              break;
            case 15:
              output("Kain scheint in Gedanken völlig woanders zu sein ... bei deinen ... Augen.");
              if ($session[user][sex]==1){
    ...
    if (!$session['user']['prefs']['nosounds']) output("<embed src=\"media/boioing.wav\" width=10 height=10 autostart=true loop=false hidden=true volume=100>",true);
              output("Kain fängt an zu spielen, aber eine Saite seiner Laute reißt plötzlich und schlägt dir flach ins Auge.`n`n`0\"`^Uuuups! Vorsicht, du wirst dir noch deine Augen ausschießen, Mensch!`0\"");
    ...
    $session['user']['donation']+=1;     
         break;
            case 18:
              output("`0Kain starrt dich gedankenvoll an. Offensichtlich komponiert er gerade ein episches Gedicht...`n`n`^H-Ä-S-S-L-I-C-H, du kannst dich nicht verstecken -- Du bist hässlich, yeah, yeah, so hässlich!");
              $session[user][charm]--;
              if ($session[user][charm]<0){
                output("`n`n`0Wenn du einen Charmepunkt hättest, hättest du ihn jetzt verloren. Aber so reißt Kain nur eine Saite seiner Laute.");
              }else{
            ...
            if ($session[user][sex]==1 && $_GET[flirt]<>""){
              ...
              if ($session[user][seenlover]==0){
                    if ($session['user']['marriedto']==4294967295){
                    if (e_rand(1,4)==1){
                      output("Du gehst rüber zu Kain, um ihn zu knuddeln und mit Küssen zu überhäufen, aber er brummelt nur etwas ");
                      switch(e_rand(1,4)){
                      case 1:
                      ...
                      output("Du und Kain nehmt euch etwas Zeit füreinander und du verlässt die Schenke mit einem zuversichtlichen Strahlen!");
                      $session['bufflist']['lover']=$buff;
                      $session['user']['charm']++;
                      output("`n`n`^Du erhältst einen Charmepunkt!");
                    }
                    $session['user']['seenlover']=1;
                      ...
                      switch($_GET[flirt]){
                          case 1:
                              if (e_rand($c,2)>=2){
                                  output("Kain grinst ein breites Grinsen. Hach, ist dieses Grübchen in seinem Kinn nicht süß??");
                                    if ($c<4) $c++;
                                }else{
                                  output("Kain hebt eine Augenbraue und fragt dich, ob du etwas im Auge hast.");
                                }
                              break;
                          case 2:
                              if (e_rand($c,4)>=4){
                                  output("Kain lächelt dich an und sagt: \"`^Du hast wunderschöne Augen`0\"");
                                    if ($c<7) $c++;
                                }else{
                                    output("Kain lächelt und winkt ... zu der Person hinter dir.");
                                }
                              break;
                          case 3:
                              if (e_rand($c,7)>=7){
                                  output("Während Kain sich bückt, um dir dein Taschentuch zurückzugeben, bewunderst du seinen knackigen Hintern.");
                                    if ($session[user][charisma]==4294967295) {
                                    ...
                                    output("Kain hebt das Taschentuch auf, putzt sich damit die Nase und gibt es dir zurück.");
                                }
                              break;
                          case 4:
                              if (e_rand($c,11)>=11){
                                  output("Kain platziert seinen Arm um deine Hüfte, geleitet dich an die Bar und kauft dir eines der köstlichsten Getränke, die es in der Schenke gibt.");
                                    if ($session[user][charisma]==4294967295) {
                                        output(" Dein Mann wird gar nicht begeistert sein, wenn er davon erfährt!");
                                        $c--;
                                    } else {
                                  ...
                                  output("Kain bedauert: \"`^Tut mir Leid, meine Dame, ich habe kein Geld zu verschenken.`0\" Dabei stülpt er seine mottenzerfressenen Taschen nach außen.");
                                    if ($c>0 && $c<10) $c--;
                                }
                              break;
                          case 5:
                              if (e_rand($c,14)>=14){
                                  output("Du läufst auf Kain zu, packst ihn am Hemd, stellst ihn auf die Beine und drückst ihm einen kräftigen, langen Kuss direkt auf seine attraktiven Lippen. Kain bricht fast zusammen - mit zerzausten Haaren und ziemlich atemlos.");
                                    if ($session[user][charisma]==4294967295) {
                                        $c--;
                                    } else {
                                        if ($c<18) $c++;
                                  ...
                                  output("Du bückst dich zu Kain herunter, um ihn auf die Lippen zu küssen, doch als sich eure Lippen gerade berühren wollen, bückt sich Kain, um sich den Schuh zuzubinden.");
                          ...
                          case 6:
                              if (e_rand($c,18)>=18){
                                output("Du stehst auf der ersten Treppenstufe und gibst Kain ein 'komm hierher' Zeichen. Er folgt dir wie ein Schoßhündchen.");
                                    if ($session['user']['turns']>0){
                                      output("Du fühlst dich ausgelaugt!  ");
                                      $session['user']['turns']-=2;
                                      if ($session['user']['turns']<0) $session['user']['turns']=0;
                                    }
                                    addnews("`@Es wurde beobachtet, wie ".$session[user][name]."`@ und `^Kain`@ gemeinsam die Treppen in der Schenke nach oben gingen.");
                                    if ($session[user][charisma]==4294967295 && e_rand(1,3)==2) {
                                        $sql = "SELECT acctid,name FROM accounts WHERE locked=0 AND acctid=".$session[user][marriedto]."";
                                          $result = db_query($sql) or die(db_error(LINK));
                                        $row = db_fetch_assoc($result);
                                        $partner=$row[name];
                                        addnews("`\$$partner hat ".$session[user][name]."`\$ wegen eines Seitensprungs mit `^Kain`\$ verlassen!");
                                        ...
                                        $sql = "UPDATE accounts SET charisma=0,marriedto=0,goldinbank=goldinbank+$getgold WHERE acctid='$row[acctid]'";
                                        db_query($sql);
                                        systemmail($row['acctid'],"`\$Seitensprung!`0","`&{$session['user']['name']}`6 geht mit Kain fremd!`nDas ist Grund genug für dich, die Scheidung einzureichen. Ab sofort bist du wieder solo.`nDu bekommst `^$getgold`6 von ihrem Vermögen auf dein Bankkonto.");
                                    }else if ($session[user][charisma]==4294967295) {
                                        output(" Dein Mann wird gar nicht begeistert sein, wenn er davon erfährt!");
                                        $c--;
                                    }else{
                            ...
                            /*  break;
                            case 7:
                                output("Du gehst zu Kain und verlangst ohne Umschweife von ihm, daß er dich heiratet.`n`nEr schaut dich ein paar Sekunden lang an.`n`n");
                              if ($c>=22){
                                    output("\"`^Natürlich, meine Liebe!`0\", sagt er. Die nächsten wochen bist du damit beschäftigt, eine gigantische Hochzeit vorzubereiten, die natürlich Kain zahlt, und danach geht es in den dunklen Wald in die Flitterwochen.");
                                    addnews("`&".$session[user][name]." und `^Kain`& haben heute feierlich den Bund der Ehe geschlossen!!!");
                                    $session['user']['marriedto']=4294967295; //int max.
                                      $session['bufflist']['lover']=$buff;
                                    $session['user']['donation']+=1;
                                }else{
                                    output("`^Kain sagt: \"`^Es tut mir Leid, offensichtlich habe ich einen falschen Eindruck erweckt. Ich denke, wir sollten einfach nur Freunde sein.`0\"  Deprimiert hast du heute kein Verlangen mehr danach, nochmal im Wald kämpfen zu gehen.");
                                    $session[user][turns]=0;
                                }
                                */
                        }
                      ...
                      output("Du denkst, es ist besser, dein Glück mit `^Kain`0 heute nicht mehr herauszufordern.");
                }
            }else{
              //sorry, no lezbo action here.
            }
        ...
        case "converse":
          output("Du schlenderst rüber zu einem Tisch, stellst den Fuß auf die Bank und lauschst der Unterhaltung:`n");
            viewcommentary("inn_essos","Zur Unterhaltung beitragen:",20);
        ...
        if (getsetting("paidales",0)<=1 || $session[user][gotfreeale]>=2) {
          ...
          if ($_GET[act]==""){
                ...
                addnav("Schwarzes Brett","inn_essos.php?op=msgboard");
                addnav("Bestechen","inn_essos.php?op=bartender&act=bribe");
                addnav("Edelsteine","inn_essos.php?op=bartender&act=gems");
                if (getsetting("paidales",0)<=1) {
                    addnav("Ale (`^$alecost`0 Gold)","inn_essos.php?op=bartender&act=ale");
                    addnav("Runde schmeißen","inn_essos.php?op=bartender&act=schmeiss");
                } else {
                    $amt=getsetting("paidales",0)-1;
                    addnav("Ale (`^".($session[user][gotfreeale]>=2?"$alecost`0 Gold":"schon bezahlt`0").")","inn_essos.php?op=bartender&act=ale");
              ...
              if ($_POST[gemcount]==""){
                    ...
                    output("<form action='inn_essos.php?op=bartender&act=gems' method='POST'><input name='gemcount' value='0'><input type='submit' class='button' value='Weggeben'>`n",true);
                    ...
                    addnav("","inn_essos.php?op=bartender&act=gems");
                ...
                output("<form action='inn_essos.php?op=bartender&act=schmeiss2' method='POST'>Die nächsten <input name='runden' id='runden' width='4'> Ale gehen auf deine Rechnung.`n",true);
                ...
                addnav("","inn_essos.php?op=bartender&act=schmeiss2");
                ...
                if ($_GET[type]==""){
                    ...
                    addnav("1 Edelstein","inn_essos.php?op=bartender&act=bribe&type=gem&amt=1");
                    addnav("2 Edelsteine","inn_essos.php?op=bartender&act=bribe&type=gem&amt=2");
                    addnav("3 Edelsteine","inn_essos.php?op=bartender&act=bribe&type=gem&amt=3");
                    addnav("$g1 Gold","inn_essos.php?op=bartender&act=bribe&type=gold&amt=$g1");
                    addnav("$g2 Gold","inn_essos.php?op=bartender&act=bribe&type=gold&amt=$g2");
                    addnav("$g3 Gold","inn_essos.php?op=bartender&act=bribe&type=gold&amt=$g3");
                }else{
                  if ($_GET[type]=="gem"){
                    ...
                    if ($try){
                        if (e_rand(0,100)<$chance){
                            output("Cedrik lehnt sich zu dir über die Theke und fragt: \"`%Was kann ich für dich tun, Kleine".($session[user][sex]?"":"r")."?`0\"");
                                if (getsetting("pvp",1)) {
                                addnav("Wer schläft oben?","inn_essos.php?op=bartender&act=listupstairs");
                            }
                            addnav("Farbenlehre","inn_essos.php?op=bartender&act=colors");
                            addnav("Spezialgebiet wechseln","inn_essos.php?op=bartender&act=specialty");
                        }else{
              ...
              output("Du schlägst mit der Faust auf die Bar und verlangst ein Ale");
                if ($session[user][drunkenness]>66){
                  ...
                  if ($session[user][gold]>=$alecost){
                      $session[user][drunkenness]+=33;
                        ...
                        output("`n`nDu drehst dich um, trinkst dieses herzhafte Gesöff auf ex und gibst ".($session[user][sex]?"Kain":"Irina"));
                ...
                addnav("Liste aktualisieren","inn_essos.php?op=bartender&act=listupstairs");
    ...
    if ($session['user']['pvpflag']=="5013-10-06 00:42:00"){
        output("`n`&(Du hast PvP-Immunität gekauft. Diese verfällt, wenn du jetzt angreifst!)`0`n`n");
              ...
              output("Cedrik lehnt sich weiter über die Bar. \"`%Du willst also was über Farben wissen, hmm?`0\" ");
                ...
                output("Deine Eingabe: ".str_replace("`","&#0096;",HTMLSpecialChars($_POST[testtext]))."`n",true);
          ...
          $expense = round(($session[user][level]*(10+log($session[user][level]))),0);
            if ($_GET[pay]){
                  ...
                  $config['innstays']--;
                    ...
                    if ($_GET[pay] == 2 || $session[user][gold]>=$expense || $session[user][boughtroomtoday]){
                        if ($session[user][loggedin]){
                            ...
                            $sql = "UPDATE accounts SET loggedin=0,rpbulb=0,location=1 $goldline WHERE acctid = ".$session[user][acctid];
                ...
                if ($session[user][boughtroomtoday]){
                    ...
                    addnav("Gehe ins Zimmer","inn_essos.php?op=room&pay=1");
                }else{
                    if ($config['innstays']>0){
                        addnav("Zeige ihm den Gutschein für ".$config['innstays']." Übernachtungen","inn_essos.php?op=room&pay=1&coupon=1");
                    }
                    ...
                    if ($session['user']['goldinbank'] >= $bankexpense && $bankexpense != $expense) {
                        output("Weil du so eine nette Person bist, bietet er dir zum Preis von `\$".$bankexpense."`0 Gold auch an, direkt von der Bank zu bezahlen. Der Preis beinhaltet " . (mb_strpos($fee, "%") ? $fee : "$fee Gold") . " Überweisungsgebühr.");
                    }
                    ...
                    addnav("Gib ihm $expense Gold","inn_essos.php?op=room&pay=1");
                    if ($session['user']['goldinbank'] >= $bankexpense) {
                        addnav("Zahle $bankexpense Gold von der Bank","inn_essos.php?op=room&pay=2");
                    }
                }
            }
            break;
    }
  addnav("Zurück zur Schenke","inn_essos.php");
}
