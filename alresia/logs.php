...
if ($_GET[op]=="mail"){
    ...
    elseif ($_GET['act']=='saveban') {
        ...
        foreach ($vals AS $this) {
            ...
            $sql.="\"$_POST[reason]\")";
            if ($_POST[type]=="ip"){
                if (mb_substr($_SERVER['REMOTE_ADDR'],0,mb_strlen($this)) == $this){
                    $sql = "";
    ...
    elseif (!empty($_POST['deleteuser']) && count($_POST['userid'])>0) {
        ...
        $result = db_query($sql);
        while ($row = db_fetch_assoc($result)) {
            if (mb_strlen($row['reason'])>50) $row['reason'] = mb_substr($row['reason'],0,47).'...';
            output('<option value="'.$row['reasonid'].'">'.HTMLSpecialChars($row['reason']).'</option>',true);
        }
        output('</select>',true);
