...
if ($session['user']['gold'] > 10)  addnav("","blumenmaedchen.php?op=send&op2=gift2");
if ($session['user']['gold'] > 25)  addnav("","blumenmaedchen.php?op=send&op2=gift3");
if ($session['user']['gold'] > 50)  addnav("","blumenmaedchen.php?op=send&op2=gift4");
if ($session['user']['gold'] > 80)  addnav("","blumenmaedchen.php?op=send&op2=gift5");
if ($session['user']['gold'] > 80)  addnav("","blumenmaedchen.php?op=send&op2=gift6");
if ($session['user']['gold'] > 100) addnav("","blumenmaedchen.php?op=send&op2=gift7");
if ($session['user']['gold'] > 250) addnav("","blumenmaedchen.php?op=send&op2=gift8");
if ($session['user']['gold'] > 700) addnav("","blumenmaedchen.php?op=send&op2=gift9");
if ($session['user']['gold'] > 1000) addnav("","blumenmaedchen.php?op=send&op2=gift10");
if ($session['user']['gold'] > 1000) addnav("","blumenmaedchen.php?op=send&op2=gift11");

if ($session['user']['gold'] < 10){
      output("`n`n`&`cDu hast nicht genug Gold um Blumen kaufen zu können.`c`0",true);
    }
output("<table border='0' cellpadding='1'>",true);
  if ($session['user']['gold'] > 10){
  output("<tr class='trhead'><td>`bBlumen`b</td><td align='center'>`bPreis`b</td></tr>",true);
    output("<tr><td><a href=\"blumenmaedchen.php?op=send&op2=gift2\">Ein Gänseblümchen</a></td><td>10 Gold</td></tr>",true);
  }
    if ($session['user']['gold'] > 25){
    output("<tr><td><a href=\"blumenmaedchen.php?op=send&op2=gift3\">Eine Wild Rose</a></td><td>25 Gold</td></tr>",true);
  }
    if ($session['user']['gold'] > 50){
    output("<tr><td><a href=\"blumenmaedchen.php?op=send&op2=gift4\">Eine Rote Rose</a></td><td>50 Gold</td></tr>",true);
  }
    if ($session['user']['gold'] > 80){
    output("<tr><td><a href=\"blumenmaedchen.php?op=send&op2=gift5\">Eine Weiße Rose</a></td><td>80 Gold</td></tr>",true);
  }
    if ($session['user']['gold'] > 80){
    output("<tr><td><a href=\"blumenmaedchen.php?op=send&op2=gift6\">Eine Schwarze Rose</a></td><td>80 Gold</td></tr>",true);
  }
    if ($session['user']['gold'] > 100){
    output("<tr><td><a href=\"blumenmaedchen.php?op=send&op2=gift7\">Straus Gänseblümchen</a></td><td>100 Gold</td></tr>",true);
  }
    if ($session['user']['gold'] > 250){
    output("<tr><td><a href=\"blumenmaedchen.php?op=send&op2=gift8\">Straus Wild Rosen</a></td><td>250 Gold</td></tr>",true);
  }
    if ($session['user']['gold'] > 700){
    output("<tr><td><a href=\"blumenmaedchen.php?op=send&op2=gift9\">Straus Rote Rosen</a></td><td>700 Gold</td></tr>",true);
  }
    if ($session['user']['gold'] > 1000){
    output("<tr><td><a href=\"blumenmaedchen.php?op=send&op2=gift10\">Straus Weiße Rosen</a></td><td>1000 Gold</td></tr>",true);
  }
    if ($session['user']['gold'] > 1000){
    output("<tr><td><a href=\"blumenmaedchen.php?op=send&op2=gift11\">Straus Schwarze Rosen</a></td><td>1000 Gold</td></tr>",true);
  }

output("</table></ul>",true);
...
addnav("","blumenmaedchen.php?op=send1&op2=gift12");
addnav("","blumenmaedchen.php?op=send1&op2=gift13");
addnav("","blumenmaedchen.php?op=send1&op2=gift14");
...
$page=(int)$_GET[limit];
addnav("Vorherige Seite","blumenmaedchen.php?op=send1&op2=$gift&limit=".($page-1)."&search=$_POST[search]");
}
$limit="".($page*$ppp).",".($ppp+1);
...
$result = db_query($sql);
if (db_num_rows($result)>$ppp) addnav("Nächste Seite","blumenmaedchen.php?op=send1&op2=$gift&limit=".($page+1)."&search=$_POST[search]");
output("`r`nÜberglücklich strahlt dich Aeris an.`n \"Für wen sind die Blumen denn bestimmt?\"`n`n");
...
addnav("","blumenmaedchen.php?op=send1&op2=$gift");
output("<table cellpadding='3' cellspacing='0' border='0'><tr class='trhead'><td>Name</td><td>Level</td><td>Geschlecht</td></tr>",true);
for ($i=0;$i<db_num_rows($result);$i++){
$row = db_fetch_assoc($result);
output("<tr class='".($i%2?"trlight":"trdark")."'><td><a href='blumenmaedchen.php?op=send2&op2=$gift&name=".HTMLEntities($row['acctid'])."'>",true);
