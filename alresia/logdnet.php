...
if ($_GET[op]==""){
    ...
    while (list($key,$val)=each($servers)){
        ...
        if (mb_substr($row[address],0,7)!="http://"){

        }else{
            output("<tr><td valign='top'><a href='".HTMLSpecialChars($row[address])."' target='_blank'>".stripslashes(HTMLSpecialChars($row[description]))."`0</a></td><td valign='top' width='130'>".HTMLSpecialChars($row[version])."</td></tr>",true);
        }
    }
    output("</table>",true);
    page_footer();
