   ...
   /* content_show begin */
    ...
    if ($_GET['act']=="takekey") {
        if (empty($_POST['ziel'])) {
            ...
            $sql = "INSERT INTO commentary (postdate,section,author,comment) VALUES (now(),'house-".$session['user']['specialmisc']['houseid']."',".$session['user']['acctid'].",'/me `^nimmt $row2[name]`^ einen Schlüssel ab. $row2[name]`^ bekommt einen Teil aus dem Schatz.')";
    ...
    elseif ($_GET['act']=="takegold"){
        ...
        if (empty($_POST['gold'])) {
            ...
            elseif ($session['user']['transferredtoday']>=getsetting("transferreceive",3)) {
                ...
                $sql = "INSERT INTO commentary (postdate,section,author,comment) VALUES (now(),'house-".$session['user']['specialmisc']['houseid']."',".$session['user']['acctid'].",'/me `\$nimmt `^$amt`\$ Gold.')";
                db_query($sql) or die(db_error(LINK));
            }
        }
        addnav("Schatzkammer","houses.php?op=drin");
    ...
    elseif ($_GET['act']=="givegold") {
        ...
        if (empty($_POST['gold'])) {
            ...
            elseif ($goldinhouse >= 5000) {
                output("`2Der Schatz ist voll.");
            }
            elseif ($amt>(5000-$goldinhouse)) {
            ...
            elseif ($session['user']['amountouttoday']+$amt > $maxout) {
                ...
                $sql="INSERT INTO commentary (postdate,section,author,comment) VALUES (now(),'house-".$session['user']['specialmisc']['houseid']."',".$session['user']['acctid'].",'/me `@deponiert `^$amt`@ Gold.')";
                db_query($sql) or die(db_error(LINK));
            }
        }
        addnav("Schatzkammer","houses.php?op=drin");
    ...
    elseif ($_GET['act']=="takegems"){
        ...
        if (empty($_POST['gems'])) {
            ...
            elseif ($amt<0) {
                ...
                $sql = "INSERT INTO commentary (postdate,section,author,comment) VALUES (now(),'house-".$session['user']['specialmisc']['houseid']."',".$session['user']['acctid'].",'/me `\$nimmt `%$amt Edelsteine`\$.')";
                db_query($sql) or die(db_error(LINK));
            }
        }
        addnav("Schatzkammer","houses.php?op=drin");
    ...
    elseif ($_GET['act']=="givegems") {
        ...
        if (empty($_POST['gems'])) {
            ...
            elseif ($gemsinhouse >= 30) {
                output("`2Der Schatz ist voll.");
            }
            elseif ($amt>(30-$gemsinhouse)) {
                output("`2Du gibst alles, aber du bekommst beim besten Willen nicht so viel in den Schatz.");
            }
            elseif ($amt<0) {
                output("`2Wenn du etwas aus dem Schatz nehmen willst, versuche nicht, etwas negatives hineinzutun.");
                ...
                $sql="INSERT INTO commentary (postdate,section,author,comment) VALUES (now(),'house-".$session['user']['specialmisc']['houseid']."',".$session['user']['acctid'].",'/me `@deponiert `%$amt Edelsteine`@.')";
                db_query($sql) or die(db_error(LINK));
            }
        }
        addnav("Schatzkammer","houses.php?op=drin");
