...
        $this->coloremote = '`&'; // => /me $farbe schreitet über den Boden. Auch für die Talkline
...
        $this->colortalkline = "`&"; // Damit das "sagt" auch in weiß ist
        $this->gamemaster = '`&';
        
...
     */
    public function ChangeDefaultColors($speak = false, $thirdperson = false, $emote = false, $gm = false) {
...
            $emote = 3;
        }
        elseif(strtolower(substr($commentary,0,3)) === '/gm') {
            $commentary = $this->gamemaster.substr($commentary,3);
            $emote = 4;
        }            
        else {
...
        $commentary = HTMLSpecialchars($commentary, ENT_QUOTES, 'ISO-8859-1');
        
...
            if(strlen($sentence) > 11 AND $wordnum > 3) {
                $numSentences++;
                $sentencesInfo['wordnum'][] = $wordnum;
                $sentencesInfo['strlen'][] = strlen($sentence);
                $numWords += $wordnum;
            }
        }
        
        //Wird grad nich korrekt berechnet
...
                    #  Vergütung RP-Punkte
                    $this->user['donation']++;
                    if($numChars >= 200) $this->user['donation']++;
                    if($numChars >= 400) $this->user['donation']++;
                    if($numChars >= 400) $this->user['gold']++;
                    if($numChars >=700) $this->user['donation']++;
                    if($numChars >=700) $this->user['gold']++;
                    if($numChars >=700) $this->user['gems']++;
                    
...
                        if($this->user['rpclass'] == 80 AND $numChars >= 200) {
                            $count = floor($numChars/500, 0); # All 500 Zeichen geben wir einen Punkt.
                            $this->user['dpchange']+= 1 + $count;
                        }
                        
                        if($this->user['rpclass'] == 90) {
...
            }
            case "opsizi":{
            $nichterlaubt = array("lodge"=>"1","jail"=>"1","superuser"=>"1","todolist"=>"1","occ"=>"1");
        
                $sektion = $this->section;
                
                // && $this->user['betarp']==1
                if($nichterlaubt[$sektion]!=1){
                    $anzahl = floor(strlen($commentary)/1000); //Donation Points für RP Chars
                    if($anzahl < 0.2){
                        $this->user['donation'];
                        //$this->user['gold'];
                        $this->user['gems'];
                    }
                    else if($anzahl >= 0.2 && $anzahl < 0.5){
                        $this->user['donation']++;
                        //$this->user['gold']+= 20;
                        $this->user['gems']+= 1;
                    }
                    else if($anzahl >= 0.5 && $anzahl < 0.9){
                        $this->user['donation']+= 4;
                        //$this->user['gold']+= 100;
                        $this->user['gems']+= 1;
                    }
                    else if($anzahl >= 1){
                        $this->user['donation']+= 8;
                        //$this->user['gold']+= 200;
                        $this->user['gems']+= 2;
                    }
                }
...
            # Wenn emote = 1 (/me), dann andere Farbe gebrauchen
...
        if ($this->emote != 4 || ($this->emote == 4 && $this->user[gm] == 1))
        {
        $sqli = 'INSERT INTO commentary (author, comment, section, emote, postdate, locked) VALUES (:author, :comment, :section, :emote, NOW(), 0)';
        $sql = 'INSERT INTO MysqlError ( Text) VALUES (`huhu`)';
        }
        
        $args = array(
            ':author' => $author,
...
        
        # Neuen Kommentar einfügen
        $sql = 'INSERT INTO `commentary` (`section`, `author`, `comment`, `postdate`, `locked`, `emote`) '
            .'VALUES (:section, :author, :comment, NOW(), 1, :emote)';
        
            $sql = 'INSERT INTO `MysqlError` ( `Text`, `Datum`) VALUES (`$mysqli->error`,`now()`)';
            
        $args = array(
            ':section' => $section,
            ':author' => $author,
...
        if($this->user['namecheck'] <=8 && $this->section != "mail.php?op=offtopic" && $this->section != "todolist-{$_GET['id']}")
            $postfield = false;
        else
            $postfield = true;
            
...
        }
        else{
            output("`n`nDu musst erst auf Freischaltung warten. :D`n`n");
        }
        
...
        rawoutput('<input type="hidden" value="'.HTMLEntities($this->section, ENT_COMPAT | ENT_HTML401, 'ISO-8859-1').'" name="section" />
...
            $linktemplate = COMMENTARY_POPUPBIO_LINKTEMPLATE;
            $link = COMMENTARY_POPUPBIO_LINK;
            }
...
        switch($row['emote']) {
            case 4:{
                $post = $this->gamemaster.$this->nl2paragraph($row['comment'])."`0".endl;;
                break;
            }
            
            case 3: {
                $lastchar = strToLower(substr($regname, -2) == "`0");
                
                switch($lastchar) {
     case 's':
     case 'z':
     case 'c':
      $post = $this->playernamecolor.substr($row['name'], 0, -2).$this->coloremote."&rsquo; ".$this->coloremote.$this->nl2paragraph($row['comment'])."`0".endl;
      break;
      
     default:
      $post = $this->playernamecolor.substr($row['name'], 0, -2).$this->coloremote."s ".$this->coloremote.$this->nl2paragraph($row['comment'])."`0".endl;
      break;
    }
                break;
            }
...
                $req = 'exportcomments.php?section='.$this->section.'&return='.self::encodeStringForURL($this->baselink);
                allownav($req);
                rawoutput($template[3], HTMLSpecialchars($req), 'RP speichern');
            }
        }
        /*output("`n`nFarbcodes:`n");
      $result = db_query("SELECT allowed, code FROM appoencode WHERE         allowed='1' ORDER BY id ASC");
            while ($row = db_fetch_assoc($result)){
                output("`$row[code]&#0096;$row[code]`0",true);
            }
            output('`n`nLegende:`0
                            <table border="0">
                            <tr><td>1</td><td>/me, ::, :</td><td>Ich mache jenes</td></tr>
                            <tr><td>2</td><td>/x, /em</td><td>Man könnte jenes machen</td></tr>
                            <tr><td>3</td><td>/ms</td><td>Ich`` könnte jenes machen</td></tr>
                            </table>',true);*/
        
        /*# Nur für Wintertal
...
                                '`2'.substr($row['name'], 0, -2)."`` ".$this->nl2paragraph($row['comment'])."`0\r\n");
                            break;
                            
                        default:
                            $comments[] = $delprefix.str_replace($search,$replace,
                                '`2'.substr($row['name'], 0, -2)."s ".$this->nl2paragraph($row['comment'])."`0\r\n");
                            break;
                    }
                    break;
                    
...
                case 1:
                    $comments[] = $delprefix.str_replace($search,$replace,
                    '`2'.$row['name'].' '.$this->nl2paragraph($row['comment'])."`0\r\n");
                    break;
                    
                default:
                    $comments[] = $delprefix.str_replace($search,$replace,
...
    }
global $_POST, $session, $REQUEST_URI, $_GET, $doublepost, $namenichtdrin;
    $doublepost = 0;
    $namenichtdrin = 0;
...
            if ((substr($commentary, 0, 2) == "/X" || substr($commentary, 0, 2) == "/x") && !ereg($session['user']['login'], $commentary)) {
                $namenichtdrin = 1;
            } elseif ($row['comment'] != stripslashes($commentary) || $row['author'] != $session['user']['acctid']) {
            // if ($row[comment]!=$commentary || $row[author]!=$session[user][acctid]){
                
                /*if ($rpplace && $section != "ooc") {
                    global $regort;
                    $regort = 1;
                    $len = strlen($commentary);
                    $fak = floor($y / 250);
                    switch(rand(1,10)) {
                        case 1:
                        case 2:
...
                        case 4:
                        case 5:
                            $session['user']['gold'] += $fak * rand(10,15);
                            break;
                        case 6:
                        case 7:
                        case 8:
                            $session['user']['donation'] += $fak;
                            break;
                        case 9:
                        case 10:
                            $session['user']['gems'] += floor($fak * 0.5);
                    }
                }*/
                
...
    global $_POST, $session, $REQUEST_URI, $_GET, $doublepost, $appoencode_str, $cntr, $namenichtdrin;
    
    if(COMMENTARY_USE_COMPABILITY_MODE) {
