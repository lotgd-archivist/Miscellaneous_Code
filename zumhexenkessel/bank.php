...
if ($_GET[op]==""){
  ...
  output("sorgfältig überfliegt.  ");
    ...
    if (db_num_rows($result)==1){
        ...
        output("<input type='hidden' name='to' value='".htmlentities_charset_charset($row['login'])."'><input type='hidden' name='amount' value='$amt'><input type='submit' class='button' value='Überweisung abschließen'></form>",true);
        ...
        for ($i=0;$i<db_num_rows($result);$i++){
            ...
            output("<option value=\"".htmlentities_charset_charset($row['login'])."\">".preg_replace("'[`].'","",$row['name'])."</option>",true);
        }
...
//             } else if ($session[user][emailaddress]==$row[emailaddress] && $row[emailaddress]){
    ...
    if (db_num_rows($result)==1){
        ...
        output("<input type='hidden' name='to' value='".htmlentities_charset_charset($row['login'])."'><input type='submit' class='button' value='Versand abschließen'></form>",true);
