<?php
function HTMLEntities_charset($text, $quotestyle=ENT_QUOTES) {
...
function appoencode($data,$priv=false){
    ...
    while( !(($x=strpos($data,"`")) === false) ){
        ...
        $output.=($priv?$append: htmlentities_charset($append));
    ...
    if ($priv){
        ...
        $output.= htmlentities_charset($data);
    }
    return $output;
}
...
//        .templatereplace("statrow",array("title"=>"Beruf","value"=>$u['jobname']))
        .templatereplace("stathead",array("title"=>"Besitz"))
...
.templatereplace("stathead",array("title"=>"Ausrüstungsinfo"))
...
.templatereplace("statrow",array("title"=>"Schmutz","value"=>$u['clean']))
.templatereplace("statrow",array("title"=>"Geruch","value"=>$odor))
.templatereplace("statrow",array("title"=>"Ansehen","value"=>grafbar($u[maxreputation],$u[reputation])))
//.templatereplace("statrow",array("title"=>"Ansehen","value"=>grafbar(100,($u[reputation]+50,$u[reputation])))
.templatereplace("statrow",array("title"=>"Trunkenheit","value"=>$u['drunkenness']))
    ...
    */
    if ($link===false){
        ...
        if ($text!=""){
            ...
            if ($key==""){
                //$nav.="<a href=\"". htmlentities_charset($link.$extra)."\" class='nav'>".appoencode($text,$priv)."<br></a>";
                //$key==""; // This is useless
            }else{
            ...
            $nav.=templatereplace("navitem",array(
                ...
                "link"=> htmlentities_charset($link.$extra), 
                "accesskey"=>$keyrep,
                "popup"=>($pop==true ? "target='_blank' onClick=\"".popup($link.$extra)."; return false;\"" : ($newwin==true?"target='_blank'":""))
                ));
            //$nav.="<a href=\"". htmlentities_charset($link.$extra)."\" $keyrep class='nav'>".appoencode($text,$priv)."<br></a>";
        }
    ...
    while(list($key,$val)=each($layout)){
        ...
        case "enum":
            ...
            while (list($k,$v)=each($info)){
                ...
                list($k,$v)=each($info);
                $optdis = $v;
                $output.="<option value='$optval'".($row[$key]==$optval?" selected":"").">". htmlentities_charset("$optval : $optdis")."</option>";
            }
            $output.="</select>";
            break;
        case "password":
            $output.="<input type='password' name='$key' value='". htmlentities_charset($row[$key])."'>";
            break;
        case "bool":
            $output.="<select name='$key'>";
            ...
            break;
        case "hidden":
            $output.="<input type='hidden' name='$key' value=\"". htmlentities_charset($row[$key])."\">". htmlentities_charset($row[$key]);
            break;
        case "viewonly":
            output(dump_item($row[$key]), true);
//            output(str_replace("{","<blockquote>{",str_replace("}","}</blockquote>", htmlentities_charset(preg_replace("'(b:[[:digit:]]+;)'","\\1`n",$row[$key])))),true);
            break;
        case "int":
            $output.="<input name='$key' value=\"". htmlentities_charset($row[$key])."\" size='5'>";
            break;
        default:
            $output.=("<input size='50' name='$key' value=\"". htmlentities_charset($row[$key])."\">");
            //output("`n$val");
        }
        output("</td></tr>",true);
    }
...
function redirect($location,$reason=false){
    ...
    if (strpos($location,"badnav.php")===false) $session[output]="<a href=\"". htmlentities_charset($location)."\">Hier klicken</a>";
...
function loadtemplate($templatename){
    if (!file_exists("templates/$templatename") || $templatename=="") $templatename="Jenu2.html";
...
function page_header($title="Willkommen in SeaQuinn"){
...
<input type="hidden" name="item_number" value="'. htmlentities_charset($session['user']['login']).":".$_SERVER['HTTP_HOST']."/".$_SERVER['REQUEST_URI'].'">
...
//   $paypalstr .= '</td></tr><tr><td span="2" align="right" valign="top"><a href="http://www.anpera.net" target="_blank"><img src="images/anpnet-klein.gif" alt="Sponsor" border="0"></a></td></tr></table>';
    ...
    $header = str_replace("{forum}", "<a href='$forumlink' target='_blank' class='motd'>SeaQuinn Forum</a>", $header);
    $footer = str_replace("{forum}", "<a href='$forumlink' target='_blank' class='motd'>SeaQuinn Forum</a>", $footer);
...
function popup_header($title="Legends of the Green Dragon"){
  global $header;
...
*/
        ...
        if ($ft=="::" || $ft=="/me" || $ft==":"){
            ...
            if ($x!==false){
                if ($linkbios)
                    $op[$i] = str_replace("&amp;","&", htmlentities_charset(substr($row[comment],0,$x)))
                    ."`0<a href='$link' style='text-decoration: none'>\n`&$row[name]`0</a>\n`& "
                    .str_replace("&amp;","&", htmlentities_charset(substr($row[comment],$x+strlen($ft))))
                        ."`0`n";
                else
                    $op[$i] = str_replace("&amp;","&", htmlentities_charset(substr($row[comment],0,$x)))
                    ."`0\n`&$row[name]`0\n`& "
...
//echo nl2br( htmlentities_charset(output_array($session)));
...
$allownonnav = array("badnav.php"=>true,"motd.php"=>true,"petition.php"=>true,"mail.php"=>true,"topwebvote.php"=>true,"chat.php"=>true,"source.php"=>true,"statistics.php"=>true,"potuse.php"=>true);
if ($session[loggedin]){
...
$session[counter]++;
$nokeeprestore=array("newday.php"=>1,"badnav.php"=>1,"motd.php"=>1,"mail.php"=>1,"petition.php"=>1,"chat.php"=>1,"statistics.php"=>1,"potuse.php"=>1);
