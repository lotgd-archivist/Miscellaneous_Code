...
if ($_GET['page']=="" && $_GET['op']==""){
    ...
    $sql = "SELECT acctid,name,login,alive,location,sex,level,laston,loggedin,lastip,uniqueid,race FROM accounts WHERE locked=0 AND loggedin=1 AND laston>'".date("Y-m-d H:i:s",(date("U")-getsetting("LOGINTIMEOUT",9999900)))."' ORDER BY level DESC, dragonkills DESC, login ASC";
}else{
