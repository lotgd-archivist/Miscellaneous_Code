...
switch ($_GET['op'])
{
    case '':
        $str_output .= '
            `g`ƒD`ƒu schlenderst in abgelegenen Teilen des Gartens umher, als du eine Stelle bemerkst, die scheinbar immer von Sonnenlicht umgeben ist, obwohl ringsherum hohe Büsche und viele Bäume wachsen.
    ...
    case 'pickfruit':
        if (user_getConfBit(1,$row['treepick']) ==0)
        {
            $str_output .= '
                `g`ƒD`ƒu versuchst, etwas vom `kBaum `gdes Lebens zu nehmen und findest...`n`n';
            ...
            $row['treepick']=user_setConfBit(1,1,$row['treepick']);
            user_set_aei(array('treepick'=>$row['treepick']));
        }
        else
        {
            $str_output .= '`g`ƒD`ƒu beschließt, den Anderen auch eine Chance zu geben...';
        }
    break;
}
