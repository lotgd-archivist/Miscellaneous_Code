...
if($_GET[op]=="continue"){
    ...
    if($session[user][maxhitpoints]>$lifecost){
        addnav("Biete $lifecost Lebenspunkte fuer Angriff","forest.php?op=str");
        addnav("Biete $lifecost Lebenspunkte fuer Verteidigung","forest.php?op=def");
        addnav("Biete $lifecost Lebenspunkte fuer Reichtum","forest.php?op=wealth");
    } else{
        addnav("Nicht genug Lebenskraft");
    }
    addnav("Flüchte in Furcht","forest.php?op=leave");
}else if ($_GET[op]=="leave"){
    // addnav("Zurueck in den Wald","forest.php");
