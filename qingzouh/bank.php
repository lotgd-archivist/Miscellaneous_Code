...
place();
output("`^`c`bNew Orleans Bank`b`c`6");
if ($_GET[op]==""){
  checkday();
...
        output("\"`3Ah ja, hier ist es. Du hast  `^".$session[user][goldinbank]." Gold`3 bei unserer ");
        output("renommierten Bank.  Kann ich sonst noch etwas für dich tun?`0\"");
    }else{
        output("\"`3Ah ja, hier ist es.  Du hast `&Schulden`3 in Höhe von `^".abs($session[user][goldinbank])." Gold`3 bei unserer ");
        output("renommierten Bank.  Kann ich sonst noch etwas für dich tun?`6\"`n`n(`iSchulden verfallen durch einen Drachenkill nicht!`i)`0");
    }
...
addnav("Zurück");
addnav('zum French Market', 'markt.php');
        addnav("zum Jackson Square","village.php");

viewcommentary("oldfrenchmarket_bank","`0Tätige deine Geschäfte:`n`n");
