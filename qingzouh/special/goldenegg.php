...
// If the account doesn't exist anymore, reset the golden egg.
$user                             = db_fetch_assoc(db_query("SELECT acctid FROM accounts, settings WHERE acctid = value AND setting = 'hasegg'"));
if (!$user) {
...
        output("Schnell verlässt dich die Kraft beim Ersteigen, des Baumes und du spürst die Macht vom Ei. Ehrfürchtig und Verlassen von deiner Motivation,
                springst du wieder hinab und verschwindest lieber schnell");
    } elseif (e_rand(1, 5) == 4) {
...
            addnews("`&" . $session['user']['name'] . " `jstarb beim Versuch, sich das `jgoldene Ei`7 zu schnappen.`0");
        }
    } else if (getsetting("hasegg", 0) != 0) {
        output("Pech gehabt! Gerade als du nach dem Ei greifen willst, stellst du fest, dass es verschwunden ist. Im Augenwinkel siehst du unten gerade noch jemanden mit `bdeinem`b Ei weglaufen...");
    } else {
...
        addnews("`&" . $session['user']['name'] . "`7 hat das goldene Ei im Wald gefunden!`0");
...
            output("`3Mitten im dichten Wald entdeckst du auf einer starken Astgabel eines gewaltigen Baumes ein riesiges Nest.`n");
            output("Du bemerkst, dass es leer ist und schenkst ihm keine weitere Aufmerksamkeit.`n`n ");
            output("`^Das goldene Ei befindet sich zur Zeit im Besitz von $row[name]`^!`n`3Willst du es " . ($row[sex] ? "ihr" : "ihm") . " nicht mal abnehmen?`n", true);
        }
        $session['user']['specialinc'] = "";
    }
}
