...
    addnews("`&".$session[user][name]."`j wurde bei einer zufälligen Begegnung im Wald vom `1Grünen Drachen`j getötet!");
    $session['user']['gold']=0;
...
                    addnews("`&".$session[user][name]."`j hat irgendwie eine zufällige Begegnung mit dem `1Grünen Drachen`j überlebt.");
...
                output("Auge in Auge mit diesem gewaltigen Biest zu finden. Spöttisch sagt er zu dir: \"`5Nicht hier.");
...
            addnav("Saint-Omar","forest.php?op=city&count=0");
...
            addnav("Saint-Omar","forest.php?op=city&count={$count}");
...
            $count=$_GET['count'];
            $count++;
            output("`t\"`qDiese Violet ist ganz schön süß, was?`t\"`nDer Drachen nickt. \"`5Ein schmackhafter, süßer Happen wäre das. ");
            output(" Eine Schande, dass sie niemals die Schenke verlässt. Aber vielleicht wirst du meinen Hunger solange stillen?.`t\"`n");
...
            output("`t\"`nDer Drachen schaut dich hungrig an. Zeit für einen Themenwechsel!");
            addnav("Themen");
            addnav("W?Das Wetter","forest.php?op=weather&count={$count}");
            addnav("G?Der Grüne Drachen","forest.php?op=dragon&count={$count}");
            addnav("Violet","forest.php?op=violet&count={$count}");
...
            $count=$_GET['count'];
            $count++;
            output("`t\"`qWusstest du, dass das Dorf Saint-Omar heißt? Ist ein ziemlich beeindruckendes Örtchen!`t\"`nDer Drachen gröhlt laut.`n\"`5Diese stinkende Stadt ist mir ");
