...
                    'linktitle'=>'Beschreibungen ändern',            // the link title of the module
...
        output("`2`bHier kannst du das Bild für dein Haus ändern.`b`n`nDas aktuelle Bild sieht so aus: `0<img src='".$row['house_ava']."' align='right'>`0`n",true);
        output("`0<form action=\"houses.php?op=drin&act=desc\" method='POST'>",true);
        output("`n`2Link zum Bild für dein Haus:`n<input name='ava' size='50' value='".$row['house_ava']."'>`n",true);
        output("<input type='submit' class='button' value='Abschicken'>",true);
        addnav("","houses.php?op=drin&act=desc");

        output("`n`n`n`n`2`bHier kannst du die Beschreibung für dein Haus ändern.`b`n`nDie aktuelle Beschreibung für Innen lautet: `0".$row['description']."`0`n");
        output("`0<form action=\"houses.php?op=drin&act=desc\" method='POST'>",true);
        output("`n`2Gib eine Beschreibung für dein Haus ein:`n<textarea class='input' name='desc' cols='37' rows='5'>".HTMLEntities(str_replace("`","``",$row['description']),ENT_QUOTES)."</textarea>`n",true);
        output("<input type='submit' class='button' value='Abschicken'>",true);
        addnav("","houses.php?op=drin&act=desc");

        output("`n`n`nDie aktuelle Beschreibung für Außen (Wohnviertel-Liste) lautet: `0".$row['description_out']."`0`n");
        output("`0<form action=\"houses.php?op=drin&act=desc\" method='POST'>",true);
        output("`n`2Gib eine Beschreibung für dein Haus ein:`n<textarea class='input' name='desc2' cols='37' rows='5'>".HTMLEntities(str_replace("`","``",$row['description_out']),ENT_QUOTES)."</textarea>`n",true);
        output("<input type='submit' class='button' value='Abschicken'>",true);
        addnav("","houses.php?op=drin&act=desc");

        output("`n`n`n`2`bHier kannst du den Eingangstext für Dein Privatzimmer ändern.`b`n`nDie aktuelle Beschreibung lautet: `0".$row['privatzimmer']."`0`n");
        output("`0<form action=\"houses.php?op=drin&act=desc\" method='POST'>",true);
        output("`n`2Gib einen Eingangstext für Dein Privatzimmer ein:`n<textarea class='input' name='desc3' cols='37' rows='5'>".HTMLEntities(str_replace("`","``",$row['privatzimmer']),ENT_QUOTES)."</textarea>`n",true);
        output("<input type='submit' class='button' value='Abschicken'>",true);
        addnav("","houses.php?op=drin&act=desc");
    }else{
...
        output("`n`n`2Die Beschreibung für Innen wurde geändert.`0`n`n".str_replace("\n","`n","".$_POST['desc']."")."`2");
        $sql = "UPDATE houses SET description='".$_POST['desc']."' WHERE houseid=".$session['user']['specialmisc']['houseid'];
        db_query($sql);
        
        output("`n`n`2Die Beschreibung für Außen wurde geändert.`0`n`n".str_replace("\n","`n","".$_POST['desc2']."")."`2");
        $sql = "UPDATE houses SET description_out='".$_POST['desc2']."' WHERE houseid=".$session['user']['specialmisc']['houseid'];
        db_query($sql);

        output("`n`n`2Die Privatzimmer-Beschreibung wurde geändert.`0`n`n".str_replace("\n","`n","".$_POST['desc3']."")."`2");
