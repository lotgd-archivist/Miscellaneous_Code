...
$session['user']['standort']="Warchild's Akademie";
...
if ($session[user][specialty] == 1)
        ...
        output(" `iFeuer Künste`i");
        else
        output("`iWeiße Magieì");
        ...
        else if ($session[user][specialty] == 4)
        {
            ...
            output("`$".$cost_low ."`^ Gold`n");
            output("`3Praktische Übung im Feuerkreis: ");
            output("`$".$cost_medium ."`^ Gold und `$1 Edelstein`^`n");
            output("`$ Warchilds `3Lehrstunde für Feuer Spielerei: ");
            output("`$".$cost_high ."`^ Gold und `$2 Edelsteine`^`n");
        }
        else
        {
            output("`3Selbststudium mit Büchern über die helle Magie: ");
            output("`$".$cost_low ."`^ Gold`n");
            output("`3Praktische Übung im Raum der Engel: ");
            output("`$".$cost_medium ."`^ Gold und `$1 Edelstein`^`n");
            output("`$ Warchilds `3Lehrstunde für Nachwuchsengel: ");
            output("`$".$cost_high ."`^ Gold und `$2 Edelsteine`^`n");
        }
    ...
    */
    if ($_GET[op1] == "enter" && $_GET[op2] == "chat")
    {
        ...
        viewcommentary("academy","Sprich",25,"spricht",1,1);
    }
        ...
        else // hier geht das Train los
        {
            ...
            switch ($rand)
            {
                ...
                if ($session[user][specialty] == 1)
                    ...
                    output(" `iFeuerkünste`i");
                else
                    output(" `iWeiße Magie`i");
    ...
    $session[user][gems] >= 1))
    {
        ...
        if ($session['user']['drunkenness'] > 0) // too drunk to learn
        {
            ...
            else if ($session[user][specialty] == 4)
            {
                ...
                output("Während Du, beinahe in das Feuer rein trittst wegen des Alkohols, und du sich richtung Ausgang bewegst (oh Mann Du bist betrunken!), kann Warchild sich ein Lächeln nicht unterdrücken.`n");
                output("Um es kurz zu machen: Du verbrennst dir nicht nur die Füße sondern auch Finger und die Nase.");
                output("Halb verbrannt versuchst du durch Heiltränke deine Wunden zu heilen.`n`n");
                output("`5Du verlierst ein paar Lebenspunkte!");
                $session[user][hitpoints] = $session[user][hitpoints]  * 0.1;
            }
            else
            {
                ...
                output("Während Du, von dem Licht so sehr geblendet wirst und wegen des Alkohols umkippst. Und Wachild dich auffängt und schmunzelt. Wie betrunken kann man nur sein?!`n");
                output("Um es kurz zu machen: Du fällst Wachild in die Arme knickst mit dem Fuß um wirst Ohnmächtig als er dich los lässt");
                output(" und schlägst dir leicht den Kopf auf. Als du zu boden fällst.");
                output("Voller Schmerzen versuchst du deine Gedanken zu sammeln und wieder zurück ins Dorf zu laufen.`n`n");
                output("`5Du verlierst ein paar Lebenspunkte!");
                $session[user][hitpoints] = $session[user][hitpoints]  * 0.1;
            }
            addnav("Zurück ins Dorf","village.php");
...
else
{
    ...
    if ($session[user][seenAcademy] == 1 || $session[user][turns]<1)
    {
        ...
        if ($session['user']['specialty'] == 4) output("`qFeuer Künsten`7");
        if ($session['user']['specialty'] == 5) output("`&Weiße Magie`7");
