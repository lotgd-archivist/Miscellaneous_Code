...
require_once "common.php";require_once "func/systemmail.php";
...
/*
    ...
    elseif ($_GET['act']=='dodeleteuser') {
        ...
        output('`n');
    }elseif (!empty($_POST['setmultiallowed']) && count($_POST['userid'])>0) {
    // Multis als erlaubt eintragen
        $userid = implode(',',$_POST['userid']);
        $sql = "UPDATE accounts SET multi_allowed=1 WHERE acctid IN ($userid)";
        db_query($sql);
        output(db_affected_rows()." Multis als erlaubt eingetragen.`n`n");
    }
    else output('`n');
    ...
    $ip = $id = $users = array();
    $sql = 'SELECT acctid,name,lastip,uniqueid,dragonkills,level,rp_only,multi_allowed FROM accounts WHERE (lastip IN (-1'.$in_ip.') OR uniqueid IN (-1'.$in_id.')) AND locked="0" ORDER BY dragonkills ASC, level ASC';
    $result = db_query($sql) or die(db_error(LINK));
    while ($row = db_fetch_assoc($result)) {
    ...
    foreach ($users AS $list) {
        if (count($list)<2 && count($list)<3) continue;
        $tmpstr = '';
        $ips = $ids = $accts = array();
        foreach ($list AS $that) {
            $tmpstr .= ('<tr><td><input type="checkbox" name="userid[]" value="'.$that['acctid'].'"></td>
                            <td>'.$that['name'].'</td>
                            <td><a href="user.php?op=edit&userid='.$that['acctid'].'&ret=multi">'.$that['acctid'].'</a></td>
                            <td>'.$that['lastip'].'</td>
                            <td>'.$that['uniqueid'].'</td>
                            <td>'.$that['dragonkills'].'</td>
                            <td>'.$that['level'].'</td>
                            <td>'.($that['rp_only']<="0"?"`^Kämpfer":"RP-Char").'</td>
                            <td>'.($that['multi_allowed']=="0"?"`4Nein":"`@Ja").'</td></tr>');
                            allownav('user.php?op=edit&userid='.$that['acctid'].'&ret=multi');
        }
        ...
        output('<tr><td colspan="6" align="left">
                        ...
                        <input type="submit" name="setmultiallowed" value="Multi erlauben">
                    </td></tr>',true);
    ...
    output("Die 10 letzten fehlgeschlagenen Logins:`n`n");
    $sql = "SELECT faillog.*,accounts.name AS absender FROM faillog LEFT JOIN accounts ON accounts.acctid=faillog.acctid WHERE 1 ORDER BY date DESC LIMIT 10";
    $result = db_query($sql) or die(db_error(LINK));
    ...
    if ($session[user][superuser]>3) addnav("Usermails","logs.php?op=mail");
