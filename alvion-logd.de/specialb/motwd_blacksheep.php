...
switch ($_GET['op']){
                ...
                switch(e_rand(1,3)){
                        ...
                        case 2:
                                if ($session['user']['specialmisc'] != $session['b_sheeps']){
                                        ...
                                        $session['user']['specialinc'] = $spi;
                                        addnav('Weiter');
                                        addnav('Kämpfe','berge.php?op=kampf');
                                }else{
                                        $gems = e_rand(2,7);
                                        if($session['user']['specialmisc']>1){
                                                ...
                                                $str_out .= '.`n`7Das kleine Schäfchen liegt tot auf der Lichtung und du bekommst leichte Skrupel, '.
                                                                'solch ein süßes Wesen umgebracht zu haben. Doch die Edelsteine, die du findest, wischen alle Skrupel weg und du sammelst sie eilig ein.`n`n'.
                                                                'Du bekommst `#'.$gems.' Edelsteine`7.';
                                        }
                                        if($session['user']['specialmisc']==1) addnews($session['user']['name'].'`7 hat ein kleines, süßes Schäfchen umgebracht`0');
                                        else addnews($session['user']['name'].'`7 hat '.$session['user']['specialmisc'].' kleine, süße Schäfchen umgebracht`0');
                                        $session['user']['specialinc'] = '';
                                        $session['user']['specialmisc'] = '';
        ...
        default:
                ...
                $session['b_sheeps'] = e_rand(3,12);
                addnav('Aktionen');
                addnav('Mit den Schäfchen schmusen',$nav_back.'?op=schmusen');
                addnav('Sonstiges');
                addnav('Zurück ins Gebirge',$nav_back.'?op=zurueck');
}
...
if ($battle){
        ...
        if ($victory){
                ...
                if ((e_rand() % 5) == 1){
                        $gems = rand(1,3);
                        $gold = e_rand(($session['user']['level'] * 20),($session['user']['level'] * 40));
                        $str_out .= 'Du findest `#'.$gems.' Edelstein'.($gems==1?'':'e').'`7 und `^'.$gold.
                                        ' Goldstücke`7 bei der Leiche des Schafes.`n';
                }
                $exp = round($session['user']['experience'] * 0.005)+mt_rand(10,25);
                $badguy = array();
                $session['user']['badguy'] = '';
                ...
                addnews($session['user']['name'].'`7 war zu schwach, um es mit kleinen, süßen Kuschelschäfchen aufzunehmen!`0');
                $badguy = array();
                $session['user']['badguy'] = '';
                $session['user']['alive'] = 0;
                $session['user']['hitpoints'] = 0;
                $session['user']['gems'] -=round($session['user']['gems'] / 4);
                $session['user']['gold'] = 0;
