...
page_header("Yang's Waffenladen");
$session[user][standort]="Waffenladen";
output("`c`b`&Yang's Waffen`0`b`c");
$tradeinvalue = round(($session[user][weaponvalue]*.75),0);
if ($HTTP_GET_VARS[op]==""){
  output("`!Yang `7steht hinter einem Ladentisch und scheint dir nur wenig Interesse entgegen zu bringen, als du eintrittst. ");
...
    output("`n`n`!Yang`7  nickt dir schließlich zu und wünscht sich, während er seinen Spitzbart streichelt, ");
...
    output("`!Yang`7 schaut dich an und sagt \"`#Ich gebe dir `^$tradeinvalue`# ");
...
          output("`!Yang`7 schaut dich eine Sekunde lang verwirrt an und kommt zu dem Schluss, dass du ein paar Schläge zuviel auf den Kopf bekommen hast. Schließlich nickt er und grinst.");
...
                    output(" `bGlück gehabt!`b  `!Yang`5 war gerade durch irgendwas am Fenster abgelenkt. Aber nochmal passiert ihm das nicht! Stolz auf deine ");
                    output("fette Beute stolzierst du über den Platz - bis dir jemand mitteilt, dass dir da noch ein Preisschild herumbaumelt...`nDu verlierst einen Charmepunkt!");
...
                    output(" `bGlück gehabt!`b `!Yang`5 war gerade durch irgendwas am Fenster abgelenkt. Aber nochmal wird ihm das nicht passieren! Stolz auf deine ");
                    output("fette Beute stolzierst du über den Platz - bis dir jemand mitteilt, dass dir da noch ein Preisschild herumbaumelt...`nDu verlierst einen Charmepunkt!");
                    $session[user][weapon] = $row[weaponname];
                    $session[user][attack]-=$session[user][weapondmg];
                    $session[user][weapondmg] = $row[damage];
...
                    output(" So schnell und unauffällig wie du kannst verlässt du den Laden. Geschafft! Als du mit deiner Beute über den Platz stolzierst, siehst du aus dem ");
                    output("Augenwinkel `#Yang`5 auf dich zurauschen. Er packt dich mit einer Hand an ".$session[user][armor]." und zerrt dich mit zur Stadtbank...`n`n");
                    output("`#Yang`5 zwingt dich mit seinen Händen eng um deinen Hals geschlungen dazu, die `^".($row['value']-$tradeinvalue)."`5 Gold, die du ihm schuldest, von der Bank zu zahlen!");
...
                      output("Während du wartest, bis `!Yang`7 in eine andere Richtung schaut, näherst du dich vorsichtig dem `5$row[weaponname]`7 und nimmst es leise vom Regal. ");
                    output("Deiner fetten Beute gewiss drehst du dich leise, vorsichtig, wie ein Ninja, zur Tür, nur um zu entdecken, ");
                    output("dass `!Yang`7 drohend in der Tür steht und dir den Weg abschneidet. Du versuchst einen Flugtritt. Mitten im Flug hörst du das \"SCHING\" eines Schwerts, ");
                    output("das seine Scheide verlässt.... dein Fuß ist weg. Du landest auf dem Beinstumpf und `!Yang`7 steht immer noch im Torbogen, das Schwert ohne Gebrauchsspuren wieder im  Halfter und mit ");
...
                    output("`b`&Du wurdest von `!Yang`& umgebracht!!!`n");
...
                    addnews("`%".$session[user][name]."`5 wurde beim Versuch, in `!Yang`5's Waffenladen zu stehlen, niedergemetzelt.");
                }
...
                if ($klau==1){ // Fall nur für Diebe
                    output("`5Mit den Fertigkeiten eines erfahrenen Diebes tauschst du `%$row[weaponname]`5 gegen `%".$session[user][weapon]."`5 aus und verlässt fröhlich pfeifend den Laden. ");
                    output(" `bGlück gehabt!`b  `!Yang`5 war gerade durch irgendwas am Fenster abgelenkt. Aber irgendwann wird er den Diebstahl bemerken und in Zukunft wesentlich besser aufpassen! Stolz auf deine ");
                    output("fette Beute stolzierst du über den Platz - bis dir jemand mitteilt, dass dir da noch ein Preisschild herumbaumelt...`nDu verlierst einen Charmepunkt!");
                    $session[user][weapon] = $row[weaponname];
                    $session[user][attack]-=$session[user][weapondmg];
                    $session[user][weapondmg] = $row[damage];
...
                } else if ($klau==2 || $klau==3) { // Diebstahl gelingt perfekt
                    output("`5Da dir das nötige Kleingold fehlt, grapschst du dir `%$row[weaponname]`5 und tauschst `%".$session[user][weapon]."`5 unauffällig dagegen aus. ");
                    output(" `bGlück gehabt!`b `!Yang`5 war gerade durch irgendwas am Fenster abgelenkt. Aber irgendwann wird er den Diebstahl bemerken und in Zukunft besser aufpassen. Stolz auf deine ");
                    output("fette Beute stolzierst du über den Platz - bis dir jemand mitteilt, dass dir da noch ein Preisschild herumbaumelt...`nDu verlierst einen Charmepunkt!");
                    $session[user][weapon] = $row[weaponname];
                    $session[user][attack]-=$session[user][weapondmg];
                    $session[user][weapondmg] = $row[damage];
...
                    output("`5Du grapschst dir `%$row[weaponname]`5 und tauschst `%".$session[user][weapon]."`5 unauffällig dagegen aus. ");
                    output(" So schnell und unauffällig wie du kannst verlässt du den Laden. Geschafft! Als du mit deiner Beute über den Platz stolzierst, siehst du aus dem ");
                    output("Augenwinkel `#Yang`5 auf dich zurauschen. Er packt dich mit einer Hand an ".$session[user][armor].".`n`n");
                    output("Er entreißt dir $row[weaponname] gewaltsam, ");
...
                    }else{
                        output("`n`4Yang hat dich so schlimm erwischt, dass eine Narbe bleiben wird.`nDu verlierst 3 Charmepunkte und fast alle Lebenspunkte.");
...
            }
        }else{
            output("`!Yang`7 nimmt dein `5".$session[user][weapon]."`7 stellt es aus und hängt sofort ein neues Preisschild dran. ");
...
            $session[user][attack]+=$session[user][weapondmg];
            $session[user][weaponvalue] = $row[value];
            output("`n`nIm Gegenzug händigt er dir ein glänzendes, neues `5$row[weaponname]`7 aus, das du probeweise im Raum schwingst. Dabei schlägst du `!Yang`7 fast den Kopf ab. ");
