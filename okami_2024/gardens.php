...
        if ($session[user][marriedto]==4294967295) output("Du denkst nochmal über deine Ehe mit ".($session[user][sex]?"`^Ignitus`0":"`5Kiana`0")." nach und überlegst, ob du ".($session[user][sex]?"ihn":"sie")." in der Kneipe besuchen sollst, oder für wen du diese Ehe aufs Spiel setzen würdest.`n");
...
    output("`b`c`2Das Paradies`0`c`b");
...
    output("`n`n");
    */
    output("`n`n`7Du betrittst das Paradies oder das was davon übergeblieben ist. ");
    output("Die Bäum was einst voller Blüten und bunter Blätter waren sind nun kahl und wirken fast schon tot.");
    output("Der einst silbrig glänzender, fröhlich plätschernder Fluss ist nun nur mehr braun und schlamig. ");
    output("Du spürst, dass eine besonnene Stimmung über diesem Garten liegt und instinktiv ist dir bewusst,`n dass hier besondere Regeln herrschen, wenn du hier geduldet werden willst.`0`n`n ");
    viewcommentary("gardens","Hier flüstern",30,"flüstert");

    addnav("Flirten","gardens.php?op=flirt1");
}

addnav("Zum Platz","village.php");
addnav("tiefer gehen","p1.php");
page_footer();
?>
