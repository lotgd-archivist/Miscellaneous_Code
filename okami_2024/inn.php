...
addcommentary();
$session[user][standort]="Brennenden Wolf";
checkday();
...
$buff = array("name"=>"`!Schutz der Liebe","rounds"=>60,"wearoff"=>"`!Du vermisst ".($session['user']['sex']?"Ignitus":"`5Kiana`")."!.`0","defmod"=>1.2,"roundmsg"=>"Deine große Liebe lässt dich an deine Sicherheit denken!","activate"=>"defense");

page_header("Brennenden Wolf");
output("<span style='color: #9900FF'>",true);
output("`c`bDer tote Wolf`b`c");
...
                ($session[user][sex]?
                "`^Ignitus`0 zu, der seine Harfe beim Feuer stimmt.":
                "`5Kiana`0 zu, die ein paar einheimischen Ale serviert.").
                " Der Barkeeper Elias steht hinter seiner Theke und quatscht mit irgendjemandem. Du kannst nicht genau verstehen "
...
    if ($session[user][sex]==0) addnav("V?Flirte mit Kiana","inn.php?op=violet");
    if ($session[user][sex]==1) addnav("V?Quatsche mit Kiana","inn.php?op=violet");
    addnav("S?Rede mit dem Barden Ignitus","inn.php?op=seth");
    addnav("Mit Freunden unterhalten","inn.php?op=converse");
    addnav("B?Spreche mit Barkeeper Elias","inn.php?op=bartender");
    if (getsetting("pvp",1)) {
...
        output("Elias kramt einen Zettel und einen Stift unter der Theke hervor und schaut dich fragend an, was er für dich schreiben soll. Offenbar ");
...
            output("Als Elias bemerkt, dass du offensichtlich nicht genug Gold hast, schnauzt er dich an: \"`%So kommen wir nicht ins Geschäft, Kleine".($session[user][sex]?"":"r").". Sieh zu, dass du Land gewinnst. Oder im Lotto.`0\"");
        }else{
            output("Mürrisch nimmt Elias dein Gold, schreibt deinen Text auf den Zettel und ohne ihn nochmal durchzulesen, heftet er ihn zu den anderen an das schwarze Brett neben der Eingangstür.");
...
        output("\"`%Du möchtest eine Nachricht am schwarzen Brett hinterlassen, ja? Wie lang soll die Nachricht denn dort zu sehen sein?`0\" fragt dich Elias fordernd und nennt die Preise.");
...
                    output("Du gehst rüber zu `5Kiana`0 und hilfst ihr dabei, ein paar Ales zu servieren. Als sie alle ausgeteilt sind, ");
...
                    output("Für ein paar Minuten tratschst du mit `5Kiana`0 über alles und nichts. Sie bietet dir eine Essiggurke an. ");
                    output("Das liegt in ihrer Natur, da sie früher Gurken angebaut und verkauft hat. Du nimmst an. Nach ein paar Minuten ");
                    output("bemerkst du die brennenden Blicke, die Elias immer häufiger in eure Richtung wirft und du beschließt, dass es besser ist, Kiana wieder ihre Arbeit machen zu lassen. ");
                }else if($HTTP_GET_VARS[act]=="fat"){
                    $charm = $session[user][charm]+e_rand(-1,1);
                    output("Kiana schaut dich ernst von oben bis unten an. Nur ein echter Freund kann wirklich ehrlich sein und genau deswegen ");
...
                      if (e_rand(1, 4)==1){
                      output("Du gehst rüber zu Kiana um sie zu knuddeln und sie auf Gesicht und Hals zu küssen, aber sie brummelt nur etwas ");
                      switch(e_rand(1,4)){
                      case 1:
...
                    } else {
                        output("Du und `5Kiana`0 nehmt euch etwas Zeit für euch selbst und du verlässt die Schenke zuversichtlich strahlend!");
...
                        output("Du starrst verträumt durch den Raum auf `5Kiana`0, die sich über einen Tisch beugt, ");
...
                          case 1:
                              if (e_rand($c,2)>=2){
                                  output("Du zwinkerst `5Kiana`0 zu und sie gibt dir ein warmes Lächeln zurück.");
                                    if ($c<4) $c++;
                                }else{
                                  output("Du zwinkerst `5Kiana`0 zu, doch sie tut so, als ob sie es nicht bemerkt hätte.");
                                }
                              break;
                          case 2:
                              if (e_rand($c,4)>=4){
                                  output("Selbstsicher schlenderst du Richtung `5Kiana`0 durch den Raum. Du nimmst ihre Hand, ");
                                    output("küsst sie sanft und hältst so für einige Sekunden inne. `5Kiana`0 ");
...
                                    if ($c<7) $c++;
                                }else{
                                  output("Selbstsicher schlenderst du Richtung `5Kiana`0 durch den Raum und greifst nach ihrer Hand.  ");
                                    output("`n`nAber `5Kiana`0 zieht ihre Hand rasch zurück und fragt dich, ob du vielleicht ein Ale haben willst.");
                                }
                              break;
                          case 3:
                              if (e_rand($c,7)>=7){
                                  output("Du lehnst mit deinem Rücken an einer hölzernen Säule und wartest, bis `5Kiana`0 in ");
...
                                  output("Du sitzt an einem Tisch und lauerst auf deine Gelegenheit. Als `5Kiana`0 bei dir vorbei kommt, ");
...
                          case 5:
                              if (e_rand($c,14)>=14){
                                  output("Du wartest, bis `5Kiana`0 an dir vorbeirauscht und gibst ihr einen Klaps auf den Hintern. Sie dreht sich um und ");
...
                          case 6:
                              if (e_rand($c,18)>=18){
                                output("Wie ein Wirbelwind braust du durch die Schenke, schnappst dir `5Kiana`0, die dir ihre Arme ");
...
                                    }
                                    addnews("`@Es wurde beobachtet, wie ".$session[user][name]."`@ und `5Kiana`@ gemeinsam die Treppen in der Schenke nach oben gingen.");
...
                                        $row = db_fetch_assoc($result);
                                        $partner=$row[name];
                                        addnews("`\$$partner hat ".$session[user][name]."`\$ wegen eines Seitensprungs mit `5Kiana`\$ verlassen!");
...
                                        db_query($sql);
                                        systemmail($row['acctid'],"`\$Seitensprung!`0","`&{$session['user']['name']}`6 geht mit Kiana fremd!`nDas ist Grund genug für dich, die Scheidung einzureichen. Ab sofort bist du wieder solo.`nDu bekommst `^$getgold`6 von seinem Vermögen auf dein Bankkonto.");
...
                                    }
                                }else{
                                  output("Wie ein Wirbelwind fegst du durch die Schenke und schnappst nach `5Kiana`0. Sie dreht sich um und ");
                                    output("schlägt dir ins Gesicht! \"`%Für was hältst du mich eigentlich?`0\" brüllt sie dich an! ");
                                    if ($c>0) $c--;
                                }
                              break;
                            case 7:
                                output("`5Kiana`0 arbeitet fieberhaft, um einige Gäste der Schenke zu bedienen. Du schlenderst zu ihr rüber, ");
...
                                    output("`n`5Kiana`0 und du heiraten in der Kirche am Ende der Strasse in ");
                                    output("einer prachtvollen Feier mit vielen rausgeputzten Mädels.");
                                    addnews("`&".$session[user][name]." und `%Kiana`& haben heute feierlich den Bund der Ehe geschlossen!!!");
...
                    }
                }else{
                  output("Du denkst, es ist besser, dein Glück mit `5Kiana`0 heute nicht mehr herauszufordern.");
                }
            }else{
...
        output("Ignitus schaut dich erwartungsvoll an.");
        addnav("Fordere Ignitus auf, dich zu unterhalten","inn.php?op=seth&subop=hear");
        if ($session[user][sex]==1){
            if ($session['user']['marriedto']==4294967295) {
                addnav("Flirte mit Ignitus", "inn.php?op=seth&flirt=1");
            } else {
                addnav("Flirt");
...
            }
        } else {
            addnav("Frage Ignitus nach seiner Meinung über dein(e/n) ".$session[user][armor],"inn.php?op=seth&act=armor");
        }
      }
            if ($HTTP_GET_VARS[act]=="armor"){
                $charm = $session[user][charm]+e_rand(-1,1);
                output("Ignitus schaut dich ernst von oben bis unten an. Nur wahre Freunde können wirklich ehrlich sein, das ist der Grund, weshalb du ");
...
          output("Ignitus räuspert sich und trinkt einen Schluck Wasser. \"Tut mir Leid, mein Hals ist einfach zu trocken.\"");
         // addnav("Return to the inn","inn.php");
        }else{
          $rnd = e_rand(0,18);
          output("Ignitus räuspert sich und fängt an:`n`n`^");
...
              output("`n`n`0Du bist dir nicht ganz sicher, was du davon halten sollst... du gehst lieber wieder weg und denkst, es ist besser, Ignitus wieder zu besuchen, wenn er sich besser fühlt. ");
                            output("Nach einer kurzen Verschnaufpause könntest du wieder ein paar böse Jungs verprügeln.");
                            $session[user][turns]++;
              break;
            case 3:
              output("Für eine Geschichte versammelt euch hier`neine Geschichte so schrecklich und hart`nüber Elias und sein gepanschtes Bier`nund wie sehr er ihn hasst, mich, den Bard'! ");
              output("`n`n`0Du stellst fest, dass er Recht hat, Eliass Bier ist wirklich eklig. Das dürfte der Grund dafür sein, warum die meisten Gäste sein Ale bevorzugen. Du kannst der Geschichte von Ignitus nicht wirklich etwas abgewinnnen, aber du findest dafür etwas Gold auf dem Boden!");
              $gain = e_rand(10,50);
              $session[user][gold]+=$gain;
              //debuglog("found $gain gold near Ignitus");
              break;
            case 4:
...
        output("Als Zwerg kannst du darüber nicht lachen. Mit grimmigem Gesichtsausdruck, der auch Ignituss Lachen zu ersticken scheint, schlägst du ihn zu Boden.");
...
              output("Ich liebe Yang, die Waffen von Yang, ich liebe Yang, die Waffen von Yang, ich liebe Yang, die Waffen von Yang, nichts tötet so gut wie die WAFFEN von ... Yang!");
              output("`n`n`0Du denkst, Ignitus ist ganz in Ordnung... jetzt willst du los, um irgendwas zu töten. Aus irgendeinem Grund denkst du an Bienen und Fisch.");
              $session[user][turns]++;
              break;
            case 7:
...
              output("`0Ignitus richtet sich auf und scheint sich auf etwas eindrucksvolles vorzubereiten. Dann rülpst er dir laut ins Gesicht. \"`^War das unterhaltsam genug?`0\"");
...
                  output("`0\"`^Welches Geräusch macht es, wenn man mit einer Hand klatscht?`0\", fragt Ignitus. Während du über diese Scherzfrage nachgrübelst, \"befreit\" Ignitus eine kleine Unterhaltungsgebühr aus deinem Goldsäckchen.");
                  output("`n`nDu verlierst 5 Gold!");
                  $session[user][gold]-=5;
                  //debuglog("lost 5 gold to Ignitus");
                } else {
                  output("`0\"`^Welches Geräusch macht es, wenn man mit einer Hand klatscht?`0\", fragt Ignitus. Während du über diese Scherzfrage nachgrübelst, versucht Ignitus eine kleine Unterhaltungsgebühr aus deinem Goldsäckchen zu befreien, findet aber nicht, was er sich erhofft hat.");
        $session['user']['donation']+=1;
                }
              break;
            case 9:
              output("Welcher Fuss muss immer zittern?`n`nDer Hasenfuss.");
              output("`n`nDu gröhlst und Ignitus lacht herzlich. Kopfschüttelnd bemerkst du einen Edelstein im Staub.");
              $session[user][gems]++;
              //debuglog("got 1 gem from Ignitus");
              break;
            case 10:
              output("Ignitus spielt eine sanfte, aber mitreißende Melodie.");
...
              break;
            case 11:
              output("Ignitus spielt dir ein düsteres Klagelied vor.");
...
              output("`n`n`0Ignitus singt immer weiter, aber du hast nicht den Wunsch herauszufinden, wie weit Ignitus zählen kann, deswegen verschwindest du leise. Nach dieser kurzen Rast fühlst du dich erholt.");
                            $session[user][hitpoints]=$session[user][maxhitpoints];
              break;
            case 13:
              output("Es war ein mal eine Dame von der Venus, ihr Körper war geformt wie ein ...");
              if ($session[user][sex]==1){
                output("`n`n`0Ignitus wird durch einen barschen Schlag ins Gesicht unterbrochen. Du fühlst dich rauflustig und gewinnst einen Waldkampf dazu.");
              }else{
                output("`n`n`0Ignitus wird durch dein plötzliches lautes Gelächter unterbrochen, das du ausstößt, ohne seinen Reim vollständig gehört haben zu müssen. So angespornt erhältst du einen zusätzlichen Waldkampf.");
              }
              $session[user][turns]++;
              break;
            case 14:
...
              output("Ignitus spielt einen stürmischen Schlachtruf für dich, der den Kriegergeist in dir erweckt.");
              output("`n`n`0Du bekommst einen zusätzlichen Waldkampf!");
              $session[user][turns]++;
              break;
            case 15:
              output("Ignitus scheint in Gedanken völlig woanders zu sein ... bei deinen ... Augen.");
              if ($session[user][sex]==1){
...
              output("Ignitus fängt an zu spielen, aber eine Saite seiner Laute reißt plötzlich und schlägt dir flach ins Auge.`n`n`0\"`^Uuuups! Vorsicht, du wirst dir noch deine Augen ausschießen, Mensch!`0\"");
...
         break;
            case 18:
              output("`0Ignitus starrt dich gedankenvoll an. Offensichtlich komponiert er gerade ein episches Gedicht...`n`n`^H-Ä-S-S-L-I-C-H, du kannst dich nicht verstecken -- Du bist hässlich, yeah, yeah, so hässlich!");
              $session[user][charm]--;
              if ($session[user][charm]<0){
                output("`n`n`0Wenn du einen Charmepunkt hättest, hättest du ihn jetzt verloren. Aber so reißt Ignitus nur eine Saite seiner Laute.");
              }else{
...
                    if ($session['user']['marriedto']==4294967295){
                    if (e_rand(1,4)==1){
                      output("Du gehst rüber zu Ignitus, um ihn zu knuddeln und mit Küssen zu überhäufen, aber er brummelt nur etwas ");
                      switch(e_rand(1,4)){
                      case 1:
...
                      output("`n`n`^Du VERLIERST einen Charmepunkt!");
                    }else{
                      output("Du und Ignitus nehmt euch etwas Zeit füreinander und du verlässt die Schenke mit einem zuversichtlichen Strahlen!");
                      $session['bufflist']['lover']=$buff;
                      $session['user']['charm']++;
                      output("`n`n`^Du erhältst einen Charmepunkt!");
                    }
...
                          case 1:
                              if (e_rand($c,2)>=2){
                                  output("Ignitus grinst ein breites Grinsen. Hach, ist dieses Grübchen in seinem Kinn nicht süß??");
                                    if ($c<4) $c++;
                                }else{
                                  output("Ignitus hebt eine Augenbraue und fragt dich, ob du etwas im Auge hast.");
                                }
                              break;
                          case 2:
                              if (e_rand($c,4)>=4){
                                  output("Ignitus lächelt dich an und sagt: \"`^Du hast wunderschöne Augen`0\"");
                                    if ($c<7) $c++;
                                }else{
                                    output("Ignitus lächelt und winkt ... zu der Person hinter dir.");
                                }
                              break;
                          case 3:
                              if (e_rand($c,7)>=7){
                                  output("Während Ignitus sich bückt, um dir dein Taschentuch zurückzugeben, bewunderst du seinen knackigen Hintern.");
                                    if ($session[user][charisma]==4294967295) {
...
                                    }
                                }else{
                                    output("Ignitus hebt das Taschentuch auf, putzt sich damit die Nase und gibt es dir zurück.");
                                }
                              break;
                          case 4:
                              if (e_rand($c,11)>=11){
                                  output("Ignitus platziert seinen Arm um deine Hüfte, geleitet dich an die Bar und kauft dir eines der köstlichsten Getränke, die es in der Schenke gibt.");
                                    if ($session[user][charisma]==4294967295) {
                                        output(" Dein Mann wird gar nicht begeistert sein, wenn er davon erfährt!");
                                        $c--;
                                    } else {
...
                                    }
                                }else{
                                  output("Ignitus bedauert: \"`^Tut mir Leid, meine Dame, ich habe kein Geld zu verschenken.`0\" Dabei stülpt er seine mottenzerfressenen Taschen nach außen.");
                                    if ($c>0 && $c<10) $c--;
                                }
                              break;
                          case 5:
                              if (e_rand($c,14)>=14){
                                  output("Du läufst auf Ignitus zu, packst ihn am Hemd, stellst ihn auf die Beine und drückst ihm einen kräftigen, langen Kuss direkt auf seine attraktiven Lippen. Ignitus bricht fast zusammen - mit zerzausten Haaren und ziemlich atemlos.");
                                    if ($session[user][charisma]==4294967295) {
                                        $c--;
                                    } else {
                                        if ($c<18) $c++;
                                    }
                                }else{
                                  output("Du bückst dich zu Ignitus herunter, um ihn auf die Lippen zu küssen, doch als sich eure Lippen gerade berühren wollen, bückt sich Ignitus, um sich den Schuh zuzubinden.");
...
                          case 6:
                              if (e_rand($c,18)>=18){
                                output("Du stehst auf der ersten Treppenstufe und gibst Ignitus ein 'komm hierher' Zeichen. Er folgt dir wie ein Schoßhündchen.");
                                    if ($session['user']['turns']>0){
                                      output("Du fühlst dich ausgelaugt!  ");
                                      $session['user']['turns']-=2;
                                      if ($session['user']['turns']<0) $session['user']['turns']=0;
                                    }
                                    addnews("`@Es wurde beobachtet, wie ".$session[user][name]."`@ und `^Ignitus`@ gemeinsam die Treppen in der Schenke nach oben gingen.");
                                    if ($session[user][charisma]==4294967295 && e_rand(1,3)==2) {
                                        $sql = "SELECT acctid,name FROM accounts WHERE locked=0 AND acctid=".$session[user][marriedto]."";
                                          $result = db_query($sql) or die(db_error(LINK));
                                        $row = db_fetch_assoc($result);
                                        $partner=$row[name];
                                        addnews("`\$$partner hat ".$session[user][name]."`\$ wegen eines Seitensprungs mit `^Ignitus`\$ verlassen!");
...
                                        $sql = "UPDATE accounts SET charisma=0,marriedto=0,goldinbank=goldinbank+$getgold WHERE acctid='$row[acctid]'";
                                        db_query($sql);
                                        systemmail($row['acctid'],"`\$Seitensprung!`0","`&{$session['user']['name']}`6 geht mit Ignitus fremd!`nDas ist Grund genug für dich, die Scheidung einzureichen. Ab sofort bist du wieder solo.`nDu bekommst `^$getgold`6 von ihrem Vermögen auf dein Bankkonto.");
                                    }else if ($session[user][charisma]==4294967295) {
                                        output(" Dein Mann wird gar nicht begeistert sein, wenn er davon erfährt!");
                                        $c--;
                                    }else{
...
                              break;
                            case 7:
                                output("Du gehst zu Ignitus und verlangst ohne Umschweife von ihm, daß er dich heiratet.`n`nEr schaut dich ein paar Sekunden lang an.`n`n");
                              if ($c>=22){
                                    output("\"`^Natürlich, meine Liebe!`0\", sagt er. Die nächsten wochen bist du damit beschäftigt, eine gigantische Hochzeit vorzubereiten, die natürlich Ignitus zahlt, und danach geht es in den dunklen Wald in die Flitterwochen.");
                                    addnews("`&".$session[user][name]." und `^Ignitus`& haben heute feierlich den Bund der Ehe geschlossen!!!");
                                    $session['user']['marriedto']=4294967295; //int max.
                                      $session['bufflist']['lover']=$buff;
                                    $session['user']['donation']+=1;
                                }else{
                                    output("`^Ignitus sagt: \"`^Es tut mir Leid, offensichtlich habe ich einen falschen Eindruck erweckt. Ich denke, wir sollten einfach nur Freunde sein.`0\"  Deprimiert hast du heute kein Verlangen mehr danach, nochmal im Wald kämpfen zu gehen.");
                                    $session[user][turns]=0;
                                }
                        }
...
                    }
                }else{
                      output("Du denkst, es ist besser, dein Glück mit `^Ignitus`0 heute nicht mehr herauszufordern.");
                }
            }else{
              //sorry, no lezbo action here.
            }
...
          if ($HTTP_GET_VARS[act]==""){
                output("Elias schaut dich irgendwie schräg an. Er ist keiner von der Sorte, die einem Mann viel weiter trauen, ");
                output("als sie ihn werfen können, was Zwergen einen entscheidenden Vorteil verleiht. Mit Ausnahme von Regionen natürlich, ");
                output("in denen Zwergenweitwurf verboten wurde.  Elias poliert ein Glas und hält es ins Licht, das durch die Tür hereinscheint, als ein Gast ");
...
                    output("`nEs stehen noch $amt frisch gefüllte und schon bezahlte Krüge mit Ale vor Elias.");
...
                    output("\"`%Du hast Edelsteine, oder?`0\", fragt dich Elias. \"`%Nun, für `^zwei Edelsteine`% werd ich dir nen magischen Trank machen!`0\"");
...
                      output("Elias starrt dich an und sagt: \"`%Du hast nich so viele Edelsteine, `bzieh los und besorg dir noch welche!`b`0\"");
                    }else{
                      output("`#Du platzierst $gemcount Edelsteine auf der Theke.");
                        if ($gemcount % 2 == 0){
                            
                        }else{
                            output("  Elias, der über deine absolute mathematische Unfähigkeit bescheid weiss, ");
                            output("gibt dir einen davon zurück.");
                            $gemcount-=1;
                        }
                        if ($gemcount>0) output("  Du trinkst den Trank, den Elias dir im Austausch für deine Edelsteine gegeben hat und.....`n`n");
...
                    output("Elias schaut dir tief in die Augen und meint nur \"`%Du hast heute schonmal eine Runde spendiert. In meiner Schenke machst du niemanden zum Säufer. Alles klar?`0\"");
                } else if ($amt>getsetting("maxales",50)){
                    output("\"`%Hast du sie noch alle, hier so mit deinem Gold anzugeben? Schau dich doch mal um, wieviele überhaupt da sind!`0\" Mit diesen Worten zeigt dir Elias einen Vogel und dreht sich gelangweilt weg. ");
                }else{
                    output("Du sprichst mit Barkeeper Elias und schiebst ihm `^$jamjam`0 Gold rüber. Dieser nickt mit dem Kopf und grölt in die Runde \"`%Die nächsten $amt Ale gehen auf $schussel !!`0\".");
...
                              //debuglog("spent {$HTTP_GET_VARS['amt']} gems on bribing Elias");
                            $try=true;
                        }
                    }else{
...
                              //debuglog("spent {$HTTP_GET_VARS['amt']} gold bribing Elias");
                        }
                    }
                    $chance*=100;
                    if ($try){
                        if (e_rand(0,100)<$chance){
                            output("Elias lehnt sich zu dir über die Theke und fragt: \"`%Was kann ich für dich tun, Kleine".($session[user][sex]?"":"r")."?`0\"");
                                if (getsetting("pvp",1)) {
...
                        }else{
                            output("Elias fängt an, die Oberfläche der Theke zu wischen, was eigentlich schon vor langer Zeit wieder einmal nötig gewesen wäre.  ");
...
                            output(" verschwunden. Du fragst wegen deinem Verlust nach, aber Elias starrt dich nur mit leerem Blick an.");
                        }
                    }else{
                        output("`n`nElias steht nur da und schaut dich ausdruckslos an.");
                    }
                }
...
                  //************************************************************************************************************************************
                    output(", aber Elias fährt unbekümmert damit fort, das Glas weiter zu polieren, an dem er gerade arbeitet. \"`%Du hast genug gehabt ".($session[user][sex]?"Mädl":"Bursche").".`0\" ");
                }else{
...
                        output(".  Elias nimmt ein Glas und schenkt schäumendes Ale aus einem angezapften Fass hinter ihm ein.  ");
                        output("Er gibt dem Glas Schwung und es rutscht über die Theke, wo du es mit deinen Kriegerreflexen fängst.  ");
                        output("`n`nDu drehst dich um, trinkst dieses herzhafte Gesöff auf ex und gibst ".($session[user][sex]?"Ignitus":"Kiana"));
...
                output("Elias legt einen Satz Schlüssel vor dich auf die Theke und sagt dir, welcher Schlüssel wessen Zimmer öffnet. Du hast die Wahl. Du könntest bei jedem reinschlüpfen und angreifen.");
...
              output("Elias lehnt sich weiter über die Bar. \"`%Du willst also was über Farben wissen, hmm?`0\" ");
                output("  Du willst gerade antworten, als du feststellst, dass das eine rhetorische Frage war.  ");
                output("Elias fährt fort: \`%Um Farbe in deine Texte zu bringen, musst du folgendes tun: Zuerst machst du ein &#0096; Zeichen ",true);
...
                    output("\"`2Ich will mein Spezialgebiet wechseln`0\", verkündest du Elias.`n`n");
                    output("Ohne ein Wort packt Elias dich am Hemd, zieht dich über die Theke und zerrt dich hinter die Fässer hinter ihm. ");
...
                    output("dreht Elias den Hahn wieder zurück und hebt einen frisch mit seinem vermutlich besten Gebräu gefüllten Krug. Das Zeug schäumt und ist von blau-grünlicher Farbe.");
...
                    output("`n`n\"`nElias fängt laut an zu lachen: \"`3Jup. Was hasten erwartet? Irgendne Art fantastisches und geheimnisvolles Ritual??? ");
...
                        redirect("index.php");
                    }else{
                        output("\"Aah, so ist das also.\", sagt Elias und hängt den Schlüssel, den er gerade geholt hat, wieder an seinen Haken hinter der Theke. ");
