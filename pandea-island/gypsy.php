...
//    if ($session[user][gold]>$cost){ // Eric Stevens
    ...
    $sql="SELECT name,alive FROM accounts WHERE locked=0 AND loggedin=1 AND laston>'".date("Y-m-d H:i:s",strtotime("-".getsetting("LOGINTIMEOUT",900)." seconds"))."' ORDER BY login ASC";
    ...
    if ($deadcount==0) $names.="`iNiemandem`i";
        output("`%Du fühlst die Anwesenheit von {$names}`%.`n`n");
    ...
    if ($session[user][transferredtoday]>getsetting("transferreceive",3)){
        ...
        output("`6Vessa5`wirft einen neidischen Blick auf dein Säckchen Edelsteine und beschließt, dir nichts mehr zu geben.");
    } else {
