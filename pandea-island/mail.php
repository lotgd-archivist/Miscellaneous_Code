...
if($_GET[op]=="del"){
        ...
        exit();
}elseif($_GET[op]=="archiv"){
        $sqltest = "SELECT mail.subject,mail.messageid,accounts.name,mail.msgfrom,mail.seen,mail.sent,mail.archiv, petitionmail.petitionid FROM mail LEFT JOIN petitionmail USING(messageid) LEFT JOIN accounts ON accounts.acctid=mail.msgfrom WHERE mail.msgto=\"".$session[user][acctid]."\" AND mail.archiv='1' ORDER BY mail.seen,mail.sent";
        $result = db_query($sqltest);
        if (db_num_rows($result)<30){
              $sql = "UPDATE mail SET archiv='1' WHERE messageid='$_GET[id]'";
              db_query($sql);
        }else{
              $session['message'] = "`\$`bDein Archiv ist mit 30 Nachrichten schon voll. Um eine neue Nachricht zu speichern, musst du wohl erstmal Platz machen!`b`0";
        }
        header("Location: mail.php");
        exit();
...
output("<a href='mail.php' class='motd'>Inbox</a><a href='mail.php?op=address' class='motd'>Mail schreiben</a>",true);
output("<a href='mail.php?archiv=yes' class='motd'>Archiv</a>",true);
output("`n`n");
...
if ($_GET[op]==""){
        ...
        if ($_GET[order]==""){
           $order="mail.seen,mail.sent DESC";
        }elseif ($_GET[order]=="msgfrom"){
           $order="mail.msgfrom";
        }elseif ($_GET[order]=="sent"){
           $order="mail.sent DESC";
        }elseif ($_GET[order]=="sent2"){
           $order="mail.sent ASC";
        }elseif ($_GET[order]=="subject"){
           $order="mail.subject";
        }
        $sql = "SELECT mail.subject,mail.messageid,accounts.name,mail.msgfrom,mail.seen,mail.sent,mail.archiv, petitionmail.petitionid FROM mail LEFT JOIN petitionmail USING(messageid) LEFT JOIN accounts ON accounts.acctid=mail.msgfrom WHERE mail.msgto=\"".$session[user][acctid]."\" AND mail.archiv='$arc' ORDER BY $order";
        $result = db_query($sql);
        if ($_GET[archiv]=="yes"){
           output("`b`iArchiv (".db_num_rows($result)."/30)`i`b");
           $archiv="&archiv=yes";
           $neueste="mail.php?archiv=yes";
        }else{
           $archiv="";
           $neueste="mail.php";
        }
        output("`n`7Sortieren nach: <a href='".$neueste."'>Ungelesen</a>|<a href='mail.php?order=msgfrom".$archiv."'>Absender</a>|<a href='mail.php?order=subject".$archiv."'>Betreff</a>|<a href='mail.php?order=sent".$archiv."'>Neu</a>|<a href='mail.php?order=sent2".$archiv."'>Alt</a>",true);
        if (db_num_rows($result)>0){
                ...
                if ($_GET[archiv]!="yes") output("`i`nOoooh, du hast keine Mails. Wie schade.`i");
                if ($_GET[archiv]=="yes") output("`i`nDu hast noch keine Mails ins Archiv gesichert.`i");
        }
        if ($_GET[archiv]!="yes") output("`n`n`iDu hast ".db_num_rows($result)." Nachrichten in deiner Mailbox`nNachrichten werden nach ".getsetting("oldmail",21)." Tagen gelöscht.");
}elseif ($_GET[op]=="read"){
        ...
        if (db_num_rows($result)>0){
                ...
                if($row[archiv]=='0')  output("<a href='mail.php?op=archiv&id=$row[messageid]' class='motd'>Ins Archiv</a>",true);
        }else{
        ...
        output("`n`2Text:`n");
        $output.="<textarea name='body' class='input' cols='60' rows='18'>".HTMLEntities($body).HTMLEntities(stripslashes($_GET['body']))."</textarea><br>";
