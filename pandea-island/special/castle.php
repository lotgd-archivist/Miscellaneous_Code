...
/********Healer********/
        ...
        if ($session[user][gems]>10) addnav("Transmutation (`^10 Gems`0)","forest.php?op=transmute"); //*** add by aragon: Ersatz für Transmutation bei Cedrik (wg. 10 DK-Rassenwahl)
        castlenav("return", $runden);
}elseif ($_GET[op]=="buy1"){
        $session['user']['specialinc']="castle.php";
        $loglev = log($session[user][level]);
        ...
        $session['user']['specialinc']="castle.php";
        $castleoptions = unserialize(gettexts('specialmisc'));
        if ($session[user][gems]>=1 && $session[user][visitedkala]<3){
                $session[user][gems]-=1;
        ...
        }else if ($session[user][visitedkala]>=3) {
        ...
        castlenav("return", $runden);
} else if ($_GET[op]=="transmute") { //*** add by aragon: Ersatz für Transmutation bei Cedrik (wg. 10 DK-Rassenwahl)
        $session['user']['specialinc']="castle.php";
...
/********Guard Fight********/
        ...
        switch ($_GET['act']) {
                /*
                case 'del':
                        $sql = 'DELETE FROM messageboard WHERE messageid='.(int)$_GET['msg'];
                        db_query($sql);
                        output("Du reisst die Nachricht vom Audienzbrett ab. Der Fall hat sich für dich erledigt.");
                        addnav('Zurück zum Audienzbrett','forest.php?op=msgboard');
                        break;
                */
                case 'add1':
                        /*
                        output("<form action=\"forest.php?op=msgboard&act=add2\" method='POST'>",true);
                        output("`nGib deine Nachricht ein:`n<input name='msg' maxlength='250' size='50'>`n",true);
                        output("<input type='submit' class='button' value='Um Audienz ersuchen'>",true);
                        addnav("","forest.php?op=msgboard&act=add2");
                        */
                        if (addmessageboard()) {
                ...
                case 'add2':
                        output("Die Palastwache schreibt deinen Text auf den Zettel und heftet ihn zu den anderen an das Audienzbrett neben dem Eingangstor.");
                        $sql = 'INSERT INTO messageboard (boardid, acctid, name, message) VALUES ('.$boardid.','.$session['user']['acctid'].',"'.$session['user']['name'].'","'.$_POST['msg'].'")';
                        db_query($sql);
                        addnav('Zurück zum Audienzbrett','forest.php?op=msgboard');
                        break;
                */
                default:
                        /*
                        $sql = 'SELECT messageid,acctid,name,message FROM messageboard WHERE boardid=1 ORDER BY messageid DESC';
                        $result = db_query($sql);
                        if (db_num_rows($result)==0){
                                output('Bislang hat sich noch niemand um eine Audienz beim König bemüht.');
                        }
                        else {
                                output("Am Audienzbrett neben dem Eingangstor flattern einige Nachrichten im Luftzug:");
                                while ($row = db_fetch_assoc($result)) {
                                        output("`n`n`& $row[name]`&:`n`^$row[message]`0 ");
                                        if ($row[acctid]==$session[user][acctid] || $session[user][superuser]==3){
                                                output("[<a href='forest.php?op=msgboard&act=del&msg=".$row['messageid']."'>entfernen</a>]",true);
                                                addnav("","forest.php?op=msgboard&act=del&msg=".$row['messageid']);
                                        }
                                }
                        }
                        */
