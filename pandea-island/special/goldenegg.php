...
if ($_GET[op]=="takeit") {
    ...
    if (getsetting("pvp",1)==0) {
        ...
        $sql = "SELECT acctid,name,sex,laston FROM accounts WHERE acctid = '".getsetting("hasegg",0)."'";
        ...
        if ($owner == $row[acctid]) {
            ...
            $session[user][hitpoints]-=$hurt;
            output("`n`n`^Du verlierst $hurt Lebenspunkte!`nDu verlierst das goldene Ei!`n");
            ...
            if (date("Y-m-d",strtotime($row['laston'])) <= date("Y-m-d",strtotime("-5 day")))
            {
            output("`3Mitten im Wald siehst du einen Greif mit einem goldenen Ei knapp an dir im Eiltempo vorbeifliegen.");
            savesetting("hasegg",stripslashes(0));
            addnews("`^Es wurde beobachtet, wie ein Greif ".$row['name']."`^ das goldene Ei entwendet hat.");
            }
            else
            {
            output("`3Mitten im dichten Wald entdeckst du auf einer starken Astgabel eines gewaltigen Baumes ein riesiges Nest.`n");
