...
// Vor dem Drachen in die Bar fliehen
...
// Grünen Drachen suchen
    ...
    output("ein Dutzend Meter über dem Boden des Waldes, wobei Geröll eine kegelförmige ");
    output("Rampe zum Eingang bildet.  Stalaktiten und Stalagmiten nahe des Einganges ");
    output("erwecken in dir den Eindruck, der Höhleneingang wäre in Wirklichkeit ");
    output("das Maul einer riesigen Bestie.");
    ...
    output("immer wieder, in einem regelmäßigen Rhythmus.");
    output("`n`nDu kletterst den Geröllhaufen hinauf, der zum Eingang der Höhle führt. Deine Schritte zerbrechen ");
    output("die scheinbaren Überreste ehemaliger Helden.");
    output("`n`nAll deine Instinkte wollen dich zur Flucht und so schnell wie möglich zurück ins warme Wirtshaus und ");
    output(" ".($session['user']['sex']?"zum noch wärmeren Seth":"zur noch wärmeren Violet")." treiben.  Was tust du?");
...
//Drachenschmiede
    ...
    if (strchr($session['user']['weapon'],"Qualitäts")){
        ...
        if (strchr($session['user']['armor'],"Qualitäts")){
            ...
            output("Hahaha! Deine Rüstung sieht ja genauso aus!");
        }else{
            output("An deiner Rüstung könnte ich allerdings noch etwas ändern. Danach würde sie `@$verteidigung`4 Rüstungsschutz haben. Allerdings verlange ich für diese Arbeit auch `^$verteidigungskosten Goldstücke`4, um meine Unkosten zu decken.");
            addnav("Rüstung verbessern","forest.php?op=powerruestung");
        }
...
//Drachenhöhle
    ...
    if ($glueck == 5){
        switch(e_rand(1,4)) {
            ...
            case 3:
                ...
                output("`n`^Du verlierst all dein Gold und 6% deiner Erfahrung!");
                output("`n`^Du kannst erst morgen wieder weiterkämpfen!");
                $session['user']['alive']=false;
                $session['user']['hitpoints']=0;
                $session['user']['gold'] = 0;
                $session['user']['experience'] *= 0.94;
                $session['user']['specialinc'] = '';
                addnews("`%".$session[user][name]."`3 fand man völlig verkohlt vor dem Eingang einer Höhle.");
                addnav("Tägliche News","news.php");
                break;
            case 4:
                output("`4Als du um die nächste Ecke gehst, steht dir ein riesiger Drache gegenüber, der den Schatz bewacht. Überrascht, wie du bist, kannst du den Hieben seiner mächtigen Klauen nicht mehr rechtzeitig ausweichen und der Drache lässt erst von dir ab, als er dich für tot hält.");
                output("`nDoch du bist nicht tot. Etwas später öffnest du mühsam die Augen, erkennst, dass der Drache nicht mehr in deiner unmittelbaren Nähe zu sein scheint, und schleppst dich mit letzter Kraft davon, bevor er zurückkommt.");
                output("`nDu hast einen Großteil deiner Lebenspunkte und 6% deiner Erfahrung verloren!");
                $session['user']['hitpoints']*=0.05;
                $session['user']['experience']*=0.94;
                addnav("Zurück in den Wald","forest.php?op=leave");
                break;
            }
    ...
    if ($glueck < 4){
        switch(e_rand(1,4)) {
            ...
            case 2:
            case 3:
                output("`4Du gehst weiter, bleibst aber an einer Schatztruhe hängen, die darauf mit einem lauten Knall umfällt, so dass das ganze Gold auf den Boden rollt. Dir ist die Truhe eigentlich völlig egal, doch als du dich wieder umdrehst, um den Weg weiterzulaufen, blickst du in die feuerroten Augen eines riesigen Drachens, der den Schatz bewacht. Du versuchst zwar, dich zu schützen und schnell zu verschwinden, doch es bringt alles nichts...");
                output("`n`^Du bist tot!");
                output("`n`^Du verlierst all dein Gold und 8% deiner Erfahrung!");
                output("`n`^Du kannst erst morgen wieder weiterkämpfen!");
                $session['user']['alive']=false;
                $session['user']['hitpoints']=0;
                $session['user']['gold'] = 0;
            ...
            case 4:
                output("`4Du gehst weiter, bleibst aber an einer Schatztruhe hängen, die darauf mit einem lauten Knall umfällt, so dass das ganze Gold auf den Boden rollt. Dir ist die Truhe eigentlich völlig egal, doch als du dich wieder umdrehst, um den Weg weiterzulaufen, blickst du in die feuerroten Augen eines riesigen Drachens, der den Schatz bewacht. Du versuchst zwar, dich zu schützen und schnell zu verschwinden, doch es bringt alles nichts...");
                output("`nDoch irgendwann hält dich der Drache für tot und lässt von dir ab - gerade noch rechtzeitig. Etwas später öffnest du mühsam die Augen, erkennst, dass der Drache nicht mehr in deiner unmittelbaren Nähe zu sein scheint, und schleppst dich mit letzter Kraft davon, bevor er zurückkommt.");
                output("`nDu hast einen Großteil deiner Lebenspunkte und 8% deiner Erfahrung verloren!");
                $session['user']['hitpoints']*=0.05;
                $session['user']['experience']*=0.92;
                addnav("Zurück in den Wald","forest.php?op=leave");
                break;
    ...
    if ($glueck == 1){
        switch(e_rand(1,5)) {
            ...
            case 4:
                ...
                break;
            case 5:
                $gold=$session['user']['level']*100+e_rand(1,100);
                output("`4Du läufst näher zum Drachen, um die schönsten Exemplare in dieser Schatzkammer zu rauben und steckst dir nebenbei die Taschen voll, doch dir fällt ein Edelstein aus der Tasche und der Drache wacht auf! Du versuchst zu flüchten, doch deine Taschen sind so voll, dass du kaum rennen kannst und dich der Drache schon nach wenigen Metern einholt...");
                output("`nMit seinen riesigen Klauen reißt das Ungetüm tiefe Wunden in dein Fleisch, lässt aber irgendwann von dir ab, als es dich tot glaubt. Doch bist du nicht wirklich tot. Etwas später öffnest du mühsam die Augen, erkennst, dass der Drache nicht mehr in deiner unmittelbaren Nähe zu sein scheint, und schleppst dich mit letzter Kraft davon, bevor er zurückkommt.");
                output("`nDu hast einen Großteil deiner Lebenspunkte und 10% deiner Erfahrung verloren!");
                output("`nEinige wenige Beutestücke sind jedoch in deinen Taschen verblieben. Du erbeutest `%einen`4 Edelstein und `^$gold`4 Goldstücke.");
                $session['user']['hitpoints']*=0.05;
                $session['user']['experience']*=0.90;
                $session['user']['gems']+=1;
                $session['user']['gold']+=$gold;
    ...
    if ($glueck < 4){
        switch(e_rand(1,6)) {
            ...
            case 4:
            case 5:
                output("`4Du läufst furchtlos zum Drachen und füllst dir die Taschen mit Gold und Edelsteinen. Als du einen besonders großen Edelstein, der dummerweise genau vor dem Maul des Ungetüms liegt, erblickst, vergisst du all deine Angst und versuchst, ihn hochzuheben. Von deinen ungestümen Anstalten wird jedoch der Drache wach und öffnet die Augen. Du bist so mit dem Edelstein beschäftigt, dass dir die Gefahr erst wieder klar wird, als dich das Feuer des Drachens umgibt und du jämmerlich verbrennst...");
                output("`n`^Du bist tot!");
                ...
                break;
            case 6:
                $gold=$session['user']['level']*200+e_rand(1,150);
                output("`4Du läufst furchtlos zum Drachen und füllst dir die Taschen mit Gold und Edelsteinen. Als du einen besonders großen Edelstein, der dummerweise genau vor dem Maul des Ungetüms liegt, erblickst, vergisst du all deine Angst und versuchst, ihn hochzuheben. Von deinen ungestümen Anstalten wird jedoch der Drache wach und öffnet die Augen. Du bist so mit dem Edelstein beschäftigt, dass dir die Gefahr erst wieder klar wird, als dich das Feuer des Drachens umgibt...");
                output("`nDoch irgendwie scheinst du selbst das überlebt zu haben, denn als du etwas später die Augen öffnest, bist du noch immer in der Höhle - der Drache muss dich wohl für tot gehalten haben. So schnell dich deine verbrannten Beine tragen, schleppst du dich davon, bevor das Monster wieder auftaucht.");
                output("`nDu hast einen Großteil deiner Lebenspunkte und 15% deiner Erfahrung verloren!");
                output("`nEinige Beutestücke sind jedoch in deinen Taschen verblieben. Du erbeutest `%zwei`4 Edelsteine und `^$gold`4 Goldstücke.");
                $session['user']['hitpoints']*=0.03;
                $session['user']['experience']*=0.85;
                $session['user']['gems']+=2;
                $session['user']['gold']+=$gold;
                addnav("Zurück in den Wald","forest.php?op=leave");
    ...
    $gold=$session['user']['level']*60+e_rand(1,50);
    ...
    $gold=$session['user']['level']*180+e_rand(1,150);
    $gems=e_rand(1,2);
    ...
    $gold=$session['user']['level']*300+e_rand(1,250);
    $gems=e_rand(3,4);
    ...
    output("`7Auf deiner Erkundungstour triffst du in einer Höhle auf eine Echsendame, die dich auch sofort anspricht: \"`4Sei gegrüßt! Ich bin Sylaya und kenne mich mit Drachen wohl so gut aus wie niemand sonst. Warum bist du zu mir gekommen?`7\"");
    ...
    if ($session['user']['hitpoints'] < $session['user']['maxhitpoints']){
        output("`7Du zeigst Sylaya deine Wunden und sie erklärt dir, dass sie sich gelegentlich mit dem Heiler aus dem Wald trifft und Heilmittel austauscht. Wenn du willst, wird sie deine Wunden versorgen.");
    ...
    //Einleitungstext
    output("`7Du hast den Wald durchquert und kommst auf der anderen Seite wieder heraus. Du siehst eine sich ewig ausdehnende Einöde vor
        dir liegen, gehst aber weiter. Nach Stunden des Laufens hast du die Orientierung komplett verloren, dafür aber einen Durst, wie du ihn dir nie
        hättest vorstellen können. Zuerst meinst du, deine Sinne würden dir einen Streich spielen, aber dann erkennst du schnell, dass du tatsächlich auf zwei
        riesige Felswände zuläufst, zwischen denen ein Tal liegt. In der Hoffnung auf Wasser sammelst du noch einmal deine letzten Kräfte und
        kommst schließlich im Tal an... könnte dies das sagenumwobene Drachental sein?");
