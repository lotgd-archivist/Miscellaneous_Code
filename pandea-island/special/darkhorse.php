...
case "":
        checkday();
        output("Eine Baumgruppe in der Nähe kommt dir bekannt vor... Du bist dir sicher, diesen Platz schon einmal gesehen zu haben.  ");
        ...
        if ($playermount['tavern']>0) output("Aber dein treues Tier scheint den Weg zu kennen."); // BoarVolk's idea
...
case "bartender":
        if ($_GET[what]==""){
                ...
                $sql="SELECT acctid,name,location,loggedin,invisible,laston,alive,housekey FROM accounts WHERE acctid=".$_GET[who];
                $result = db_query($sql) or die(db_error(LINK));
                $row = db_fetch_assoc($result);
            ...
            if ($invisible=='0'){
                ...
                // end houses
                if ($row[location]==9) $loc="am Pranger";
                if ($invisible=='0'){
                output("\"`7$row[name]`7 wurde zuletscht $loc geschehen".($row[alive]?"":", ischt inzwischen aber ... geschtorben ... worden")."!`0\"");
        }else{
                output("\"`7$row[name]`7 wurde zuletscht $loc geschehen, ischt inzwischen aber ... geschtorben ... worden!`0\"");
        }
...
case "oldman":
        ...
        case "":
                ...
                break;
        case "stones":
                $stones = unserialize(gettexts('specialmisc'));
                ...
                updatetexts('specialmisc',serialize($stones));
                break;
        case "guess":
                if ($session[user][gold]>0){
                        ...
                        if ($bet<=0){
                                ...
                                updatetexts('specialmisc',e_rand(1,100));
                                ...
                                if ($_POST[guess]!==NULL){
                                        $try = (int)$_GET['try'];
                                        if ($_POST['guess']==gettexts('specialmisc')){
                                                if ($try == 1) {
                                                ...
                                                if ($_GET['try']>=6){
                                                        ...
                                                        if ((int)$_POST['guess']>gettexts('specialmisc')){
        ...
        case "dice":
                if ($session[user][gold]>0){
                        ...
                        if ($bet<=0){
                                ...
                                if ($_GET[what]!="keep"){
                                        updatetexts('specialmisc',e_rand(1,6));
                                        $try=$_GET['try'];
                                        $try++;
                                        output("Du würfelst deinen ".($try==1?"ersten":($try==2?"zweiten":"dritten"))." Versuch und es erscheint die `b".gettexts('specialmisc')."`b`n`n");
                                        ...
                                        output("Dein endgültiger Wurf war `b".gettexts('specialmisc')."`b, der alte Mann wird jetzt versuchen, das zu überbieten:`n`n");
                                        $r = e_rand(1,6);
                                        output("Der alte Mann wirft eine $r...`n");
                                        if ($r>gettexts('specialmisc') || $r==6){
                                                output("\"`7Ich glaub ich behalte diesen Wurf!`0\"`n");
                                        }else{
                                                $r = e_rand(1,6);
                                                output("Der alte Mann würfelt nochmal und bekommt eine $r...`n");
                                                if ($r>=gettexts('specialmisc')){
                                                        output("\"`7Ich glaub ich behalte diesen Wurf!`0\"`n");
                                                }else{
                                                        $r = e_rand(1,6);
