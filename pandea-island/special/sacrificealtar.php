...
//    if ($session[user][level] != 1) addnav("Ein schwaches Monster","forest.php?op=Sacrifice&type=Creature&Difficulty=Weak");
    addnav("Ein paar Blumen","forest.php?op=Sacrifice&type=Flowers");
...
$session[user][turns]--;
            ...
            elseif ($_GET[flower]=="Daisies"){
                ...
                switch(e_rand(1,5)){
                    case 1:
                        ...
                        output("`n`nAls Dein Geist in die Schatten abtaucht, denkst Du noch \"`&Sie irren sich, ich denke, ");
  ...
  output("weil Du an ihrem heligen Altar verweilst.");
    ...
    output("`@Du trägst dein Geschenk, `^".$badguyname."`@, zurück zum Altar. Du legst den toten Leichnahm auf den ");
...
if ($battle){
  include("battle.php");
    ...
    if (getsetting("dropmingold",0)){
        ...
        output("`b`\$Du wurdest von $badguy[creaturename] getötet!`0`b`n");
