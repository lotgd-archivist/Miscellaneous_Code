...
if ($_GET[op]==""){
    output("`5Nicht ganz zufällig steht die kleine Holzhütte von `#Pegasus`5 neben `!MightyEs`5 Hause. ");
    output("So hat er immer ein bewunderndes Auge auf seine hübsche `#Pegasus`5 und zugleich kann er sie beschützen. ");
    if($session[user][thefttoday]) {
        output("Ihr misstrauischer Blick lässt keine Zweifel daran, dass sie bei der ersten falschen Bewegung ");
        output("mit dem Finger schnippen würde, um `!MightyE`5 wie ein Racheengel vor dir erscheinen zu lassen. ");
    }
    output("Pegasus weist mit einer einladenden Handbewegung in das Innere ihrer Hütte, deren gewagte Einrichtung dir farbenprächtig entgegenleuchtet. ");
    output("Die Rüstungen liegen sorgsam auf den Ausstellungstischen ausgebreitet und dein Abenteurerherz beginnt bei diesem Anblick schnell zu schlagen. ");
    /*
    ...
    */
    ...
    $result = db_query($sql) or die(db_error(LINK));
    output("`5Du probierst ein Stück nach dem anderen an und drehst und wendest dich vor dem mannsgroßen Spiegel an der Wand. ");
    output("Du wusstest doch schon immer, dass du in allem toll aussiehst! ");
    output("`#Pegasus`5 steht neben dir und hilft dir in die Rüstungen. ");
    output("Während du dich umziehst, begutachtet sie dein(e/n) ".$session[user][armor]." und bietet dir im Tausch `^$tradeinvalue Goldstücke`5 an.`n`n ");
    /*
...
//            addnav("","armor.php?op=buy&id=$row[armorid]");
    ...
    if (db_num_rows($result)==0){
        ...
        if ($row[value]>($session[user][gold]+$tradeinvalue)){
            ...
            output("`#Pegasus`5 nimmt dein Gold".($tradeinvalue==0?"":" und sehr zu deiner Überraschung nimmt sie auch dein(e/n) `%".$session[user][armor]."`5  hängt ein Preisschild dran und legt die Rüstung hübsch zu den anderen").". ");
