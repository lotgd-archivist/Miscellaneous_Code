...
if (db_num_rows($result) > 0){
    ...
    if ($master[creaturename] == "Gadriel der Elfen-Waldläufer" && $session['user']['race'] == 'Elf') {
...
//    $exparray=array(1=>100,300,602,1012,1540,2207,3041,4085,5395,7043,9121,11740,15037,19171,24330);
    ...
    $expprodk=array(1=>10,22,37,57,80,107,138,174,214,259,309,363,424,490,563);
    ...
    if ($_GET[op]==""){
        output("Der Klang von Kampf umfängt dich. Das Geklirre von Waffen in mächtigen Kämpfen lässt dein Kriegerherz höher schlagen. ");
    ...
    if (!$session['prefs']['nosounds']) output("<embed src=\"media/bigbong.wav\" width=10 height=10 autostart=true loop=false hidden=true volume=100>",true);
            ...
            if ($session[user][experience]>=$exprequired){
                ...
                updatetexts('badguy',createstring($master));
    ...
    if (!$session['prefs']['nosounds']) output("<embed src=\"media/cheer.wav\" width=10 height=10 autostart=true loop=false hidden=true volume=100>",true);
