...
//Anfang Angels Edelsteinmod for www.pandea-island.de
    ...
    if($session['user']['gemsinbank']<1) $session['user']['gemsinbank']=0;
      ...
      if($session['user']['putingems']==1){
      ...
      }elseif ($session['user']['gemsinbank']>=$session['user']['gemsvolume']){
         ...
         <input type='submit' class='button' value='Einlagern'>`n`iGib 0 oder gar nichts ein, um den Tresor voll zu machen oder alles einzuzahlen.`i</form>",true);
         output("<script language='javascript'>document.getElementById('input').focus();</script>",true);
         addnav("","bank.php?op=gemsfinish");
      }
}else if($_GET['op']=="gemsfinish"){
        $max=$session['user']['gemsvolume'];
        $in=$session['user']['gemsinbank'];
        $free=$max-$in;
        $_POST[amount]=abs((int)$_POST[amount]);
        if ($_POST[amount]==0){
                if ($session[user][gems]>$free) $_POST[amount]=$free;
                else $_POST[amount]=$session[user][gems];
        }
...
}else if($_GET['op']=="getgems"){
      if($session['user']['gemsinbank']<1){
         ...
         <input type='submit' class='button' value='Abholen'>`n`iGib 0 oder gar nichts ein, um den Tresor leer zu machen.`i</form>",true);
         output("<script language='javascript'>document.getElementById('input').focus();</script>",true);
         addnav("","bank.php?op=getgemsfinish");
      }
}else if($_GET['op']=="getgemsfinish"){
        $in=$session[user][gemsinbank];
        $_POST[amount]=abs((int)$_POST[amount]);
        if ($_POST[amount]==0 || $_POST[amount] > $in){    /* gems-in-bank negativ-prevention */
                $_POST[amount]=$in;
        }
