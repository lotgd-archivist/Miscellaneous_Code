...
// output("`b`c`\$IN ÜBERARBEITUNG`b`c`0`n");
        ...
        if (db_num_rows($result)>0){
                ...
                if ($session[user][charisma]<=$row[charisma]) $flirtnum=$session[user][charisma];
                if ($row[charisma]<$session[user][charisma]) $flirtnum=$row[charisma];
                $sql2="SELECT * FROM race WHERE name='".$row['race']."'";
                $result2 = db_query($sql2) or die(db_error(LINK));
                $row2 = db_fetch_assoc($result2);
                $sql3="SELECT * FROM race WHERE name='".$session['user']['race']."'";
                ...
                if (ac_check($row)){
                        output("`\$`bDas geht doch nicht!!`b Du kannst doch nicht mit deinen eigenen Charakteren oder deiner eigenen Familie flirten!");
                         addnav("Zurück zum Garten","gardens.php");
                } else if ($session[user][marriedto]!=$row[acctid] && $session[user][acctid]!=$row[marriedto] && in_array($row2['rid'],unserialize($row3['cantmarry'])) || in_array($row3['rid'],unserialize($row2['cantmarry']))) {
                        output("Du wartest im Garten auf `6$row[name]`0 und beobachtest ".($session[user][sex]?"ihn":"sie")." eine Weile. Bei näherer Betrachtung stellst du aber fest, dass ".$session['user']['race']."en und ".$row['race']."en vielleicht doch niemals zusammen passen werden.");
        ...
        if (!$session['prefs']['nosounds']) output("<embed src=\"media/vogel.wav\" width=10 height=10 autostart=true loop=false hidden=true volume=100>",true);
        /*
        ...
        output("`n`n");
        */
        output("`n`n`v Du betrittst den Garten und  plötzliche Ruhe umfängt dich. ");
        output("Durch den Garten schlängelt sich ein silbrig glänzender, gemächlich plätschernder Fluss, über den sich eine Brücke aus weißem Stein spannt.");
        output("Nur wenige Meter nach der Brücke ist ein kleiner Wasserfall, der in einem herrlichen See mündet, auf dessen gegenüberliegender Seite der Fluss wieder seinen Lauf nimmt.");
        output("Hier und dort in den natürlichen Nischen der Büsche, in den kleinen Buchten am See und auf der kleinen Lichtung im nahegelegenen Trauerweidenhain, siehst du das eine oder andere Paar, das sich leise unterhält, sich die ewige Liebe schwört oder sich in den Armen liegt. Du spürst, dass eine besonnene Stimmung über diesem Garten liegt, die nicht nur von den unzähligen exotischen und heimischen Blumen und Pflanzen ausgeht die überall wachsen und erblühen.");
        output("Im hinteren, etwas verwilderten Teil des Gartens findest du zufällig eine warme Quelle, die ihr Wasser in ein kleines, natürliches Becken ergießt.");
        output("Instinktiv ist dir bewusst, dass hier besondere Regeln herrschen, wenn du hier geduldet werden willst.`n`n ");
