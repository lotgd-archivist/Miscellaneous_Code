...
if ($session['user']['loggedin']){
        ...
        output("Du bekommst automatisch 25 Punkte für jeden geworbenen Charakter, der es bis Level 5 schafft.
        `n`n
        Woher weiß die Seite, dass du eine Person geworben hast?`n
  ...
  ".getsetting("serverurl","http://".$_SERVER['SERVER_NAME'].dirname($_SERVER['REQUEST_URI']))."/referral.php?r=". rawurlencode($session['user']['login'])."`n`n
          `0Hast du vielleicht sogar eine eigene Homepage? Wenn du über deine Homepage für uns Werbung machen willst haben wir hier einen Banner für dich, den du
          ganz einfach mit dem darunter stehenden Code auf deine Seite einbinden kannst!`n`n");
          output("<img src='banner1.gif' border=0>",true);
          output("`n");
          output("`q<a href='".getsetting('serverurl','http://'.$_SERVER['SERVER_NAME'].dirname($_SERVER['REQUEST_URI'])).'/referral.php?r='.rawurlencode($session['user']['login'])."'><img src='http://www.pandea-island.de/banner1.gif' border=0></a>");
          output("`n`n");
          output("
        `0Dadurch wird die Seite wissen, dass du die Person warst, die den neuen Spieler hergeschickt hat. Wenn er dann zum ersten Mal Level 5 erreicht, bekommst du deine Punkte!");
