...
if ($_GET[op] == ""){
    ...
    if ($session[user][gold] > 49 and $session[user][turns] > 0) addnav("E?Zahle Eintritt (50 Gold)","battlearena.php?op=pay");
...
if ($_GET[op] == "prefight"){
    ...
    updatetexts('specialmisc',$badguy['creaturehealth']);
    $_GET[op]="fight";
}
if ($_GET[op] == "fight"){
$battle=true;
...
*/
        ...
        output("`n");
                    for ($i=0;$i<1;$i+=.02){
            if ($badguy[creaturehealth] > gettexts('specialmisc') * $i){
