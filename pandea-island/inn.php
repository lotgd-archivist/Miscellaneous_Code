...
page_header("Die Boar's Head Kneipe");
output("<span style='color: #9900FF'>",true);
output("`c`bDie Boar's Head Kneipe`b`c");
if ($_GET[op]=="strolldown"){
        output("Wieder mal bereit fürs Abenteuer schlenderst du die Treppen der Kneipe runter! ");
}
if ($_GET[op]==""){
        output("Die grob hölzerne Tür quietscht in ihren Angeln, als du Cedriks Kneipe betrittst. ");
}
if ($_GET[op]=="" || $_GET[op]=="strolldown"){
        output("Rauch, Gelächter und Stimmengewirr schlagen dir entgegen. Als du dich umsiehst, entdeckst du hinter dem Tresen Cedrik, der gewissenhaft einen Krug mit Ale füllt. ");
        if (getsetting("pvp",1)) {
                output("Zwischen all den anderen Menschen siehst du die komplett in schwarz gehüllte Gestalt von Dag Durnick, der seinen Hut tief ins Gesicht gezogen hat. ");
        }
        output("`5Violet`0 balanciert einige Krüge an dir vorbei und wirft dir ein Lächeln zu. ");
        output("Hinter ihr erblickst du `^Seth`0 am Kamin, der wieder einmal versucht, eine Maid mit seinem Gesange zu betören. ");
        output("Hier willst bleiben, wenn es dich dürstet, du ein warmes Bette suchst und die neusten Nachrichten hören oder am schwarzen Brette lesen willst. ");


        output("`n`nDie Uhr am Kamin zeigt `6".getgametime()."`0.");

  output("`n`nDu schlenderst rüber zu einem Tisch, stellst den Fuß auf die Bank und lauschst der Unterhaltung:`n");
        ...
        addnav("B?Schwarzes Brett ansehen","inn.php?op=readmsgboard");
        addnav("C?Spreche mit Barkeeper Cedrik","inn.php?op=bartender");
        if (getsetting("pvp",1)) {
  ...
  case "readmsgboard":
                ...
                addnav("Nachricht hinterlassen","inn.php?op=msgboard");
          break;
        case "msgboard":
                if ($_GET[act]=="add1"){
                        $msgprice=$session[user][level]*6*(int)$_GET[amt];
                        if (isset($_POST['insertblackboard'])) {
                                ...
                                formmessageboard('inn','Ans schwarze Brett','Gib deine Nachricht ein:');
                        }
                } else{
                        ...
                        $sql = 'SELECT COUNT(*) AS zahl FROM messageboard WHERE boardid="inn" AND acctid="'.$session['user']['acctid'].'"';
                        $result = db_query($sql);
                        $row = db_fetch_assoc($result);
                        if ($row['zahl']>0) output("`nEr macht dich noch darauf aufmerksam, dass er deine alte Nachricht entfernen wird, wenn du jetzt eine neue anbringen willst.");
                }
                break;
          case "violet":
                        /*
                          ...
                          if ($session[user][seenlover]==0){
                                    if ($session['user']['marriedto']==4294967295){
                                          ...
                                          output("`n`n`^Du VERLIERST einen Charmepunkt!");
                                                ...
                                                output("Du und `5Violet`0 nehmt euch etwas Zeit für euch selbst und du verlässt die Kneipe zuversichtlich strahlend!");
                                        ...
                                        $session['user']['seenlover']=1;
                                          ...
                                          switch($_GET[flirt]){
                                                  ...
                                                  case 4:
                                                          if (e_rand($c,11)>=11){
        if (!$session['prefs']['nosounds']) output("<embed src=\"media/giggle.wav\" width=10 height=10 autostart=true loop=false hidden=true volume=100>",true);
                                                  ...
                                                  case 6:
                                                          if (e_rand($c,18)>=18){
                                                            output("Wie ein Wirbelwind braust du durch die Kneipe, schnappst dir `5Violet`0, die dir ihre Arme ");
                                                                        ...
                                                                        addnews("`@Es wurde beobachtet, wie ".$session[user][name]."`@ und `5Violet`@ gemeinsam die Treppen in der Kneipe nach oben gingen.");
                                                                  ...
                                                                  output("Wie ein Wirbelwind fegst du durch die Kneipe und schnappst nach `5Violet`0. Sie dreht sich um und ");
                                                                        output("schlägt dir ins Gesicht! \"`%Für was hältst du mich eigentlich?`0\" brüllt sie dich an! ");
                                                                        if ($c>0) $c--;
                                                                }
                                                          break;
                                                        case 7:
                                                                output("`5Violet`0 arbeitet fieberhaft, um einige Gäste der Kneipe zu bedienen. Du schlenderst zu ihr rüber, ");
      ...
      if ($_GET[subop]=="hear"){
         ...
         // addnav("Return to the inn","inn.php");
          ...
          switch ($rnd){
            ...
            case 1:
              ...
              break;
            case 2:
        if (!$session['prefs']['nosounds']) output("<embed src=\"media/ragtime.mid\" width=10 height=10 autostart=true loop=false hidden=true volume=100>",true);
        ...
        $session['user']['donation']+=1;
              break;
            case 6:
        if (!$session['prefs']['nosounds']) output("<embed src=\"media/matlock.mid\" width=10 height=10 autostart=true loop=false hidden=true volume=100>",true);
              ...
              break;
            case 7:
        if (!$session['prefs']['nosounds']) output("<embed src=\"media/burp.wav\" width=10 height=10 autostart=true loop=false hidden=true volume=100>",true);
        ...
        if (!$session['prefs']['nosounds']) output("<embed src=\"media/indianajones.mid\" width=10 height=10 autostart=true loop=false hidden=true volume=100>",true);
        ...
        if (!$session['prefs']['nosounds']) output("<embed src=\"media/eternal.mid\" width=10 height=10 autostart=true loop=false hidden=true volume=100>",true);
              output("`n`nDeine Stimmung fällt und du wirst heute nicht mehr so viele Bösewichte erschlagen.");
              $session[user][turns]--;
              if ($session[user][turns]<0) $session[user][turns]=0;
              break;
            case 12:
        if (!$session['prefs']['nosounds']) output("<embed src=\"media/babyphan.mid\" width=10 height=10 autostart=true loop=false hidden=true volume=100>",true);
              ...
              break;
            case 13:
              output("Es war einmal eine Dame von der Venus, ihr Körper war geformt wie ein ...");
              if ($session[user][sex]==1){
              ...
              break;
            case 14:
        if (!$session['prefs']['nosounds']) output("<embed src=\"media/knightrider.mid\" width=10 height=10 autostart=true loop=false hidden=true volume=100>",true);
            ...
            case 15:
              ...
              break;
            case 16:
        if (!$session['prefs']['nosounds']) output("<embed src=\"media/boioing.wav\" width=10 height=10 autostart=true loop=false hidden=true volume=100>",true);
            ...
            case 18:
              ...
              break;
                        ...
                        if ($session[user][sex]==1 && $_GET[flirt]<>""){
                          ...
                          if ($session[user][seenlover]==0){
                                    ...
                                    if (e_rand(1,4)==1){
                                          ...
                                          output("Du und Seth nehmt euch etwas Zeit füreinander und du verlässt die Kneipe mit einem zuversichtlichen Strahlen!");
                                          $session['bufflist']['lover']=$buff;
                                          $session['user']['charm']++;
                                          output("`n`n`^Du erhältst einen Charmepunkt!");
                                        }
                                    $session['user']['seenlover']=1;
                                          ...
                                          switch($_GET[flirt]){
                                                  ...
                                                  case 4:
                                                          if (e_rand($c,11)>=11){
                                                                  output("Seth platziert seinen Arm um deine Hüfte, geleitet dich an die Bar und kauft dir eines der köstlichsten Getränke, die es in der Kneipe gibt.");
                                                                        if ($session[user][charisma]==4294967295) {
                                                                                output(" Dein Mann wird gar nicht begeistert sein, wenn er davon erfährt!");
                                                                                $c--;
                                                                        } else {
                                                  ...
                                                  case 6:
                                                          if (e_rand($c,18)>=18){
                                                            output("Du stehst auf der ersten Treppenstufe und gibst Seth ein 'komm hierher' Zeichen. Er folgt dir wie ein Schoßhündchen.");
                                                                        ...
                                                                        addnews("`@Es wurde beobachtet, wie ".$session[user][name]."`@ und `^Seth`@ gemeinsam die Treppen in der Kneipe nach oben gingen.");
                                                                        if ($session[user][charisma]==4294967295 && e_rand(1,3)==2) {
                                                                                $sql = "SELECT acctid,name FROM accounts WHERE locked=0 AND acctid=".$session[user][marriedto]."";
                                                                                  $result = db_query($sql) or die(db_error(LINK));
                                                                                $row = db_fetch_assoc($result);
                ...
                if (getsetting("paidales",0)<=1 || $session[user][gotfreeale]>=2) {
                        $alecost = $session[user][level]*8;
                } else {
                        $alecost = 0;
                }
                $xmascost = $session[user][level]*10;
                $metcost = $session[user][level]*9;
                $rumcost = $session[user][level]*10;
                $grogcost = $session[user][level]*15;
                $weincost = $session[user][level]*20;
                  if ($_GET[act]==""){
                                ...
                                output("die Kneipe verläßt. Dann verzieht er das Gesicht, spuckt auf das Glas ");
                                ...
                                if (getsetting("paidales",0)<=1) {
                                        ...
                                        addnav("Met (`^$metcost`0 Gold)","inn.php?op=bartender&act=met");
                                        addnav("Rum (`^$rumcost`0 Gold)","inn.php?op=bartender&act=rum");
                                        addnav("Grog (`^$grogcost`0 Gold)","inn.php?op=bartender&act=grog");
                                        addnav("Wein (`^$weincost`0 Gold)","inn.php?op=bartender&act=wein");
                                        #addnav("Glühwein (`^$xmascost`0 Gold)","inn.php?op=bartender&act=punsch");
                                        addnav("Runde schmeißen (Ale)","inn.php?op=bartender&act=schmeiss");
                                } else {
                          ...
                          if ($_POST[gemcount]==""){
                                        ...
                                        output("<input type='radio' name='wish' value='4'> Vergessen</form>`n",true);
                                        //output("<input type='radio' name='wish' value='5'> Transmutation</form>",true); rausgenommen wg. rassenwahl
                                }else{
                                ...
                                if ($session[user][gold]<$jamjam){
                                        ...
                                        output("Cedrik schaut dir tief in die Augen und meint nur \"`%Du hast heute schonmal eine Runde spendiert. In meiner Kneipe machst du niemanden zum Säufer. Alles klar?`0\"");
                                        ...
                                        output("Cedrik schaut dich schief an und murmelt: \"Immer diese Säufer die nicht mehr zählen können und 0 Ales ausgeben wollen...\"");
                                }else{
                                        ...
                                        if ($amt>(e_rand(getsetting("alecharme",5),getsetting("maxales",50))+e_rand(0,15)-e_rand(0,15))){
                        ...
                        $session[user][charm]+=1;
                          ...
                          output("Du schlägst mit der Faust auf die Bar und verlangst ein Ale. ");
                                if ($session[user][drunkenness]>66){
                                  //************************************************************************************************************************************
                                        output("Doch Cedrik fährt unbekümmert damit fort, das Glas weiter zu polieren, an dem er gerade arbeitet. \"`%Du hast genug gehabt ".($session[user][sex]?"Mädl":"Bursche").".`0\" ");
                                }else{
                                                  ...
                                                  case 1:
                                                        case 2:
                                                          output("`&Du fühlst dich gesund!");
                                                                //$session[user][hitpoints]+=round($session[user][maxhitpoints]*.1,0);
                                                                if ($session[user][dragonkills] < 2) $pmax = 15;
                                                                else $pmax = round(10*(15-log($session[user][dragonkills])),0);
                                                                $session[user][hitpoints]+=round(mt_rand(50,$pmax)/1000*$session[user][maxhitpoints],0);
                                                                break;
                                                        case 3:
                                                          output("`&Du fühlst dich lebhaft!");
                                                                $session[user][turns]++;
                                                }
                                                $session[bufflist][101] = array("name"=>"`#Rausch","rounds"=>10,"wearoff"=>"Dein Rausch verschwindet.","atkmod"=>1.05,"roundmsg"=>"Du hast einen ordentlichen Rausch am laufen.","activate"=>"offense");
                                        }else{
                                          output("Du hast aber nicht genug Geld bei dir. Wie kannst du ein Ale haben wollen, wenn du das Geld dafür nicht hast!?!");
                                        }
                                }
                        }else if ($_GET[act]=="met"){
                          output("Du schlägst mit der Faust auf die Bar und verlangst Met. ");
                                if ($session[user][drunkenness]>66){
                                        output("Doch Cedrik fährt unbekümmert damit fort, das Glas weiter zu polieren, an dem er gerade arbeitet. \"`%Du hast genug gehabt ".($session[user][sex]?"Mädl":"Bursche").".`0\" ");
                                }else{
                                  if ($session[user][gold]>=$metcost){
                                          $session[user][drunkenness]+=33;
                                                $session[user][gold]-=$metcost;
                                                output("Cedrik nimmt ein sauberes Horn und füllt dir Met hinein. ");
                                                output("Er reicht es dir und du kehrst ihm den Rücken zu, um deinen Blick durch die Kneipe schweifen zu lassen.`n`n");
                                                switch(e_rand(1,3)){
                                                  case 1:
                                                        case 2:
                                                          output("`&Du fühlst dich gesund!");
                                                ...
                                                $session[bufflist][101] = array("name"=>"`#Rausch","rounds"=>10,"wearoff"=>"Dein Rausch verschwindet.","atkmod"=>1.15,"roundmsg"=>"Du hast einen ordentlichen Rausch am laufen.","activate"=>"offense");
                                        }else{
                                          output("Du hast aber nicht genug Geld bei dir. Wie kannst du Met haben wollen, wenn du das Geld dafür nicht hast!?!");
                                        }
                                }
                        }else if ($_GET[act]=="rum"){
                          output("Du schlägst mit der Faust auf die Bar und verlangst einen guten Rum. ");
                                if ($session[user][drunkenness]>66){
                                        output("Doch Cedrik fährt unbekümmert damit fort, das Glas weiter zu polieren, an dem er gerade arbeitet. \"`%Du hast genug gehabt ".($session[user][sex]?"Mädl":"Bursche").".`0\" ");
                                }else{
                                  if ($session[user][gold]>=$rumcost){
                                          $session[user][drunkenness]+=42;
                                                $session[user][gold]-=$rumcost;
                                                output("Cedrik nimmt eins der Gläser und füllt dir den Rum ab. ");
                                                output("Er schiebt es dir über die Theke und steckt sich das Gold dafür ein.`n`n");
                                                switch(e_rand(1,3)){
                                                  case 1:
                                                        case 2:
                                                          output("`&Du fühlst dich gesund!");
                                                ...
                                                $session[bufflist][101] = array("name"=>"`#Rausch","rounds"=>18,"wearoff"=>"Dein Rausch verschwindet.","atkmod"=>1.20,"roundmsg"=>"Du hast einen ordentlichen Rausch am laufen.","activate"=>"offense");
                                        }else{
                                          output("Du hast aber nicht genug Geld bei dir. Wie kannst du Rum haben wollen, wenn du das Geld dafür nicht hast!?!");
                                        }
                                }
                        }else if ($_GET[act]=="grog"){
                          output("Du schlägst mit der Faust auf die Bar und verlangst einen Grog. ");
                                if ($session[user][drunkenness]>66){
                                        output("Doch Cedrik fährt unbekümmert damit fort, das Glas weiter zu polieren, an dem er gerade arbeitet. \"`%Du hast genug gehabt ".($session[user][sex]?"Mädl":"Bursche").".`0\" ");
                                }else{
                                  if ($session[user][gold]>=$grogcost){
                                          $session[user][drunkenness]+=38;
                                                $session[user][gold]-=$grogcost;
                                                output("Cedrik poliert sichtlich unbeeindruckt von deiner Art der Bestellung das Glas noch fertig und gibt dir dann deinen Grog. ");
                                                output("Er setzt ein künstliches Lächeln auf und wendet sich grummelnd von dir ab.`n`n");
                                                switch(e_rand(1,3)){
                                                  case 1:
                                                        case 2:
                                                          output("`&Du fühlst dich gesund!");
                                                ...
                                                $session[bufflist][101] = array("name"=>"`#Rausch","rounds"=>17,"wearoff"=>"Dein Rausch verschwindet.","atkmod"=>1.25,"roundmsg"=>"Du hast einen ordentlichen Rausch am laufen.","activate"=>"offense");
                                        }else{
                                          output("Du hast aber nicht genug Geld bei dir. Wie kannst du Grog haben wollen, wenn du das Geld dafür nicht hast!?!");
                                        }
                                }
                        }else if ($_GET[act]=="wein"){
                          output("Du gehst auf die Theke zu forderst Cedrik dazu auf dir seine besten Weine zu zeigen.");
                                if ($session[user][drunkenness]>66){
                                        output("Doch Cedrik fährt unbekümmert damit fort, das Glas weiter zu polieren, an dem er gerade arbeitet. \"`%Du hast genug gehabt ".($session[user][sex]?"Mädl":"Bursche").".`0\" ");
                                }else{
                                  if ($session[user][gold]>=$weincost){
                                          $session[user][drunkenness]+=33;
                                                $session[user][gold]-=$weincost;
                                                output("Er holt 2 Flaschen unter der Theke hervor und gibt dir von beiden zu probieren. Allerdings kommt dir bei deinem zweiten Versuch, einem Weißwein, fast dein letztes Essen wieder hoch und du entscheidest dich doch lieber für den Rotwein.");
                                                output("Er füllt dir ein Glas voll ein und kassiert eifrig das Gold dafür.`n`n");
                                                switch(e_rand(1,5)){
                                                    case 1:
                                                    case 2:
                                                    case 3:
                                                        case 4:
                                                          output("`&Du fühlst dich gesund!");
                                                                ...
                                                                break;
                                                        case 5:
                                                          output("`&Du fühlst dich müde...");
                                                                $session[user][turns]--;
                                                }
                                                $session[bufflist][101] = array("name"=>"`#Rausch","rounds"=>14,"wearoff"=>"Dein Rausch verschwindet.","atkmod"=>1.3,"roundmsg"=>"Du hast einen ordentlichen Rausch am laufen.","activate"=>"offense");
                                        }else{
                                          output("Du hast aber nicht genug Geld bei dir. Wie kannst du von Cedrik verlangen dir Wein zeigen zu lassen, wenn du das Geld dafür nicht hast!?!");
                                        }
                                }
                        }else if ($_GET[act]=="punsch"){
                          output("Du schlägst mit der Faust auf die Bar und verlangst einen Glühwein. ");
                                if ($session[user][drunkenness]>66){
                                        output("Doch Cedrik fährt unbekümmert damit fort, das Glas weiter zu polieren, an dem er gerade arbeitet. \"`%Du hast genug gehabt ".($session[user][sex]?"Mädl":"Bursche").".`0\" ");
                                }else{
                                  if ($session[user][gold]>=$xmascost){
                                          $session[user][drunkenness]+=33;
                                                $session[user][gold]-=$xmascost;
                                                output("Cedrik nimmt eine Tasse und schenkt dir einen warmen Glühwein mit Schuss ein. ");
                                                output("Er bringt dir die Tasse und wünscht dir Frohe Weihnachten. ");
                                                output("`n`nDu drehst dich um, und lässt dich von diesem warmen Zeugs innerlich wärmen.`n`n");
                                                switch(e_rand(1,3)){
                                                  case 1:
                                                        case 2:
                                                          output("`&Du fühlst dich gesund!");
                                                ...
                                                $session[bufflist][102] = array("name"=>"`#Weihnachtsrausch","rounds"=>10,"wearoff"=>"Dein Rausch verschwindet.","atkmod"=>1.25,"roundmsg"=>"Du hast einen ordentlichen Rausch am laufen.","activate"=>"offense");
                                        }else{
                                          output("Du hast aber nicht genug Geld bei dir. Wie kannst du Punsch haben wollen, wenn du das Geld dafür nicht hast!?!");
                                        }
                                }
                                ...
                                /*if ($session['user']['race']['nopvp']==1) {
                                        output('`@Nachdem du nochmal genau nachgedacht hast, bist du dann doch der Meinung, dass du
                                                                        keiner Kriegerrasse angehörst und wohl kaum Chancen hättest.');
                                } elseif
                          ...
                          output("Cedrik lehnt sich weiter über die Bar. \"`%Du willst also was über Farben wissen, hmm?`0\" ");
                                ...
                                output("`e&#0096;e `E&#0096;E `z&#0096;z `Z&#0096;Z `x&#0096;x`n",true);
                                output("`u&#0096;u `f&#0096;f `F&#0096;F `d&#0096;d `D&#0096;D `y&#0096;y `G&#0096;G`n",true);
                                output("`+&#0096;+ `a&#0096;a `A&#0096;A `k&#0096;k`n",true);
                                ...
                                if ($_GET[specialty]=="" && $_GET[specialty2]==""){
                                        ...
                                        //ANGELMOD START
                                        $sql = "SELECT * FROM skills WHERE activated='1' ORDER BY id ASC";
                                        $result = db_query($sql) or die(sql_error($sql));
                                        $max = db_num_rows($result);
                                        for($i=0;$i<$max;$i++){
                                           $row = db_fetch_assoc($result);
                                           addnav("".$row[name]."",preg_replace("/[&?]c=[[:digit:]-]*/","",$REQUEST_URI)."&specialty2=$row[id]");
                                        }
                                }else{
                                      if ($_GET[specialty2]==""){
                                        ...
                                        output("`n`n\"`nCedrik fängt laut an zu lachen: \"`3Jup. Was hasten erwartet? Irgendne Art fantastisches und geheimnisvolles Ritual??? ");
                                        output("Du bist in Ordnung, Kleiner... spiel nur niemals Poker, ok?`0\"");
                                        output("`n`n\"`Ach, nochwas.");
                                        output("Deine neue Fertigkeit wirst du trainieren müssen, um wirklich gut darin zu sein.`0\"");
                                        //addnav("Return to the inn","inn.php");
                                        $session[user][skill]=$_GET[specialty2];
                        ...
                        if ($_GET[pay]){
                                ...
                                if ($session[user][boughtroomtoday]){
                                        ...
                                        output("`n`nDu willst dich nicht von deinem Gold trennen und fängst an darüber zu debattieren, dass man in den Feldern auch kostenlos "
                                                                ."schlafen könnte. Schließlich siehst du aber ein, dass ein Zimmer in der Kneipe vielleicht der sicherere Platz zum Schlafen ist, da es schwieriger für Herumstreicher sein dürfte, "
