...
if ($_GET[op] == "wkkauf"){
    ...
    if ($_GET[id]>"" && $_GET[pid]>""){
        ...
        $sql = "SELECT login,name,level,sex,acctid FROM accounts WHERE $search locked=0 AND alive=1 AND acctid<>".$session[user][acctid]." AND lastip<>'".$session[user][lastip]."' AND (age > $days OR dragonkills > 0 OR pk > 0 OR experience > $exp) ORDER BY login,level LIMIT $limit";
        $result = db_query($sql);
